...
1. **Standardized Field Names Across Files**
...
graph TB
    ...
    subgraph "Style Mapping Hub"
        STYLES[`/api/styles` br/>Master Mapping Table]
    end
    ...
    subgraph "Yarn Layer"
        YARN_ID[`/api/yarn/active`<br/>Master Yarn DB]
        YARN_INV[`/api/yarn/active`<br/>Current Stock]
    end
...
### 2. `GET /api/styles` - Fetch styles from eFab
...
- `Style#` ← All inventory files (F01, G00, G02, I01)
- `Style#` → `Style_BOM.csv[Style#]`
- `fStyle#` ← `Sales Activity [Style#]`
- Both `fStyle#` and `Style#` → `eFab_SO_List[fStyle#, Style#]`

### 3. eFab_SO_List = `GET /api/sales-order/plan/list' - Fetch sales orders
...
### 4. Inventory API (F01, G00, G02, I01)
...
| Field            | Type     | Maps To             | Description           |
| ---------------- | -------- | ------------------- | --------------------- |
| **Style#** | String   | eFab_Styles.fStyle# | Fabric style code     |
...
### 6 `QuadS_finishedFabricLis = tGET /api/styles/greige/active` & /api/styles/finished/active
...
### 7.  `GET /api/knitorder/list` - Fetch knit orders `
