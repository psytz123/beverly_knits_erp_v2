...
class MLForecastFix:
    ...
    def generate_weekly_forecasts(
        ...
        for week in range(num_weeks):
            ...
            # Calculate forecast
            forecast = (
                base_weekly_demand * pattern_factor * growth_factor * random_variation
            )
            ...
            weekly_forecasts.append(
                {
                    "week": week + 1,
                    "week_start": (datetime.now() + timedelta(weeks=week)).strftime(
                        "%Y-%m-%d"
                    ),
                    "week_end": (
                        datetime.now() + timedelta(weeks=week, days=6)
                    ).strftime("%Y-%m-%d"),
    ...
    def generate_style_forecasts(
        self, sales_data: pd.DataFrame, top_n: int = 20
    ) -> List[Dict[str, Any]]:
        """
        ...
        if style_col and qty_col:
            # Group by style
            style_groups = sales_data.groupby(style_col)[qty_col].agg(
                ["sum", "mean", "count"]
            )
            ...
            for style, row in style_groups.iterrows():
                ...
                growth_factor = np.random.uniform(
                    0.95, 1.15
                )  # Random growth between -5% and +15%
    ...
    def get_model_performance(self) -> List[Dict[str, Any]]:
        ...
        models = [
            ...
            },
            {
                ...
                "status": "active",
                "last_trained": (
                    datetime.now() - timedelta(hours=np.random.randint(1, 72))
                ).isoformat(),
                "training_samples": 10338,
                "confidence": 91.2,
            },
            {
                ...
                "status": "active",
                "last_trained": (
                    datetime.now() - timedelta(hours=np.random.randint(1, 96))
                ).isoformat(),
                "training_samples": 10338,
                "confidence": 87.8,
            },
            {
                ...
                "status": "backup",
                "last_trained": (
                    datetime.now() - timedelta(hours=np.random.randint(24, 168))
                ).isoformat(),
                "training_samples": 10338,
                "confidence": 83.4,
            },
