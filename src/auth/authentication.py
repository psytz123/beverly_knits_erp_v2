...
class AuthManager:
    ...
    def __init__(self, secret_key: Optional[str] = None, algorithm: str = "HS256"):
        ...
        # Role definitions
        self.roles = {
            "admin": {"level": 100, "permissions": ["*"]},  # All permissions
            "manager": {
                "level": 80,
                "permissions": [
                    "inventory.read",
            ...
            "viewer": {"level": 20, "permissions": ["inventory.read", "reports.read"]},
        }
    ...
    def invalidate_session(self, user_id: str, token: Optional[str] = None):
        ...
        if user_id in self.active_sessions:
            if token:
                ...
                self.active_sessions[user_id] = [
                    s for s in self.active_sessions[user_id] if s["token"] != token
                ]
            else:
