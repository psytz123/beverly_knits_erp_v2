...
class KnitOrderGenerator:
        ...
        for style, required_quantity in net_requirements.items():
            ...
            if priorities and style in priorities:
                ...
                priority = self._determine_priority(
                    required_quantity, demand_dates, style
                )
            ...
            if required_quantity > self.max_order_quantity:
                ...
                for i in range(num_orders):
                    batch_order = self._create_order(
                        style=style,
                        quantity=min(
                            qty_per_order, required_quantity - (i * qty_per_order)
                        ),
                        priority=priority,
    ...
    def _determine_priority(
        self, quantity: float, demand_dates: Optional[Dict], style: str
    ) -> Priority:
    ...
    def _generate_production_notes(
        self, style: str, quantity: float, priority: Priority
    ) -> str:
    ...
    def export_orders(
        self, orders: List[Dict], format: str = "json", path: Optional[Path] = None
    ) -> Optional[str]:
        ...
        for order in orders:
            ...
            for field in required_fields:
                if field not in order or order[field] is None:
                    errors.append(
                        f"Order {order.get('order_id', 'unknown')}: Missing {field}"
                    )
            ...
            if "quantity_lbs" in order:
                if order["quantity_lbs"] <= 0:
                    errors.append(
                        f"Order {order['order_id']}: Invalid quantity {order['quantity_lbs']}"
                    )
                elif order["quantity_lbs"] < self.min_order_quantity:
                    warnings.append(
                        f"Order {order['order_id']}: Quantity below minimum ({order['quantity_lbs']} < {self.min_order_quantity})"
                    )
            ...
            if "production_start_date" in order and "demand_date" in order:
                try:
                    start = datetime.strptime(
                        order["production_start_date"], "%Y-%m-%d"
                    )
                    demand = datetime.strptime(order["demand_date"], "%Y-%m-%d")
                    if start > demand:
                        errors.append(
                            f"Order {order['order_id']}: Production starts after demand date"
                        )
                except:
...
def generate_orders_from_requirements(
    net_requirements: Dict[str, float], priority: str = "NORMAL"
) -> List[Dict]:
    ...
    priorities = {
        style: priority_map.get(priority, Priority.NORMAL)
