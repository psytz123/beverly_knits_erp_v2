...
class MachineWorkCenterMapper:
    ...
    def __init__(self, data_path: Optional[str] = None):
        ...
        self.data_path = (
            data_path
            or "/mnt/c/finalee/beverly_knits_erp_v2/data/production/5/ERP Data"
        )
    ...
    def load_quads_data(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.style_to_work_center)} style→work center mappings"
            )
            return True
    ...
    def load_machine_report(self) -> bool:
        ...
        try:
            ...
            if not machine_file:
                logger.error(
                    f"Machine report file not found. Tried: {[str(f) for f in possible_files]}"
                )
                return False
            ...
            for _, row in df.iterrows():
                ...
                if machine_pattern and machine_pattern != "nan":
                    ...
                    machine_id = (
                        machine_number
                        if machine_number and machine_number != "nan"
                        else machine_pattern
                    )
            ...
            total_machines = sum(
                len(machines) for machines in self.work_center_to_machines.values()
            )
            logger.info(
                f"Loaded {len(self.work_center_to_machines)} work centers with {total_machines} machines"
            )
    ...
    def _build_mapping_chain(self):
        ...
        logger.info(
            f"Built mapping chain for {len(self.style_to_machine_chain)} styles"
        )
    ...
    def _update_work_center_summaries(self):
        ...
        for work_center, machine_ids in self.work_center_to_machines.items():
            ...
            assigned_styles = [
                style
                for style, wc in self.style_to_work_center.items()
                if wc == work_center
            ]
    ...
    def get_mapping_statistics(self) -> Dict:
        ...
        return {
            ...
            "avg_machines_per_work_center": total_machines
            / max(1, len(self.work_center_to_machines)),
    ...
    def validate_mappings(self) -> Dict[str, List[str]]:
        ...
        for style in self.style_to_work_center.keys():
            ...
            if work_center not in self.work_center_to_machines:
                styles_without_wc.append(
                    f"Style {style} → Work Center {work_center} (no machines)"
                )
        ...
        empty_work_centers = [
            wc for wc, machines in self.work_center_to_machines.items() if not machines
        ]

        if empty_work_centers:
            issues["warnings"].extend(
                [f"Empty work center: {wc}" for wc in empty_work_centers]
            )
        ...
        if duplicate_machines:
            issues["errors"].extend(
                [f"Duplicate machine: {m}" for m in duplicate_machines]
            )
    ...
    def export_mappings_to_json(self, file_path: Optional[str] = None) -> str:
        ...
        export_data = {
            ...
            "work_center_info": {
                k: v.to_dict() for k, v in self.work_center_info.items()
            },
        ...
        if not file_path:
            file_path = (
                f"/tmp/machine_mappings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
if __name__ == "__main__":
    ...
    if mapper.load_all_data():
        ...
        for wc_id, wc_info in mapper.get_work_center_summary().items():
            print(
                f"  {wc_id}: {wc_info.machine_count} machines, {len(wc_info.active_styles)} styles"
            )
