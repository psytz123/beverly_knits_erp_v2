...
class EnhancedProductionAnalyzer:
    ...
    def get_comprehensive_production_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_orders = len(
                self.knit_orders_data[self.knit_orders_data["Status"] != "Completed"]
            )
            total_orders = len(self.knit_orders_data)

            # Calculate on-time delivery rate
            if (
                "Delivery_Date" in self.knit_orders_data.columns
                and "Actual_Delivery" in self.knit_orders_data.columns
            ):
                ...
                on_time_rate = (
                    (len(on_time) / total_orders * 100) if total_orders > 0 else 0
                )
            else:
                # Estimate based on status
                completed = len(
                    self.knit_orders_data[
                        self.knit_orders_data["Status"] == "Completed"
                    ]
                )
                on_time_rate = (
                    (completed / total_orders * 85) if total_orders > 0 else 0
                )  # Assume 85% of completed are on-time

            # Calculate production efficiency
            if (
                "Planned_Quantity" in self.knit_orders_data.columns
                and "Actual_Quantity" in self.knit_orders_data.columns
            ):
            ...
            capacity_utilization = (
                (current_week_production / weekly_capacity * 100)
            ...
            return {
                ...
                "recommendations": self._generate_recommendations(
                    bottlenecks, efficiency, capacity_utilization
                ),
            }
    ...
    def _get_current_week_production(self) -> float:
        ...
        try:
            ...
            if "Order_Date" in self.knit_orders_data.columns:
                self.knit_orders_data["Order_Date"] = pd.to_datetime(
                    self.knit_orders_data["Order_Date"]
                )
            ...
            for stage in self.production_stages:
                ...
                for status, count in status_counts.items():
                    if status_stage_map.get(status, "") == stage:
                        stage_orders += count
                        if "Quantity" in self.knit_orders_data.columns:
                            stage_data = self.knit_orders_data[
                                self.knit_orders_data["Status"] == status
                            ]
                ...
                stages.append(
                    {
                        ...
                        "utilization": (
                            round(stage_orders / max(total_orders, 1) * 100, 1)
                            if "total_orders" in locals()
                            else 0
                        ),
                    }
                )
    ...
    def _get_priority_orders(self) -> List[Dict[str, Any]]:
        ...
        if "Delivery_Date" in self.knit_orders_data.columns:
            self.knit_orders_data["Delivery_Date"] = pd.to_datetime(
                self.knit_orders_data["Delivery_Date"]
            )
            today = pd.Timestamp.now()
    ...
    def _calculate_wip_value(self) -> float:
        ...
        incomplete = self.knit_orders_data[
            self.knit_orders_data["Status"] != "Completed"
        ]
        ...
        if "Order_Value" in incomplete.columns:
            ...
            for idx, row in incomplete.iterrows():
                ...
                product = (
                    str(row.get("Product", "")).lower() if "Product" in row else ""
                )
    ...
    def _calculate_average_lead_time(self) -> float:
        ...
        if (
            "Order_Date" in self.knit_orders_data.columns
            and "Completion_Date" in self.knit_orders_data.columns
        ):
            completed = self.knit_orders_data[
                self.knit_orders_data["Status"] == "Completed"
            ]
            if not completed.empty:
                ...
                lead_times = (
                    completed["Completion_Date"] - completed["Order_Date"]
                ).dt.days
    ...
    def _generate_recommendations(
        self, bottlenecks: List, efficiency: float, capacity: float
    ) -> List[str]:
        ...
        if bottlenecks:
            recommendations.append(
                f"Address {len(bottlenecks)} identified bottlenecks to improve flow"
            )

        if efficiency < 80:
            recommendations.append(
                "Focus on improving production efficiency through process optimization"
            )

        if capacity > 90:
            recommendations.append(
                "Consider adding shifts or equipment to handle high capacity utilization"
            )
        elif capacity < 60:
            recommendations.append(
        ...
        if not recommendations:
            recommendations.append(
                "Production running smoothly - maintain current performance"
            )
