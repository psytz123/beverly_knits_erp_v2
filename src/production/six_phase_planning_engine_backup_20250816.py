...
class SixPhasePlanningEngine:
    ...
    def validate_data_requirements(self):
        ...
        if self.sales_data is None or self.sales_data.empty:
            ...
            required_sales_cols = ['Date', 'Quantity', 'Customer']
        ...
        if self.inventory_data is None or self.inventory_data.empty:
            ...
            required_inv_cols = ['Item Code', 'On Hand', 'Unit Cost']
            missing_cols = [col for col in required_inv_cols 
                           if col not in self.inventory_data.columns]
            if missing_cols:
                validation_results['errors'].append(
                    f"Missing inventory columns: {missing_cols}"
                )
                validation_results['valid'] = False
        ...
        if self.bom_data is None or self.bom_data.empty:
            ...
            required_bom_cols = ['Parent', 'Component', 'Quantity']
            missing_cols = [col for col in required_bom_cols 
                           if col not in self.bom_data.columns]
            if missing_cols:
                validation_results['warnings'].append(
                    f"Missing BOM columns: {missing_cols}"
                )
            
            validation_results['data_summary']['bom'] = {
                'rows': len(self.bom_data),
                'status': 'Available',
                'unique_parents': self.bom_data['Parent'].nunique() if 'Parent' in self.bom_data.columns else 0
            }
        
        # Check supplier data
        if self.supplier_data is None or self.supplier_data.empty:
            validation_results['warnings'].append(
    ...
    def _internal_execute_cycle(self, callback=None) -> List[PlanningPhaseResult]:
        ...
        if callback:
            callback.update(1, 'Running', 'Executing forecast unification')
        phase1_result = self.execute_phase_with_logging(
            self.phase1_forecast_unification,
            "Phase 1: Forecast Unification"
        )
        self.phase_results.append(phase1_result)
        if callback:
            callback.complete_phase(1, "Forecast Unification", phase1_result.status == "Completed")
        
        # Phase 2: BOM Explosion
        if callback:
            callback.update(2, 'Running', 'Executing BOM explosion')
        phase2_result = self.execute_phase_with_logging(
            self.phase2_bom_explosion,
            "Phase 2: BOM Explosion"
        )
        self.phase_results.append(phase2_result)
        if callback:
            callback.complete_phase(2, "BOM Explosion", phase2_result.status == "Completed")
        
        # Phase 3: Inventory Netting
        if callback:
            callback.update(3, 'Running', 'Executing inventory netting')
        phase3_result = self.execute_phase_with_logging(
            self.phase3_inventory_netting,
            "Phase 3: Inventory Netting"
        )
        self.phase_results.append(phase3_result)
        if callback:
            callback.complete_phase(3, "Inventory Netting", phase3_result.status == "Completed")
        
        # Phase 4: Procurement Optimization
        if callback:
            callback.update(4, 'Running', 'Executing procurement optimization')
        phase4_result = self.execute_phase_with_logging(
            self.phase4_procurement_optimization,
            "Phase 4: Procurement Optimization"
        )
        self.phase_results.append(phase4_result)
        if callback:
            callback.complete_phase(4, "Procurement Optimization", phase4_result.status == "Completed")
        
        # Phase 5: Supplier Selection
        if callback:
            callback.update(5, 'Running', 'Executing supplier selection')
        phase5_result = self.execute_phase_with_logging(
            self.phase5_supplier_selection,
            "Phase 5: Supplier Selection"
        )
        self.phase_results.append(phase5_result)
        if callback:
            callback.complete_phase(5, "Supplier Selection", phase5_result.status == "Completed")
        
        # Phase 6: Output Generation
        if callback:
            callback.update(6, 'Running', 'Generating final output')
        phase6_result = self.execute_phase_with_logging(
            self.phase6_output_generation,
            "Phase 6: Output Generation"
        )
        self.phase_results.append(phase6_result)
        if callback:
            callback.complete_phase(6, "Output Generation", phase6_result.status == "Completed")
