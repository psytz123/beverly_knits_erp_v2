...
class EnhancedTimePhasedPlanning:
    ...
    def calculate_enhanced_balance(
        ...
        for week_offset in range(self.planning_horizon_weeks):
            ...
            week_data = {
                ...
                "coverage_days": (
                    (balance / (demand / 7)) if demand > 0 else float("inf")
                ),
            }
    ...
    def _generate_po_suggestion(
        self, yarn_id: str, weekly_analysis: List[Dict], first_shortage_week: int
    ) -> Dict[str, Any]:
        """
        ...
        order_by_week = self.current_week + max(
            0, weeks_until_shortage - lead_time_weeks
        )
    ...
    def generate_procurement_plan(
        self, yarn_shortages: List[Dict], budget_limit: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        procurement_plan["total_quantity"] = sum(
            ...
            for po in procurement_plan["po_suggestions"]
    ...
    def analyze_supply_chain_risk(
        ...
        if critical_count > 5:
            ...
            risk_assessment["recommendations"].append(
                "Expedite PO processing for critical yarns"
            )
            ...
            risk_assessment["recommendations"].append(
                "Implement preventive ordering strategy"
            )

        # Add supplier-based recommendations
        if supplier_performance:
            low_performers = [
                s for s, score in supplier_performance.items() if score < 0.8
            ]
            if low_performers:
                risk_assessment["risk_factors"].append(
                    f'Supplier reliability issues: {", ".join(low_performers)}'
                )
                risk_assessment["recommendations"].append(
                    "Diversify supplier base for critical yarns"
                )
