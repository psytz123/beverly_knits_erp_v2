...
class EnhancedProductionPipeline:
    ...
    def process_knit_orders(self) -> Dict[str, Any]:
        ...
        result["recommendations"] = self._generate_recommendations(
            result["pipeline"], result["bottlenecks"]
        )
    ...
    def _calculate_stage_metrics(self) -> List[Dict[str, Any]]:
        ...
        stage_col = self._find_column(
            ["Stage", "Status", "Production Stage", "Current Stage"]
        )
        if not stage_col:
    ...
    def _analyze_by_status_patterns(self) -> List[Dict[str, Any]]:
        ...
        status_col = self._find_column(
            ["Status", "Order Status", "Current Status", "Stage"]
        )
        ...
        if status_col:
            for stage_name, keywords in patterns.items():
                ...
                for keyword in keywords:
                    mask |= self.knit_orders[status_col].str.contains(
                ...
                if not stage_df.empty:
                    metrics = {
                        ...
                        "on_time_rate": self._calculate_on_time_rate(stage_df),
                        "late_orders": self._count_late_orders(stage_df),
                        "bottleneck_status": self._determine_bottleneck_status(
                            stage_df
                        ),
                        "avg_cycle_time": "N/A",
                        "wip_value": self._calculate_wip_value(stage_df),
    ...
    def _calculate_total_quantity(self, df: pd.DataFrame) -> float:
        ...
        qty_col = self._find_column(
            ["Qty Ordered (lbs)", "Qty", "Quantity", "Order Qty"], df
        )
        if qty_col:
    ...
    def _calculate_completion_rate(self, df: pd.DataFrame) -> float:
        ...
        comp_col = self._find_column(
            ["Completion", "Progress", "%Complete", "Completion %"], df
        )
        if comp_col:
    ...
    def _calculate_on_time_rate(self, df: pd.DataFrame) -> float:
        ...
        date_col = self._find_column(
            ["Start Date", "Quoted Date", "Due Date", "Delivery Date"], df
        )
        if date_col:
            try:
                df_copy = df.copy()
                ...
                on_time = len(
                    df_copy[(df_copy[date_col] >= today) | df_copy[date_col].isna()]
                )
    ...
    def _count_late_orders(self, df: pd.DataFrame) -> int:
        ...
        if date_col:
            try:
                ...
                if balance_col:
                    late_orders = df_copy[
                        (df_copy[date_col] < today) & (df_copy[balance_col] > 0)
                    ]
    ...
    def _calculate_summary(self) -> Dict[str, Any]:
        ...
        if all([qty_col, g00_col, shipped_col, balance_col]):
            for idx, row in self.knit_orders.iterrows():
                ...
                if (
                    abs(calculated_balance - recorded_balance) > 1
                ):  # Allow 1 lb tolerance
                    balance_errors += 1
        ...
        summary = {
            ...
            "data_accuracy_note": (
                f"{balance_errors} orders have incorrect balance calculations"
                if balance_errors > 0
                else "All balance calculations verified"
            ),
            "unique_styles": 0,
            "unique_customers": 0,
            "avg_order_size": 0,
        ...
        if summary["total_orders"] > 0:
            summary["avg_order_size"] = (
                summary["total_quantity_ordered"] / summary["total_orders"]
            )
    ...
    def _identify_critical_orders(self) -> List[Dict[str, Any]]:
        ...
        try:
            self.knit_orders[date_col] = pd.to_datetime(
                self.knit_orders[date_col], errors="coerce"
            )
            today = pd.Timestamp.now()
            ...
            for _, row in critical_df.head(10).iterrows():
                order = {
                    ...
                    "days_until_due": (
                        (row[date_col] - today).days if pd.notna(row[date_col]) else 0
                    ),
    ...
    def _generate_recommendations(
        self, pipeline: List[Dict], bottlenecks: List[Dict]
    ) -> List[str]:
        ...
        if critical_bottlenecks:
            recommendations.append(
                f"URGENT: Address critical bottleneck in {critical_bottlenecks[0]['stage']} - {critical_bottlenecks[0]['impact']}"
            )
        ...
        if avg_efficiency < 70:
            recommendations.append(
                f"Overall efficiency is low ({avg_efficiency:.1f}%). Review production scheduling and resource allocation."
            )
        ...
        if total_wip > 500:
            recommendations.append(
                f"High WIP levels ({total_wip} orders). Consider implementing pull systems to reduce inventory."
            )
        ...
        if total_late > 10:
            recommendations.append(
                f"{total_late} orders are late. Expedite processing and communicate with customers."
            )

        if not recommendations:
            recommendations.append(
                "Production pipeline is operating normally. Continue monitoring for changes."
            )
    ...
    def _find_column(
        self, possible_names: List[str], df: Optional[pd.DataFrame] = None
    ) -> Optional[str]:
