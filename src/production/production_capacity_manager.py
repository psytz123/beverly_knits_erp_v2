...
try:
    ...
    try:
        from src.production.machine_mapper import (
            get_machine_mapper,
            MachineWorkCenterMapper,
        )
    except ImportError:
        logger.warning(
            "MachineWorkCenterMapper not available - machine-level features disabled"
        )
...
class ProductionCapacityManager:
    ...
    def __init__(self, capacity_file_path: Optional[str] = None):
        ...
        self.machine_suggested_workload_lbs = (
            {}
        )  # machine_id -> suggested/unassigned work in lbs
        self.machine_forecasted_workload_lbs = (
            {}
        )  # machine_id -> forecasted demand in lbs
    ...
    def load_machine_workloads(self):
        ...
        try:
            ...
            if not knit_orders_path.exists():
                logger.warning(
                    "Knit orders file not found, using default machine utilization"
                )
                return
                    ...
                    if unassigned_workload > 0:
                        ...
                        for machine_id in machines:
                            ...
                            if machine_id_str in self.machine_suggested_workload_lbs:
                                ...
                                self.machine_suggested_workload_lbs[machine_id_str] = (
                                    workload_per_machine
                                )

                            # Also add to main workload for utilization calculation
                            if machine_id_str in self.machine_workload_lbs:
                                self.machine_workload_lbs[
                                    machine_id_str
                                ] += workload_per_machine
                            else:
                                self.machine_workload_lbs[machine_id_str] = (
                                    workload_per_machine
                                )
                        ...
                        logger.info(
                            f"Distributed {unassigned_workload:.0f} lbs for style {style} across {len(machines)} machines"
                        )
            ...
            total_assigned = len(
                [
                    m
                    for m in self.machine_workload_lbs
                    if m not in self.machine_suggested_workload_lbs
                ]
            )
            total_suggested = len(self.machine_suggested_workload_lbs)
            logger.info(
                f"Loaded workloads for {len(self.machine_workload_lbs)} machines from knit orders"
            )
            logger.info(f"  - {total_assigned} machines with direct assignments")
            logger.info(
                f"  - {total_suggested} machines with distributed/suggested workload"
            )
    ...
    def load_capacity_data(self, file_path: str) -> bool:
        ...
        try:
            ...
            for _, row in self.capacity_data.iterrows():
                ...
                if capacity < 0:
                    logger.warning(
                        f"Negative capacity for style {style}: {capacity}. Using default."
                    )
            ...
            logger.info(
                f"Loaded {len(self.style_capacities)} style capacities from {file_path}"
            )
            return True
    ...
    def optimize_production_schedule(
        self, production_requests: List[Dict], max_daily_capacity: float = 10000
    ) -> List[Dict]:
        """
        ...
        sorted_requests = sorted(
            production_requests, key=lambda x: x.get("combined_score", 0), reverse=True
        )
            ...
            while not scheduled:
                ...
                if current_load + capacity <= max_daily_capacity:
                    ...
                    schedule_item = {
                        ...
                        "end_date": (check_date + timedelta(days=days_needed)).strftime(
                            "%Y-%m-%d"
                        ),
                    ...
                    for day in range(int(np.ceil(days_needed))):
                        day_date = (check_date + timedelta(days=day)).strftime(
                            "%Y-%m-%d"
                        )
    ...
    def get_capacity_summary(self) -> Dict:
        ...
        return {
            ...
            "median_capacity": (
                np.median(positive_capacities) if positive_capacities else 0
            ),
            "total_daily_capacity": sum(positive_capacities),
            "excellent_efficiency_styles": len(
                [c for c in positive_capacities if c >= 2000]
            ),
            "good_efficiency_styles": len(
                [c for c in positive_capacities if 1000 <= c < 2000]
            ),
            "average_efficiency_styles": len(
                [c for c in positive_capacities if 500 <= c < 1000]
            ),
            "below_average_efficiency_styles": len(
                [c for c in positive_capacities if 100 <= c < 500]
            ),
            "poor_efficiency_styles": len([c for c in positive_capacities if c < 100]),
        }

    def validate_production_request(
        self, style: str, quantity_lbs: float, deadline_days: int = None
    ) -> Tuple[bool, str]:
        """
        ...
        if deadline_days and days_needed > deadline_days:
            return (
                False,
                f"Cannot produce {quantity_lbs} lbs in {deadline_days} days. Need {days_needed:.1f} days",
            )

        if quantity_lbs > capacity * 30:  # More than a month's production
            return (
                False,
                f"Quantity {quantity_lbs} lbs exceeds monthly capacity of {capacity*30:.0f} lbs",
            )
    ...
    def assign_style_to_machine(self, machine_id: str, style: str) -> bool:
        ...
        if self.machine_mapper and machine_id in self.machine_mapper.machine_info:
            ...
            self.update_machine_utilization(
                machine_id, 75.0
            )  # Assume 75% when assigned
            return True
        return False
    ...
    def get_machine_level_status(self, active_only: bool = False) -> Dict[str, Any]:
        ...
        for machine_id, machine_info in self.machine_mapper.machine_info.items():
            ...
            suggested_workload_lbs = self.machine_suggested_workload_lbs.get(
                machine_id, 0
            )
            forecasted_workload_lbs = self.machine_forecasted_workload_lbs.get(
                machine_id, 0
            )

            # Calculate total potential workload
            total_workload = (
                workload_lbs + suggested_workload_lbs + forecasted_workload_lbs
            )
            ...
            status = {
                ...
                "work_center": self.machine_mapper.get_work_center_for_machine(
                    machine_id
                ),
                ...
                "days_of_work": (
                    round(total_workload / self.default_capacity, 1)
                    if total_workload > 0
                    else 0
                ),
                "status": (
                    "RUNNING"
                    if workload_lbs > 0
                    else ("PENDING" if suggested_workload_lbs > 0 else "IDLE")
                ),
            }
        ...
        for work_center, summary in self.get_all_work_centers_summary().items():
            if active_only:
                ...
                wc_machines = [
                    m for m in machine_status if m["work_center"] == work_center
                ]
                if not wc_machines:
                    continue
        ...
        avg_utilization = (
            np.mean([m["utilization"] for m in machine_status]) if machine_status else 0
        )
    ...
    def optimize_machine_assignments(
        self, production_requests: List[Dict]
    ) -> List[Dict]:
        """
        ...
        for request in sorted_requests:
            ...
            if best_machine:
                ...
                days_needed = (
                    quantity / capacity
                    if capacity > 0
                    else quantity / self.default_capacity
                )
                ...
                new_utilization = min(
                    100.0, best_utilization + (days_needed * 10)
                )  # Rough estimate
...
def get_capacity_manager(
    capacity_file_path: Optional[str] = None,
