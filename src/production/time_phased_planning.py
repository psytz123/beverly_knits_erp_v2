...
class TimePhasedPlanning:
    ...
    def calculate_weekly_balance(
        ...
        logger.debug(
            f"Yarn {yarn_id}: Starting balance {starting_balance}, Past due +{past_due} = {current_balance}"
        )
    ...
    def identify_shortage_periods(
        self, yarn_id: str, weekly_balances: Dict[str, float]
        ...
        if shortage_periods:
            logger.info(
                f"Yarn {yarn_id}: {len(shortage_periods)} shortage periods identified"
            )

        return shortage_periods

    def _find_recovery_week(
        self, weekly_balances: Dict[str, float], shortage_week: str
    ) -> Optional[str]:
        """
    ...
    def calculate_expedite_requirements(
        self,
        yarn_id: str,
        shortage_timeline: List[Tuple[str, float, Optional[str]]],
        weekly_receipts: Dict[str, float],
    ) -> List[Dict[str, Any]]:
        """
        ...
        for shortage_week, shortage_amount, recovery_week in shortage_timeline:
            ...
            if expedite_candidate:
                recommendation = {
                    ...
                    "expedite_amount": min(
                        shortage_amount, expedite_candidate["amount"]
                    ),
    ...
    def _find_best_expedite_candidate(
        self, target_week: str, needed_amount: float, weekly_receipts: Dict[str, float]
    ...
    def calculate_yarn_coverage_weeks(
        self, yarn_id: str, current_balance: float, weekly_demand: Dict[str, float]
    ) -> float:
        """
        ...
        coverage_weeks = (
            current_balance / weekly_avg_demand if weekly_avg_demand > 0 else 999.0
        )
    ...
    def generate_shortage_summary(
        self,
        yarn_id: str,
        weekly_balances: Dict[str, float],
        weekly_receipts: Dict[str, float],
        weekly_demand: Dict[str, float],
    ) -> Dict[str, Any]:
        """
        ...
        expedite_recs = self.calculate_expedite_requirements(
            yarn_id, shortage_periods, weekly_receipts
        )
        ...
        coverage_weeks = self.calculate_yarn_coverage_weeks(
            yarn_id, current_balance, weekly_demand
        )
    ...
    def process_yarn_time_phased(
        ...
        theoretical_balance = yarn_data.get(
            "theoretical_balance", yarn_data.get("Theoretical Balance", 0)
        )
...
def main():
    ...
    if result["expedite_recommendations"]:
        ...
        for rec in result["expedite_recommendations"]:
            print(
                f"Expedite {rec['expedite_amount']:,.2f} lbs from {rec['expedite_from_week']} to {rec['expedite_to_week']}"
            )
