...
class EnhancedProductionSuggestionsV2:
    ...
    def calculate_demand_confidence(self, style: str) -> Dict[str, Any]:
        ...
        if (
            hasattr(self.analyzer, "sales_orders_data")
            and self.analyzer.sales_orders_data is not None
        ):
                ...
                if not style_orders.empty:
                    ...
                    if "Status" in style_orders.columns:
                        active_orders = style_orders[
                            style_orders["Status"].str.contains(
                        ...
                        if not active_orders.empty:
                            ...
                            result["sales_order_qty"] = float(
                                active_orders.get(
                                    "Balance", active_orders.get("Ordered", pd.Series())
                                ).sum()
                            )
                            result["confidence_score"] = (
                                1.0  # 100% confidence for actual orders
                            )
        ...
        if (
            hasattr(self.analyzer, "sales_data")
            and self.analyzer.sales_data is not None
        ):
            ...
            if style_col:
                style_sales = self.analyzer.sales_data[
                    self.analyzer.sales_data[style_col] == style
                ]
                ...
                if not style_sales.empty:
                    ...
                    demand_col = style_sales.get(
                        "Ordered", style_sales.get("Quantity", pd.Series())
                    )
                    if not demand_col.empty:
                        ...
                        if demand_col.dtype == "object":
                            ...
                            demand_col = (
                                demand_col.astype(str)
                                .str.replace(",", "")
                                .str.replace("$", "")
                            )
                            # Extract numeric values
                            demand_col = demand_col.str.extract(
                                r"([\d.]+)", expand=False
                            )
                            demand_col = pd.to_numeric(
                                demand_col, errors="coerce"
                            ).fillna(0)
                    ...
                    if (
                        "Ordered" in style_sales.columns
                        or "Quantity" in style_sales.columns
                    ):
                        demand_series = style_sales.get(
                            "Ordered", style_sales.get("Quantity", pd.Series())
                        )
                        # Convert string values with commas to numeric
                        if demand_series.dtype == "object":
                            # Remove commas, dollar signs, and any non-numeric characters
                            demand_series = (
                                demand_series.astype(str)
                                .str.replace(",", "")
                                .str.replace("$", "")
                            )
                            # Extract numeric values
                            demand_series = demand_series.str.extract(
                                r"([\d.]+)", expand=False
                            )
                            demand_series = pd.to_numeric(
                                demand_series, errors="coerce"
                            ).fillna(0)
                    ...
                    if (
                        "Invoice Date" in style_sales.columns
                        or "Date" in style_sales.columns
                    ):
                        date_col = (
                            "Invoice Date"
                            if "Invoice Date" in style_sales.columns
                            else "Date"
                        )
                        try:
                            style_sales["parsed_date"] = pd.to_datetime(
                                style_sales[date_col]
                            )
                            style_sales = style_sales.sort_values("parsed_date")

                            # Check for consistent ordering pattern
                            date_diffs = (
                                style_sales["parsed_date"].diff().dt.days.dropna()
                            )
                            if len(date_diffs) > 0:
                                ...
                                if avg_interval > 0 and not pd.isna(std_interval):
                                    interval_cv = (
                                        std_interval / avg_interval
                                        if avg_interval > 0
                                        else 1
                                    )
                                    trend_confidence = max(
                                        0, min(1, 1 - (interval_cv / 2))
                                    )
    ...
    def calculate_inventory_urgency(self, style: str) -> Dict[str, Any]:
            ...
            for stage, inv_data in self.analyzer.inventory_data.items():
                ...
                if (
                    ...
                    if not style_inv.empty:
                        balance = style_inv.get(
                            "Balance", style_inv.get("On Hand (lbs)", pd.Series())
                        ).sum()
        ...
        if (
            ...
            if "Style#" in self.analyzer.sales_orders_data.columns:
                ...
                if not style_orders.empty:
                    urgency["allocated"] = float(
                        style_orders.get("Balance", pd.Series()).sum()
                    )
        ...
        if demand_confidence["sales_order_qty"] > 0:
            ...
            daily_demand = (
                demand_confidence["historical_demand"] / 30
            )  # Assume 30-day history
    ...
    def calculate_priority_score_v2(
        self,
        style: str,
        demand_confidence: Dict,
        inventory_urgency: Dict,
        ...
        base_priority = self.PRIORITY_CATEGORIES.get(
            demand_confidence["demand_source"], 10
        )
        ...
        weighted_score += (
            demand_source_score * self.scoring_weights["demand_source"] * 100
        )

        # 2. Demand confidence score (25% weight)
        confidence_score = demand_confidence["confidence_score"]
        weighted_score += (
            confidence_score * self.scoring_weights["demand_confidence"] * 100
        )

        # 3. Inventory urgency score (20% weight)
        urgency_score = inventory_urgency["urgency_score"]
        weighted_score += (
            urgency_score * self.scoring_weights["inventory_urgency"] * 100
        )

        # 4. Material availability score (15% weight)
        weighted_score += (
            material_availability * self.scoring_weights["material_availability"] * 100
        )
    ...
    def check_material_availability(
        ...
        for _, bom_row in style_bom.iterrows():
            ...
            if yarn_id and str(yarn_id) != "nan":
                yarn_needed = (
                    (quantity * bom_percent / 100) if bom_percent > 0 else quantity
                )
                ...
                if available_yarn >= yarn_needed:
                    ...
                    issues.append(
                        f"Yarn {yarn_id}: need {yarn_needed:.0f} lbs, short {shortage:.0f} lbs"
                    )
                    available = False
            ...
            if not style_sales.empty:
                ...
                if "Unit Price" in style_sales.columns:
                    ...
                    if price_series.dtype == "object":
                        ...
                        price_series = price_series.str.extract(
                            r"([\d.]+)", expand=False
                        )
                        price_series = pd.to_numeric(
                            price_series, errors="coerce"
                        ).fillna(0)
    ...
    def calculate_suggested_quantity(
        self, style: str, demand_confidence: Dict, inventory_urgency: Dict
    ) -> float:
        ...
        # For forecasted demand, adjust based on confidence
            ...
            if demand_confidence["demand_variability"] > 0.5:
                safety_stock = (
                    suggested_qty * 0.3
                )  # High variability = more safety stock
            else:
        ...
        try:
                ...
                if "Style#" in self.analyzer.sales_orders_data.columns:
                    ...
                    if "Status" in self.analyzer.sales_orders_data.columns:
                        active_orders = self.analyzer.sales_orders_data[
                            self.analyzer.sales_orders_data["Status"].str.contains(
                                "Open|Active|Pending", case=False, na=False
                            )
                        ]
                        sales_order_styles.update(active_orders["Style#"].unique())
                    else:
                        sales_order_styles.update(
                            self.analyzer.sales_orders_data["Style#"].unique()
                        )
            ...
            if (
                ...
                if "Style#" in self.analyzer.sales_data.columns:
                    ...
                    if (
                        hasattr(self.analyzer, "style_mapper")
                        and self.analyzer.style_mapper
                    ):
                        for style in sales_styles[:50]:  # Limit for performance
                            mapped = self.analyzer.style_mapper.map_sales_to_bom(
                                str(style)
                            )
                            if mapped:
            ...
            bom_styles = set()
            if (
                hasattr(self.analyzer, "bom_data")
                and self.analyzer.bom_data is not None
            ):
                bom_df = self.analyzer.bom_data
                if isinstance(bom_df, dict) and "data" in bom_df:
                    bom_df = bom_df["data"]
            ...
            all_styles = (
                list(sales_order_styles)
                + [s for s in historical_styles if s not in sales_order_styles]
                + [
                    s
                    for s in bom_styles
                    if s not in sales_order_styles and s not in historical_styles
                ]
            )
            ...
            for style in all_styles[:200]:
                ...
                if (
                    inventory_urgency["days_of_supply"] > self.forecast_horizon_days
                    and not demand_confidence["has_sales_order"]
                    and demand_confidence["confidence_score"] < 0.3
                ):
                    continue

                # Calculate suggested quantity
                suggested_qty = self.calculate_suggested_quantity(
                    style, demand_confidence, inventory_urgency
                )
                ...
                material_available, material_issues, material_score = (
                    self.check_material_availability(style, suggested_qty)
                )
                ...
                if demand_confidence["has_sales_order"]:
                    rationale_parts.append(
                        f"ACTIVE SALES ORDER: {demand_confidence['sales_order_qty']:.0f} lbs"
                    )
                elif demand_confidence["confidence_level"] == "HIGH":
                    rationale_parts.append(
                        f"High confidence forecast ({demand_confidence['confidence_score']:.0%})"
                    )
                elif demand_confidence["confidence_level"] == "MEDIUM":
                    rationale_parts.append(
                        f"Medium confidence forecast ({demand_confidence['confidence_score']:.0%})"
                    )
                else:
                    rationale_parts.append(
                        f"Low confidence forecast ({demand_confidence['confidence_score']:.0%})"
                    )

                # Add urgency information
                if inventory_urgency["stockout_risk"] in ["critical", "high"]:
                    rationale_parts.append(
                        f"{inventory_urgency['stockout_risk'].upper()} stockout risk - {inventory_urgency['days_of_supply']:.1f} days supply"
                    )

                # Add demand metrics
                if demand_confidence["historical_demand"] > 0:
                    rationale_parts.append(
                        f"Historical: {demand_confidence['historical_demand']:.0f} lbs"
                    )
                    if demand_confidence["data_points"] > 0:
                        rationale_parts.append(
                            f"{demand_confidence['data_points']} orders"
                        )

                # Add material status
                if not material_available:
                    rationale_parts.append(
                        f"Material constraints: {len(material_issues)} issues"
                    )
                ...
                suggestion = {
                    ...
                    "historical_demand": round(
                        demand_confidence["historical_demand"], 0
                    ),
                    "data_points": demand_confidence["data_points"],
                    "demand_variability": round(
                        demand_confidence["demand_variability"], 2
                    ),
                    ...
                    "material_status": (
                        ...
                        else f"{len(material_issues)} material issues"
                    ),
            ...
            for s in top_suggestions:
                category_counts[s["priority_category"]] = (
                    category_counts.get(s["priority_category"], 0) + 1
                )
            ...
            summary = {
                ...
                "critical_stockout_risk": len(
                    [s for s in top_suggestions if s["stockout_risk"] == "critical"]
                ),
                "high_stockout_risk": len(
                    [s for s in top_suggestions if s["stockout_risk"] == "high"]
                ),
                "material_available": len(
                    [s for s in top_suggestions if s["yarn_available"]]
                ),
                "material_shortage": len(
                    [s for s in top_suggestions if not s["yarn_available"]]
                ),
                "total_suggested_production": sum(
                    s["suggested_quantity_lbs"] for s in top_suggestions
                ),
                "average_priority_score": (
                    round(np.mean([s["priority_score"] for s in top_suggestions]), 1)
                    if top_suggestions
                    else 0
                ),
                "average_confidence": (
                    round(np.mean([s["confidence_score"] for s in top_suggestions]), 2)
                    if top_suggestions
                    else 0
                ),
            }
            ...
            if summary["sales_order_driven"] > 0:
                recommendations.append(
                    f"PRIORITY: {summary['sales_order_driven']} styles have active sales orders - produce immediately"
                )

            if summary["high_confidence_forecast"] > 0:
                recommendations.append(
                    f"High confidence: {summary['high_confidence_forecast']} styles with reliable demand forecast"
                )

            if summary["low_confidence_forecast"] > summary["high_confidence_forecast"]:
                recommendations.append(
                    "Many low-confidence forecasts - consider gathering more sales data before production"
                )

            # Risk-based recommendations
            if summary["critical_stockout_risk"] > 0:
                recommendations.append(
                    f"URGENT: {summary['critical_stockout_risk']} styles at critical stockout risk"
                )

            # Material recommendations
            if summary["material_shortage"] > summary["material_available"]:
                recommendations.append(
                    "Material constraints affecting majority - prioritize yarn procurement"
                )
            ...
            if avg_conf < 0.5:
                recommendations.append(
                    "Overall low demand confidence - focus on sales order fulfillment"
                )
            elif avg_conf > 0.75:
                recommendations.append(
                    "High overall confidence - production plans are reliable"
                )
...
def create_enhanced_suggestions_v2(analyzer):
    ...
    try:
        ...
        if suggestions_result.get("status") == "success" and suggestions_result.get(
            "suggestions"
        ):
            ...
            if enhanced:
                suggestions_result["summary"]["critical_priority"] = len(
                    [s for s in enhanced if s.get("priority_rank") == "CRITICAL"]
                )
                suggestions_result["summary"]["high_priority"] = len(
                    [s for s in enhanced if s.get("priority_rank") == "HIGH"]
                )
                suggestions_result["summary"]["total_suggested_qty"] = sum(
                    s.get("suggested_quantity_lbs", 0) for s in enhanced
                )

                # Add business logic recommendations
                business_recommendations = []

                critical_count = suggestions_result["summary"].get(
                    "critical_priority", 0
                )
                if critical_count > 0:
                    business_recommendations.append(
                        f"URGENT: {critical_count} critical priority items need immediate production"
                    )

                material_shortage = len(
                    [s for s in enhanced if not s.get("yarn_available", True)]
                )
                if material_shortage > 0:
                    business_recommendations.append(
                        f"Material shortage affecting {material_shortage} styles - prioritize procurement"
                    )

                seasonal = len(
                    [s for s in enhanced if s.get("style_category") == "SEASONAL"]
                )
                if seasonal > 0:
                    business_recommendations.append(
                        f"{seasonal} seasonal items detected - adjust lead times accordingly"
                    )

                if business_recommendations:
                    suggestions_result["recommendations"].extend(
                        business_recommendations
                    )
