...
try:
    ...
    try:
        ...
        logger.warning(
            "ColumnStandardizer not available, using fallback column detection"
        )
...
class PlanningProgressCallback:
    ...
    def update(self, phase, status, details=None):
        ...
        logger.info(
            f"Progress Update: Phase {phase}/{self.total_phases} - {status} ({progress['percentage']:.1f}%)"
        )
    ...
    def execute_with_timeout(self, func, timeout_seconds=60):
        ...
        with self._lock:  # Thread-safe access
            ...
            try:
                ...
                future.cancel()
                raise TimeoutError(
                    f"Planning execution exceeded {timeout_seconds} seconds"
                )
...
class SixPhasePlanningEngine:
    ...
    def __init__(self, data_path: Path = None, enable_parallel: bool = True):
        self.data_path = (
            Path(data_path)
            if data_path
            else Path("/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data")
        )
        self.phase_results = []
        ...
        logger.info(
            f"Six-Phase Planning Engine initialized with data path: {data_path}"
        )

    def set_erp_data(
        self, sales_data=None, inventory_data=None, bom_data=None, supplier_data=None
    ):
    ...
    def validate_data_requirements(self):
        ...
        # Check sales data
        if self.sales_data is None or (
            hasattr(self.sales_data, "empty") and self.sales_data.empty
        ):
            ...
            date_col = next(
                (col for col in date_cols if col in self.sales_data.columns), None
            )
            qty_col = next(
                (col for col in qty_cols if col in self.sales_data.columns), None
            )
            ...
            missing_cols = [
                col for col in required_sales_cols if col not in self.sales_data.columns
            ]
            if missing_cols:
        ...
        if self.inventory_data is None or (
            hasattr(self.inventory_data, "empty") and self.inventory_data.empty
        ):
            ...
            item_col = next(
                (col for col in item_cols if col in self.inventory_data.columns), None
            )
            qty_col = next(
                (col for col in qty_cols if col in self.inventory_data.columns), None
            )
        ...
        # Check BOM data
        if self.bom_data is None or (
            hasattr(self.bom_data, "empty") and self.bom_data.empty
        ):
            validation_results["warnings"].append(
            ...
            parent_col = next(
                (col for col in parent_cols if col in self.bom_data.columns), None
            )
            component_col = next(
                (col for col in component_cols if col in self.bom_data.columns), None
            )
            qty_col = next(
                (col for col in qty_cols if col in self.bom_data.columns), None
            )
            ...
            if not parent_col or not component_col or not qty_col:
                ...
                validation_results["warnings"].append(f"Missing BOM columns: {missing}")

            validation_results["data_summary"]["bom"] = {
                "rows": len(self.bom_data),
                "status": "Available",
                "unique_parents": (
                    self.bom_data[parent_col].nunique() if parent_col else 0
                ),
            }

        # Check supplier data
        if self.supplier_data is None or (
            hasattr(self.supplier_data, "empty") and self.supplier_data.empty
        ):
            validation_results["warnings"].append(
    ...
    def execute_phase_with_logging(self, phase_func, phase_name):
        ...
        try:
            ...
            logger.debug(
                f"Phase output data size: {self._get_data_size(result.output_data)}"
            )
            return result
    ...
    def _internal_execute_cycle(self, callback=None) -> List[PlanningPhaseResult]:
        ...
        phase1_result = self.execute_phase_with_logging(
            self.phase1_demand_consolidation, "Phase 1: Demand Consolidation"
        )
        self.phase_results.append(phase1_result)
        if callback:
            callback.complete_phase(
                1, "Demand Consolidation", phase1_result.status == "Completed"
            )
        ...
        phase2_result = self.execute_phase_with_logging(
            self.phase2_inventory_assessment, "Phase 2: Inventory Assessment"
        )
        self.phase_results.append(phase2_result)
        if callback:
            callback.complete_phase(
                2, "Inventory Assessment", phase2_result.status == "Completed"
            )
        ...
        phase3_result = self.execute_phase_with_logging(
            self.phase3_net_requirements, "Phase 3: Net Requirements Calculation"
        )
        self.phase_results.append(phase3_result)
        if callback:
            callback.complete_phase(
                3, "Net Requirements Calculation", phase3_result.status == "Completed"
            )

        # Phase 4: BOM Explosion (moved from phase 2, only for net requirements)
        if callback:
            callback.update(
                4, "Running", "Executing BOM explosion for net requirements"
            )
        ...
        if callback:
            callback.complete_phase(
                4, "BOM Explosion", phase4_result.status == "Completed"
            )

        # Phase 5: Procurement & Production Planning (combined)
        if callback:
            callback.update(
                5, "Running", "Executing procurement and production planning"
            )
        ...
        if callback:
            callback.complete_phase(
                5, "Procurement & Production", phase5_result.status == "Completed"
            )
        ...
        phase6_result = self.execute_phase_with_logging(
            self.phase6_optimization_output, "Phase 6: Optimization & Output Generation"
        )
        self.phase_results.append(phase6_result)
        if callback:
            callback.complete_phase(
                6, "Optimization & Output", phase6_result.status == "Completed"
            )

        logger.info("=" * 60)
        logger.info("PLANNING ENGINE EXECUTION COMPLETED")
        logger.info(
            f"Total phases completed: {len([p for p in self.phase_results if p.status == 'Completed'])}/{len(self.phase_results)}"
        )
        logger.info("=" * 60)
    ...
    def execute_full_planning_cycle(
        self, max_time: int = 60, callback=None, validate_data=True
    ) -> List[PlanningPhaseResult]:
        """
        ...
        if validate_data:
            ...
            if not validation["valid"]:
                ...
                logger.info(
                    f"Data validation passed - Processing {validation['data_summary']}"
                )
        ...
        try:
            ...
            execute_func = lambda: self._internal_execute_cycle(
                callback=self.progress_callback
            )

            result = timeout_manager.execute_with_timeout(
                execute_func, timeout_seconds=max_time
            )
        ...
        try:
            ...
            if sales_data is not None and len(sales_data) > 0:
                ...
                accuracy_metrics = self._calculate_forecast_accuracy(
                    ts_data, self.unified_forecast
                )

                # ENHANCED: Analyze inventory risk and stockout probability
                inventory_risk_analysis = self._analyze_inventory_stockout_risk(
                ...
                details = {
                    ...
                    "high_risk_items": len(
                        inventory_risk_analysis.get("high_risk", [])
                    ),
                    "medium_risk_items": len(
                        inventory_risk_analysis.get("medium_risk", [])
                    ),
                    "stockout_probability": inventory_risk_analysis.get(
                        "avg_stockout_prob", 0
                    ),
                    "critical_shortage_items": inventory_risk_analysis.get(
                        "critical_items", []
                    ),
                }
                ...
                status = "Completed"
                logger.info(
                    f"Phase 1 completed: {len(forecasts)} forecast sources unified"
                )
            else:
    ...
    def phase3_net_requirements(self) -> PlanningPhaseResult:
        ...
        try:
            if (
                not hasattr(self, "unified_forecast")
                or self.unified_forecast is None
                or self.unified_forecast.empty
            ):
            ...
            for style in self.unified_forecast["style"].unique():
                ...
                demands.append(
                    {"style": style, "quantity": style_demand, "date": required_date}
                )

            # Process all demands through multi-level netting
            netting_results = self.netting_system.batch_net_demands(
                demands, parallel=True
            )
            ...
            for result in netting_results:
                ...
                i01_inventory = (
                    self.inventory_data.get("I01", {}).get(style, 0)
                ...
                g00_inventory = (
                    self.inventory_data.get("G00", {}).get(style, 0)
                    if remaining_demand > 0
                    else 0
                )
                g02_inventory = (
                    self.inventory_data.get("G02", {}).get(style, 0)
                    if remaining_demand > 0
                    else 0
                )
                ...
                ko_balance = (
                    self.inventory_data.get("KO", {}).get(style, 0)
                    if remaining_demand > 0
                    else 0
                )
            ...
            inventory_coverage = (
                ((total_demand - total_net) / total_demand * 100)
                if total_demand > 0
                else 0
            )

            logger.info(
                f"✅ Net requirements calculated for {len(net_requirements)} styles"
            )
            logger.info(
                f"   Total demand: {total_demand:.0f}, Net requirement: {total_net:.0f}"
            )
        ...
        return PlanningPhaseResult(
            ...
            details={
                ...
                "inventory_coverage": (
                    inventory_coverage if "inventory_coverage" in locals() else 0
                ),
            },
            errors=errors,
            warnings=warnings,
    ...
    def phase4_bom_explosion_net(self) -> PlanningPhaseResult:
        ...
        try:
            ...
            if not hasattr(self, "net_requirements") or not self.net_requirements:
                warnings.append(
                    "No net requirements to process - all demand covered by inventory"
                )
                ...
                return PlanningPhaseResult(
                    ...
                    execution_time=(datetime.now() - start_time).total_seconds(),
                    details={
                        "message": "No BOM explosion needed - all demand covered by inventory"
                    },
                    errors=[],
                    warnings=warnings,
            ...
            if bom_data is not None:
                ...
                yarn_consumption = self._enhanced_yarn_consumption_with_fabric_specs(
                    self.exploded_bom, self.unified_forecast
                )
                ...
                production_schedule = self._calculate_production_timing(
                    self.unified_forecast, yarn_consumption
                )

                details = {
                    "parent_items": (
                        len(bom_data["Parent Item"].unique())
                        if "Parent Item" in bom_data.columns
                        else 0
                    ),
                    ...
                    "total_yarn_consumption_lbs": sum(
                        y.get("total_required_lbs", y.get("total_required", 0))
                        for y in yarn_consumption
                    ),
                    "critical_yarn_items": len(
                        [y for y in yarn_consumption if y.get("priority") == "High"]
                    ),
                    "conversion_method": (
                        yarn_consumption[0].get("conversion_method", "standard")
                        if yarn_consumption
                        else "none"
                    ),
                    "production_start_date": production_schedule.get(
                        "production_start_date", "TBD"
                    ),
                    "yarn_order_deadline": production_schedule.get(
                        "yarn_order_deadline", "TBD"
                    ),
                ...
                status = "Completed"
                logger.info(
                    f"Phase 2 completed: {len(self.exploded_bom)} BOM items exploded"
                )
            else:
        ...
        try:
            ...
            if inventory_data is not None and self.exploded_bom is not None:
                ...
                self.net_requirements = self._calculate_net_requirements(
                    self.exploded_bom, inventory_data
                )
                ...
                yarn_procurement_timing = self._calculate_yarn_procurement_timing(
                    yarn_shortage_analysis, self.unified_forecast
                )

                # Identify critical shortages
                critical_items = self._identify_critical_shortages(
                    self.net_requirements
                )

                # Calculate inventory coverage
                coverage_analysis = self._analyze_inventory_coverage(
                    inventory_data, self.net_requirements
                )

                # Handle both standardized (lowercase) and original column names
                planning_balance_col = (
                    "planning_balance"
                    if "planning_balance" in inventory_data.columns
                    else "Planning Balance"
                )
                on_order_col = (
                    "on_order" if "on_order" in inventory_data.columns else "On Order"
                )

                details = {
                    "on_hand_inventory": (
                        f"{inventory_data[planning_balance_col].sum():,.0f} units"
                        if planning_balance_col in inventory_data.columns
                        else "0 units"
                    ),
                    "on_order_quantity": (
                        f"{inventory_data[on_order_col].sum():,.0f} units"
                        if on_order_col in inventory_data.columns
                        else "0 units"
                    ),
                    "gross_requirements": (
                        f"{self.exploded_bom['total_required'].sum():,.0f} units"
                        if hasattr(self.exploded_bom, "sum")
                        else "TBD"
                    ),
                    "net_requirements": (
                        f"{self.net_requirements['net_required'].sum():,.0f} units"
                        if hasattr(self.net_requirements, "sum")
                        else "TBD"
                    ),
                    ...
                    "yarn_shortages_identified": (
                        yarn_shortage_analysis.get("total_yarn_items", 0)
                        if isinstance(yarn_shortage_analysis, dict)
                        else 0
                    ),
                    "critical_yarn_shortages": (
                        yarn_shortage_analysis.get("critical_shortages", 0)
                        if isinstance(yarn_shortage_analysis, dict)
                        else 0
                    ),
                    "total_yarn_shortage_value": (
                        sum(y.get("estimated_cost", 0) for y in yarn_procurement_timing)
                        if isinstance(yarn_procurement_timing, list)
                        else 0
                    ),
                    "earliest_order_date": (
                        min(
                            [
                                p.get("recommended_order_date", "9999-12-31")
                                for p in yarn_procurement_timing
                            ]
                        )
                        if yarn_procurement_timing
                        and isinstance(yarn_procurement_timing, list)
                        and len(yarn_procurement_timing) > 0
                        else "N/A"
                    ),
                    "yarn_procurement_actions": (
                        len(yarn_procurement_timing)
                        if isinstance(yarn_procurement_timing, list)
                        else 0
                    ),
                }
                ...
                status = "Completed"
                logger.info(
                    f"Phase 3 completed: Net requirements calculated for {len(self.net_requirements)} items"
                )
            else:
    ...
    def phase5_procurement_production(self) -> PlanningPhaseResult:
        ...
        try:
            if self.net_requirements is not None and len(self.net_requirements) > 0:
                    ...
                    if item["net_required"] > 0:
                        ...
                        recommendation = ProcurementRecommendation(
                            ...
                            recommended_quantity=max(
                                eoq_result["eoq"], item["net_required"]
                            ),
                            eoq=eoq_result["eoq"],
                ...
                supplier_optimization = self._optimize_supplier_allocation(
                    self.procurement_plan
                )
                ...
                details = {
                    ...
                    "multi_sourcing_items": supplier_optimization.get(
                        "multi_sourced", 0
                    ),
                ...
                status = "Completed"
                logger.info(
                    f"Phase 4 completed: {len(self.procurement_plan)} items optimized"
                )
            else:
        ...
        try:
            ...
            if not self.procurement_plan or len(self.procurement_plan) == 0:
                ...
                if self.inventory_data is not None and not self.inventory_data.empty:
                    ...
                    low_stock_items = (
                        ...
                        if "Planning Balance" in self.inventory_data.columns
                        else pd.DataFrame()
                    )
            ...
            if self.procurement_plan and len(self.procurement_plan) > 0:
                ...
                self.supplier_assignments = self._assign_suppliers(
                    self.procurement_plan, supplier_scores
                )
                ...
                status = "Completed"
                logger.info(
                    f"Phase 5 completed: {len(self.supplier_assignments)} supplier assignments made"
                )
            else:
    ...
    def _phase6_output_generation_old(self) -> PlanningPhaseResult:
        ...
        try:
            ...
            try:
                self.final_output["planning_horizon"] = (
                    f"{self.config.get('forecast_horizon', 90)} days"
                )
            except Exception as e:
            ...
            try:
                self.final_output["supplier_assignments"] = (
                    self.supplier_assignments
                    if hasattr(self, "supplier_assignments")
                    else {}
                )
            except Exception as e:
            ...
            try:
                self.final_output["risk_mitigation"] = (
                    self._generate_risk_mitigation_plan()
                )
            except Exception as e:
            ...
            try:
                self.final_output["approval_workflow"] = (
            ...
            details = {
                ...
                "approval_workflow": self.final_output["approval_workflow"].get(
                    "status", "Pending"
                ),
                "next_review_date": (datetime.now() + timedelta(days=7)).strftime(
                    "%Y-%m-%d"
                ),
            ...
            status = "Completed"
            logger.info(
                f"Phase 6 completed: {len(self.final_output['procurement_orders'])} purchase orders generated"
            )
            ...
            for _, item in inventory_data.iterrows():
                ...
                if daily_consumption > 0:
                    ...
                    stockout_prob = 1 / (
                        1
                        + np.exp(
                            -(self.config["critical_inventory_days"] - days_of_supply)
                            / 10
                        )
                    )
        ...
        try:
            ...
            if fabric_style:
                ...
                style_match = fabric_specs[
                    fabric_specs.apply(
                        lambda row: any(
                            fabric_style.lower() in str(cell).lower()
                ...
                if not style_match.empty:
                    ...
                    weight_cols = [
                        col
                        for col in style_match.columns
                        if any(
                            weight_term in col.lower()
                    ...
                    if weight_cols:
                        ...
                        if pd.notna(weight_value) and isinstance(
    ...
    def _exponential_smoothing_forecast(
        self, ts_data: pd.DataFrame, alpha: float = 0.3
    ) -> Dict:
    ...
    def _ml_forecast(self, ts_data: pd.DataFrame, model_type: str) -> Dict:
        ...
        try:
            ...
            feature_cols = [
                col for col in ts_data.columns if col not in ["Date", "Qty Shipped"]
            ]
    ...
    def _prophet_forecast(self, ts_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            forecast_values = (
                forecast["yhat"].tail(self.config["forecast_horizon"]).tolist()
            )
    ...
    def _calculate_forecast_accuracy(
        self, actual_data: pd.DataFrame, forecast: Dict
    ) -> Dict:
        """Calculate forecast accuracy metrics"""
        ...
        if forecast and "daily_forecast" in forecast:
            ...
            mape = (
                abs(forecast_mean - actual_mean) / actual_mean * 100
                if actual_mean > 0
                else 0
            )
    ...
    def _explode_bom(self, bom_data: pd.DataFrame, forecast) -> pd.DataFrame:
        ...
        if len(bom_data) > 100:
            ...
            for _, bom_item in bom_data.iterrows():
                ...
                parent_demand = (
                    total_demand / len(bom_data["Parent Item"].unique())
                    if "Parent Item" in bom_data.columns
                    else total_demand
                )
    ...
    def _explode_bom_parallel(
        self, bom_data: pd.DataFrame, total_demand: float
    ) -> pd.DataFrame:
        ...
        chunks = [
            bom_data.iloc[i : i + chunk_size]
            for i in range(0, len(bom_data), chunk_size)
        ]
    ...
    def _parallel_forecast(self, sales_data: pd.DataFrame) -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            futures = {
                executor.submit(
                    self._moving_average_forecast, ts_data
                ): "moving_average",
                executor.submit(
                    self._exponential_smoothing_forecast, ts_data
    ...
    def _analyze_variants(self, exploded_bom: pd.DataFrame) -> Dict:
        ...
        if "description" in exploded_bom.columns:
            ...
            dyed_items = exploded_bom[
                exploded_bom["description"].str.contains("dyed", case=False, na=False)
            ]
            greige_items = exploded_bom[
                exploded_bom["description"].str.contains(
                    "greige|grey", case=False, na=False
                )
            ]
        ...
        return {"variant_count": variant_count, "critical_count": critical_count}
    ...
    def _calculate_net_requirements(
        self, exploded_bom: pd.DataFrame, inventory: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        desc_col = (
            "description" if "description" in inventory.columns else "Description"
        )
        balance_col = (
            "planning_balance"
            if "planning_balance" in inventory.columns
            else "Planning Balance"
        )
        ...
        for _, bom_item in exploded_bom.iterrows():
            ...
            if desc_col in inventory.columns:
                matching = inventory[
                    inventory[desc_col].str.contains(
                        bom_item.get("description", ""), case=False, na=False
                    )
                ]
                if not matching.empty:
                    on_hand = (
                        matching[balance_col].sum()
                        if balance_col in matching.columns
                        else 0
                    )
                    on_order = (
                        matching[order_col].sum()
                        if order_col in matching.columns
                        else 0
                    )
                    supplier = (
                        matching[supplier_col].iloc[0]
                        if supplier_col in matching.columns
                        else "TBD"
                    )
                    unit_cost = (
                        matching[cost_col].iloc[0]
                        if cost_col in matching.columns
                        else 3.5
                    )
        ...
        if (
            ...
            and balance_col in inventory.columns
        ):
    ...
    def _identify_critical_shortages(self, net_requirements: pd.DataFrame) -> List:
        ...
        if "net_required" in net_requirements.columns:
            ...
            critical = shortage_items.nlargest(10, "net_required")[
                "description"
            ].tolist()

        return critical

    def _analyze_inventory_coverage(
        self, inventory: pd.DataFrame, net_requirements: pd.DataFrame
    ) -> Dict:
        ...
        if consumed_col in inventory.columns and balance_col in inventory.columns:
            ...
            avg_coverage_days = (
                current_inventory / daily_consumption if daily_consumption > 0 else 0
            )
            ...
            return {"avg_coverage_days": avg_coverage_days, "anomalies": anomalies}
    ...
    def _calculate_eoq(self, item: pd.Series) -> Dict:
        ...
        if annual_demand > 0 and unit_cost > 0:
            ...
            total_cost = (
                annual_holding_cost + annual_ordering_cost + (annual_demand * unit_cost)
            )

            # Calculate savings vs current method (assume current = demand)
            current_cost = (
                annual_demand * unit_cost * 1.1
            )  # Assume 10% higher without optimization
            savings = max(0, current_cost - total_cost)

            return {"eoq": eoq, "total_cost": total_cost, "savings": savings}

        return {"eoq": self.config["min_order_quantity"], "total_cost": 0, "savings": 0}
    ...
    def _generate_rationale(self, item: pd.Series, eoq_result: Dict) -> str:
        ...
        priority = self._determine_priority(item)
        savings_pct = (
            (eoq_result["savings"] / eoq_result["total_cost"] * 100)
            if eoq_result["total_cost"] > 0
            else 0
        )
    ...
    def _optimize_supplier_allocation(
        self, procurement_plan: List[ProcurementRecommendation]
    ) -> Dict:
        ...
        avg_adjustment = (
            total_adjustment / len(procurement_plan) if procurement_plan else 0
        )

        return {"avg_adjustment": avg_adjustment, "multi_sourced": multi_sourced}
        ...
        try:
            ...
            if inventory_file.exists():
                ...
                if "Supplier" in data.columns:
                    return (
                        data[["Supplier", "Cost/Pound", "Planning Balance"]]
                        .groupby("Supplier")
                        .agg({"Cost/Pound": "mean", "Planning Balance": "sum"})
                        .reset_index()
                    )
        except Exception as e:
    ...
    def _evaluate_suppliers(self, supplier_data: pd.DataFrame) -> Dict:
        ...
        if not supplier_data.empty and "Supplier" in supplier_data.columns:
            for _, supplier in supplier_data.iterrows():
                ...
                cost_score = 100 - min(
                    100, supplier.get("Cost/Pound", 50)
                )  # Lower cost = higher score
                volume_score = min(
                    100, supplier.get("Planning Balance", 0) / 100
                ...
                supplier_scores[supplier_name] = {
                    ...
                    "risk_level": (
                        "Low"
                        if total_score > 85
                        else "Medium" if total_score > 70 else "High"
                    ),
                }
    ...
    def _assign_suppliers(
        self, procurement_plan: List[ProcurementRecommendation], supplier_scores: Dict
    ) -> Dict:
    ...
    def _analyze_supplier_risks(self, assignments: Dict) -> Dict:
        ...
        return {
            "high_risk_count": sum(
                ...
                if supplier_counts[s] > total_assignments * 0.3
            ),
            "medium_risk_count": sum(
                1
                for s in supplier_counts
                if 0.15 < supplier_counts[s] / total_assignments <= 0.3
            ),
            "diversification_index": diversification_index,
            "contingency_count": max(
                0, 3 - len(supplier_counts)
            ),  # Need at least 3 suppliers
        }
    ...
    def _check_supplier_financial_health(self, supplier_scores: Dict) -> Dict:
        ...
        concerning = [
            s
            for s, scores in supplier_scores.items()
        ...
        if self.procurement_plan:
            for i, rec in enumerate(self.procurement_plan):
                po = {
                    ...
                    "quantity": rec.recommended_quantity,
                    "unit_cost": (
                        rec.total_cost / rec.recommended_quantity
    ...
    def _analyze_yarn_shortages(self, exploded_bom, inventory_data) -> Dict:
        ...
        try:
            if (
                exploded_bom is not None
                and not exploded_bom.empty
                and inventory_data is not None
                and not inventory_data.empty
            ):
                # Get unique yarn items from BOM
                if "item_code" in exploded_bom.columns:
                    yarn_items = exploded_bom[
                        exploded_bom["item_code"].str.contains(
                            "YRN|YARN", case=False, na=False
                        )
                    ]
                    ...
                    for _, item in yarn_items.iterrows():
                        ...
                        if "Item Code" in inventory_data.columns:
                            inv_item = inventory_data[
                                inventory_data["Item Code"] == item_code
                            ]
                            if not inv_item.empty:
                                available = (
                                    inv_item["Planning Balance"].sum()
                                    if "Planning Balance" in inv_item.columns
                                    else 0
                                )
                ...
                return {
                    ...
                    "critical_shortages": len(
                        [s for s in shortages.values() if s["urgency"] == "High"]
                    ),
                    "total_shortage_quantity": sum(
                        s["shortage"] for s in shortages.values()
                    ),
    ...
    def _create_approval_workflow(self) -> Dict:
        ...
        if (
            ...
            and self.final_output
            and "procurement_orders" in self.final_output
        ):
            total_value = sum(
                po["total_value"]
                for po in self.final_output.get("procurement_orders", [])
            )
        else:
            total_value = 0
        ...
        if total_value < 50000:
            workflow["levels"].append(
                {"level": 1, "approver": "Procurement Manager", "threshold": "$50,000"}
            )
            ...
            workflow["levels"].append(
                {
                    "level": 3,
                    "approver": "C-Level Executive",
                    "threshold": "Above $250,000",
                }
            )
    ...
    def _calculate_planning_kpis(self) -> Dict:
        ...
        # Forecast accuracy
        if self.unified_forecast:
            kpis["forecast_confidence"] = (
                self.unified_forecast.get("confidence", 0) * 100
            )
        ...
        if self.supplier_assignments:
            kpis["supplier_diversification"] = len(
                set(a["primary_supplier"] for a in self.supplier_assignments.values())
            )
    ...
    def _export_results(self) -> Dict:
        ...
        try:
            ...
            if self.procurement_plan:
                csv_file = (
                    self.data_path
                    / f"procurement_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                )
                procurement_df = pd.DataFrame(
                    [asdict(rec) for rec in self.procurement_plan]
                )
    ...
    def _calculate_production_timing(self, forecast, yarn_consumption):
        ...
        try:
            ...
            schedule = {
                "production_start_date": datetime.now()
                + timedelta(days=self.config["production_lead_time"]),
                "yarn_order_deadline": datetime.now()
                + timedelta(days=7),  # Order yarn ASAP
                "critical_milestones": [],
            ...
            for i, yarn in enumerate(yarn_consumption[:5]):  # Top 5 yarns
                milestone = {
                    ...
                    "required_delivery": schedule["production_start_date"]
                    - timedelta(days=3),
                    ...
                    procurement_plan.append(
                        {
                            ...
                            "estimated_cost": shortage_data.get("shortage", 0)
                            * 6.0,  # Rough estimate
                        }
                    )
                    ...
                    procurement_plan.append(
                        {
                            ...
                            "estimated_cost": yarn_shortage.get("shortage", 0)
                            * 6.0,  # Rough estimate
                        }
                    )
            ...
            if isinstance(exploded_bom, list):
                for bom_item in exploded_bom:
                    ...
                    if "yarn" in component.lower() or "lycra" in component.lower():
                        ...
                        required_qty_lbs = self._convert_yards_to_pounds(
                            required_qty_yards, fabric_style
                        )

                        yarn_requirements[component] += (
                            required_qty_lbs * self.config["yarn_safety_buffer"]
                        )
                ...
                yarn_consumption.append(
                    {
                        ...
                        "weekly_requirement_lbs": quantity_lbs
                        / (self.config["forecast_horizon"] / 7),
                        "priority": (
                            "High" if quantity_lbs > 500 else "Medium"
                        ),  # Adjusted for pounds
                        "unit": "lbs",
                        "conversion_method": (
                            "fabric_specs" if fabric_specs is not None else "standard"
                        ),
                    }
                )

            return sorted(
                yarn_consumption, key=lambda x: x["total_required_lbs"], reverse=True
            )
        except Exception as e:
...
class PlanningEngineAdapter:
        ...
        for result in self.last_results:
            ...
            if result.phase_number == 1 and self.engine.unified_forecast:
                ...
                phase_data["procurement_summary"] = {
                    ...
                    "total_value": sum(
                        r.total_cost for r in self.engine.procurement_plan
                    ),
                    "urgent_items": len(
                        [
                            r
                            for r in self.engine.procurement_plan
    ...
    def get_critical_alerts(self):
        ...
        if self.engine and self.last_results:
            ...
            if (
                hasattr(self.engine, "net_requirements")
                and self.engine.net_requirements is not None
            ):
    ...
    def get_inventory_analysis(self):
        ...
        if (
            "planning_balance" in self.inventory_data.columns
            and "cost_per_pound" in self.inventory_data.columns
        ):
            analysis["total_value"] = (
                self.inventory_data["planning_balance"]
                * self.inventory_data["cost_per_pound"]
            ).sum()
            analysis["low_stock_items"] = len(
                self.inventory_data[self.inventory_data["planning_balance"] < 100]
            )
            analysis["overstocked_items"] = len(
                self.inventory_data[self.inventory_data["planning_balance"] > 10000]
            )
    ...
    def get_supplier_risk_assessment(self):
        ...
        supplier_summary = (
            self.inventory_data.groupby("supplier")
            .agg({"planning_balance": "sum", "cost_per_pound": "mean"})
            .to_dict("index")
            if "planning_balance" in self.inventory_data.columns
            else {}
        )
...
if __name__ == "__main__":
    ...
    if recommendations:
        ...
        for rec in recommendations[:3]:
            print(
                f"  - {rec['description']}: {rec['quantity']} units (Priority: {rec['priority']})"
            )
