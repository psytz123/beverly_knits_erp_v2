...
class PlanningDataAPI:
    ...
    def _get_demand_forecast(self, request: PlanningRequest) -> Dict[str, Any]:
        ...
        forecast_data["summary"] = {
            ...
            "total_forecasted_demand": sum(
                f["summary"]["total_demand"] for f in forecast_data["forecasts"]
            ),
    ...
    def _get_material_requirements(self, request: PlanningRequest) -> Dict[str, Any]:
        ...
        for product in products:
            ...
            materials = [
                {
                    "material_id": f"MAT_{product}_{i}",
                    "quantity_per_unit": np.random.randint(1, 5),
                }
                for i in range(1, 4)
            ]
            ...
            for material in materials:
                mat_req = {
                    ...
                    "required_quantity": requirement["planned_quantity"]
                ...
                mat_req["order_date"] = (
                    datetime.now() - timedelta(days=mat_req["lead_time_days"])
                ).isoformat()
        ...
        mrp_data["summary"] = {
            ...
            "materials_with_shortage": sum(
                1 for m in all_materials if m["shortage"] > 0
            ),
    ...
    def _get_capacity_analysis(self, request: PlanningRequest) -> Dict[str, Any]:
        ...
        for wc in work_centers:
            ...
            for day in range(request.planning_horizon_days):
                ...
                daily_data["utilization_pct"] = min(
                    100,
                    (daily_data["planned_hours"] / daily_data["available_hours"]) * 100,
                )
                analysis["daily_capacity"].append(daily_data)
                analysis["planned_load"] += daily_data["planned_hours"]

            analysis["utilization"] = (
                analysis["planned_load"] / analysis["capacity_hours"]
            ) * 100
            analysis["status"] = (
                "overloaded"
                if analysis["utilization"] > 100
                else "optimal" if analysis["utilization"] > 70 else "underutilized"
            )
        ...
        capacity_data["summary"] = {
            ...
            "avg_utilization": np.mean(
                [a["utilization"] for a in capacity_data["capacity_analysis"]]
            ),
            "overloaded_centers": sum(
                1
                for a in capacity_data["capacity_analysis"]
                if a["status"] == "overloaded"
            ),
            "total_capacity_hours": sum(
                a["capacity_hours"] for a in capacity_data["capacity_analysis"]
            ),
            "total_planned_hours": sum(
                a["planned_load"] for a in capacity_data["capacity_analysis"]
            ),
        }
    ...
    def _get_production_schedule(self, request: PlanningRequest) -> Dict[str, Any]:
        ...
        for product in products:
            ...
            for _ in range(num_orders):
                start_day = np.random.randint(
                    0, max(1, request.planning_horizon_days - 5)
                )
                ...
                order = {
                    ...
                    "end_date": (
                        datetime.now() + timedelta(days=start_day + duration)
                    ).isoformat(),
                    "duration_days": duration,
                    "status": np.random.choice(
                        ["planned", "confirmed", "in_progress"], p=[0.5, 0.3, 0.2]
                    ),
                    "priority": np.random.choice(
                        ["low", "normal", "high"], p=[0.2, 0.6, 0.2]
                    ),
                    "work_center": np.random.choice(
                        ["WC_CUTTING", "WC_SEWING", "WC_FINISHING"]
                    ),
                }
        ...
        schedule_data["summary"] = {
            ...
            "total_quantity": sum(
                o["quantity"] for o in schedule_data["scheduled_orders"]
            ),
            "orders_by_status": {},
            "orders_by_priority": {},
        }
        ...
        for status in ["planned", "confirmed", "in_progress"]:
            count = sum(
                1 for o in schedule_data["scheduled_orders"] if o["status"] == status
            )
        ...
        for priority in ["low", "normal", "high"]:
            count = sum(
                1
                for o in schedule_data["scheduled_orders"]
    ...
    def _get_procurement_plan(self, request: PlanningRequest) -> Dict[str, Any]:
        ...
        for material in materials:
            ...
            if np.random.random() > 0.3:  # 70% chance of needing procurement
                po = {
                    ...
                    "requested_date": (
                        datetime.now() + timedelta(days=np.random.randint(7, 30))
                    ).isoformat(),
                    "lead_time_days": np.random.randint(7, 30),
                    "status": np.random.choice(
                    ...
                    "urgency": np.random.choice(
                        ["normal", "urgent", "critical"], p=[0.7, 0.2, 0.1]
                    ),
                }
        ...
        procurement_data["summary"] = {
            ...
            "total_value": sum(
                po["total_value"] for po in procurement_data["purchase_orders"]
            ),
            "unique_materials": len(
                set(po["material_id"] for po in procurement_data["purchase_orders"])
            ),
            "unique_suppliers": len(
                set(po["supplier"] for po in procurement_data["purchase_orders"])
            ),
            "orders_by_status": {},
            "urgent_orders": sum(
                1
                for po in procurement_data["purchase_orders"]
                if po["urgency"] in ["urgent", "critical"]
            ),
        }
    ...
    def create_planning_session(
        self, session_name: str, parameters: Dict[str, Any]
    ) -> str:
    ...
    def update_planning_phase(
        self, session_id: str, phase: PlanningPhase, phase_data: Dict[str, Any]
    ) -> bool:
...
def create_planning_request(
    request_type: str, horizon_days: int, products: Optional[List[str]] = None, **kwargs
) -> PlanningRequest:
