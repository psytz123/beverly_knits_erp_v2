...
class ProductionFlowTracker:
    ...
    def _calculate_yarn_requirements(
        self, fabric_quantity: float, bom_data: Dict[str, float]
    ) -> Dict[str, float]:
        ...
        for yarn_id, percentage in bom_data.items():
            ...
            yarn_pounds = (
                fabric_quantity * percentage * 0.45
    ...
    def move_batch_to_stage(
        ...
        if target_stage not in valid_transitions:
            raise ValueError(
                f"Invalid transition from {batch.current_stage} to {target_stage}"
            )
        ...
        # Update stage inventory
        self._update_stage_inventory(
            batch.style_id, batch.current_stage, target_stage, move_quantity
        )
    ...
    def _get_valid_transitions(
        self, current_stage: ProductionStage
    ...
    def calculate_f01_replenishment_needs(
        self, safety_stock_days: int = 20
    ) -> Dict[str, float]:
        """
    ...
    def generate_production_plan(
        self,
        replenishment_needs: Dict[str, float],
        bom_data: Dict[str, Dict[str, float]],
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_yarn_impact_analysis(
        self, production_plan: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    def export_production_metrics(self) -> Dict[str, Any]:
        ...
        for stage in ProductionStage:
            if stage.is_wip:
                batches_in_stage = [
                    b
                    for b in self.production_batches.values()
                ...
                metrics["stage_metrics"][stage.value] = {
                    ...
                    "average_age_days": (
                        np.mean([b.days_in_production for b in batches_in_stage])
                        if batches_in_stage
                        else 0
                    ),
                }
...
if __name__ == "__main__":
    ...
    for order in plan[:3]:  # Show first 3 orders
        print(
            f"   Style {order['style_id']}: {order['quantity']} units, Priority: {order['priority']}"
        )
