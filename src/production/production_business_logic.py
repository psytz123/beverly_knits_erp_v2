...
class ProductionBusinessLogic:
    ...
    def calculate_demand_forecast(
        self, style: str, sales_history: pd.DataFrame, forecast_horizon: int = None
    ) -> Dict:
        """
        ...
        if date_col and qty_col:
            try:
                # Convert to datetime
                sales_history[date_col] = pd.to_datetime(
                    sales_history[date_col], errors="coerce"
                )
                ...
                if not sales_history.empty:
                    ...
                    date_range = (
                        sales_history[date_col].max() - sales_history[date_col].min()
                    ).days
                    ...
                    if date_range > 0:
                        ...
                        # Weekly pattern
                        sales_history["week"] = (
                            sales_history[date_col].dt.isocalendar().week
                        )
                        weekly_sales = sales_history.groupby("week")[qty_col].sum()
                        weekly_avg = (
                            weekly_sales.mean() if not weekly_sales.empty else 0
                        )
                        ...
                        monthly_avg = (
                            monthly_sales.mean() if not monthly_sales.empty else 0
                        )

                        # Calculate trend
                        if date_range >= 30:
                            first_month = sales_history[
                                sales_history[date_col]
                                <= sales_history[date_col].min() + timedelta(days=30)
                            ]
                            last_month = sales_history[
                                sales_history[date_col]
                                >= sales_history[date_col].max() - timedelta(days=30)
                            ]

                            first_month_qty = (
                                first_month[qty_col].sum()
                                if not first_month.empty
                                else 0
                            )
                            last_month_qty = (
                                last_month[qty_col].sum() if not last_month.empty else 0
                            )

                            if first_month_qty > 0:
                                growth_rate = (
                                    last_month_qty - first_month_qty
                                ) / first_month_qty
                                if growth_rate > 0.1:
                        ...
                        if daily_avg > 0:
                            ...
                            if sales_history[qty_col].std() > 0:
                                cv = (
                                    sales_history[qty_col].std()
                                    / sales_history[qty_col].mean()
                                )
                                if cv > 0.5:
        ...
        if forecast["forecast_qty"] == 0 and qty_col:
            ...
            if avg_order > 0 and order_count > 0:
                ...
                forecast["forecast_qty"] = (
                    avg_order * order_count * (forecast_horizon / 30)
                )
    ...
    def calculate_inventory_urgency(
        self, style: str, current_inventory: float, allocated_qty: float, forecast: Dict
    ) -> Dict:
        """
        ...
        daily_demand = forecast.get("forecast_qty", 0) / max(
            1, self.FORECAST_HORIZON_DAYS
        )
        ...
        if urgency["available"] < urgency["reorder_point"]:
            ...
            urgency["reorder_qty"] = max(
                self.MIN_PRODUCTION_BATCH, max_stock - urgency["available"]
            )

            # Round to batch size
            batch_size = self.MIN_PRODUCTION_BATCH
            urgency["reorder_qty"] = (
                np.ceil(urgency["reorder_qty"] / batch_size) * batch_size
            )

            # Cap at maximum batch
            urgency["reorder_qty"] = min(
                urgency["reorder_qty"], self.MAX_PRODUCTION_BATCH
            )

        return urgency

    def calculate_production_priority(
        self,
        style: str,
        customer: str,
        forecast: Dict,
        ...
        priority["factors"] = {
            ...
            "inventory_urgency": urgency.get("urgency_score", 0)
            * self.WEIGHT_INVENTORY_URGENCY,
            "material_availability": material_availability
            * self.WEIGHT_MATERIAL_AVAILABILITY,
            "production_efficiency": self.calculate_production_efficiency(style)
            * self.WEIGHT_PRODUCTION_EFFICIENCY,
        }
    ...
    def calculate_production_schedule(
        self, suggestions: List[Dict], max_capacity_per_day: float = 10000
    ) -> List[Dict]:
        """
    ...
    def validate_production_suggestion(
        ...
        if qty < self.MIN_PRODUCTION_BATCH:
            issues.append(
                f"Quantity {qty} below minimum batch {self.MIN_PRODUCTION_BATCH}"
            )
            is_valid = False
        ...
        if qty > self.MAX_PRODUCTION_BATCH:
            issues.append(
                f"Quantity {qty} exceeds maximum batch {self.MAX_PRODUCTION_BATCH}"
            )
...
def apply_business_logic(suggestions: List[Dict], analyzer=None) -> List[Dict]:
    ...
    for suggestion in suggestions:
        ...
        if (
            analyzer
            and hasattr(analyzer, "sales_data")
            and analyzer.sales_data is not None
        ):
            if "Style#" in analyzer.sales_data.columns:
                sales_history = analyzer.sales_data[
                    analyzer.sales_data["Style#"] == style
                ]
        ...
        urgency = logic.calculate_inventory_urgency(
            style, current_inv, allocated, forecast
        )
        ...
        priority = logic.calculate_production_priority(
            style, customer, forecast, urgency, material_availability, profitability
        )
