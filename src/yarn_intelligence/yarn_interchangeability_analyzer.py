...
class YarnInterchangeabilityAnalyzer:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if len(groups) == 3 and all(
                    g.replace(".", "").isdigit() for g in groups
                ):
                ...
                # Handle single denier values (150D, 200DEN, etc.)
                    ...
                    x in desc_str for x in ["D", "DEN", "DENIER"]
                ):
    ...
    def categorize_materials(self):
        ...
        for yarn_file, df in self.yarn_data.items():
            for _, row in df.iterrows():
                ...
                material_analysis = self._analyze_material_composition(
                    material_type, blend, row.get("description", "")
                )

                # Create material signature that separates spun and filament yarns
                yarn_structure = self._determine_yarn_structure(
                    row.get("description", "")
                )
    ...
    def _analyze_material_composition(
        self, material_type: str, blend: str, description: str
    ) -> Dict:
        """
        ...
        if material_percentages:
            primary_material = max(
                material_percentages.keys(), key=lambda k: material_percentages[k]
            )
        ...
        compatibility_group = self._get_compatibility_group(
            primary_material, blend_category, material_percentages
        )
        ...
        return {
            ...
            "substitution_flexibility": self._calculate_substitution_flexibility(
                material_percentages, blend_category
            ),
        }
    ...
    def _get_compatibility_group(
        self, primary_material: str, blend_category: str, percentages: Dict
    ) -> str:
        """
    ...
    def _calculate_substitution_flexibility(
        self, percentages: Dict, blend_category: str
    ) -> str:
                ...
                for size_group_idx, size_yarns in enumerate(size_compatible_groups):
                    if len(size_yarns) >= 2:
                        ...
                        interchangeable_groups[group_key] = {
                            ...
                            "suppliers": list(
                                set(yarn["supplier"] for yarn in size_yarns)
                            ),
                            "supplier_count": len(
                                set(yarn["supplier"] for yarn in size_yarns)
                            ),
                            ...
                            "compatibility_score": group_analysis[
                                "compatibility_score"
                            ],
                            "substitution_risk": group_analysis["substitution_risk"],
                            "material_flexibility": group_analysis[
                                "material_flexibility"
                            ],
                            "recommended_priority": group_analysis[
                ...
                for compatible_sig in compatible_sigs:
                    if compatible_sig in self.material_categories:
                        ...
                        if combo_key not in processed_combinations:
                            ...
                            combined_yarns = (
                                primary_yarns + self.material_categories[compatible_sig]
                            )
    ...
    def _analyze_group_compatibility(self, yarns: List[Dict]) -> Dict:
        ...
        for yarn in yarns:
            ...
            if "material_analysis" in yarn:
                ...
                flexibility_scores.append(
                    {"low": 1, "medium": 2, "high": 3}.get(flexibility, 2)
                )

        # Calculate compatibility score (0-100)
        avg_flexibility = (
            sum(flexibility_scores) / len(flexibility_scores)
        ...
        compatibility_score = min(
            100, (avg_flexibility * 20) + (supplier_diversity * 10) + 20
        )
        ...
        return {
            "cost_analysis": {
                ...
                "cost_spread_pct": (
                    ((max(costs) - min(costs)) / min(costs) * 100)
                    if len(costs) > 1
                    else 0
                ),
            },
            ...
            "material_flexibility": {
                ...
                "flexibility_range": (
                    [min(flexibility_scores), max(flexibility_scores)]
                    if flexibility_scores
                    else [2, 2]
                ),
            },
    ...
    def _sizes_within_tolerance(self, size1: Dict, size2: Dict) -> bool:
        ...
        spun_types = {
            ...
            "probably_spun",
            "metric_spun",
            "tex_specified",  # TEX can be spun or filament, but treat conservatively
        }
        ...
        if (type1_is_spun and type2_is_filament) or (
            type1_is_filament and type2_is_spun
        ):
    ...
    def get_substitution_recommendations(self, target_desc_id) -> Dict:
        ...
        for group_key, group_data in self.interchangeable_groups.items():
            if any(yarn["desc_id"] in target_ids for yarn in group_data["yarns"]):
                ...
                alternatives = [
                    yarn
                    for yarn in group_data["yarns"]
                    if yarn["desc_id"] not in target_ids
                ]

                # Sort alternatives by cost if available
                if group_data.get("cost_range"):
                    alternatives.sort(
                        key=lambda x: self._extract_cost_data(x) or float("inf")
                    )
    ...
    def generate_interchangeability_report(self) -> str:
        ...
        report.append("")
        report.append(
            "🚨 CRITICAL TEXTILE RULE: SPUN AND FILAMENT YARNS ARE NEVER INTERCHANGEABLE"
        )
        report.append("   - Cotton spun (30/1, 24/1) ≠ Filament (1/150/48, 150D)")
        report.append(
            "   - Different manufacturing processes create incompatible structures"
        )
        report.append(
            "   - Only yarns within the SAME structure type can be substituted"
        )
        report.append("")
        ...
        if total_groups > 0:
            report.append(
                f"Average Alternatives per Group: {total_alternatives/total_groups:.1f}"
            )
        report.append("")
        ...
        for group_key, group_data in sorted_groups:
            ...
            report.append(
                f"  Supplier Diversity: {group_data['supplier_count']} suppliers"
            )

            if (
                group_data.get("cost_range")
                and group_data["cost_range"].get("min") is not None
            ):
                cost_range = group_data["cost_range"]
                report.append(
                    f"  Cost Range: ${cost_range['min']:.2f} - ${cost_range['max']:.2f} (avg: ${cost_range['avg']:.2f})"
                )
...
def main():
    ...
    if "error" not in recommendations:
        print(
            f"Target Yarn: {recommendations['target_yarn'].get('supplier', 'Unknown')} - {recommendations['target_yarn'].get('description', 'N/A')}"
        )
