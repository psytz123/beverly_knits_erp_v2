...
class YarnAggregationIntelligence:
    def __init__(
        self, data_path="D:\\Agent-MCP-1-ddd\\Agent-MCP-1-dd\\ERP Data\\prompts\\5"
    ):
    ...
    def load_data(self):
        ...
        try:
            ...
            yarn_data = self.yarn_inventory[
                ["Desc#", "Description", "Supplier", "Color"]
            ].copy()
            yarn_data.columns = ["desc_id", "description", "supplier", "color"]

            # Extract material type from description
            yarn_data["type"] = yarn_data["description"].apply(
                self._extract_material_type
            )
        ...
        for group_id, yarns in self.yarn_groups.items():
            ...
            if isinstance(yarns, list) and len(yarns) > 1:
                group_info = {
                    ...
                    "yarns": [
                        y["desc_id"] if isinstance(y, dict) else str(y) for y in yarns
                    ],
                    "material": (
                        group_id.split("|")[0] if "|" in group_id else "Unknown"
                    ),
                ...
                if len(yarn_list) > 1:
                    group_info = {
                        ...
                        "color": (
                            group_id.split("|")[-1] if "|" in group_id else "Unknown"
                        ),
                    }
                    group_analysis.append(group_info)
    ...
    def calculate_aggregated_balances(self):
        ...
        for group_id, yarns in self.yarn_groups.items():
            ...
            group_inventory = self.yarn_inventory[
                self.yarn_inventory["Desc#"].isin(yarn_ids)
            ]
            ...
            if not group_inventory.empty:
                ...
                aggregated[group_id] = {
                    ...
                    "can_substitute": aggregated_shortage == 0
                    and len(individual_shortages) > 0,
                }
        ...
        for _, neg_yarn in negative_yarns.iterrows():
            ...
            for group_id, yarns in self.yarn_groups.items():
                ...
                if str(yarn_id) in [str(g) for g in group_yarn_ids]:
                    ...
                    group_inventory = self.yarn_inventory[
                        self.yarn_inventory["Desc#"].isin(
                            [int(g) if str(g).isdigit() else g for g in group_yarn_ids]
                        )
                    ]
                        ...
                        opportunities.append(
                            {
                                ...
                                "group_id": group_id,
                                "total_group_balance": group_inventory[
                                    "Planning Balance"
                                ].sum(),
    ...
    def generate_aggregation_report(self):
        ...
        report = {
            "summary": {
                ...
                "groups_with_alternatives": sum(
                    1 for g in groups if g["yarn_count"] > 1
                ),
...
if __name__ == "__main__":
    ...
    if analyzer.load_data():
        ...
        print(
            f"Groups with Alternatives: {report['summary']['groups_with_alternatives']}"
        )
        print(
            f"Substitution Opportunities: {report['summary']['substitution_opportunities']}"
        )
        print(
            f"Potential Shortage Reduction: {report['summary']['potential_shortage_reduction']:,.2f} lbs"
        )
