...
class YarnCompatibilityMLTrainer:
    ...
    def __init__(
        self, data_path: str = "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/5"
    ):
    ...
    def extract_material_properties(self, description: str) -> Dict:
        ...
        if cotton_match:
            ...
            properties["cotton_percent"] = (
                100
                if not any(x in desc_lower for x in ["blend", "poly", "spandex"])
                else 50
            )
        ...
        if poly_match:
            ...
            properties["polyester_percent"] = (
                100
                if not any(x in desc_lower for x in ["blend", "cotton", "spandex"])
                else 50
            )
    ...
    def calculate_compatibility_score(
        self, yarn1_props: Dict, yarn2_props: Dict
    ) -> float:
        """
        ...
        try:
            ...
            if training_data:
                ...
                scores = cross_val_score(
                    self.compatibility_model, X_train, y_train, cv=5
                )
                logger.info(
                    f"Model trained with accuracy: {scores.mean():.2f} (+/- {scores.std() * 2:.2f})"
                )
    ...
    def predict_substitutions(
        self, shortage_yarn_id: str, available_yarns: List[Dict]
    ) -> List[Dict]:
        """
        ...
        for yarn in available_yarns:
            ...
            if yarn_id in self.material_properties:
                ...
                combined = np.concatenate(
                    [shortage_features, candidate_features]
                ).reshape(1, -1)

                # Predict compatibility
                compatibility_prob = self.compatibility_model.predict_proba(combined)[
                    0
                ][1]
                ...
                detailed_score = self.calculate_compatibility_score(
                    yarn1_props, yarn2_props
                )
    ...
    def save_trained_model(
        self,
        output_path: str = "/mnt/d/Agent-MCP-1-ddd/trained_yarn_compatibility_model.json",
    ):
        """
    ...
    def train_and_generate_substitutions(self):
        ...
        if yarn_inv_file.exists():
            ...
            if self.train_compatibility_model(yarn_df):
                ...
                shortage_yarns = yarn_df[
                    yarn_df.get("Planning_Balance", yarn_df.get("planning_balance", 0))
                    < 0
                ]
                available_yarns = yarn_df[
                    yarn_df.get("Planning_Balance", yarn_df.get("planning_balance", 0))
                    > 100
                ]
                ...
                for _, shortage_row in shortage_yarns.iterrows():
                    yarn_id = shortage_row.get(
                        "desc_num", shortage_row.get("Desc#", "")
                    )
                    shortage_amt = abs(
                        shortage_row.get(
                            "Planning_Balance", shortage_row.get("planning_balance", 0)
                        )
                    )
                    ...
                    if yarn_id:
                        ...
                        recommendations = self.predict_substitutions(
                            yarn_id, available_yarns.to_dict("records")
                        )

                        if recommendations:
                            substitution_recommendations[yarn_id] = {
                                "description": shortage_row.get(
                                    "description", shortage_row.get("Description", "")
                                ),
                                ...
                                "total_available": sum(
                                    r["available_qty"] for r in recommendations
                                ),
                                "coverage_percent": (
                                    min(
                                        100,
                                        sum(r["available_qty"] for r in recommendations)
                                        / shortage_amt
                                        * 100,
                                    )
                                    if shortage_amt > 0
...
def main():
    ...
    if recommendations:
        ...
        for yarn_id, rec in sorted_recs:
            ...
            print(
                f"  Best substitute: {rec['substitutes'][0]['yarn_id'] if rec['substitutes'] else 'None'}"
            )
            if rec["substitutes"]:
                print(
                    f"  Compatibility: {rec['substitutes'][0]['compatibility_score']:.2f}"
                )
