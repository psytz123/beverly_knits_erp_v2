...
class IntelligentYarnMatcher:
    ...
    def normalize_color(self, color_str):
        ...
        color_match = re.search(
            r"(BLUE|RED|GREEN|YELLOW|GREY|GRAY|BROWN|PINK|ORANGE|PURPLE)", color_upper
        )
        if color_match:
            ...
            if size1.get("type") == size2.get("type"):
                if size1.get("type") == "FILAMENT":
                    ...
                    if "denier" in size1 and "denier" in size2:
                        ratio = min(size1["denier"], size2["denier"]) / max(
                            size1["denier"], size2["denier"]
                        )
                        if ratio >= 0.95:
                    ...
                    if "count" in size1 and "count" in size2:
                        ratio = min(size1["count"], size2["count"]) / max(
                            size1["count"], size2["count"]
                        )
                        if ratio >= 0.95:
...
def main():
    ...
    try:
        ...
        print(
            "Error: Could not connect to API. Please ensure server is running on port 5005"
        )
        return
    ...
    print(
        f"Found {len(shortage_yarns)} shortage yarns, {len(available_yarns)} available yarns"
    )
    ...
    for shortage_yarn in shortage_yarns:
                ...
                candidates.append(
                    {
                        ...
                        "available_qty": float(
                            avail_yarn.get("theoretical_balance", 0)
                        ),
                        ...
                        "compatibility": (
                            ...
                            else "MEDIUM" if compatibility >= 0.65 else "LOW"
                        ),
        ...
        candidates.sort(
            key=lambda x: (x["compatibility_score"], x["available_qty"]), reverse=True
        )
        ...
        if top_candidates:
            ...
            primary_material = (
                max(mat_comp.items(), key=lambda x: x[1])[0] if mat_comp else "UNKNOWN"
            )
            ...
            coverage_percent = min(
                100, (total_available / shortage_amt * 100) if shortage_amt > 0 else 0
            )
            ...
            substitutions[yarn_id] = {
                ...
                "supply_chain_risk": (
                    ...
                    else "MEDIUM" if len(top_candidates) >= 2 else "HIGH"
                ),
            }

    # Sort by shortage amount (highest first)
    sorted_substitutions = dict(
        sorted(substitutions.items(), key=lambda x: x[1]["shortage_qty"], reverse=True)
    )
    ...
    output = {
        ...
        "mitigation_percentage": float(
            (mitigatable_shortage / total_shortage * 100) if total_shortage > 0 else 0
        ),
    ...
    print(
        f"Mitigatable shortage: {mitigatable_shortage:.0f} lbs ({mitigatable_shortage/total_shortage*100:.1f}%)"
    )
        ...
        for i, (yarn_id, rec) in enumerate(list(sorted_substitutions.items())[:5], 1):
            ...
            if rec["substitutes"]:
                ...
                for j, sub in enumerate(rec["substitutes"][:3], 1):
                    print(
                        f"   {j}. {sub['substitute_id']}: {sub['description'][:40]}..."
                    )
                    print(f"      Supplier: {sub['supplier']}")
                    print(f"      Available: {sub['available_qty']:.0f} lbs")
                    print(
                        f"      Compatibility: {sub['compatibility']} ({sub['compatibility_score']:.2f})"
                    )
