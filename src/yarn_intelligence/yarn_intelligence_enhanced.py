...
class YarnIntelligenceEngine:
        ...
        try:
            ...
            if demand_files:
                ...
                if demand_files:
                    ...
                    if (
                        "Yarn" in self.yarn_demand.columns
                        and "Desc#" not in self.yarn_demand.columns
                    ):
                        self.yarn_demand.rename(columns={"Yarn": "Desc#"}, inplace=True)
                    print(
                        f"Loaded yarn demand: {len(self.yarn_demand)} yarns from {demand_file.name}"
                    )
                else:
            ...
            print(
                f"Loaded demand by style: {len(self.yarn_by_style)} style-yarn combinations"
            )
            ...
            self.yarn_inventory = pd.read_excel(
                self.data_path / "yarn_inventory (2).xlsx"
            )
    ...
    def analyze_yarn_criticality(self):
        ...
        for idx, yarn_inv in self.yarn_inventory.iterrows():
            ...
            shortage = max(
                0, -planning_balance
            )  # Convert negative balance to positive shortage
            ...
            if "Desc#" in self.yarn_demand.columns:
                ...
                if not demand_row.empty:
                    weekly_demand = (
                        demand_row["Demand This Week"].iloc[0]
                        if "Demand This Week" in demand_row.columns
                        else 0
                    )
                ...
                critical_analysis.append(
                    {
                        ...
                        "theoretical_balance": (
                            float(theoretical_balance)
                            if not pd.isna(theoretical_balance)
                            else 0.0
                        ),
                        "allocated": (
                            float(allocated) if not pd.isna(allocated) else 0.0
                        ),
                        "on_order": float(on_order) if not pd.isna(on_order) else 0.0,
                        "planning_balance": (
                            float(planning_balance)
                            if not pd.isna(planning_balance)
                            else 0.0
                        ),
                        "balance": (
                            float(planning_balance)
                        ...
                        "weekly_demand": (
                            float(weekly_demand) if not pd.isna(weekly_demand) else 0.0
                        ),
                        "weeks_of_supply": (
                            float(weeks_of_supply)
                            if not pd.isna(weeks_of_supply)
                            else 0.0
                        ),
                        "criticality_score": (
                            float(criticality_score)
                            if not pd.isna(criticality_score)
                            else 0.0
                        ),
        ...
        return {
            ...
            "summary": {
                ...
                "critical_count": int(
                    sum(1 for y in critical_analysis if y["risk_level"] == "CRITICAL")
                ),
                "high_count": int(
                    sum(1 for y in critical_analysis if y["risk_level"] == "HIGH")
                ),
                "total_shortage": float(sum(y["shortage"] for y in critical_analysis)),
                "yarns_with_shortage": int(
                    sum(1 for y in critical_analysis if y["shortage"] > 0)
                ),
            },
        }
    ...
    def analyze_weekly_demand_pattern(self):
        ...
        if weekly_analysis:
            ...
            avg_weekly_demand = sum(
                w["total_demand"] for w in weekly_analysis.values()
            ) / len(weekly_analysis)
        else:
            peak_week = None
            avg_weekly_demand = 0
    ...
    def analyze_style_yarn_requirements(self):
            ...
            style_analysis.append(
                {
                    ...
                    "this_week_demand": (
                        float(style_data["This Week"].sum())
                        if "This Week" in style_data.columns
                        else 0
                    ),
                }
            )
        ...
        return {
            ...
            "summary": {
                ...
                "unique_yarns": int(
                    self.yarn_by_style["Yarn"].nunique()
                    if "Yarn" in self.yarn_by_style.columns
                    else 0
                ),
            },
        }
    ...
    def analyze_expected_deliveries(self):
        ...
        if "Desc#" in self.expected_yarn.columns:
            ...
            for yarn_id, group in yarn_groups:
                total_on_order = (
                    group["PO Order Amt"].sum()
                    if "PO Order Amt" in group.columns
                    else 0
                )
                total_received = (
                    group["Rec&#39;d"].sum() if "Rec&#39;d" in group.columns else 0
                )
                ...
                for col in ["This Week", "Unscheduled or Past Due"]:
                    if col in group.columns:
                        ...
                        if amount > 0:
                            deliveries.append({"period": col, "amount": float(amount)})
        ...
        return {
            ...
            "summary": {
                ...
                "unique_yarns": (
                    int(self.expected_yarn["Desc#"].nunique())
                    if "Desc#" in self.expected_yarn.columns
                    else 0
                ),
                "total_on_order": (
                    float(self.expected_yarn["PO Order Amt"].sum())
                    if "PO Order Amt" in self.expected_yarn.columns
                    else 0
                ),
                "past_due": (
                    float(self.expected_yarn["Unscheduled or Past Due"].sum())
                    if "Unscheduled or Past Due" in self.expected_yarn.columns
                    else 0
                ),
            },
        }
    ...
    def generate_procurement_recommendations(self):
        ...
        recommendations.sort(
            key=lambda x: (0 if x["urgency"] == "CRITICAL" else 1, -x["shortage"])
        )
        ...
        return {
            ...
            "summary": {
                ...
                "total_order_quantity": float(
                    sum(r["recommended_order"] for r in recommendations)
                ),
                "total_estimated_cost": float(
                    sum(r["estimated_cost"] for r in recommendations)
                ),
                "critical_orders": int(
                    sum(1 for r in recommendations if r["urgency"] == "CRITICAL")
                ),
                "high_priority_orders": int(
                    sum(1 for r in recommendations if r["urgency"] == "HIGH")
                ),
            },
        }
    ...
    def find_substitution_opportunities(self):
        ...
        negative_yarns = self.yarn_inventory[
            self.yarn_inventory[planning_col] < 0
        ].copy()
        ...
        substitution_opportunities.sort(
            key=lambda x: x["shortage_amount"], reverse=True
        )
        ...
        return {
            ...
            "summary": {
                ...
                "total_shortage_with_substitutes": sum(
                    s["shortage_amount"] for s in substitution_opportunities
                ),
    ...
    def _calculate_compatibility_score(
        self, yarn1_info: dict, yarn2_info: dict
    ) -> float:
        ...
        if yarn1_info.get("interchangeability_group") == yarn2_info.get(
