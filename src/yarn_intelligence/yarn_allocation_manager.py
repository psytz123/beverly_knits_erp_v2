...
class YarnAllocationManager:
    ...
    def calculate_priority(self, ko: pd.Series) -> float:
        ...
        if pd.notna(ko.get("Shipped (lbs)", 0)) and ko.get("Shipped (lbs)", 0) > 0:
            ...
            logger.debug(
                f"KO {ko.get('Actions', 'Unknown')}: Added {self.priority_weights['production_started']} for production started"
            )
        ...
        if ko.get("is_sales_order", False) or ko.get("SO#", ""):
            ...
            logger.debug(
                f"KO {ko.get('Actions', 'Unknown')}: Added {self.priority_weights['sales_order_driven']} for sales order"
            )
        ...
        if pd.notna(ko.get("Quoted Date")):
            try:
                ...
                urgency_score = (
                    max(0, 100 - days_until_due)
                    * self.priority_weights["days_until_due"]
                )
                score += urgency_score
                logger.debug(
                    f"KO {ko.get('Actions', 'Unknown')}: Added {urgency_score:.1f} for urgency ({days_until_due} days)"
                )

            except Exception as e:
                logger.warning(
                    f"Could not parse date for KO {ko.get('Actions', 'Unknown')}: {e}"
                )
    ...
    def allocate_yarn_for_ko(
        self, ko: pd.Series, bom_data: pd.DataFrame, available_yarn: Dict[str, float]
    ) -> Dict:
        """
        ...
        qty_lbs = ko.get("Qty Ordered (lbs)", 0) - ko.get(
            "G00 (lbs)", 0
    ...
    def suggest_reallocation(
        self, yarn_shortage: Dict[str, float], current_allocations: pd.DataFrame
    ) -> List[Dict]:
        """
        ...
        for yarn_id, shortage_qty in yarn_shortage.items():
            ...
            for _, alloc in available_allocs.iterrows():
                ...
                realloc_qty = min(
                    alloc["quantity_allocated"], shortage_qty - cumulative_available
                )
                ...
                suggestions.append(
                    {
                        ...
                        "shortage_after_reallocation": max(
                            0, shortage_qty - cumulative_available
                        ),
                    }
                )
            ...
            report_data.append(
                {
                    ...
                    "allocation_rate": (
                        (total_allocated / total_requested * 100)
    ...
    def optimize_allocation_schedule(
        self, knit_orders: pd.DataFrame, yarn_availability: pd.DataFrame
    ) -> Dict:
        """
        ...
        summary = {
            ...
            "can_start_immediately": sum(
                1 for s in allocation_schedule.values() if s["can_start"]
            ),
            "hard_allocations": sum(
                1
                for s in allocation_schedule.values()
                if s["allocation_type"] == "HARD"
            ),
            "soft_allocations": sum(
                1
                for s in allocation_schedule.values()
                if s["allocation_type"] == "SOFT"
            ),
        ...
        logger.info(
            f"Allocation optimization complete: {summary['allocated_kos']}/{summary['total_kos']} KOs allocated"
        )
...
def test_yarn_allocation_manager():
    ...
    yarn_availability = pd.DataFrame(
        {"Yarn_ID": ["Y001", "Y002", "Y003"], "Available": [500, 1000, 750]}
    )

    # Test optimization
    print("\n\nTesting Allocation Optimization...")
    optimization_result = manager.optimize_allocation_schedule(
        sample_kos, yarn_availability
    )
