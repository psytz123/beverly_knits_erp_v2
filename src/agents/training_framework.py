...
class TrainingMetrics:
    ...
    def meets_requirements(self, config: AgentConfig) -> bool:
        ...
        return (
            self.accuracy >= config.min_accuracy
            and self.response_time_ms <= config.max_response_time_ms
...
class BaseAgent(ABC):
    ...
    def practice_scenario(self, scenario_name: str) -> TrainingMetrics:
        ...
        response_time = (
            (end_time - start_time).total_seconds() * 1000 / len(scenario_data["cases"])
        )
    ...
    def certify(self) -> bool:
        ...
        if not self.performance_metrics.meets_requirements(self.config):
            logger.warning(
                f"Agent does not meet minimum requirements for certification"
            )
            return False
    ...
    def train_all_agents(self, epochs: int = 100) -> Dict[AgentRole, TrainingMetrics]:
        ...
        for role, agent in self.agents.items():
            ...
            if not training_data.empty:
                ...
                logger.info(
                    f"Training complete for {role.value}: accuracy={metrics.accuracy:.2%}"
                )
            else:
    ...
    def evaluate_all_agents(self) -> Dict[AgentRole, TrainingMetrics]:
        ...
        for role, agent in self.agents.items():
            ...
            if not test_data.empty:
                ...
                results[role] = metrics
                logger.info(
                    f"Evaluation complete for {role.value}: accuracy={metrics.accuracy:.2%}"
                )
