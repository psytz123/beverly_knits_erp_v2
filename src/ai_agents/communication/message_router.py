...
class MessageRouter:
    ...
    def register_agent(
        ...
        try:
            ...
            self.logger.info(
                f"Agent registered: {agent_id} with {len(capabilities)} capabilities"
            )
            return True
    ...
    async def broadcast_message(
        self, message: AgentMessage, agent_filter: Optional[Callable] = None
    ) -> int:
        """
    ...
    async def _deliver_message(self, message: AgentMessage):
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    f"Message delivered: {message.correlation_id} -> {target_agent_id}"
                )

            except Exception as e:
                self.logger.error(
                    f"Failed to deliver message to {target_agent_id}: {str(e)}"
                )
                self.dead_letter_queue.append(message)
                self.router_metrics["messages_failed"] += 1
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            ).total_seconds(),  # TODO: Track actual uptime
            "performance": {
                "average_routing_time_ms": self.router_metrics[
                    "average_routing_time_ms"
                ],
                "success_rate": self._calculate_success_rate(),
                "queue_utilization": min(
                    self.message_queue.qsize() / 1000, 1.0
                ),  # Assume max 1000
            },
            "agents": {
    ...
    def _calculate_success_rate(self) -> float:
        ...
        total_messages = (
            self.router_metrics["messages_routed"]
            + self.router_metrics["messages_failed"]
        )
        if total_messages == 0:
            return 1.0
