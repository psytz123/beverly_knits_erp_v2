...
class AgentMessage:
    ...
    def is_expired(self) -> bool:
        ...
        return datetime.now() > (
            self.timestamp + timedelta(seconds=self.timeout_seconds)
        )
    ...
    async def process_message(self, message: AgentMessage) -> Optional[AgentMessage]:
        ...
        try:
            ...
            if message.is_expired():
                self.logger.warning(
                    f"Received expired message: {message.correlation_id}"
                )
                return AgentMessage(
                    agent_id=self.agent_id,
                    target_agent_id=message.agent_id,
            ...
            return AgentMessage(
                ...
                target_agent_id=message.agent_id,
                message_type=MessageType.ERROR,
                payload={"error": "PROCESSING_ERROR", "details": str(e)},
                correlation_id=message.correlation_id,
            )

    def register_message_handler(
        self, message_type: MessageType, handler: Callable[[AgentMessage], AgentMessage]
    ):
