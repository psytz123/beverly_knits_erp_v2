...
class ProjectMetrics:
    ...
    def schedule_variance(self) -> float:
        ...
        if self.planned_duration_weeks == 0:
            return 0.0
        return (
            (self.planned_duration_weeks - self.actual_duration_weeks)
            / self.planned_duration_weeks
        ) * 100
...
class ImplementationProjectManagerAgent(BaseAgent):
    ...
    async def assess_customer_complexity(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            implementation_plan = self._generate_implementation_plan(
                customer_profile, complexity_score
            )
            ...
            self.logger.info(
                f"Customer assessment completed for {customer_id}: complexity={complexity_score:.2f}"
            )
    ...
    def _calculate_complexity_score(self, customer_profile: CustomerProfile) -> float:
        ...
        factors = {
            "company_size": self.complexity_factors["company_size"].get(
                customer_profile.company_size.value, 1.0
            ),
            "data_volume": self.complexity_factors["data_volume"].get(
                customer_profile.data_volume_estimate, 1.0
            ),
            "integration": self.complexity_factors["integration_complexity"].get(
                customer_profile.integration_complexity, 1.0
            ),
            "change_readiness": self.complexity_factors["change_readiness"].get(
                customer_profile.change_management_readiness, 1.0
            ),
            "erp_experience": self.complexity_factors["erp_experience"].get(
                customer_profile.previous_erp_experience, 1.0
            ),
        }
    ...
    def _generate_implementation_plan(
        self, customer_profile: CustomerProfile, complexity_score: float
    ) -> Dict[str, Any]:
    ...
    def _generate_milestones(
        self, customer_profile: CustomerProfile, duration_weeks: int
    ...
    def _perform_risk_assessment(
        ...
        return {
            ...
            "overall_risk_score": (
                sum(r.risk_score for r in risks) / len(risks) if risks else 0
            ),
    ...
    def _identify_challenges(
        ...
        if complexity_score > 7:
            challenges.append(
                "Managing high system complexity and integration requirements"
            )
        ...
        if customer_profile.data_volume_estimate == "Enterprise":
            challenges.append(
                "Managing large-scale data migration with minimal downtime"
            )
...
eFab AI Project Manager
    ...
    async def monitor_project_progress(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(overdue_milestones) > 0:
                ...
                len([m for m in milestones if m.status == MilestoneStatus.AT_RISK]) > 0
            ):
            ...
            recommendations = self._generate_recommendations(
                customer_id, milestones, project_status
            )
            ...
            self.logger.error(
                f"Failed to monitor project progress for {customer_id}: {str(e)}"
            )
            return {"error": str(e)}
    ...
    async def predict_timeline_completion(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            project_start = min(
                m.actual_start_date for m in completed_milestones if m.actual_start_date
            )
            ...
            if velocity > 0:
                ...
                confidence = min(
                    0.95, max(0.5, 1.0 - (abs(weeks_remaining - 3) / 10))
                )  # Higher confidence for 3-week remaining
            else:
    ...
    async def _handle_notification(
        ...
        if notification_type == "MILESTONE_UPDATE":
                ...
                for milestone in self.project_milestones[customer_id]:
                    if milestone.milestone_id == milestone_id:
                        ...
                        if status == "COMPLETED":
                            ...
                            status == "IN_PROGRESS" and not milestone.actual_start_date
                        ):
                            milestone.actual_start_date = datetime.now()

                        self.logger.info(
                            f"Milestone {milestone_id} updated: {status} ({progress}%)"
                        )
                        break
            ...
            if customer_id in self.active_projects:
                ...
                self.logger.info(
                    f"Customer {customer_id} transitioned to phase: {new_phase}"
                )
                    ...
                    for risk in risks:
                        if risk.status == "IDENTIFIED":
                            ...
                            if risk.risk_score > 0.7:
                                self.logger.warning(
                                    f"High risk detected for {customer_id}: {risk.description}"
                                )
