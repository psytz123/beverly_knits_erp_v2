...
class DataMigrationIntelligenceAgent(BaseAgent):
    ...
    def _initialize(self):
        ...
        # Load Beverly Knits patterns for training data
        self._extract_beverly_patterns()
    ...
    def _load_column_variations(self) -> Dict[str, List[str]]:
        ...
        return {
            # Inventory columns
            "yarn_id": ["yarn_id", "yarnid", "yarn_code", "yarn_number", "item_id", "desc#", "desc_num", "material_code"],
            "description": ["description", "desc", "yarn_desc", "material_desc", "item_desc", "product_desc"],
            "planning_balance": ["planning_balance", "planning balance", "balance", "available", "on_hand", "stock_balance"],
            "allocated": ["allocated", "reserved", "committed", "assigned", "planned_usage"],
            "safety_stock": ["safety_stock", "safety stock", "min_stock", "minimum_stock", "reorder_point"],
            ...
            # Production columns  
            "style_number": ["style_number", "style#", "fstyle#", "style_id", "product_code", "part_number"],
            ...
            # BOM columns
            "parent_item": ["parent_item", "parent", "finished_good", "assembly", "product"],
            "component_item": ["component_item", "component", "child_item", "material", "ingredient"],
            "usage_qty": ["usage_qty", "usage", "quantity_per", "consumption", "requirement"],
    ...
    def _extract_beverly_patterns(self):
        """Extract patterns from Beverly Knits ERP for training data"""
        try:
            # Load Beverly Knits schemas as training examples
            beverly_patterns = {
                "yarn_inventory_schema": {
                    "columns": ["Desc#", "Planning Balance", "Allocated", "Safety Stock"],
                    "patterns": ["desc_variations", "balance_calculations", "allocation_logic"]
                },
                "bom_schema": {
                    "columns": ["Style#", "Yarn ID", "Consumption", "Unit"],
                    "patterns": ["style_mapping", "consumption_calculations", "unit_conversions"]
                },
                "production_schema": {
                    "columns": ["Order#", "Machine", "Style", "Quantity", "Due Date"],
                    "patterns": ["order_sequencing", "machine_assignment", "capacity_planning"]
                }
            }
            
            self.schema_templates["beverly_knits"] = beverly_patterns
            self.logger.info("Beverly Knits patterns extracted for training")
            
        except Exception as e:
            self.logger.error(f"Error extracting Beverly patterns: {str(e)}")
    ...
    async def _discover_data_sources(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Automatically discover and analyze legacy data sources"""
