...
class PerformanceOptimizationAgent(BaseAgent):
    ...
    def _initialize(self):
        ...
        self.register_message_handler(
            MessageType.REQUEST, self._handle_performance_request
        )
        self.register_message_handler(
            MessageType.NOTIFICATION, self._handle_performance_notification
        )
    ...
    async def _handle_performance_notification(
    ...
    async def _monitor_performance(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metrics_to_monitor = payload.get(
            ...
            try:
                ...
                if metric_value is not None:
                    ...
                    if len(self.metric_history[metric_type_str]) > 1000:
                        self.metric_history[metric_type_str] = self.metric_history[
                            metric_type_str
                        ][-1000:]

                    # Generate alerts if thresholds exceeded
                    if metric.alert_level in [
                        AlertSeverity.HIGH,
                        ...
                        alert = {
                            ...
                            "threshold_exceeded": threshold_config.get(
                                "critical"
                                if metric.alert_level == AlertSeverity.CRITICAL
                                else "warning"
                            ),
                ...
                self.logger.error(
                    f"Error monitoring metric {metric_type_str}: {str(e)}"
                )
                continue
        ...
        return {
            "monitoring_status": {
                ...
                "next_collection": (
                    datetime.now() + timedelta(seconds=self.collection_interval)
                ).isoformat(),
            },
    ...
    async def _analyze_performance(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_type_str, metric_history in self.metric_history.items():
            ...
            if baseline_comparison and metric_type_str in self.baselines:
                ...
                deviation = (
                    (current_avg - baseline.baseline_value) / baseline.baseline_value
                ) * 100
                ...
                analysis_results[metric_type_str]["baseline_comparison"] = {
                    ...
                    "performance_status": (
                        "DEGRADED" if abs(deviation) > 15 else "STABLE"
                    ),
                }
        ...
        optimization_recommendations = (
            await self._generate_optimization_recommendations(analysis_results)
        )
    ...
    async def _generate_optimization_recommendations(
        ...
        for metric_type, analysis in analysis_results.items():
            try:
                ...
                self.logger.error(
                    f"Error generating recommendations for {metric_type}: {str(e)}"
                )
                continue
        ...
        for rec in recommendations:
            ...
            effort_score = {"HIGH": 1, "MEDIUM": 2, "LOW": 3}[
                rec["effort_estimate"]
    ...
    def _assess_overall_health(self, analysis_results: Dict[str, Any]) -> str:
        ...
        for metric_type, analysis in analysis_results.items():
            ...
            if analysis.get("trend") == "INCREASING" and metric_type in [
                "RESPONSE_TIME",
                "ERROR_RATE",
                "CPU_UTILIZATION",
            ]:
            ...
            anomaly_rate = analysis.get("anomalies_detected", 0) / analysis.get(
                "sample_count", 1
            )
    ...
    async def _establish_baselines(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_type_str in metrics_to_baseline:
            if metric_type_str not in self.metric_history:
                monitoring_recommendations.append(
                    f"Start collecting data for {metric_type_str}"
                )
                continue
            ...
            if len(recent_metrics) < 10:  # Need minimum sample size
                monitoring_recommendations.append(
                    f"Insufficient data for {metric_type_str} baseline - need at least 10 samples"
                )
                continue
            ...
            coefficient_variation = (
                (std_dev / baseline_value) * 100 if baseline_value != 0 else 0
            )
            ...
            baseline_quality[metric_type_str] = {
                ...
                "stability_assessment": (
                    "STABLE" if coefficient_variation < 15 else "VARIABLE"
                ),
            }
        ...
        return {
            ...
            "summary": {
                ...
                "high_quality_baselines": sum(
                    1
                    for q in baseline_quality.values()
                    if q["quality_rating"] == "HIGH"
                ),
    ...
    async def _get_performance_report(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_type, analysis in analysis_result["analysis_results"].items():
            summary_metrics[metric_type] = {
                ...
                "health_status": self._get_metric_health_status(
                    metric_type, analysis["average"]
                ),
            }
        ...
        active_alerts_summary = {
            ...
            "alert_types": list(
                set(alert.get("metric_type") for alert in self.active_alerts.values())
            ),
        }
        ...
        performance_report = {
            ...
            "executive_summary": {
                ...
                "metrics_analyzed": analysis_result["analysis_summary"][
                    "metrics_analyzed"
                ],
                "trends_identified": analysis_result["analysis_summary"][
                    "trends_identified"
                ],
                "recommendations_available": analysis_result["analysis_summary"][
            ...
            "baseline_status": {
                metric_type: {
                    ...
                    "baseline_age_days": (
                        (
                            datetime.now()
                            - self.baselines[metric_type].established_date
                        ).days
        ...
        if include_recommendations:
            performance_report["optimization_recommendations"] = analysis_result[
                "optimization_recommendations"
            ]
                ...
                for metric_type in PerformanceMetricType:
                    ...
                    if metric_value is not None:
                        metric = PerformanceMetric(
                            metric_type=metric_type,
                            value=metric_value,
                            unit=self.default_thresholds.get(metric_type, {}).get(
                                "unit", ""
                            ),
                            timestamp=datetime.now(),
                            source=self.agent_id,
