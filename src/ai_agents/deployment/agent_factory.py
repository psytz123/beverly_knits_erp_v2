...
class AgentTemplate:
    ...
    async def create_instance(
        self, agent_id: str, configuration: Dict[str, Any] = None
    ) -> BaseAgent:
        ...
        if self._requires_custom_initialization():
            ...
            instance = self.agent_class(agent_id=agent_id, configuration=final_config)
    ...
    async def _create_with_custom_init(
...
class AgentFactory:
    ...
    def _initialize_builtin_templates(self):
        """Initialize built-in agent templates"""
        from ..implementation.project_manager_agent import (
    ...
    async def create_agent(
        ...
        try:
            ...
            if not template.can_create_instance(context):
                self.logger.error(
                    f"Cannot create agent {agent_id} - requirements not met"
                )
                self.factory_metrics["creation_failures"] += 1
                return None
            ...
            if not self._reserve_resources(template.resource_requirements):
                self.logger.error(
                    f"Cannot create agent {agent_id} - insufficient resources"
                )
                self.factory_metrics["creation_failures"] += 1
                return None
        ...
        try:
            ...
            for template_name, config in core_agents:
                ...
                if agent:
                    ...
                    self.logger.warning(
                        f"Failed to create {template_name} agent for customer {customer_id}"
                    )
            ...
            if industry_template and industry_template in self.agent_templates:
                agent = await self.create_agent(
                    template_name=industry_template, customer_id=customer_id
                )
            ...
            self.logger.info(
                f"Created {len(created_agents)} agents for customer {customer_id}"
            )
            return created_agents

        except Exception as e:
            self.logger.error(
                f"Failed to create customer agent stack for {customer_id}: {str(e)}"
            )
    ...
    def get_customer_agents(self, customer_id: str) -> List[BaseAgent]:
        ...
        return [
            self.active_agents[aid] for aid in agent_ids if aid in self.active_agents
        ]
    ...
    def get_factory_status(self) -> Dict[str, Any]:
        ...
        utilization = (
            (used_resources / total_resources * 100) if total_resources > 0 else 0
        )
    ...
    def get_agent_info(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "created_at": (
                metadata.get("created_at").isoformat()
