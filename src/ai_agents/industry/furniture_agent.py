...
class FurnitureManufacturingAgent(BaseAgent):
    ...
    def _initialize(self):
        ...
        self.register_message_handler(
            MessageType.REQUEST, self._handle_furniture_request
        )
    ...
    async def _optimize_wood_yield(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_material = sum(
            mat["raw_board_feet_needed"] for mat in material_requirements
        )
        waste_percentage = (
            (total_waste / total_material * 100) if total_material > 0 else 0
        )
    ...
    async def _generate_furniture_bom(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for hardware in hardware_list:
            hardware_cost = (
                hardware.get("cost_per_unit", 0.0)
                * hardware.get("quantity_per_piece", 1)
                * quantity
            )
        ...
        # Lead time analysis
        max_lead_time = max(
            (item.get("lead_time_days", 0) for item in detailed_bom), default=0
        )
            ...
            "material_costs": {
                ...
                "material_cost_breakdown": {
                    "wood": sum(
                        ...
                        if "wood" in item["category"].lower()
                    ),
                    "hardware": sum(
                        item["extended_cost"]
                        for item in detailed_bom
                    ...
                    "other": sum(
                        item["extended_cost"]
                        for item in detailed_bom
                        if item["category"] not in ["HARDWARE"]
                        and "wood" not in item["category"].lower()
                    ),
                },
            },
            "lead_time_analysis": {
                    ...
                    [
                        {
                            ...
                            + timedelta(days=max_lead_time - item["lead_time_days"]),
                        }
                        for item in detailed_bom
                    ],
    ...
    async def _schedule_finishing(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        booth_capacity = payload.get(
            "finish_booth_capacity", {"booths": 2, "pieces_per_booth": 10}
        )
        environmental = payload.get(
            "environmental_conditions", {"temperature": 70, "humidity": 45}
        )
            ...
            finish_schedule.append(
                {
                    ...
                    "completion_time": (
                        current_time + timedelta(hours=adjusted_cure_hours)
                    ).isoformat(),
        ...
        utilization = (
            (used_hours / total_booth_hours * 100) if total_booth_hours > 0 else 0
        )
            ...
            "bottleneck_analysis": {
                "critical_finish_type": max(
                    ...
                    key=lambda x: sum(
                        ...
                        for item in finish_schedule
                        if item["finish_type"] == x
                    ),
                    default="NONE",
                ),
    ...
    async def _configure_custom_order(self, payload: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if custom_type == "dimension_change":
                ...
                if abs(size_change) > 50:
                    feasibility_issues.append(
                        "Dimension change exceeds 50% - may require design re-engineering"
                    )
                cost_impact["customization_cost"] += (
                    abs(size_change) * 5
                )  # $5 per % change
                ...
                if finish_complexity == "COMPLEX":
                    ...
                    feasibility_issues.append(
                        "Complex finish may extend lead time by 1-2 weeks"
                    )

        cost_impact["total_cost"] = (
            cost_impact["base_cost"] + cost_impact["customization_cost"]
        )
        ...
        if cost_impact["total_cost"] > max_budget:
            feasibility_issues.append(
                f"Total cost ${cost_impact['total_cost']} exceeds budget ${max_budget}"
            )
        ...
        return {
            "feasibility_analysis": {
                ...
                "complexity_rating": (
                    "HIGH"
                    if len(customizations) > 3
                    else "MEDIUM" if len(customizations) > 1 else "LOW"
                ),
            },
    ...
    async def _analyze_seasonal_demand(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for product_type_str, quantity in product_mix.items():
            try:
                ...
                pattern = self.seasonal_demand_patterns.get(
                    product_type,
        ...
        return {
            ...
            "peak_season_products": [
                product
                for product, data in seasonal_forecast.items()
