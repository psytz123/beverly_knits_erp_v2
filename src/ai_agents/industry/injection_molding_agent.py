...
class InjectionMoldingAgent(BaseAgent):
    ...
    async def _optimize_process_parameters(
        ...
        base_properties = self.material_properties.get(
            material_name, self.material_properties["ABS"]
        )
        ...
        quality_predictions = {
            "dimensional_accuracy": (
                "±0.002 inches" if mold_temp > 160 else "±0.005 inches"
            ),
            "surface_finish": (
                "Excellent" if mold_temp > base_properties["mold_temp"] + 15 else "Good"
            ),
    ...
    async def _schedule_mold_maintenance(
        ...
        for mold in mold_inventory:
            ...
            cycles_remaining = maintenance_cycles - (
                current_cycles % maintenance_cycles
            )
            urgency_percentage = (
                (maintenance_cycles - cycles_remaining) / maintenance_cycles
            ) * 100
            ...
            risk_level = (
                "HIGH"
                if urgency_percentage > 85
                else "MEDIUM" if urgency_percentage > 60 else "LOW"
            )
        ...
        maintenance_schedule.sort(
            key=lambda x: (Priority(x["priority"]).score, -x["urgency_percentage"]),
            reverse=True,
        )
        ...
        return {
            ...
            "downtime_impact": {
                ...
                "weeks_of_capacity_needed": total_downtime_hours
                / maintenance_capacity.get("hours_per_week", 40),
    ...
    async def _analyze_defects(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        most_common_defect = (
            max(defect_summary.keys(), key=defect_summary.get)
            if defect_summary
            else None
        )
        ...
        if most_common_defect:
            try:
                ...
                possible_causes = self.defect_causes.get(
                    defect_enum, ["Unknown causes"]
                )
                ...
                root_causes[most_common_defect] = {
                    ...
                    "percentage_of_total": (
                        defect_summary[most_common_defect]
                        / sum(defect_summary.values())
                    )
                    * 100,
                }
                ...
                root_causes[most_common_defect] = {
                    "possible_causes": ["Unknown defect type"]
                }
        ...
        return {
            "root_cause_analysis": {
                ...
                "total_defect_rate": (
                    sum(defect_summary.values()) / len(defect_data) * 100
                    if defect_data
                    else 0
                ),
            },
            ...
            "recommendations": [
                "Focus immediate attention on reducing "
                + (most_common_defect or "defects"),
    ...
    async def _optimize_material_usage(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for material, total_grams in material_requirements.items():
            ...
            runner_waste = sum(
                job.get("runner_weight_grams", 10)
                * (job.get("quantity", 1) // job.get("cavity_count", 1))
                for job in production_jobs
            ...
            # Recycling potential
            can_recycle_runners = recycling_capabilities.get(
                "regrind_capability", False
            )
            ...
            material_cost_per_gram = material_inventory.get(material, {}).get(
                "cost_per_gram", 0.003
            )  # $0.003/gram default
            recycling_value = (
                recyclable_material * material_cost_per_gram * 0.3
            )  # 30% value recovery
            waste_cost_avoided = (
                recyclable_material * 0.001
            )  # $0.001/gram disposal cost avoided

            cost_savings["recycling_value"] += recycling_value
            cost_savings["waste_reduction"] += waste_cost_avoided

        cost_savings["total_savings"] = (
            cost_savings["recycling_value"] + cost_savings["waste_reduction"]
        )
        ...
        material_plan = {
            ...
            "total_waste_kg": sum(
                data["total_waste_grams"] for data in waste_analysis.values()
            )
            / 1000,
            "recycling_potential_kg": sum(
                data["recyclable_grams"] for data in waste_analysis.values()
            )
            / 1000,
            "net_waste_kg": sum(
                data["net_waste_grams"] for data in waste_analysis.values()
            )
            / 1000,
        }
    ...
    async def _calculate_cycle_time(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        material_data = self.material_properties.get(
            material_name, self.material_properties["ABS"]
        )
    ...
    async def _optimize_energy_consumption(
        ...
        production_schedule = payload.get("production_schedule", [])
        energy_rates = payload.get(
            "energy_rates", {"peak": 0.12, "off_peak": 0.08}
        )  # $/kWh
        ...
        for job in production_schedule:
            ...
            if job.get("material_temperature", 450) > 500:
                heating_savings = (
                    heating_power_kw * 0.1
                )  # 10% savings with optimized heating
                optimizations.append(
                    {
                        "strategy": "Optimize barrel heating zones",
                        "energy_savings_kwh": heating_savings
                        * (cycle_time * quantity)
                        / 3600,
                        "cost_savings": heating_savings
                        * 0.1
            ...
            hydraulic_savings = (
                hydraulic_power_kw * 0.15
            )  # 15% with variable speed drives
            optimizations.append(
                {
                    "strategy": "Install variable frequency drives",
                    "energy_savings_kwh": hydraulic_savings
                    * (cycle_time * quantity)
                    / 3600,
            ...
            if job.get("priority", "NORMAL") != "URGENT":
                schedule_savings = total_energy_kwh * (
                    energy_rates["peak"] - energy_rates["off_peak"]
                )
                optimizations.append(
                    {
        ...
        return {
            "energy_analysis": {
                "total_consumption_kwh": sum(
                    job["current_energy_kwh"] for job in energy_optimization
                ),
                ...
                "savings_percentage": (
                    (
                        ...
                        / sum(job["current_cost"] for job in energy_optimization)
                    )
                    * 100
            ...
            "payback_analysis": {
                ...
                "payback_months": (
                    (15000 / (total_energy_savings * 250 / 12))
