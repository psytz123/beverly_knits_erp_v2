...
from .deployment.agent_factory import (
    AgentFactory,
    AgentTemplate,
    AgentType,
    DeploymentStrategy,
...
from ..agents.training_framework import (
...
class AgentSystemInitializer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = (
            config_path or self.base_path / "config" / "agent_training_config.json"
        )
    ...
    async def _initialize_core_infrastructure(self):
        ...
        self.initialization_status["components_initialized"].append(
            "core_infrastructure"
        )
        ...
        if hasattr(self.message_router, "register_agent"):
            self.message_router.register_agent(
                ...
                agent_info={
                    ...
                    "capabilities": [
                        cap.name for cap in self.orchestrator.capabilities
                    ],
                },
    ...
    async def _validate_system_readiness(self):
        ...
        if self.orchestrator:
            ...
            validation_results["orchestrator_health"] = (
                orchestrator_status["registered_agents"] > 0
            )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {"initialization_status": self.initialization_status, "components": {}}
        ...
        if self.agent_factory:
            status["components"][
                "agent_factory"
            ] = self.agent_factory.get_factory_status()

        if self.orchestrator:
            status["components"][
                "orchestrator"
            ] = self.orchestrator.get_orchestrator_status()
...
async def initialize_ai_agents(
    config_path: Optional[str] = None, mode: str = "full"
) -> Dict[str, Any]:
