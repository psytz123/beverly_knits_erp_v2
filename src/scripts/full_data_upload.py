...
class FullDataUploader:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if so_number and so_number != "nan":
                    ...
                    customer_name = str(
                        row.get("Sold To", row.get("Customer", "Unknown"))
                    )
    ...
    def verify_upload(self):
        ...
        queries = [
            ...
            (
                "Yarn Inventory",
                "SELECT COUNT(*) FROM production.yarn_inventory_ts WHERE snapshot_date = %s",
            ),
            (
                "Sales Orders",
                "SELECT COUNT(*) FROM production.sales_orders_ts WHERE snapshot_date = %s",
            ),
            (
                "Knit Orders",
                "SELECT COUNT(*) FROM production.knit_orders_ts WHERE snapshot_date = %s",
            ),
