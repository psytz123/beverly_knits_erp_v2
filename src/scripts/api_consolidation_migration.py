...
class APIConsolidationMigrator:
    ...
    def get_current_feature_flags(self) -> Dict:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/admin/feature-flags", timeout=5
            )
    ...
    def phase2_test_consolidated_endpoints(self) -> bool:
        ...
        for endpoint in endpoints:
            ...
            phase_report["endpoints_tested"].append(
                {"name": endpoint["name"], "success": success}
            )
    ...
    def phase4_monitor_usage(self, duration_seconds: int = 60) -> Dict:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/admin/deprecation-report", timeout=5
            )
            if response.status_code == 200:
                phase_report["deprecation_report"] = response.json()
                report = response.json()
                print(
                    f"✓ Total deprecated calls: {report.get('total_deprecated_calls', 0)}"
                )
                print(
                    f"✓ Unique deprecated endpoints: {report.get('unique_deprecated_endpoints', 0)}"
                )
            ...
            self.migration_report["errors"].append(
                f"Failed to get deprecation report: {e}"
            )
...
def main():
    ...
    parser.add_argument(
        "--url", default=BASE_URL, help=f"Base URL of the server (default: {BASE_URL})"
    )
