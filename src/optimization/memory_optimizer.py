...
class MemoryOptimizer:
    ...
    def __init__(self):
        ...
        )  # Weak references to cached objects
        ...
        logger.info(
            f"MemoryOptimizer initialized. Initial memory: {self.initial_memory:.2f} MB"
        )
    ...
    def optimize_dataframe(
        ...
        if len(df) > self.MAX_DATAFRAME_SIZE:
            logger.warning(
                f"DataFrame '{name}' exceeds size limit ({len(df)} > {self.MAX_DATAFRAME_SIZE}). Truncating..."
            )
        ...
        if len(df) < initial_rows:
            logger.info(
                f"Removed {initial_rows - len(df)} duplicate rows from '{name}'"
            )
        ...
        if reduction > 10:  # Log significant reductions
            logger.info(
                f"Optimized DataFrame '{name}': {initial_memory:.2f} MB -> {final_memory:.2f} MB ({reduction:.1f}% reduction)"
            )
    ...
    def limit_cache_size(self, cache_dict: Dict) -> Dict:
        ...
        if cache_size_mb > self.MAX_CACHE_SIZE_MB:
            ...
            logger.warning(
                f"Cache size exceeded limit ({cache_size_mb:.2f} MB > {self.MAX_CACHE_SIZE_MB} MB). "
    ...
    def collect_garbage(self, force: bool = False) -> Dict[str, Any]:
        ...
        with self.gc_lock:
            ...
            if stats["memory_freed_mb"] > 10:  # Log significant collections
                logger.info(
                    f"Garbage collection freed {stats['memory_freed_mb']:.2f} MB "
    ...
    def monitor_memory_usage(self) -> Optional[Dict[str, Any]]:
        ...
        if memory_percent > self.MEMORY_WARNING_PERCENT:
            ...
            logger.warning(
                f"High memory usage detected: {current_memory:.2f} MB ({memory_percent:.1f}%)"
            )
    ...
    def setup_automatic_cleanup(self, interval_minutes: int = 15):
        ...
        cleanup_task()
        logger.info(
            f"Automatic memory cleanup scheduled every {interval_minutes} minutes"
        )
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(result, pd.DataFrame) and len(result) > max_rows:
                logger.warning(
                    f"Function {func.__name__} returned DataFrame with {len(result)} rows. "
            ...
            # Check if result is a dict containing DataFrames
                ...
                for key, value in result.items():
                    if isinstance(value, pd.DataFrame) and len(value) > max_rows:
                        logger.warning(
                            f"Function {func.__name__} returned DataFrame '{key}' with {len(value)} rows. "
                            f"Truncating to {max_rows} rows."
                        )
...
def test_memory_optimizer():
    ...
    print(
        f"  Optimized size: {df_optimized.memory_usage(deep=True).sum() / 1024 / 1024:.2f} MB"
    )
