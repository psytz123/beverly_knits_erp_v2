...
class PerformanceProfiler:
        ...
        for i in range(samples):
            try:
                ...
                if response.status_code < 400:
                    ...
                    errors += 1
                    logger.warning(
                        f"Error response from {endpoint}: {response.status_code}"
                    )
    ...
    def run_load_test(
        ...
        logger.info(
            f"Running load test on {endpoint} with {concurrent_users} users for {duration_seconds}s..."
        )
        ...
        logger.info(
            f"Load test complete. Processed {len(results['requests'])} requests."
        )
        ...
        if self.profiles:
            ...
            report["summary"] = {
                ...
                "slowest_endpoint": max(
                    self.profiles.values(), key=lambda p: p.avg_response_time
                ).endpoint,
                ...
                "critical_bottlenecks": sum(
                    1 for b in report["bottlenecks"] if b["severity"] == "CRITICAL"
                ),
            }
...
def test_performance_profiler():
    ...
    for bottleneck in bottlenecks:
        print(
            f"    - {bottleneck['type']}: {bottleneck['endpoint']} ({bottleneck['severity']})"
        )
