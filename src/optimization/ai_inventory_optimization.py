...
class InventoryIntelligenceAPI:
    ...
    def get_yarn_forecast(
        self, yarn_id: str, historical_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if window_size > 0:
                moving_avg = np.convolve(
                    demand_series, np.ones(window_size) / window_size, mode="valid"
                )
                baseline_forecast = (
                    moving_avg[-1] if len(moving_avg) > 0 else np.mean(demand_series)
                )
            else:
                baseline_forecast = (
                    np.mean(demand_series) if len(demand_series) > 0 else 100
                )
            ...
            for i in range(forecast_horizon):
                ...
                forecast_val = (
                    baseline_forecast * trend_factor * seasonality * (1 + noise)
                )
            ...
            if len(demand_series) > 1:
                mape = (
                    np.mean(np.abs(np.diff(demand_series) / demand_series[:-1])) * 100
                )
            ...
            return {
                ...
                "accuracy_metrics": {"mape": round(mape, 2), "rmse": round(rmse, 2)},
            }
    ...
    def optimize_safety_stock(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            reduction_percentage = (
                (traditional_safety_stock - optimized_safety_stock)
                / traditional_safety_stock
            ) * 100
            ...
            return {
                ...
                "factors": {
                    "demand_variability": (
                        "high" if demand_std > demand_mean * 0.5 else "low"
                    ),
                    "supplier_reliability": supplier_reliability,
    ...
    def get_reorder_recommendation(
        ...
        try:
            ...
            confidence = min(
                0.9, 0.5 + (demand_rate / 100)
            )  # More demand = higher confidence
...
class AIInventoryOptimizer:
    ...
    def optimize_inventory_levels(
        ...
        for item in current_inventory:
            ...
            product_demand = (
                demand_history[demand_history["product_id"] == product_id]
                if "product_id" in demand_history.columns
                else pd.DataFrame()
            )
    ...
    def _calculate_optimal_levels(
        ...
        daily_demand = (
            demand_data.groupby("date")["quantity"].sum()
            if "quantity" in demand_data.columns
            else pd.Series([current_stock / 30])
        )
        ...
        eoq = (
            np.sqrt((2 * avg_demand * 365 * ordering_cost) / holding_cost)
            if holding_cost > 0
            else avg_demand * 30
        )
...
class DynamicSafetyStockOptimizer:
    ...
    def optimize_safety_stock(
        ...
        new_safety_stock = max(
            ...
            min(
                new_safety_stock,
                self.config.max_safety_stock_days * demand_history["quantity"].mean(),
            ),
        )
        ...
        return {
            ...
            "service_level_gap": service_level_actual
            - self.config.service_level_target,
        }
