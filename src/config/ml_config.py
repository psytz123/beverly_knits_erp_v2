...
ARIMA_CONFIG = ModelConfig(
    ...
    training_params={"method": "lbfgs", "maxiter": 100, "disp": False},
    target="demand",
...
PROPHET_CONFIG = ModelConfig(
    ...
    training_params={"include_history": True, "uncertainty_samples": 1000},
    target="demand",
    validation_split=0.2,
...
XGBOOST_CONFIG = ModelConfig(
    ...
    features=[
        "lag_1",
        "lag_7",
        "lag_30",  # Lagged features
        "rolling_mean_7",
        "rolling_mean_30",  # Rolling statistics
        "month",
        "quarter",
        "dayofweek",  # Time features
        "price",
        "inventory_level",  # Business features
        "is_holiday",
        "is_weekend",  # Calendar features
    ],
    target="demand",
    validation_split=0.2,
    test_split=0.1,
...
YARN_SUBSTITUTION_CONFIG = ModelConfig(
    ...
    training_params={"cv_folds": 5, "scoring": "f1_weighted"},
    features=[
        "color_similarity",
        "weight_similarity",
        "material_match",
        ...
        for name, config in MODEL_REGISTRY.items():
            ...
            if config.min_accuracy_threshold > 0.95:
                issues["warnings"].append(
                    f"{name}: very high accuracy threshold ({config.min_accuracy_threshold})"
                )

            if config.retrain_frequency_days > 30:
                issues["warnings"].append(
                    f"{name}: infrequent retraining ({config.retrain_frequency_days} days)"
                )

        # Check global config
        if ML_GLOBAL_CONFIG["max_parallel_jobs"] > os.cpu_count():
            issues["warnings"].append(
                f"max_parallel_jobs ({ML_GLOBAL_CONFIG['max_parallel_jobs']}) > CPU count ({os.cpu_count()})"
            )
