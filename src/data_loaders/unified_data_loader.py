...
try:
    ...
    try:
        ...
        logger.warning(
            "ColumnStandardizer not available, column standardization disabled"
        )
...
class ConsolidatedDataLoader:
    ...
    def __init__(
        ...
        logger.info(
            f"Consolidated Data Loader initialized - {self.max_workers} workers, caching enabled"
        )
        ...
        try:
            ...
            if config_path.exists():
                with open(config_path, "r") as f:
                    ...
                    self.use_database = (
                        config.get("data_source", {}).get("primary") == "database"
                    )
    ...
    def _standardize_columns(self, df: pd.DataFrame, file_type: str) -> pd.DataFrame:
        ...
        if file_type == "yarn_inventory":
            ...
            planning_col = (
                ColumnStandardizer.find_column(
                    df, ["Planning Balance", "Planning_Balance", "Planning_Ballance"]
                )
                if ColumnStandardizer
            ...
            if planning_col not in df.columns:
                theoretical_col = (
                    ColumnStandardizer.find_column(
                        df, ["Theoretical Balance", "Theoretical_Balance"]
                    )
                    if ColumnStandardizer
                    else "Theoretical Balance"
                )
                allocated_col = (
                    ColumnStandardizer.find_column(df, ["Allocated", "allocated"])
                    if ColumnStandardizer
                    else "Allocated"
                )
                on_order_col = (
                    ColumnStandardizer.find_column(df, ["On Order", "On_Order"])
                    if ColumnStandardizer
                    else "On Order"
                )
                ...
                if all(
                    col in df.columns
                    for col in [theoretical_col, allocated_col, on_order_col]
                ):
                    # CRITICAL: Allocated is ALREADY NEGATIVE in files
                    df["Planning Balance"] = (
                        df[theoretical_col] + df[allocated_col] + df[on_order_col]
                    )
    ...
    def _load_file(
        self, file_patterns: Union[str, List[str]], file_type: str
        ...
        try:
            ...
            if df is not None and len(df) > 0:
                ...
                if any(val in column_names for val in first_row_values if val != "nan"):
                    logger.warning(
                        f"Detected header row as data in {file_type}, removing..."
                    )
    ...
    def load_yarn_inventory(self) -> Optional[pd.DataFrame]:
        ...
        if self.use_database and DATABASE_AVAILABLE:
            try:
                ...
                if self.db_conn:
                    ...
                    logger.info(
                        f"Loaded yarn inventory from database: {len(df)} records"
                    )
            except Exception as e:
        ...
        self.load_times["yarn_inventory"] = time.time() - start_time
        logger.info(
            f"Loaded yarn inventory: {len(df) if df is not None else 0} records in {self.load_times['yarn_inventory']:.2f}s"
        )
    ...
    def load_sales_orders(self) -> Optional[pd.DataFrame]:
        ...
        self.load_times["sales_orders"] = time.time() - start_time
        logger.info(
            f"Loaded sales orders: {len(df) if df is not None else 0} records in {self.load_times['sales_orders']:.2f}s"
        )
    ...
    def load_bom(self) -> Optional[pd.DataFrame]:
        ...
        self.load_times["bom"] = time.time() - start_time
        logger.info(
            f"Loaded BOM: {len(df) if df is not None else 0} records in {self.load_times['bom']:.2f}s"
        )
    ...
    def load_all_data_parallel(self) -> Dict[str, pd.DataFrame]:
        ...
        logger.info(
            f"Parallel load complete: {total_records} total records in {total_time:.2f}s"
        )
        logger.info(
            f"Cache performance: {self.cache_stats['hits']} hits, {self.cache_stats['misses']} misses"
        )
    ...
    def load_knit_orders(self) -> Optional[pd.DataFrame]:
        ...
        self.load_times["knit_orders"] = time.time() - start_time
        logger.info(
            f"Loaded knit orders: {len(df) if df is not None else 0} records in {self.load_times['knit_orders']:.2f}s"
        )
    ...
    def load_styles(self) -> Optional[pd.DataFrame]:
        ...
        self.load_times["styles"] = time.time() - start_time
        logger.info(
            f"Loaded styles: {len(df) if df is not None else 0} records in {self.load_times['styles']:.2f}s"
        )
    ...
    def validate_data_integrity(self) -> Dict[str, List[str]]:
        ...
        if yarn_df is not None:
            ...
            if "Planning_Balance" not in yarn_df.columns:
                ...
                if all(
                    col in yarn_df.columns
                    for col in ["Theoretical_Balance", "Allocated", "On_Order"]
                ):
                    calc_balance = (
                        yarn_df["Theoretical_Balance"]
                        + yarn_df["Allocated"]
                        + yarn_df["On_Order"]
                    )
                    discrepancies = (
                        yarn_df["Planning_Balance"] - calc_balance
                    ).abs() > 0.01
                    if discrepancies.any():
                        yarn_issues.append(
                            f"Planning Balance calculation errors: {discrepancies.sum()} rows"
                        )
...
def test_unified_loader():
    ...
    print(
        f"   Yarn Inventory: {len(yarn_df) if yarn_df is not None else 'Failed'} records"
    )

    sales_df = loader.load_sales_orders()
    print(
        f"   Sales Orders: {len(sales_df) if sales_df is not None else 'Failed'} records"
    )
...
def integrate_database_loader(erp_instance):
    ...
    def enhanced_load():
        ...
        try:
            ...
            logger.warning(
                f"Consolidated loading failed: {e}. Falling back to original."
            )
