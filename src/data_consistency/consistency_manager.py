...
class DataConsistencyManager:
    ...
    def standardize_columns(
        cls, df: pd.DataFrame, inplace: bool = False
    ) -> pd.DataFrame:
        """
    ...
    def calculate_yarn_shortage(cls, yarn_row: pd.Series) -> Dict[str, Any]:
        ...
        theoretical_balance = cls._get_value(
            yarn_row, "theoretical_balance", planning_balance
        )
        allocated = abs(
            cls._get_value(yarn_row, "allocated", 0)
        )  # Allocated is typically negative
        ...
        urgency_score = min(
            100, (shortage_amount / abs(cls.SHORTAGE_THRESHOLDS["critical"])) * 100
        )
    ...
    @classmethod
    def aggregate_yarn_requirements(
        cls, bom_df: pd.DataFrame, production_df: pd.DataFrame
    ) -> Dict[str, float]:
        """
    ...
    def validate_data_consistency(
        ...
        validation_results["statistics"]["critical_shortages"] = sum(
            1 for s in shortage_analysis if s["risk_level"] == "CRITICAL"
        )
        ...
        validation_results["statistics"]["yarn_requirements_count"] = len(
            yarn_requirements
        )
        validation_results["statistics"]["total_required_lbs"] = sum(
            yarn_requirements.values()
        )
        ...
        report = {
            ...
            "shortages": {
                "summary": {
                    ...
                    "critical_count": sum(
                        1 for s in all_shortages if s["risk_level"] == "CRITICAL"
                    ),
                    "high_count": sum(
                        1 for s in all_shortages if s["risk_level"] == "HIGH"
                    ),
                    "total_shortage_lbs": sum(
                        s["shortage_amount"] for s in all_shortages
                    ),
                },
            ...
            "recommendations": cls._generate_recommendations(
                validation, all_shortages, yarn_requirements
            ),
        }
    ...
    def _generate_recommendations(
        cls, validation: Dict, shortages: List[Dict], requirements: Dict[str, float]
    ) -> List[Dict]:
        ...
        if validation["warnings"]:
            bom_warnings = [
                w for w in validation["warnings"] if w["type"] == "MISSING_BOM"
            ]
            if bom_warnings:
                recommendations.append(
                    {
