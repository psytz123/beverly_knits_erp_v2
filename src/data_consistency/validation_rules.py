...
class DataValidationRules:
                ...
                results["warnings"].append(
                    {
                        ...
                        "sample_yarns": (
                            discrepancies["Desc#"].head(5).tolist()
                            if "Desc#" in df.columns
                            else []
                        ),
                    }
    ...
    def validate_bom(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "Style#" in df.columns:
            ...
            results["statistics"]["avg_yarns_per_style"] = (
                len(df) / unique_styles if unique_styles > 0 else 0
            )
                ...
                results["warnings"].append(
                    {
                        ...
                        "affected_styles": invalid_yarns["Style#"]
                        .unique()
                        .tolist()[:10],
                    }
                )
    ...
    def validate_production_orders(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        order_col = (
            ...
            else "Order#" if "Order#" in df.columns else None
        )
        if order_col:
        ...
        if all(
            col in df.columns
            for col in ["Qty Ordered (lbs)", "Shipped (lbs)", "Balance (lbs)"]
        ):
            # Balance should equal Ordered - Shipped
            df["calc_balance"] = df["Qty Ordered (lbs)_numeric"] - df[
                "Shipped (lbs)_numeric"
            ].fillna(0)
            discrepancies = df[
                abs(df["Balance (lbs)_numeric"] - df["calc_balance"]) > 1
            ]
                ...
                results["warnings"].append(
                    {
                        ...
                        "sample_orders": (
                            discrepancies[order_col].head(5).tolist()
                            if order_col
                            else []
                        ),
                    }
    ...
    def cross_validate_data(
        inventory_df: pd.DataFrame, bom_df: pd.DataFrame, production_df: pd.DataFrame
    ) -> Dict[str, Any]:
        """
                    ...
                    results["orphaned_data"].append(
                        {
                            ...
                            "total_value": orphaned_with_inventory[
                                "Planning Balance"
                            ].sum(),
                            "sample_yarns": orphaned_with_inventory["Desc#"]
                            .head(10)
                            .tolist(),
                        }
                    )
