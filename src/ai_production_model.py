...
try:
    ...
    from src.production.production_capacity_manager import (
        get_capacity_manager,
        ProductionCapacityManager,
    )
    from src.ml_models.production_recommendation_ml import (
        get_ml_model,
        ProductionRecommendationModel,
    )
except ImportError:
    try:
        from production.machine_mapper import (
            get_machine_mapper,
            MachineWorkCenterMapper,
        )
        from production.production_capacity_manager import (
            get_capacity_manager,
            ProductionCapacityManager,
        )
        from ml_models.production_recommendation_ml import (
            get_ml_model,
            ProductionRecommendationModel,
        )
    except ImportError:
        logger.warning(
            "Could not import production components - some features will be disabled"
        )
...
class ProductionOptimization:
    ...
    optimization_type: (
        str  # "REBALANCE", "PRIORITY_ADJUST", "CAPACITY_ADD", "SCHEDULE_CHANGE"
    )
...
class AIProductionModel:
    ...
    def _generate_simulation_data(self):
        ...
        logger.info(
            f"Generated simulation data for {len(self.historical_utilization)} machines"
        )
    ...
    def detect_bottlenecks(self) -> List[BottleneckAlert]:
        ...
        for (
            work_center,
            machine_ids,
        ) in self.machine_mapper.work_center_to_machines.items():
            if not machine_ids:
                continue
            ...
            for machine_id in machine_ids:
                ...
                # Get assigned style
                assigned_style = self.capacity_manager.get_machine_assignment(
                    machine_id
                )
                if assigned_style:
                    affected_styles.add(assigned_style)
                    style_capacity = self.capacity_manager.get_style_capacity(
                        assigned_style
                    )
            ...
            severity = self._calculate_bottleneck_severity(
                avg_utilization, len(machine_ids)
            )
            ...
            if severity != BottleneckSeverity.NONE:
                ...
                delay_days = self._estimate_delay_impact(
                    capacity_shortage, total_capacity
                )

                # Generate recommendation
                recommendation = self._generate_bottleneck_recommendation(
                    severity, work_center, avg_utilization
                )

                # Calculate urgency score
                urgency = self._calculate_urgency_score(
                    severity, delay_days, len(affected_styles)
                )
    ...
    def _calculate_bottleneck_severity(
        self, utilization: float, machine_count: int
    ) -> BottleneckSeverity:
    ...
    def _estimate_delay_impact(
        self, capacity_shortage: float, total_capacity: float
    ) -> float:
    ...
    def _generate_bottleneck_recommendation(
        self, severity: BottleneckSeverity, work_center: str, utilization: float
    ) -> str:
    ...
    def generate_production_forecast(
        ...
        seasonal_factor = 1.0 + 0.1 * math.sin(
            2 * math.pi * datetime.now().timetuple().tm_yday / 365
        )
    ...
    def get_factory_floor_ai_insights(self) -> Dict[str, Any]:
        ...
        machine_status = (
            self.capacity_manager.get_machine_level_status()
    ...
    def _calculate_ai_kpis(
        ...
        optimizations: List[ProductionOptimization],
        ...
        high_bottlenecks = len(
            [b for b in bottlenecks if b.severity == BottleneckSeverity.HIGH]
        )
...
if __name__ == "__main__":
    ...
    for i, bottleneck in enumerate(bottlenecks[:3]):  # Show top 3
        print(
            f"  {i+1}. WC {bottleneck.work_center}: {bottleneck.severity.value} "
    ...
    for i, opt in enumerate(optimizations[:3]):  # Show top 3
        print(
            f"  {i+1}. {opt.optimization_type}: {opt.potential_improvement_percent:.1f}% improvement"
        )
    ...
    print(
        f"  Optimization opportunities: {ai_kpis['optimization_potential']['opportunities']}"
    )
    print(
        f"  Actionable insights: {len(insights['ai_analysis']['actionable_insights'])}"
    )
