...
class ForecastAccuracyMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"Forecast Accuracy Monitor initialized with {accuracy_threshold*100}% threshold"
        )
    ...
    def _init_database(self):
        ...
        # Create indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_yarn_forecast ON forecast_metrics(yarn_id, forecast_date)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_accuracy ON forecast_metrics(accuracy)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_alerts ON performance_alerts(severity, resolved)"
        )
    ...
    def track_forecast(
        ...
        try:
            ...
            conn.commit()
            logger.info(
                f"Tracked forecast for {yarn_id} with {horizon_weeks}-week horizon"
            )
    ...
    def evaluate_forecast(
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Evaluated forecast for {yarn_id}: Accuracy = {metrics['accuracy']:.2%}"
                )
    ...
    def _calculate_metrics(
        ...
        # Accuracy (1 - MAPE)
        metrics["accuracy"] = (
            1 - metrics["mape"] if metrics["mape"] is not None else 1.0
        )
    ...
    def _check_accuracy_alert(self, metrics: AccuracyMetrics):
        ...
        if metrics.accuracy < self.accuracy_threshold:
            severity = (
                "critical"
                if metrics.accuracy < (self.accuracy_threshold - 0.1)
                else "warning"
            )
    ...
    def get_accuracy_report(
        self, yarn_id: Optional[str] = None, period_days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        if all_accuracies:
            report["overall_metrics"] = {
                ...
                "target_achievement_rate": report["yarns_meeting_target"]
                / len(all_accuracies),
            }
    ...
    def start_continuous_monitoring(self, check_interval_hours: int = 1):
        ...
        logger.info(
            f"Started continuous monitoring with {check_interval_hours}-hour interval"
        )
