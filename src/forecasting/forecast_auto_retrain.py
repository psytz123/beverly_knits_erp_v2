...
class AutomaticRetrainingSystem:
    ...
    def __init__(
        ...
        self.accuracy_monitor = ForecastAccuracyMonitor(
            db_path="forecast_accuracy.db", accuracy_threshold=0.90
        )
    ...
    def retrain_models(self) -> Dict[str, Any]:
        ...
        results = {
            ...
            "average_accuracy": (
                np.mean(list(model_accuracies.values())) if model_accuracies else 0
            ),
        }
    ...
    def schedule_retraining(self):
        ...
        if self.retrain_schedule == "weekly":
            ...
            if self.retrain_day == "monday":
                ...
                schedule.every().tuesday.at(f"{self.retrain_hour:02d}:00").do(
                    self.retrain_models
                )
            elif self.retrain_day == "wednesday":
                schedule.every().wednesday.at(f"{self.retrain_hour:02d}:00").do(
                    self.retrain_models
                )
            elif self.retrain_day == "thursday":
                schedule.every().thursday.at(f"{self.retrain_hour:02d}:00").do(
                    self.retrain_models
                )
            elif self.retrain_day == "friday":
                schedule.every().friday.at(f"{self.retrain_hour:02d}:00").do(
                    self.retrain_models
                )
            elif self.retrain_day == "saturday":
                schedule.every().saturday.at(f"{self.retrain_hour:02d}:00").do(
                    self.retrain_models
                )
            elif self.retrain_day == "sunday":
                schedule.every().sunday.at(f"{self.retrain_hour:02d}:00").do(
                    self.retrain_models
                )
    ...
    def get_training_status(self) -> Dict[str, Any]:
        ...
        if accuracy_report and accuracy_report.get("overall_metrics"):
            status["current_accuracy"] = accuracy_report["overall_metrics"].get(
                "average_accuracy", 0
            )
            status["yarns_meeting_target"] = accuracy_report.get(
                "yarns_meeting_target", 0
            )
...
def integrate_with_erp(app):
    ...
    retrain_system = AutomaticRetrainingSystem(
        retrain_schedule="weekly", retrain_day="sunday", retrain_hour=2
    )
    ...
    def get_accuracy_report():
        ...
        report = retrain_system.accuracy_monitor.get_accuracy_report(
...
if __name__ == "__main__":
    ...
    print(
        f"Status: {json.dumps(retrain_system.get_training_status(), indent=2, default=str)}"
    )
