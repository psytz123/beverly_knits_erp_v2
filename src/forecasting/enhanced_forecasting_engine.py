...
class ForecastConfig:
    ...
    def __post_init__(self):
        if self.ensemble_weights is None:
            self.ensemble_weights = {"prophet": 0.4, "xgboost": 0.35, "arima": 0.25}
...
class EnhancedForecastingEngine:
    ...
    def __init__(self, config: ForecastConfig = None):
        ...
        logger.info(
            f"Enhanced Forecasting Engine initialized with {len(self.models)} models"
        )
        logger.info(
            f"Target: {self.config.min_accuracy_threshold*100}% accuracy at {self.config.horizon_weeks}-week horizon"
        )
    ...
    def forecast(
        ...
        if self.config.use_historical and not historical_data.empty:
            historical_forecast = self._forecast_from_historical(
                yarn_id, historical_data
            )
        ...
        accuracy_metrics = self._calculate_accuracy_metrics(
            final_forecast, historical_data
        )
    ...
    def _forecast_from_historical(
        self, yarn_id: str, data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        if "prophet" in self.models and PROPHET_AVAILABLE:
            try:
                ...
                ensemble_weights.append(
                    self.config.ensemble_weights.get("prophet", 0.33)
                )
        ...
        if "xgboost" in self.models and XGBOOST_AVAILABLE:
            try:
                ...
                ensemble_weights.append(
                    self.config.ensemble_weights.get("xgboost", 0.33)
                )
            except Exception as e:
        ...
        if ensemble_predictions:
            ...
            # Weighted average
            final_pred = sum(
                pred * weight for pred, weight in zip(ensemble_predictions, weights)
            )
            ...
            return pd.DataFrame(
                {"date": forecast_dates, "forecast": final_pred, "type": "historical"}
            )

        return pd.DataFrame()

    def _forecast_from_orders(
        self, yarn_id: str, order_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        if "order_date" in order_data.columns:
            order_data["week"] = pd.to_datetime(order_data["order_date"]).dt.to_period(
                "W"
            )
        ...
        forecast_dates = pd.date_range(
            start=datetime.now(), periods=self.config.horizon_weeks, freq="W"
        )
        ...
        for date in forecast_dates:
            ...
            if week in weekly_demand.index:
                ...
                forecast_values.append(
                    weekly_demand.mean() if len(weekly_demand) > 0 else 0
                )

        return pd.DataFrame(
            {"date": forecast_dates, "forecast": forecast_values, "type": "order_based"}
        )

    def _combine_forecasts(
        self, historical: Optional[pd.DataFrame], orders: Optional[pd.DataFrame]
    ) -> pd.DataFrame:
    ...
    def _prophet_forecast(self, data: pd.DataFrame) -> np.ndarray:
        ...
        prophet_data = pd.DataFrame({"ds": data.index, "y": data.values.flatten()})
        ...
        future = model.make_future_dataframe(
            periods=self.config.horizon_weeks, freq="W"
        )
    ...
    def _create_lag_features(
        self, data: np.ndarray, lag: int = 4
    ...
    def _calculate_accuracy_metrics(
        ...
        if len(historical) < 10:
            return {"mape": None, "rmse": None, "mae": None, "accuracy": None}
    ...
    def _calculate_confidence_intervals(self, forecast: pd.DataFrame) -> pd.DataFrame:
        ...
        std_dev = (
            ...
            if len(forecast) > 1
            else forecast["forecast"].mean() * 0.1
        )
        ...
        confidence_intervals["lower_bound"] = confidence_intervals["lower_bound"].clip(
            lower=0
        )
    ...
    def retrain_models(
        ...
        model_accuracies = {"prophet": [], "xgboost": [], "arima": []}
...
if __name__ == "__main__":
    ...
    config = ForecastConfig(
        horizon_weeks=9, min_accuracy_threshold=0.90, retrain_frequency="weekly"
    )
