...
class InventoryForecastPipeline:
    ...
    def run_forecast_pipeline(
        ...
        try:
            ...
            if product_ids is None:
                product_ids = (
                    historical_data["product_id"].unique()
    ...
    def _prophet_forecast(
        ...
        try:
            ...
            prophet_data = pd.DataFrame({"ds": data.index, "y": data["quantity"]})
            ...
            future = model.make_future_dataframe(
                periods=max(self.config.forecast_horizons)
            )
    ...
    def _ml_forecast(
        self, data: pd.DataFrame, model_type: str, product_id: str
    ) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            for horizon in self.config.forecast_horizons:
                ...
                future_features["day_of_week"] = (
                    last_features["day_of_week"].iloc[0] + horizon
                ) % 7
            ...
            logger.error(
                f"ML forecast error for {product_id} with {model_type}: {str(e)}"
            )
            return None
    ...
    def _create_features(self, data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Rolling statistics
            df["rolling_mean_7"] = (
                df["quantity"].rolling(window=7, min_periods=1).mean()
            )
            df["rolling_mean_30"] = (
                df["quantity"].rolling(window=30, min_periods=1).mean()
            )
    ...
    def _ensemble_forecast(
        ...
        weights = {"prophet": 0.4, "rf": 0.3, "gbm": 0.3}
    ...
    def load_models(self) -> bool:
        ...
        try:
            for model_type in ["rf", "gbm"]:
                model_path = (
                    Path(self.config.model_save_path) / f"{model_type}_model.pkl"
                )
                scaler_path = (
                    Path(self.config.model_save_path) / f"{model_type}_scaler.pkl"
                )
...
# Module exports
__all__ = ["InventoryForecastPipeline", "PipelineConfig"]
