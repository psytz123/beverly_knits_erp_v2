...
from .enhanced_forecasting_engine import (
...
class ForecastValidationSystem:
    ...
    def __init__(self, accuracy_target: float = 0.90, confidence_level: float = 0.95):
        """
    ...
    def backtest_yarn(
        ...
        model_accuracies = {"prophet": [], "xgboost": [], "arima": [], "ensemble": []}
                ...
                if len(actuals) > 0 and len(predictions) > 0:
                    ...
                    if non_zero_mask.any():
                        ...
                        if result.confidence_intervals is not None:
                            lower = result.confidence_intervals["lower_bound"].values[
                                : len(actuals)
                            ]
                            upper = result.confidence_intervals["upper_bound"].values[
                                : len(actuals)
                            ]
        ...
        ensemble_accuracy = (
            np.mean(model_accuracies["ensemble"]) if model_accuracies["ensemble"] else 0
        )
        ...
        logger.info(
            f"Backtest complete for {yarn_id}: Accuracy = {ensemble_accuracy:.2%}"
        )

        return result

    def validate_forecast_system(
        self, test_data: Dict[str, pd.DataFrame], sample_size: Optional[int] = None
    ) -> ValidationReport:
        """
        ...
        if all_accuracies:
            ...
            yarns_meeting_target = sum(
                1 for acc in all_accuracies if acc >= self.accuracy_target
            )
        ...
        logger.info(
            f"Target achievement: {yarns_meeting_target}/{len(all_accuracies)} yarns"
        )
        logger.info(f"Validation {'PASSED' if validation_passed else 'FAILED'}")

        return report

    def _analyze_weekly_accuracy(
        self, yarn_id: str, data: pd.DataFrame, accuracy_by_week: Dict[int, List[float]]
    ):
    ...
    def generate_validation_plots(self, output_dir: str = "validation_plots"):
        ...
        if latest_report.accuracy_by_week:
            ...
            plt.bar(
                ...
                accuracies,
                color=[
                    "green" if acc >= self.accuracy_target else "red"
                    for acc in accuracies
                ],
            )
            plt.axhline(
                y=self.accuracy_target,
                color="blue",
                linestyle="--",
            ...
            plt.savefig(
                output_path / "accuracy_by_week.png", dpi=100, bbox_inches="tight"
            )
            plt.close()
        ...
        if latest_report.model_rankings:
            ...
            plt.savefig(
                output_path / "model_comparison.png", dpi=100, bbox_inches="tight"
            )
            plt.close()
        ...
        if self.backtest_results:
            ...
            plt.savefig(
                output_path / "accuracy_distribution.png", dpi=100, bbox_inches="tight"
            )
            plt.close()
    ...
    def export_validation_report(self, filepath: str = "validation_report.json"):
        ...
        if self.backtest_results:
            backtest_summary = {
                ...
                "average_ensemble_accuracy": np.mean(
                    [r.ensemble_accuracy for r in self.backtest_results.values()]
                ),
                "average_confidence_coverage": np.mean(
                    [
                        r.confidence_intervals_coverage
                        for r in self.backtest_results.values()
                    ]
                ),
                "average_forecast_bias": np.mean(
                    [r.forecast_bias for r in self.backtest_results.values()]
                ),
            }
    ...
    def continuous_validation(
        self, data_source: callable, interval_hours: int = 24, sample_size: int = 50
    ):
        """
            ...
            while True:
                try:
                    ...
                    if not report.validation_passed:
                        logger.warning(
                            f"Validation failed! Average accuracy: {report.average_accuracy:.2%}"
                        )
        ...
        logger.info(
            f"Started continuous validation with {interval_hours}-hour interval"
        )
...
if __name__ == "__main__":
    ...
    for i in range(20):
        ...
        test_data[yarn_id] = pd.DataFrame({"date": dates, "Consumed": consumed})
    ...
    print(
        f"Target Achievement: {report.yarns_meeting_target}/{report.total_yarns_tested}"
    )
