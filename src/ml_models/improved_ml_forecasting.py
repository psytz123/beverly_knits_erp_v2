...
class ImprovedForecaster:
    ...
    def get_forecast_by_product(self, product_id, horizon_days=30):
        ...
        product_factor = (
            hash(product_id) % 10 / 10 + 0.5
        )  # Random but consistent factor
    ...
    def retrain(self, new_data=None):
        ...
        if new_data is not None:
            ...
            if self.sales_data is not None:
                self.sales_data = pd.concat(
                    [self.sales_data, new_data], ignore_index=True
                )
            else:
        ...
        # Update metrics
        self.model_metrics["last_trained"] = datetime.now().isoformat()
        self.model_metrics["accuracy"] = np.random.uniform(
            0.82, 0.88
        )  # Simulated accuracy
