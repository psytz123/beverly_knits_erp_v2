...
class MLForecastIntegration:
    ...
    def __init__(self, data_path=None):
        self.data_path = (
            data_path or Path(__file__).parent.parent / "ERP Data" / "prompts" / "5"
        )
    ...
    def prepare_sales_data(self, sales_df=None):
        ...
        # Aggregate by date
        daily_sales = (
            sales_df.groupby(sales_df["Date"].dt.date)
            .agg({"Qty Shipped": "sum"})
            .reset_index()
        )
    ...
    def train_forecast_model(self, force_retrain=False):
        ...
        try:
            if self.engine_type == "full":
                ...
                forecast_output = self.forecast_engine.generate_forecast_output(
                    raw_data
                )
    ...
    def _create_forecast_output(self, results):
        ...
        if "forecast" in best_result and best_result["forecast"] is not None:
            ...
            output["summary"] = {
                ...
                "peak_day": (
                    output["daily_forecasts"][np.argmax(forecast_values)]["date"]
                    if output["daily_forecasts"]
                    else None
                ),
                "forecast_volatility": (
                    float(np.std(forecast_values) / np.mean(forecast_values) * 100)
                    if np.mean(forecast_values) > 0
                    else 0
                ),
            }
    ...
    def get_inventory_recommendations(self, current_inventory_df):
        ...
        if next_30_days:
            ...
            for _, item in current_inventory_df.iterrows():
                ...
                if current_stock > 0:
                    weeks_of_supply = (
                        current_stock / avg_weekly_demand
                        ...
                        recommendations.append(
                            {
                                ...
                                "recommended_order": avg_weekly_demand
                                * 4,  # 4 weeks safety stock
                            }
                        )
...
if __name__ == "__main__":
    ...
    if "error" not in result:
        print(
            f"   Success! Best model: {result.get('best_model')} ({result.get('best_accuracy', 0):.1f}% accuracy)"
        )
        print(
            f"   90-day total forecast: {result.get('summary', {}).get('total_forecast', 0):,.0f} units"
        )
    else:
