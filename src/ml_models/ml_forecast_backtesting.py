...
class MLForecastBacktester:
    ...
    def _initialize_models(self):
        ...
        if ML_AVAILABLE:
            ...
            try:
                ...
                self.models["lstm"] = (
                    "lstm"  # Will be created during fit with proper input shape
                )
            except ImportError:
    ...
    def prepare_time_series(
        self, data: pd.DataFrame, date_col: str, value_col: str, group_col: str = None
        ...
        if group_col and group_col in data.columns:
            ...
            for group_id in data[group_col].unique():
                ...
                # Aggregate by day
                daily = (
                    group_data.groupby(group_data[date_col].dt.date)[value_col]
                    .sum()
                    .reset_index()
                )
    ...
    def backtest_model(
        ...
        """
        results = {"model": model_name, "metrics": {}, "predictions": [], "actuals": []}
        ...
        try:
            if model_name == "prophet":
                ...
                # LSTM model
                predictions = self._lstm_forecast(
                    train_data, test_data, forecast_horizon
                )
            else:
                # Sklearn models
            ...
            if len(predictions) == len(actuals) and len(predictions) > 0:
                ...
                results["metrics"] = {
                    ...
                    "r2": float(r2),
                    "forecast_accuracy": (
                        float(100 - min(mape, 100)) if not np.isinf(mape) else 0
                    ),
                }
    ...
    def _prophet_forecast(self, train_data: pd.DataFrame, horizon: int) -> np.ndarray:
        ...
        try:
            ...
            prophet_data = pd.DataFrame(
                {"ds": pd.to_datetime(train_data["date"]), "y": train_data["value"]}
            )
    ...
    def _lstm_forecast(
        self, train_data: pd.DataFrame, test_data: pd.DataFrame, horizon: int
    ) -> np.ndarray:
    ...
    def run_complete_backtest(self, data: Dict[str, pd.DataFrame]) -> Dict:
                ...
                for series_id, ts_data in list(time_series.items())[:5]:  # Test top 5
                    ...
                    for model_name in self.models.keys():
                        ...
                        if "metrics" in result and result["metrics"]:
                            print(
                                f"   {model_name}: MAPE={result['metrics']['mape']:.2f}%, "
                    ...
                    best_model = min(
                        ...
                        key=lambda x: (
                            x[1]["metrics"].get("mape", float("inf"))
                            if "metrics" in x[1]
                            else float("inf")
                        ),
                    )
            ...
            if yarn_cols:
                for idx, row in data["yarn_demand"].head(5).iterrows():
                    ...
                    if len(weekly_values) >= 4:
                        # Create dates
                        dates = pd.date_range(
                            end=datetime.now(), periods=len(weekly_values), freq="W"
                        )

                        ts_data = pd.DataFrame({"date": dates, "value": weekly_values})
                        ...
                        for model_name in [
                            ...
                            result = self.backtest_model(
                                ts_data, model_name, test_size=0.25, forecast_horizon=4
                            )
        ...
        # Sort by performance
        sorted_rankings = sorted(model_rankings.items(), key=lambda x: x[1]["avg_mape"])

        print("\n🏆 Model Rankings (by Average MAPE):")
        for rank, (model, stats) in enumerate(sorted_rankings, 1):
            print(
                f"{rank}. {model}: {stats['avg_mape']:.2f}% "
    ...
    def generate_backtest_report(self, results: Dict) -> str:
        ...
        for forecast_type in ["sales_forecast", "yarn_demand_forecast"]:
            if results[forecast_type]:
                report.append(f"\n{forecast_type.upper()}:")
                for series_id, series_results in list(results[forecast_type].items())[
                    :3
                ]:
                    report.append(f"\n  {series_id}:")
                    for model_name, result in series_results.items():
    ...
    def plot_forecast_comparison(self, results: Dict, series_id: str):
        ...
        plot_file = (
            self.data_path
            / f"backtest_plot_{series_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
