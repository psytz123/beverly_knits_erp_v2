...
class SklearnModel(BaseMLModel):
    ...
    def __init__(
        self, name: str, model_class, model_params: Dict = None, use_scaler: bool = True
    ):
        ...
        df = pd.DataFrame(
            {
                "ds": (
                    ...
                    else pd.date_range(start="2025-01-01", periods=len(X))
                ),
                "y": y.values,
            }
        )
...
class LSTMModel(BaseMLModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series):
        ...
        X_seq_scaled = self.scaler.fit_transform(
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        X_seq_scaled = self.scaler.transform(
            X_seq.reshape(-1, X_seq.shape[-1])
        ).reshape(X_seq.shape)
...
class MLValidationSystem:
    ...
    def prepare_features(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if target_col in data.columns and len(data) > 7:
            features_df[f"{target_col}_rolling_7_mean"] = (
                data[target_col].rolling(7).mean()
            )
            features_df[f"{target_col}_rolling_7_std"] = (
                data[target_col].rolling(7).std()
            )
            features_df[f"{target_col}_rolling_30_mean"] = (
                data[target_col].rolling(30).mean()
            )
        ...
        if len(common_index) == 0:
            ...
            features_df = pd.DataFrame(
                {"trend": np.arange(len(target))}, index=target.index
            )
        ...
        if len(final_features) < 10:
            raise ValueError(
                f"Insufficient data after preprocessing: {len(final_features)} samples"
            )

        return final_features, final_target

    def validate_model(
        self, model: BaseMLModel, X: pd.DataFrame, y: pd.Series, dataset_name: str
        ...
        try:
            ...
            if len(X) > 50 and model.model_type not in ["lstm", "prophet"]:
                try:
                    tscv = TimeSeriesSplit(
                        n_splits=min(
                            self.config["validation"]["cv_folds"], len(X) // 20
                        )
                    )
            ...
            error_analysis = {
                ...
                "error_skewness": (
                    float(errors.skew()) if hasattr(errors, "skew") else 0
                ),
            }
    ...
    ) -> Dict[str, Dict[str, ModelValidationResult]]:
        """Validate all models on all datasets"""
        logger.info(
            f"Starting validation of {len(self.models)} models on {len(data_sources)} datasets"
        )
    ...
    def create_ensemble_predictions(
        ...
        try:
            ...
            if "weighted_average" in self.config["ensemble"]["methods"]:
                ...
                mape_weighted = (
                    mean_absolute_percentage_error(y_test, weighted_avg) * 100
                )
    ...
    def generate_validation_report(self) -> Dict:
        ...
        for dataset_name, dataset_results in self.results.items():
            ...
            report["dataset_analysis"][dataset_name] = {
                ...
                "best_model": (
                    min(dataset_results.items(), key=lambda x: x[1].mape)[0]
                    if dataset_results
                    else None
                ),
            }
        ...
        if all_maes and all_mapes:
            ...
            model_avg_mapes = {
                name: np.mean(perfs)
                for name, perfs in model_performances.items()
            ...
            if model_avg_mapes:
                ...
                report["model_rankings"] = [
                    {"model": name, "avg_mape": mape} for name, mape in sorted_models
                ]
        ...
        # Save report
        report_path = (
            "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/BKI_comp/ml_validation_report.json"
        )
    ...
    def _generate_ml_recommendations(self, report: Dict) -> List[str]:
        ...
        if "model_rankings" in report and len(report["model_rankings"]) > 1:
            ...
            recommendations.append(
                f"Consider ensemble of top 3 models: {', '.join(top_3)}"
            )
        ...
        if "model_rankings" in report:
            for model_info in report["model_rankings"]:
                if model_info["avg_mape"] > 50:
                    recommendations.append(
                        f"{model_info['model']} shows poor performance - consider parameter tuning or removal"
                    )
                elif model_info["avg_mape"] < 10:
                    recommendations.append(
                        f"{model_info['model']} shows excellent performance - prioritize for production use"
                    )
...
def main():
    ...
    for data_path in data_paths:
        if data_path.exists():
            for file_path in data_path.glob("*.csv"):
                try:
                    data_sources[f"{data_path.name}_{file_path.stem}"] = pd.read_csv(
                        file_path
                    )
            ...
            for file_path in data_path.glob("*.xlsx"):
                try:
                    data_sources[f"{data_path.name}_{file_path.stem}"] = pd.read_excel(
                        file_path
                    )
                    print(f"Loaded: {file_path.name}")
                except Exception as e:
    ...
    if not data_sources:
        ...
        synthetic_data = pd.DataFrame(
            {
                "Date": dates,
                "Quantity": np.random.poisson(100, 100)
                + 10 * np.sin(np.arange(100) * 2 * np.pi / 30),
