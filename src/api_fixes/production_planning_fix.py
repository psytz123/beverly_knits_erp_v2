...
def get_production_planning_fixed(data_loader) -> Dict[str, Any]:
    ...
    try:
        ...
        for idx, order in knit_orders.iterrows():
            ...
            g00_stage = (
                safe_float(order.get("G00 (lbs)", 0))
            ...
            production_item = {
                ...
                "completion_percentage": (
                    round((shipped / quantity_ordered * 100), 1)
        ...
        production_schedule.sort(
            key=lambda x: (priority_order.get(x["priority"], 4), x["days_until_due"])
        )
        ...
        capacity_analysis = {
            ...
            "utilization_percentage": min(
                100, (total_planned / (daily_capacity * 30)) * 100
            ),  # 30-day utilization
        ...
        summary = {
            ...
            "critical_orders": len(
                [p for p in production_schedule if p["priority"] == "Critical"]
            ),
            "high_priority_orders": len(
                [p for p in production_schedule if p["priority"] == "High"]
            ),
        }
