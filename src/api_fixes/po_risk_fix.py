    ...
    try:
        ...
        for idx, row in knit_orders.iterrows():
            ...
            order_info = {
                ...
                "g00_stage": (
                    safe_float(row.get("G00 (lbs)", 0))
                    if pd.notna(row.get("G00 (lbs)"))
                    else 0
                ),
            }
            ...
            order_info["due_date"] = (
                due_date.isoformat() if pd.notna(due_date) else None
            )
            order_info["start_date"] = (
                start_date.isoformat() if pd.notna(start_date) else None
            )
            ...
            order_info["production_stage"] = (
                "Greige" if order_info["g00_stage"] > 0 else "Knitting"
            )
        ...
        risk_analysis.sort(
            key=lambda x: (
                risk_priority.get(x["risk_level"], 5),
        ...
        summary = {
            ...
            "overdue_orders": len(
                [o for o in risk_analysis if o["risk_level"] == "OVERDUE"]
            ),
            "critical_orders": len(
                [o for o in risk_analysis if o["risk_level"] == "CRITICAL"]
            ),
            "high_risk_orders": len(
                [o for o in risk_analysis if o["risk_level"] == "HIGH"]
            ),
            "medium_risk_orders": len(
                [o for o in risk_analysis if o["risk_level"] == "MEDIUM"]
            ),
            "low_risk_orders": len(
                [o for o in risk_analysis if o["risk_level"] == "LOW"]
            ),
            "complete_orders": len(
                [o for o in risk_analysis if o["status"] == "Complete"]
            ),
            "in_progress_orders": len(
                [o for o in risk_analysis if o["status"] == "In Progress"]
            ),
            "not_started_orders": len(
                [o for o in risk_analysis if o["status"] == "Not Started"]
            ),
