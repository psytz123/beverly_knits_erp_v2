...
def get_production_suggestions_fixed(data_loader) -> Dict[str, Any]:
    ...
    try:
        ...
        for idx, order in knit_orders.iterrows():
            ...
            g00_stage = (
                float(order.get("G00 (lbs)", 0))
                if pd.notna(order.get("G00 (lbs)"))
                else 0
            )
            ...
            completion_pct = (
                (shipped / quantity_ordered * 100) if quantity_ordered > 0 else 0
            )
            ...
            if yarn_inventory is not None and not yarn_inventory.empty:
                ...
                total_yarn = (
                    yarn_inventory["Physical Inventory"].sum()
                    if "Physical Inventory" in yarn_inventory.columns
                    else 0
                )
                material_available = total_yarn / max(
                    1, len(knit_orders)
                )  # Rough allocation
        ...
        material_available_count = len(
            [s for s in top_suggestions if "Available" in s["material_status"]]
        )
        material_shortage_count = len(
            [s for s in top_suggestions if "Shortage" in s["material_status"]]
        )
        ...
        summary = {
            ...
            "high_priority": len(
                [s for s in top_suggestions if s["priority"] == "HIGH"]
            ),
            "medium_priority": len(
                [s for s in top_suggestions if s["priority"] == "MEDIUM"]
            ),
        ...
        return {"status": "error", "message": str(e), "suggestions": [], "summary": {}}
