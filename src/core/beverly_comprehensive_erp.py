...
try:
    from data_loaders.unified_data_loader import (
    ...
    print(
        "[OK] Consolidated data loader available - includes optimized caching (100x+), parallel loading (4x), and database support"
    )
except ImportError:
...
try:
    ...
    def get_inventory_risk_report(*_args, **_kwargs):
        ...
        analyzer = get_inventory_analyzer()
        return {
            "risk_assessment": analyzer.analyze_all(),
...
try:
    from data_sync.sharepoint_data_connector import (
        SharePointDataConnector,
        integrate_sharepoint_with_erp,
    )
...
try:
    from data_sync.exclusive_data_config import (
        ExclusiveDataConfig,
        configure_exclusive_data_source,
    )

    EXCLUSIVE_DATA_CONFIG = True
    print(
        "Exclusive data configuration loaded - will ONLY use SharePoint ERP Data folder"
    )
except ImportError as e:
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if FEATURE_FLAGS_AVAILABLE and should_log_deprecated_usage():
                logger.warning(
                    f"Deprecated API called: {request.path} → {new_endpoint}"
                )
...
def intercept_deprecated_endpoints():
    ...
    redirect_map = {
        ...
        "/api/pipeline/inventory-risks": (
            "/api/inventory-intelligence-enhanced",
            {"view": "risks", "source": "pipeline"},
        ),
        ...
        "/api/yarn-forecast-shortages": (
            "/api/yarn-intelligence",
            {"forecast": "true", "analysis": "shortage"},
        ),
        "/api/ai/yarn-forecast": (
            "/api/yarn-intelligence",
            {"forecast": "true", "ai": "true"},
        ),
        ...
        "/api/ai/optimize-safety-stock": (
            "/api/inventory-intelligence-enhanced",
            {"ai": "true", "action": "optimize-safety"},
        ),
        "/api/ai/reorder-recommendation": (
            "/api/inventory-intelligence-enhanced",
            {"ai": "true", "action": "reorder"},
        ),
        "/api/ai/ensemble-forecast": (
            "/api/ml-forecast-detailed",
            {"ai": "true", "method": "ensemble"},
        ),
    ...
    if request.path in redirect_map:
        ...
        if request.path not in api_call_tracking:
            api_call_tracking[request.path] = {
                "count": 0,
                "redirected_to": new_endpoint,
            }
...
DATA_PATH = Path(
    os.environ.get("DATA_PATH", "/mnt/c/finalee/beverly_knits_erp_v2/data")
)
...
def has_complete_data(path):
    ...
    has_yarn = any(path.glob("**/yarn_inventory*.xlsx")) or any(
        path.glob("**/Yarn_Inventory*.xlsx")
    )
...
print(
    f"[INFO] Data files found: {len(list(DATA_PATH.glob('*'))) if DATA_PATH.exists() else 0}"
)
...
if CACHE_MANAGER_AVAILABLE:
    ...
    cache_manager = CacheManager(
        cache_dir=cache_dir, default_ttl=CACHE_DURATION, max_memory_items=1000
    )
...
if FABRIC_CONVERSION_AVAILABLE:
    try:
        fabric_converter = FabricConversionEngine(
            erp_host="http://localhost:5006", data_path="data/raw"
        )
        print(
            f"OK Fabric Conversion Engine initialized with {len(fabric_converter.conversion_cache)} fabric specs"
        )
    except Exception as e:
...
class InventoryAnalyzer:
    ...
    def analyze_inventory_levels(self, current_inventory, forecast):
        ...
        for product in current_inventory:
            ...
            required_inventory = (
                daily_demand * self.lead_time_days * self.safety_stock_multiplier
            )
    ...
    def analyze_inventory(self, inventory_data=None):
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                critical_items, high_risk_items
            ),
            "summary": {
    ...
    def _calculate_eoq(
        self, annual_demand, ordering_cost=100, holding_cost_rate=0.25, unit_cost=10
    ):
        """Calculate Economic Order Quantity"""
        if (
            annual_demand <= 0
            or ordering_cost <= 0
            or holding_cost_rate <= 0
            or unit_cost <= 0
        ):
            return 0
...
class InventoryManagementPipeline:
    ...
    def run_complete_analysis(
        self, sales_data=None, inventory_data=None, yarn_data=None
    ):
        ...
        try:
            ...
            if self.supply_chain_ai and hasattr(
                self.supply_chain_ai, "demand_forecast"
            ):
            ...
            if inventory_data is not None:
                ...
                inventory_analysis = self.inventory_analyzer.analyze_inventory_levels(
                    current_inventory=current_inventory, forecast=sales_forecast
                )
                ...
                production_plan = self.generate_production_plan(
                    inventory_analysis=inventory_analysis, forecast=sales_forecast
                )
    ...
    def _generate_simple_forecast(self, sales_data):
        ...
        if hasattr(sales_data, "iterrows"):
            for _, row in sales_data.iterrows():
                ...
                forecast[item_id] = (
                    row.get("Consumed", row.get("Sales", 0)) * 1.1
                )  # 10% growth
    ...
    def _generate_recommendations(self, analysis_results):
        ...
        if "inventory_analysis" in analysis_results:
            critical_items = [
                item
                for item in analysis_results["inventory_analysis"]
                if item["shortage_risk"] in ["CRITICAL", "HIGH"]
            ]
            if critical_items:
                recommendations.append(
...
class SalesForecastingEngine:
    ...
    def initialize_ml_engines(self):
        ...
        try:
            ...
            self.ml_engines["random_forest"] = RandomForestRegressor(
    ...
    def forecast_with_consistency(self, style_data, horizon_days=90):
            ...
            try:
                ...
                if "ensemble" in forecast_results:
                    ...
                    for model in ["XGBoost", "Prophet", "ARIMA"]:
                        if (
                            model in forecast_results
                            and "forecast" in forecast_results[model]
                        ):
                ...
                print(
                    f"ML forecasting failed: {str(e)}, falling back to weighted average"
                )
                result["method"] = "fallback_to_weighted"
                result["forecast"] = self._calculate_weighted_average(
                    style_data, consistency_score
                )
        ...
        # Low consistency (CV >= 0.7, score < 0.3): React to orders only
            ...
            result["recommendation"] = (
                "Monitor actual orders only - pattern too variable for forecasting"
            )
    ...
    def _extract_seasonality_patterns(self, data):
        ...
        if len(data) >= 14:
            patterns["weekly"] = {
                ...
                "pattern": (
                    "multiplicative"
                    if self._is_multiplicative_seasonality(data, 7)
                    else "additive"
                ),
            }
        ...
        if len(data) >= 60:
            patterns["monthly"] = {
                ...
                "pattern": (
                    "multiplicative"
                    if self._is_multiplicative_seasonality(data, 30)
                    else "additive"
                ),
            }
        ...
        if len(data) >= 365:
            patterns["yearly"] = {
                ...
                "pattern": (
                    "multiplicative"
                    if self._is_multiplicative_seasonality(data, 365)
                    else "additive"
                ),
            }
    ...
    def _extract_promotion_effects(self, data):
        ...
        if "Qty Shipped" in data.columns:
            ...
            sales_col = (
                data.select_dtypes(include=[np.number]).columns[0]
        ...
        if sales_col:
            ...
            effects["promotion_impact"] = (
                (data[sales_col][promotion_periods].mean() / rolling_mean.mean())
                if promotion_periods.sum() > 0
                else 1.0
            )
            effects["avg_promotion_duration"] = self._calculate_avg_duration(
                promotion_periods
            )
    ...
    def _extract_customer_segments(self, data):
        ...
        if "Customer" in data.columns:
            ...
            if not customer_sales.empty:
                ...
                customer_sales_sorted = customer_sales.sort_values(
                    by=customer_sales.columns[0], ascending=False
                )
                cumsum = customer_sales_sorted["percentage"].cumsum()

                segments["top_20_percent_customers"] = len(cumsum[cumsum <= 0.8]) / len(
                    customer_sales
                )
                segments["concentration_ratio"] = (
                    cumsum.iloc[int(len(cumsum) * 0.2)] if len(cumsum) > 5 else 0
                )
                segments["customer_diversity"] = (
                    1 - (customer_sales["percentage"] ** 2).sum()
                )  # Herfindahl index
    ...
    def _extract_cyclical_patterns(self, data):
        ...
        try:
            ...
            detrended = (
                data[sales_col]
                - data[sales_col].rolling(window=30, min_periods=1).mean()
            )
            ...
            if len(detrended) > 50:
                ...
                acf_values = [
                    detrended.autocorr(lag=i)
                    for i in range(1, min(40, len(detrended) // 2))
                ]

                # Find significant lags
                significant_lags = [
                    i + 1 for i, v in enumerate(acf_values) if abs(v) > 0.2
                ]

                features["cycle_length"] = (
    ...
    def train_models(self, sales_data, features):
        ...
        try:
            ...
            if ts_data is None or len(ts_data) < 30:
                return self._get_fallback_forecast_results(
            ...
            ml_logger.critical(
                f"Critical error in model training: {str(e)}\n{traceback.format_exc()}"
            )
    ...
    def _prepare_time_series(self, sales_data):
        ...
        try:
            ...
            if not date_col or not value_col:
                ...
                date_col = (
                    sales_data.select_dtypes(include=["datetime64"]).columns[0]
                    if len(sales_data.select_dtypes(include=["datetime64"]).columns) > 0
                    else None
                )
                value_col = (
                    sales_data.select_dtypes(include=[np.number]).columns[0]
                    if len(sales_data.select_dtypes(include=[np.number]).columns) > 0
                    else None
                )
    ...
    def _train_arima(self, ts_data, features):
        ...
        if not STATSMODELS_AVAILABLE or len(ts_data) < 30:
            return {
                ...
                "mape": 100,
                "model": None,
                "error": "ARIMA unavailable or insufficient data",
            }
    ...
    def _train_prophet(self, ts_data, features):
        ...
        if not ML_AVAILABLE or len(ts_data) < 30:
            return self._get_fallback_model_result(
                "Prophet", "Prophet unavailable or insufficient data"
            )
        ...
        try:
            ...
            seasonality_mode = (
                "multiplicative"
                if features.get("seasonality", {}).get("weekly", {}).get("pattern")
                == "multiplicative"
                else "additive"
            )
            ...
            return {
                ...
                "mape": 100,
                "model": None,
                "error": f"Prophet training failed: {str(e)}",
            }
    ...
    def _train_lstm(self, ts_data, features):
        ...
        try:
            ...
            return {
                ...
                "mape": 100,
                "model": None,
                "error": f"LSTM training failed: {str(e)}",
            }
    ...
    def _train_xgboost(self, ts_data, features):
        ...
        if not XGBOOST_AVAILABLE or len(ts_data) < 60:
            return {
                ...
                "mape": 100,
                "model": None,
                "error": "XGBoost unavailable or insufficient data",
            }
        ...
        try:
            ...
            for window in [7, 14, 30]:
                X[f"rolling_mean_{window}"] = (
                    ts_data["y"].rolling(window, min_periods=1).mean()
                )
                X[f"rolling_std_{window}"] = (
                    ts_data["y"].rolling(window, min_periods=1).std()
                )
                X[f"rolling_min_{window}"] = (
                    ts_data["y"].rolling(window, min_periods=1).min()
                )
                X[f"rolling_max_{window}"] = (
                    ts_data["y"].rolling(window, min_periods=1).max()
                )
            ...
            if features.get("seasonality", {}).get("weekly"):
                X["weekly_strength"] = features["seasonality"]["weekly"].get(
                    "strength", 0
                )

            if features.get("promotions"):
                X["promotion_impact"] = features["promotions"].get(
            ...
            for i in range(self.forecast_horizon):
                ...
                for window in [7, 14, 30]:
                    recent_values = [
                        last_features[f"lag_{k}"].values[0]
                        for k in range(1, min(window + 1, 31))
                    ]
    ...
    def _create_ensemble(self, model_results):
        ...
        valid_models = {
            k: v for k, v in model_results.items() if v.get("forecast") is not None
        }
        ...
        ensemble_accuracy = sum(
            weights[name] * model.get("accuracy", 0)
            for name, model in valid_models.items()
        )
    ...
    def generate_forecast_output(self, sales_data):
            ...
            if ensemble.get("forecast") is not None:
                ...
                for i in range(self.forecast_horizon):
                    daily_forecasts.append(
                        {
                            "date": (base_date + pd.Timedelta(days=i)).strftime(
                                "%Y-%m-%d"
                            ),
                ...
                output["summary"] = {
                    ...
                    "peak_day": daily_forecasts[np.argmax(ensemble["forecast"])][
                        "date"
                    ],
                    "lowest_day": daily_forecasts[np.argmin(ensemble["forecast"])][
                        "date"
                    ],
                    "forecast_volatility": float(
                        ensemble["forecast"].std() / ensemble["forecast"].mean() * 100
                    ),
                }
        ...
        output["validation"] = {
            ...
            "confidence_level": (
                "HIGH"
                if best_accuracy >= 90
                else "MEDIUM" if best_accuracy >= 80 else "LOW"
            ),
        }
            ...
            for _, row in self.bom_data.iterrows():
                ...
                if yarn_id:
                    ...
                    if (
                        product
                        and product
                        not in self.yarn_requirements[yarn_id]["products_using"]
                    ):
                        self.yarn_requirements[yarn_id]["products_using"].append(
                            product
                        )
                    ...
                    procurement_list.append(
                        {
                            ...
                            "priority": (
                                "CRITICAL"
                                if current_stock < 0
                                else "HIGH" if shortage > required * 0.5 else "MEDIUM"
                            ),
                        }
                    )
...
class CapacityPlanningEngine:
    ...
    def calculate_finite_capacity_requirements(
        self, production_plan, time_horizon_days=30
    ):
        ...
        for product, quantity in production_plan.items():
            ...
            machine_hours = (
                quantity * 0.5
            )  # Placeholder - should come from routing data
    ...
    def optimize_capacity_allocation(self, demand_forecast, capacity_constraints):
        ...
        for product, demand in demand_forecast.items():
            ...
            allocation_plan[product] = {
                ...
                "utilization": (
                    allocated / available_capacity.get(product, 1)
                    if available_capacity.get(product, 0) > 0
                    else 0
                ),
            }
    ...
    def _get_available_capacity(self):
        ...
        total_capacity_per_day = sum(
            cap for machine, cap in machine_capacities.items() if machine != "default"
        )
    ...
    def get_production_data(self):
        ...
        try:
            ...
            if kpi_row:
                ...
                return {
                    "timestamp": datetime.now().isoformat(),
                    "overallEquipmentEffectiveness": kpi_data[
        ...
        return [
            {
                ...
                "trend": (
                    "up"
                    if kpi_data["overall_equipment_effectiveness"] >= 75
                    else "down"
                ),
                "status": (
                    "good"
                    if kpi_data["overall_equipment_effectiveness"] >= 85
                    else "warning"
                ),
            },
            {
...
class ProductionScheduler:
    ...
    def __init__(self):
        ...
        self.priority_rules = [
            ...
            "SPT",
            "CR",
        ]  # Earliest Due Date, Shortest Processing Time, Critical Ratio

    def generate_finite_capacity_schedule(
        self, orders, capacity_constraints, scheduling_horizon_days=30
    ):
        ...
        for order in sorted_orders:
            ...
            start_time = self._find_earliest_slot(
                order, available_capacity, current_time
            )
    ...
    def _get_setup_time(self, from_product, to_product):
        ...
        if from_product == to_product:
            return 0
        return self.setup_matrix.get(
            (from_product, to_product), 1
        )  # Default 1 hour setup
...
class TimePhasedMRP:
    ...
    def calculate_time_phased_requirements(
        self, demand_forecast, bom, lead_times, planning_horizon_weeks=12
    ):
        ...
        for week in range(planning_horizon_weeks):
            ...
            for product, quantity in demand_forecast.get(week, {}).items():
                ...
                materials = self._explode_bom_with_timing(
                    product, quantity, bom, week, lead_times
                )
    ...
    def calculate_safety_stock(
        self, item, demand_variability, lead_time, service_level=0.95
    ):
        ...
        try:
            ...
            safety_stock = (
                1.65 * demand_variability * (lead_time**0.5)
            )  # Approx for 95% service level

        return safety_stock

    def _explode_bom_with_timing(
        self, product, quantity, bom, current_week, lead_times
    ):
...
class ManufacturingSupplyChainAI:
    ...
    def __init__(self, data_path, column_mapping=None):
        ...
        if PLANNING_ENGINE_AVAILABLE:
            try:
                ...
                self.planning_engine = SixPhasePlanningEngine(
                    data_path=planning_data_path
                )
                print(
                    f"Six-Phase Planning Engine initialized with data path: {planning_data_path}"
                )
            except Exception as e:
    ...
    def load_all_data(self):
        ...
        try:
                ...
                try:
                    ...
                    if yarn_file:
                        self.raw_materials_data = (
                            pd.read_excel(yarn_file)
                            if yarn_file.suffix == ".xlsx"
                            else pd.read_csv(yarn_file)
                        )
                    ...
                    if knit_orders_file:
                        self.knit_orders = (
                            pd.read_excel(knit_orders_file)
                            if knit_orders_file.suffix == ".xlsx"
                            else pd.read_csv(knit_orders_file)
                        )
            ...
            if PARALLEL_LOADER_AVAILABLE and hasattr(self, "parallel_loader"):
                print(
                    "[FAST] Using parallel data loader for fast concurrent loading..."
                )
                ...
                self.raw_materials_data = parallel_results.get(
                    "yarn_inventory", pd.DataFrame()
                )
                self.yarn_data = self.raw_materials_data  # For compatibility
                self.bom_data = parallel_results.get(
                    "bom", pd.DataFrame()
                )  # parallel loader returns 'bom'
                self.sales_data = parallel_results.get(
                    "sales_orders", pd.DataFrame()
                )  # parallel loader returns 'sales_orders'
                self.knit_orders_data = parallel_results.get(
                ...
                if (
                    not self.sales_data.empty
                    and "fStyle#" in self.sales_data.columns
                ...
                if STYLE_MAPPER_AVAILABLE and style_mapper and not self.bom_data.empty:
                    if "Style#" in self.bom_data.columns:
                        ...
                        print(
                            f"[OK] Style mapper initialized with {len(bom_styles)} BOM styles"
                        )
                ...
                if (
                    ...
                    and column_standardizer
                    and not self.raw_materials_data.empty
                ):
                    try:
                        self.raw_materials_data = (
                            column_standardizer.standardize_columns(
                                self.raw_materials_data
                            )
                        )
            ...
            print(
                f"Looking for yarn inventory: {yarn_inventory_file}, exists: {yarn_inventory_file.exists()}"
            )
            if yarn_inventory_file.exists():
                ...
                if (
                    STANDARDIZATION_AVAILABLE
                    and column_standardizer
                    and self.raw_materials_data is not None
                ):
                    try:
                        self.raw_materials_data = (
                            column_standardizer.standardize_columns(
                        ...
                        print(
                            f"[OK] Standardized yarn data: {len(self.raw_materials_data)} rows"
                        )
                    except Exception as e:
                        print(f"⚠️ Standardization failed, using raw data: {e}")
                ...
                inventory_files = (
                    list(primary_data_path.glob("*inventory*.xlsx"))
                ...
                if inventory_files:
                    ...
                    if (
                        ...
                        try:
                            ...
                            print(
                                f"[OK] Standardized fallback yarn data: {len(self.raw_materials_data)} rows"
                            )
                        except Exception as e:
                            print(
                                f"⚠️ Fallback standardization failed, using raw data: {e}"
                            )
            ...
            if self.raw_materials_data is not None and not STANDARDIZATION_AVAILABLE:
                ...
                if self.yarn_data is None:
                    self.yarn_data = self.raw_materials_data
                    print(
                        f"OK Yarn data assigned (legacy method): {len(self.yarn_data)} rows"
                    )
            ...
            if not sales_files:
                sales_files = (
                    list(primary_data_path.glob("*SO_List*.csv"))
            ...
            if sales_files:
                ...
                if STANDARDIZATION_AVAILABLE and column_standardizer:
                    try:
                        self.sales_data = column_standardizer.standardize_columns(
                            self.sales_data
                        )
                ...
                if (
                    "fStyle#" in self.sales_data.columns
                    and "Style#" not in self.sales_data.columns
                ):
                    self.sales_data.rename(columns={"fStyle#": "Style#"}, inplace=True)
            ...
            if bom_file and bom_file.exists():
                ...
                if DAY0_FIXES_AVAILABLE:
                    try:
                        ...
                        if (
                            hasattr(self, "raw_materials_data")
                            and self.raw_materials_data is not None
                        ):
                            self.raw_materials_data = (
                                column_system.standardize_dataframe(
                                    self.raw_materials_data
                                )
                            )
                            print(
                                f"[DAY0] Standardized {len(column_system.applied_mappings)} columns in yarn inventory"
                            )

                        if hasattr(self, "bom_data") and self.bom_data is not None:
                            self.bom_data = column_system.standardize_dataframe(
                                self.bom_data
                            )
                            print(f"[DAY0] Standardized BOM columns")

                        if hasattr(self, "sales_data") and self.sales_data is not None:
                            self.sales_data = column_system.standardize_dataframe(
                                self.sales_data
                            )
                ...
                if STANDARDIZATION_AVAILABLE and column_standardizer:
                    try:
                        self.bom_data = column_standardizer.standardize_columns(
                            self.bom_data
                        )
                ...
                bom_files = list(primary_data_path.glob("*[Bb][Oo][Mm]*.csv")) + list(
                    self.data_path.glob("*[Bb][Oo][Mm]*.csv")
                )
                if bom_files:
            ...
            component_files = list(self.data_path.glob("*component*.xlsx")) + list(
                self.data_path.glob("*assembly*.xlsx")
            )
            if component_files:
                self.components_data = pd.read_excel(component_files[0])

            # Load finished goods data
            finished_files = list(self.data_path.glob("*finished*.xlsx")) + list(
                self.data_path.glob("*product*.xlsx")
            )
            if finished_files:
            ...
            for stage in [
                ...
                stage_files = list(primary_data_path.glob(f"*{stage}*.xlsx")) + list(
                    self.data_path.glob(f"*{stage}*.xlsx")
                )
            ...
            demand_files = (
                list(primary_data_path.glob("*[Dd]emand*.xlsx"))
    ...
    def initialize_time_phased_data(self):
        ...
        try:
            ...
            if po_file:
                ...
                print(
                    f"[TIME-PHASED] Loaded {len(po_data) if po_data is not None and hasattr(po_data, '__len__') else 0} PO records"
                )
                ...
                print(
                    f"[TIME-PHASED] ✓ Successfully loaded PO deliveries for {len(self.yarn_weekly_receipts)} yarns"
                )
                print(f"[TIME-PHASED] ✓ Time-phased planning is now ENABLED")

            else:
                print(
                    "[TIME-PHASED] Expected_Yarn_Report not found in any of these locations:"
                )
    ...
    def initialize_yarn_demand_scheduler(self):
        ...
        try:
            ...
            try:
                from src.data_loaders.efab_report_downloader_enhanced import (
                    EFabReportDownloaderEnhanced as EFabReportDownloader,
                )

                print(
                    "[SCHEDULER] Using enhanced downloader with auto-refresh and retry logic"
                )
            ...
            def refresh_yarn_demand():
                ...
                try:
                    print(
                        f"[SCHEDULER] Starting Yarn Demand refresh at {datetime.now()}"
                    )
                    ...
                    target_path = (
                        Path(analyzer_instance.data_path)
                        / "5"
                        / "ERP Data"
                        / "Expected_Yarn_Report.xlsx"
                    )
                    ...
                    if downloader.download_latest(target_path):
                        print(
                            f"[SCHEDULER] Downloaded Yarn Demand report to {target_path}"
                        )
            ...
            def run_scheduler():
                ...
                print(
                    f"[SCHEDULER] Yarn Demand refresh scheduled for {', '.join(refresh_times)}"
                )
    ...
    def get_yarn_time_phased_data(self, yarn_id: str) -> dict:
        ...
        try:
            from src.production.time_phased_planning import (
                TimePhasedPlanning,
                create_mock_demand_schedule,
            )
            ...
            if self.yarn_data is not None and not self.yarn_data.empty:
                ...
                yarn_matches = self.yarn_data[
                    self.yarn_data["Desc#"].astype(str) == str(yarn_id)
                ]
            ...
            return {
                "error": f"Error processing time-phased data for yarn {yarn_id}: {e}"
            }
    ...
    def load_knit_orders(self):
        ...
        if (
            hasattr(self, "knit_orders_data")
            and self.knit_orders_data is not None
            and not self.knit_orders_data.empty
        ):
        ...
        try:
            ...
            ko_file_paths = [
                Path(
                    "/mnt/c/finalee/beverly_knits_erp_v2/data/production/5/eFab_Knit_Orders.xlsx"
                ),
                Path(
                    "/mnt/c/finalee/beverly_knits_erp_v2/data/production/5/ERP Data/eFab_Knit_Orders.xlsx"
                ),
                Path(
                    "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/prompts/5/eFab_Knit_Orders_20250816.xlsx"
                ),
                Path(
                    "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/prompts/5/eFab_Knit_Orders_20250810 (2).xlsx"
                ),
                Path(
                    "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/prompts/4/eFab_Knit_Orders_20250810.xlsx"
                ),
            ]
            ...
            if ko_file is None:
                ...
                for data_dir in [
                    ...
                    if ko_files:
                        ko_file = sorted(ko_files, key=lambda x: x.stat().st_mtime)[
                            -1
                        ]  # Most recent
                        break
            ...
            if ko_file and ko_file.exists():
                ...
                for old_name, new_name in column_mapping.items():
                    if (
                        old_name in self.knit_orders.columns
                        and new_name not in self.knit_orders.columns
                    ):
                ...
                qty_ordered = (
                    pd.to_numeric(
                        self.knit_orders["Qty_Ordered_Lbs"], errors="coerce"
                    ).fillna(0)
                    if "Qty_Ordered_Lbs" in self.knit_orders.columns
                    else 0
                )
                g00 = (
                    pd.to_numeric(self.knit_orders["G00_Lbs"], errors="coerce").fillna(
                        0
                    )
                    if "G00_Lbs" in self.knit_orders.columns
                    else 0
                )
                shipped = (
                    pd.to_numeric(
                        self.knit_orders["Shipped_Lbs"], errors="coerce"
                    ).fillna(0)
                    if "Shipped_Lbs" in self.knit_orders.columns
                    else 0
                )
                seconds = (
                    pd.to_numeric(
                        self.knit_orders["Seconds_Lbs"], errors="coerce"
                    ).fillna(0)
                    if "Seconds_Lbs" in self.knit_orders.columns
                    else 0
                )

                self.knit_orders["Calculated_Balance"] = (
                    qty_ordered - g00 - shipped - seconds
                )

                # Add status flags for better tracking
                self.knit_orders["Is_Active"] = (
                    self.knit_orders["Calculated_Balance"] > 0
                )
                ...
                shipped_col = (
                    ...
                    if "Shipped_Lbs" in self.knit_orders.columns
                    else pd.Series(0, index=self.knit_orders.index)
                )
                ...
                qty_ordered_pct = (
                    ...
                    if "Qty_Ordered_Lbs" in self.knit_orders.columns
                    else pd.Series(1, index=self.knit_orders.index)
                )
                qty_ordered_pct = qty_ordered_pct.replace(
                    0, 1
                )  # Avoid division by zero

                completed = g00_col + shipped_col

                self.knit_orders["Completion_Percentage"] = (
                    completed / qty_ordered_pct * 100
                ).round(1)
                ...
                self.knit_orders["KO_Status"] = self.knit_orders.apply(
                    classify_ko_status, axis=1
                )
                ...
                for col in date_columns:
                    if col in self.knit_orders.columns:
                        self.knit_orders[col] = pd.to_datetime(
                            self.knit_orders[col], errors="coerce"
                        )
    ...
    def enhance_knit_order_data(self):
        ...
        # Calculate actual balance (Qty Ordered - G00 - Shipped - Seconds)
        qty_ordered = (
            pd.to_numeric(self.knit_orders["Qty_Ordered_Lbs"], errors="coerce").fillna(
                0
            )
            if "Qty_Ordered_Lbs" in self.knit_orders.columns
            else 0
        ...
        g00 = (
            pd.to_numeric(self.knit_orders["G00_Lbs"], errors="coerce").fillna(0)
            if "G00_Lbs" in self.knit_orders.columns
            else 0
        )
        shipped = (
            pd.to_numeric(self.knit_orders["Shipped_Lbs"], errors="coerce").fillna(0)
            if "Shipped_Lbs" in self.knit_orders.columns
            else 0
        )
        seconds = (
            pd.to_numeric(self.knit_orders["Seconds_Lbs"], errors="coerce").fillna(0)
            if "Seconds_Lbs" in self.knit_orders.columns
            else 0
        )
            ...
            if len(numeric_cols) > 1:
                ...
                if "Qty Shipped" in X.columns:
                    ...
                    for name, model in self.ml_models.items():
                        try:
                            if name == "prophet":
                                prophet_df = sales_data[["Date", "Qty Shipped"]].rename(
                                    columns={"Date": "ds", "Qty Shipped": "y"}
                                )
    ...
    def generate_alerts(self):
        ...
        if self.raw_materials_data is not None:
            ...
            if "Planning Balance" in self.raw_materials_data.columns:
                planning_col = find_column(
                    self.raw_materials_data, PLANNING_BALANCE_VARIATIONS
                )
                if planning_col:
                    low_stock = self.raw_materials_data[
                        self.raw_materials_data[planning_col] < 500
                    ]
                else:
    ...
    def validate_data_integrity(self):
        ...
        if self.raw_materials_data is not None:
            ...
            missing_cols = [
                ...
                if col not in self.raw_materials_data.columns
            ]
            if missing_cols:
                validation_results["errors"].append(
                    f"Missing columns in raw materials data: {missing_cols}"
                )
            else:
                validation_results["passed"].append(
                    "All required raw materials columns present"
                )

            # Check for negative values where they shouldn't exist
            if "Planning Balance" in self.raw_materials_data.columns:
                negative_balance = self.raw_materials_data[
                    self.raw_materials_data["Planning Balance"] < 0
                ]
                if len(negative_balance) > 0:
                    validation_results["errors"].append(
                        f"{len(negative_balance)} items with negative balance"
                    )
                else:
            ...
            null_percentage = (
                self.raw_materials_data.isnull().sum() / len(self.raw_materials_data)
            ) * 100
            high_null_cols = null_percentage[null_percentage > 20].index.tolist()
            if high_null_cols:
                validation_results["warnings"].append(
                    f"High null values in: {high_null_cols}"
                )
            ...
            if "Cost/Pound" in self.raw_materials_data.columns:
                ...
                if cost_stats["std"] > cost_stats["mean"] * 0.5:
                    validation_results["warnings"].append(
                        "High cost variance detected - review pricing"
                    )
        ...
        if self.sales_data is not None:
            ...
            if "Date" in self.sales_data.columns:
                try:
                    ...
                    if date_range > 365:
                        validation_results["warnings"].append(
                            f"Sales data spans {date_range} days - consider time segmentation"
                        )
                except (ValueError, TypeError, pd.errors.ParserError) as e:
                    validation_results["errors"].append(
                        f"Invalid date format in sales data: {e}"
                    )

            # Validate price-quantity relationships
            if (
                "Qty Shipped" in self.sales_data.columns
                and "Unit Price" in self.sales_data.columns
            ):
                zero_price = self.sales_data[
                    (self.sales_data["Qty Shipped"] > 0)
                    & (self.sales_data["Unit Price"] <= 0)
                ]
                if len(zero_price) > 0:
                    validation_results["errors"].append(
                        f"{len(zero_price)} orders with zero/negative price"
                    )
    ...
    def explode_bom_multilevel(self, finished_good_code, quantity=1, max_levels=10):
        ...
        def explode_level(item_code, qty_needed, level=0):
            ...
            parent_col = self._find_column(
                self.bom_data, ["parent", "Parent Item", "Finished Good", "Assembly"]
            )
            child_col = self._find_column(
                self.bom_data, ["child", "Component", "Material", "Part"]
            )
            qty_col = self._find_column(
                self.bom_data, ["quantity", "Quantity", "Qty Per", "Usage"]
            )
            ...
            if parent_col and child_col:
                ...
                for _, component in components.iterrows():
                    ...
                    qty_per = (
                        component[qty_col]
                        if qty_col and pd.notna(component[qty_col])
                        else 1
                    )
    ...
    def explode_textile_bom(self, style_id, fabric_yards):
        ...
        try:
            ...
            yarn_calc_result = fabric_converter.calculate_yarn_requirements(
                style_id, fabric_yards
            )
            ...
            requirements.update(
                {
                    "conversion_details": {
                        "fabric_yards_requested": yarn_calc_result.get(
                            "fabric_yards_requested", fabric_yards
                        ),
                        "fabric_yards_with_waste": yarn_calc_result.get(
                            "fabric_yards_with_waste", 0
                        ),
                        "fabric_pounds": yarn_calc_result.get("fabric_pounds", 0),
                        "fabric_waste_factor": yarn_calc_result.get(
                            "fabric_waste_factor", 1.03
                        ),
                        "yarn_shrinkage_factor": yarn_calc_result.get(
                            "yarn_shrinkage_factor", 1.02
                        ),
    ...
    def backtest_forecasts(self, test_period_days=30):
        ...
        if self.sales_data is None or len(self.sales_data) < 60:
            backtest_results["recommendations"].append(
                "Insufficient data for backtesting (need 60+ records)"
            )
        ...
        try:
            ...
            if "Date" in sales_data.columns and "Qty Shipped" in sales_data.columns:
                ...
                if split_point > 30:
                    ...
                    if best_mape < 10:
                        backtest_results["recommendations"].append(
                            "Forecast accuracy excellent - maintain current approach"
                        )
                    elif best_mape < 20:
                        backtest_results["recommendations"].append(
                            "Good accuracy - consider ensemble methods for improvement"
                        )
                    else:
                        backtest_results["recommendations"].append(
                            "High forecast error - review data quality and model selection"
                        )
    ...
    def verify_calculations(self):
        ...
        if self.raw_materials_data is not None:
            ...
            consumed = (
                pd.to_numeric(self.raw_materials_data["Consumed"], errors="coerce")
                .fillna(0)
                .sum()
                if "Consumed" in self.raw_materials_data.columns
                else 0
            )
            balance = (
                pd.to_numeric(
                    self.raw_materials_data["Planning Balance"], errors="coerce"
                )
                .fillna(0)
                .sum()
                if "Planning Balance" in self.raw_materials_data.columns
            ...
            if (
                "Cost/Pound" in self.raw_materials_data.columns
                and "Planning Balance" in self.raw_materials_data.columns
            ):
                # Ensure numeric types for calculations
                planning_balance = pd.to_numeric(
                    self.raw_materials_data["Planning Balance"], errors="coerce"
                ).fillna(0)
                cost_per_pound = pd.to_numeric(
                    self.raw_materials_data["Cost/Pound"], errors="coerce"
                ).fillna(0)
            ...
            numeric_cols = self.raw_materials_data.select_dtypes(
                include=[np.number]
            ).columns
    ...
    def calculate_comprehensive_kpis(self):
        ...
        yarn_inventory = (
            self.yarn_data if self.yarn_data is not None else self.raw_materials_data
        )
        ...
        if yarn_inventory is not None:
            try:
                ...
                print(
                    f"OK KPI calculation: {total_yarns} yarns, inventory value: {kpis['inventory_value']}"
                )
        ...
        if self.sales_data is not None:
            try:
                ...
                def parse_price(price_str):
                    ...
                    match = re.search(
                        r"[\d,]+\.?\d*", str(price_str).replace("$", "").strip()
                    )
                    if match:
                ...
                if "Line Price" in self.sales_data.columns:
                    self.sales_data["parsed_line_price"] = self.sales_data[
                        "Line Price"
                    ].apply(parse_price)
                    sales_revenue = self.sales_data["parsed_line_price"].sum()
                    kpis["sales_revenue"] = f"${sales_revenue:,.0f}"
                elif (
                    "Unit Price" in self.sales_data.columns
                    and "Yds_ordered" in self.sales_data.columns
                ):
                    # Fallback: calculate from unit price * quantity
                    self.sales_data["parsed_unit_price"] = self.sales_data[
                        "Unit Price"
                    ].apply(parse_price)
                    sales_revenue = (
                        self.sales_data["parsed_unit_price"]
                        * self.sales_data["Yds_ordered"]
                    ).sum()
                    kpis["sales_revenue"] = f"${sales_revenue:,.0f}"
                else:
                ...
                if hasattr(self, "knit_orders") and self.knit_orders is not None:
                    if (
                        "Qty Ordered (lbs)" in self.knit_orders.columns
                        and "Shipped (lbs)" in self.knit_orders.columns
                    ):
                        total_ordered = self.knit_orders["Qty Ordered (lbs)"].sum()
                        total_shipped = self.knit_orders["Shipped (lbs)"].sum()
                        fill_rate = (
                            (total_shipped / total_ordered * 100)
                            if total_ordered > 0
                            else 0
                        )
                ...
                print(
                    f"OK Sales KPIs: {active_orders} orders, value: {kpis.get('order_value', '$0')}"
                )
            ...
            kpis.update(
                {"active_knit_orders": 0, "order_value": "$0", "order_fill_rate": "0%"}
            )
        ...
        if hasattr(self, "knit_orders") and self.knit_orders is not None:
            try:
                ...
                if (
                    "G00 (lbs)" in self.knit_orders.columns
                    and "Shipped (lbs)" in self.knit_orders.columns
                ):
                    ...
                    if total_in_process > 0:
                        process_efficiency = (
                            total_shipped / (total_in_process + total_shipped)
                        ) * 100
            except:
                process_efficiency = 0
        ...
        if yarn_inventory is not None and "cost_per_pound" in yarn_inventory.columns:
            ...
            total_cost = (
                (
                    yarn_inventory["planning_balance"]
                    * yarn_inventory["cost_per_pound"]
                ).sum()
                if "planning_balance" in yarn_inventory.columns
                else 0
            )
        ...
        kpis.update(
            {
                ...
                "procurement_savings": (
                    f"${procurement_savings:,.0f}" if procurement_savings > 0 else "$0"
                ),
                "optimization_rate": (
                    f"{min(100, process_efficiency * 1.2):.1f}%"
                    if process_efficiency > 0
                    else "0%"
                ),
                "forecast_accuracy": f"{forecast_accuracy:.1f}%",
                "process_efficiency": (
                    f"{process_efficiency:.1f}%" if process_efficiency > 0 else "0%"
                ),
    ...
    def get_6_phase_planning_results(self):
        ...
        forecast_sources = len(list(self.data_path.glob("*Yarn_Demand*.xlsx"))) + (
            1 if self.sales_data is not None else 0
        )
        reliability_score = validation["data_quality_score"] / 100
        outliers = len(validation["warnings"]) + len(validation["errors"])
        status = (
            "Completed"
            if self.demand_forecast is not None or self.sales_data is not None
            else "Pending"
        )
        ...
        phases.append(
            {
                ...
                "details": {
                    ...
                    "Bias Correction": (
                        "Applied" if status == "Completed" else "Pending"
                    ),
        ...
        skus_processed = (
            self.bom_data["Parent Item"].nunique() if self.bom_data is not None else 0
        )
        material_requirements = (
            self.bom_data["Quantity"].sum()
            if self.bom_data is not None and "Quantity" in self.bom_data.columns
            else 0
        )
        ...
        on_hand = (
            self.raw_materials_data["Planning Balance"].sum()
            if self.raw_materials_data is not None
            and "Planning Balance" in self.raw_materials_data.columns
            else 0
        )
        on_order = (
            self.raw_materials_data["On Order"].sum()
            if self.raw_materials_data is not None
            and "On Order" in self.raw_materials_data.columns
            else 0
        )
        total_demand = (
            self.demand_forecast["Demand"].sum()
            if self.demand_forecast is not None
            and "Demand" in self.demand_forecast.columns
            else (
                self.sales_data["Qty Shipped"].sum()
                if self.sales_data is not None
                else 0
            )
        )
        ...
        total_savings = (
            sum(
                float(rec["savings_potential"].replace("$", "").replace(",", ""))
        ...
        cost_reduction = (
            (
                total_savings
                / (on_hand * self.raw_materials_data["Cost/Pound"].mean())
                * 100
            )
            if self.raw_materials_data is not None and on_hand > 0
            else 0
        )
        ...
        suppliers = (
            self.raw_materials_data["Supplier"].nunique()
            if self.raw_materials_data is not None
            else 0
        )
        supplier_risks = self.get_supplier_risk_intelligence()
        high_risk_suppliers = len(
            [s for s in supplier_risks if s["risk_level"] == "High"]
        )
        ...
        phases.append(
            {
                ...
                "details": {
                    ...
                    "Risk Scoring": (
                        "Multi-criteria optimization applied"
                        if status == "Completed"
                        else "Pending"
                    ),
                    "High-Risk Suppliers": f"{high_risk_suppliers} flagged",
                    "Financial Health": (
                        "All suppliers verified" if status == "Completed" else "Pending"
                    ),
                },
            }
        )
        ...
        status = (
            "Completed"
            if all(p["status"] == "Completed" for p in phases)
            else "Pending"
        )
        phases.append(
            {
                ...
                "details": {
                    ...
                    "Approval Workflow": (
                        "Pending C-level review"
                        if status == "Completed"
                        else "Awaiting prior phases"
                    ),
                },
            }
        )
    ...
    def get_ml_forecasting_insights(self):
        ...
        if self.sales_data is None or len(self.sales_data) == 0:
            return self._get_ml_error_response(
        ...
        try:
            ...
            forecast_output = forecasting_engine.generate_forecast_output(
                self.sales_data
            )
            ...
            try:
                ...
                return self._get_ml_error_response(
                    f"All forecasting methods failed: {str(e)}"
                )
    ...
    def _get_ml_fallback_forecast(self, reason):
        ...
        try:
            ...
            if self.sales_data is not None and len(self.sales_data) > 0:
                ...
                numeric_cols = self.sales_data.select_dtypes(
                    include=[np.number]
                ).columns
                if len(numeric_cols) > 0:
                    ...
                    return [
                        {
                            ...
                            "forecast_value": (
                                float(avg_value) if not pd.isna(avg_value) else 0
                            ),
                        }
                    ]
        except Exception as e:
    ...
    def _format_forecast_results(self, forecast_output):
        ...
        return sorted(
            formatted_results,
            key=lambda x: float(x["accuracy"].replace("%", "")),
            reverse=True,
        )
    ...
    def _prepare_time_series_data(self):
        ...
        try:
            ...
            date_col = self._find_column(
                sales_copy, ["Date", "Order Date", "Ship Date", "Transaction Date"]
            )
            qty_col = self._find_column(
                sales_copy, ["Qty Shipped", "Quantity", "Units", "Amount"]
            )

            if date_col and qty_col:
                sales_copy["date"] = pd.to_datetime(
                    sales_copy[date_col], errors="coerce"
                )
                time_series_data = (
                    sales_copy.groupby("date")[qty_col].sum().reset_index()
                )
                ...
                for result in ml_results:
                    if "model" in result and result["model"] != "Error":
                        ...
                        self.ml_models_cache[model_name] = {
                            "mape": float(
                                result.get("mape", "100.0%").replace("%", "")
                            ),
                            "accuracy": float(
                                result.get("accuracy", "0.0%").replace("%", "")
                            ),
    ...
    def detect_demand_anomalies(self, threshold_std=2.5, lookback_days=30):
        ...
        try:
            ...
            date_col = self._find_column(
                sales_copy, ["Date", "Order Date", "Ship Date"]
            )
            qty_col = self._find_column(
                sales_copy, ["Qty Shipped", "Quantity", "Units"]
            )
            ...
            if date_col and qty_col:
                ...
                daily_demand["rolling_mean"] = (
                    daily_demand["demand"]
                    .rolling(window=lookback_days, min_periods=1)
                    .mean()
                )
                daily_demand["rolling_std"] = (
                    daily_demand["demand"]
                    .rolling(window=lookback_days, min_periods=1)
                    .std()
                )
                ...
                daily_demand["z_score"] = (
                    daily_demand["demand"] - daily_demand["rolling_mean"]
                ) / (daily_demand["rolling_std"] + 0.001)
                daily_demand["is_anomaly"] = (
                    abs(daily_demand["z_score"]) > threshold_std
                )
    ...
    def _detect_demand_patterns(self, daily_demand):
        ...
        if len(daily_demand) > 60:
            if "date" in daily_demand.columns:
                ...
                daily_demand["day_of_week"] = pd.to_datetime(
                    daily_demand["date"], errors="coerce"
                ).dt.dayofweek
                    ...
                    patterns.append(
                        {
                            ...
                            "strength": "Detected",
                            "action": "Adjust weekly schedules",
                        }
                    )
    ...
    def generate_90_day_forecast(self, confidence_level=0.95, product_filter=None):
        ...
        try:
            ...
            if self.sales_data is None or len(self.sales_data) == 0:
                ...
                return self._get_default_forecast_results(
                    "No sales data available", confidence_level
                )
            ...
            if date_col and qty_col:
                ...
                if product_filter:
                    product_col = self._find_column(
                        sales_copy, ["Product", "Item", "SKU", "Style"]
                    )
                    if product_col:
                        sales_copy = sales_copy[
                            sales_copy[product_col] == product_filter
                        ]
                ...
                if best_model_name == "Prophet" and ML_AVAILABLE:
                    try:
                        ...
                        forecast_results = self._simple_90_day_forecast(
                            daily_demand, confidence_level
                        )
                    except Exception as e:
                        ml_logger.error(
                            f"Prophet forecasting failed: {str(e)}\n{traceback.format_exc()}"
                        )
                        forecast_results = self._simple_90_day_forecast(
                            daily_demand, confidence_level
                        )
            ...
            forecast_results = self._get_default_forecast_results(
                f"Invalid data: {str(e)}", confidence_level
            )
        except Exception as e:
            ml_logger.error(
                f"Unexpected error in 90-day forecast: {str(e)}\n{traceback.format_exc()}"
            )
            forecast_results = self._get_default_forecast_results(
                f"Forecast failed: {str(e)}", confidence_level
            )
    ...
    def _calculate_seasonality_from_sales(self):
        ...
        try:
            date_col = self._find_column(
                self.sales_data, ["Invoice Date", "Date", "Order Date"]
            )
            qty_col = self._find_column(
                self.sales_data, ["Qty Shipped", "Quantity", "Units"]
            )
    ...
    def _calculate_holding_cost_rate(self, unit_cost):
        ...
        if (
            self.raw_materials_data is not None
            and "cost" in self.column_mapping["raw_materials"]
        ):
            cost_col = self._find_column(
                self.raw_materials_data, self.column_mapping["raw_materials"]["cost"]
            )
            if cost_col:
    ...
    def _process_eoq_item(self, item, columns, seasonality_factor):
        ...
        consumed_value = (
            item[columns["consumed"]]
            if columns["consumed"] and pd.notna(item[columns["consumed"]])
            else 0
        )
        ...
        unit_cost = (
            item[columns["cost"]]
            if columns["cost"] and pd.notna(item[columns["cost"]])
            else 5.0
        )
        ...
        # Get supplier info
        supplier_name = (
            str(item[columns["supplier"]]).lower()
            if columns["supplier"] and pd.notna(item[columns["supplier"]])
            else "unknown"
        )
        ...
        supplier_factor = (
            ...
            else 0.9 if len(supplier_name) < 10 else 1.0
        )
        ...
        safety_stock = self._calculate_safety_stock(
            adjusted_demand, lead_time, lead_time_std, demand_cv, service_level
        )
        ...
        current_stock = (
            ...
            if columns["balance"] and pd.notna(item[columns["balance"]])
            else 0
        )
        total_cost = (
            (eoq / 2 * holding_cost)
            + (adjusted_demand / eoq * ordering_cost)
            + (safety_stock * holding_cost)
        )
        ...
        return {
            ...
            "stock_status": (
                ...
                else "Low" if current_stock < reorder_point else "Adequate"
            ),
        }
    ...
    def calculate_dynamic_eoq(self, item_code=None, seasonality_factor=1.0):
        ...
        try:
            ...
            desc_col = self._find_column(
                self.raw_materials_data, ["Desc#", "Description", "Item Code"]
            )
            consumed_col = self._find_column(
                self.raw_materials_data, ["Consumed", "Usage", "Monthly Usage"]
            )
            balance_col = self._find_column(
                self.raw_materials_data,
                ["Theoretical Balance", "Planning Balance", "Stock"],
            )
            cost_col = self._find_column(
                self.raw_materials_data, ["Cost/Pound", "Unit Cost", "Cost"]
            )
            supplier_col = self._find_column(
                self.raw_materials_data, ["Supplier", "Vendor"]
            )
            ...
            if item_code and desc_col:
                items_to_process = self.raw_materials_data[
                    self.raw_materials_data[desc_col]
                    .astype(str)
                    .str.contains(str(item_code), case=False, na=False)
                ]
                if items_to_process.empty:
                    return []
            else:
                items_to_process = (
                    self.raw_materials_data.nlargest(20, consumed_col)
                    if consumed_col
                    else self.raw_materials_data.head(20)
                )
            ...
            return sorted(results, key=lambda x: x["adjusted_demand"], reverse=True)[
                :20
            ]
    ...
    def _get_eoq_recommendation(
        self, seasonality_factor, current_stock, reorder_point, safety_stock
    ):
    ...
    def _calculate_delivery_score(self, cost_variance, volume_reliability):
        ...
        delivery_reliability = max(
            0.5, min(1.0, 0.95 - cost_variance + (volume_reliability * 0.1))
        )
    ...
    def _calculate_quality_score(self, avg_cost, cost_std, market_benchmarks):
        ...
        quality_premium = (
            avg_cost / market_benchmarks["avg_cost"]
            if market_benchmarks["avg_cost"] > 0
            else 1.0
        )
        consistency_score = (
            max(0, 1 - (cost_std / market_benchmarks["cost_std"]))
            if market_benchmarks["cost_std"] > 0
            else 0.8
        )
        return min(
            100, max(50, 70 + (quality_premium - 1) * 15 + consistency_score * 15)
        )
    ...
    def _calculate_lead_time_score(self, item_count, total_volume, market_avg_volume):
        ...
        volume_factor = (
            min(1.0, total_volume / market_avg_volume) if market_avg_volume > 0 else 0.5
        )
    ...
    def _get_risk_classification(self, risk_score):
        ...
        if risk_score >= 75:
            return (
                "Critical",
                "Immediate action required: Find alternative suppliers",
                "Urgent",
            )
        elif risk_score >= 50:
    ...
    def calculate_supplier_risk_score(self):
        ...
        try:
            ...
            supplier_col = self._find_column(
                self.raw_materials_data, ["Supplier", "Vendor", "Source"]
            )
            cost_col = self._find_column(
                self.raw_materials_data, ["Cost/Pound", "Unit Cost", "Cost"]
            )
            ...
            market_benchmarks = {}
            if cost_col:
                market_benchmarks["avg_cost"] = self.raw_materials_data[
                    cost_col
                ].median()
            ...
            if balance_col:
                market_benchmarks["avg_volume"] = (
                    self.raw_materials_data.groupby(supplier_col)[balance_col]
                    .sum()
                    .median()
                )
            else:
            ...
            for supplier, group in supplier_groups:
                ...
                volume_reliability = min(
                    1.0, total_volume / (market_benchmarks["avg_volume"] * 2)
                )
                delivery_reliability = max(
                    0.5, min(1.0, 0.95 - cost_variance + (volume_reliability * 0.1))
                )
                ...
                quality_score = min(
                    100,
                    max(50, 70 + (quality_premium - 1) * 15 + consistency_score * 15),
                )
                ...
                lead_time_factor = min(1.0, item_count / 10)
                volume_factor = (
                    min(1.0, total_volume / market_benchmarks["avg_volume"])
                    if market_benchmarks["avg_volume"] > 0
                    else 0.5
                )
                ...
                total_risk_score = sum(
                    risk_matrix[key] * risk_weights[key] for key in risk_matrix
                )
                ...
                if total_risk_score >= 75:
                    ...
                    risk_level = "High"
                    mitigation_strategy = (
                        "Diversify supply base, develop contingency plans"
                    )
                ...
                supplier_risk_scores.append(
                    {
                        ...
                        "risk_matrix": {
                            ...
                            "concentration": round(
                                risk_matrix["concentration_risk"], 1
                            ),
                        },
                        ...
                        "recommendation": (
                            "Immediate supplier review required"
                            if total_risk_score >= 75
                            else (
                                ...
                                if total_risk_score >= 50
                                    ...
                                    else "Maintain strategic partnership"
                                )
                            )
                        ),
                    }
        ...
        return sorted(
            supplier_risk_scores, key=lambda x: x["total_risk_score"], reverse=True
        )

    def run_advanced_planning_cycle(self):
        """Execute advanced 6-phase planning with new engines and real knit orders"""
        planning_results = {"timestamp": datetime.now().isoformat(), "phases": {}}
        ...
        try:
            ...
            if knit_orders_status["status"] == "success":
                planning_results["phases"]["demand_analysis"] = {
                    "knit_orders": {
                        ...
                        "overdue_orders": knit_orders_status["timeline"][
                            "overdue_orders"
                        ],
                        "next_week_orders": knit_orders_status["timeline"][
                            "next_week_orders"
                        ],
                        "total_balance_lbs": knit_orders_status["summary"][
                            "total_balance_lbs"
                        ],
                    },
                    "yarn_requirements": {
                        "total_yarn_needed_lbs": (
                            yarn_requirements["summary"]["total_yarn_required_lbs"]
                            if yarn_requirements["status"] == "success"
                            else 0
                        ),
                        "unique_yarns": (
                            yarn_requirements["summary"]["unique_yarns_needed"]
                            if yarn_requirements["status"] == "success"
                            else 0
                        ),
                        "bom_coverage": (
                            yarn_requirements["summary"]["mapping_coverage"]
                            if yarn_requirements["status"] == "success"
                            else 0
                        ),
                    },
            ...
            if self.sales_data is not None and len(self.sales_data) > 0:
                ...
                planning_results["phases"]["demand_forecast"] = {
            ...
            if hasattr(self, "capacity_planner"):
                ...
                production_orders = (
                    self._get_production_orders()
                )  # This now uses real knit orders
                ...
                capacity_req = (
                    self.capacity_planner.calculate_finite_capacity_requirements(
                        production_plan
                    )
                )
                ...
                bottlenecks = self.capacity_planner.identify_capacity_bottlenecks(
                    capacity_util
                )
                allocation = self.capacity_planner.optimize_capacity_allocation(
                    production_plan, capacity_req
                )
            ...
            if hasattr(self, "mrp_engine") and yarn_requirements["status"] == "success":
                ...
                # Apply lot sizing
                sized_req = self.mrp_engine.apply_lot_sizing_rules(
                    time_phased_req, "EOQ"
                )

                # Add shortage analysis
                critical_shortages = [
                    yarn for yarn in allocation_check if yarn["shortage_lbs"] > 0
                ]
                ...
                planning_results["phases"]["mrp"] = {
                    ...
                    "total_shortage_lbs": sum(
                        y["shortage_lbs"] for y in critical_shortages
                    ),
                }
            ...
            if hasattr(self, "production_scheduler"):
                orders = (
                    self._get_production_orders()
                )  # This now loads real knit orders
                ...
                optimized_seq = self.production_scheduler.optimize_setup_sequences(
                    schedule
                )
                critical_path = self.production_scheduler.calculate_critical_path(
                    schedule
                )
            ...
            if yarn_requirements["status"] == "success":
                ...
                quality_predictions["yarn_quality_risks"] = {
                    ...
                    "risk_level": (
                        "high"
                        if len(unmatched_styles) > 20
                        else "medium" if len(unmatched_styles) > 10 else "low"
                    ),
                    "recommendation": (
                        "Update BOM mappings for accurate yarn tracking"
                        if len(unmatched_styles) > 0
                        else "BOM coverage good"
                    ),
                }
            ...
            if knit_orders_status["status"] == "success":
                ...
                delivery_plan["urgent_deliveries"] = {
                    ...
                    "priority": (
                        "CRITICAL"
                        if overdue_info["count"] > 50
                        else "HIGH" if overdue_info["count"] > 20 else "NORMAL"
                    ),
                }
            ...
            planning_results["summary"] = self._generate_planning_summary(
                planning_results
            )
    ...
    def _convert_forecast_to_weekly(self, forecast):
        ...
        if isinstance(forecast, dict) and "predictions" in forecast:
            for week in range(12):  # 12 weeks
                weekly[week] = {
                    f"product_{i}": pred["quantity"]
                    / 4  # Divide monthly by 4 for weekly
    ...
    def _get_production_orders(self):
        ...
        try:
            ...
            knit_orders_path = str(
                DATA_PATH / "eFab_Knit_Orders_20250810 (2).xlsx"
            )  # Use DATA_PATH/5
            if os.path.exists(knit_orders_path):
                knit_orders = pd.read_excel(knit_orders_path)
                # Apply standardization if available
                if STANDARDIZATION_AVAILABLE:
                    knit_orders = ColumnStandardizer.standardize_dataframe(
                ...
                for idx, row in knit_orders.head(50).iterrows():
                    ...
                    days_until_start = (
                        (start_date - pd.Timestamp.now()).days
                    ...
                    if balance > 0:
                        ...
                        processing_hours = (
                            balance / 100
                        )  # 100 lbs per hour capacity assumption
    ...
    def calculate_yarn_requirements_for_knit_orders(self):
        ...
        try:
            ...
            if hasattr(self, "knit_orders_data") and self.knit_orders_data is not None:
                ...
                knit_orders_path = (
                    self.data_path / "5" / "eFab_Knit_Orders_20250810 (2).xlsx"
                )
                if not knit_orders_path.exists():
                    knit_orders_path = (
                        self.data_path / "4" / "eFab_Knit_Orders_20250810.xlsx"
                    )
                    if not knit_orders_path.exists():
                        return {
                            "status": "error",
                            "message": "Knit orders file not found",
                        }
                knit_orders = pd.read_excel(knit_orders_path)
                # Apply standardization if available
                if STANDARDIZATION_AVAILABLE:
            ...
            balance_col = (
                "balance_lbs"
                if "balance_lbs" in knit_orders.columns
                else "Balance (lbs)"
            )
            if balance_col in knit_orders.columns:
                ...
                return {
                    "status": "error",
                    "message": f"Balance column not found in knit orders",
                }
            ...
            for _, order in active_orders.iterrows():
                ...
                style_col = (
                    "style_id"
                    if "style_id" in active_orders.columns
                    else "Style#" if "Style#" in active_orders.columns else "Style #"
                )
                ...
                bom_style_col = (
                    "style_id"
                    if "style_id" in bom_data.columns
                    else "Style#" if "Style#" in bom_data.columns else "Style_ID"
                )
                ...
                if not style_bom.empty:
                    ...
                    for _, bom_row in style_bom.iterrows():
                        ...
                        yarn_col = (
                            "yarn_id"
                            if "yarn_id" in style_bom.columns
                            else "desc#" if "desc#" in style_bom.columns else "Yarn_ID"
                        )
                        bom_pct_col = (
                            "bom_percentage"
                            if "bom_percentage" in style_bom.columns
                            else "BOM_Percentage"
                        )
                        ...
                        order_col_req = (
                            "order_number"
                            if "order_number" in active_orders.columns
                            else "Order #"
                        )
                        yarn_requirements[yarn_id]["orders_using"].append(
                            order.get(order_col_req, "")
                        )
            ...
            if (
                self.raw_materials_data is not None
                and "Yarn ID" in self.raw_materials_data.columns
            ):
                        ...
                        balance_col_yarn = (
                            ...
                            if "Planning Balance" in yarn_inventory.columns
                                ...
                                if "planning_balance" in yarn_inventory.columns
                                    ...
                                    "Weight_KG"
                                    if "Weight_KG" in yarn_inventory.columns
                                    else None
                                )
                            )
                        )
                        available = (
                            yarn_inventory[balance_col_yarn].sum()
                            if balance_col_yarn
                            else 0
                        )
                        allocated = (
                            yarn_inventory["Allocated"].sum()
                            if "Allocated" in yarn_inventory.columns
                            else 0
                        )
                        ...
                        yarn_allocation_check.append(
                            {
                                ...
                                "shortage_lbs": max(
                                    0, requirements["total_required_lbs"] - available
                                ),
            ...
            yarn_allocation_check = sorted(
                yarn_allocation_check, key=lambda x: x["shortage_lbs"], reverse=True
            )

            # Calculate summary statistics
            total_yarn_required = sum(
                req["total_required_lbs"] for req in yarn_requirements.values()
            )
            total_orders_mapped = len(
                set(
                    order
                    for req in yarn_requirements.values()
            ...
            return {
                ...
                "summary": {
                    ...
                    "mapping_coverage": (
                        (total_orders_mapped / len(active_orders) * 100)
    ...
    def analyze_knit_orders_status(self):
        ...
        try:
            ...
            qty_ordered_col = (
                "qty_ordered_lbs"
                if "qty_ordered_lbs" in knit_orders.columns
                else "Qty Ordered (lbs)"
            )
            balance_col = (
                "balance_lbs"
                if "balance_lbs" in knit_orders.columns
            ...
            shipped_col = (
                "shipped_lbs"
                if "shipped_lbs" in knit_orders.columns
                else "Shipped (lbs)"
            )
            g00_col = "g00_lbs" if "g00_lbs" in knit_orders.columns else "G00 (lbs)"

            total_ordered = (
                knit_orders[qty_ordered_col].sum()
                if qty_ordered_col in knit_orders.columns
                else 0
            )
            total_balance = (
                knit_orders[balance_col].sum()
                if balance_col in knit_orders.columns
                else 0
            )
            total_shipped = (
                knit_orders[shipped_col].sum()
                if shipped_col in knit_orders.columns
                else 0
            )
            total_g00 = (
                knit_orders[g00_col].sum() if g00_col in knit_orders.columns else 0
            )
            ...
            shipped_col_check = (
                shipped_col if shipped_col in knit_orders.columns else "Shipped (lbs)"
            )
            balance_col_check = (
                balance_col if balance_col in knit_orders.columns else "Balance (lbs)"
            )

            not_started = (
                knit_orders[
                    knit_orders[g00_col_check].isna()
                    & knit_orders[shipped_col_check].isna()
                ]
                if g00_col_check in knit_orders.columns
                and shipped_col_check in knit_orders.columns
                else knit_orders.iloc[0:0]
            )
            in_progress = (
                knit_orders[
                    knit_orders[g00_col_check].notna()
                    & knit_orders[shipped_col_check].isna()
                ]
                if g00_col_check in knit_orders.columns
                and shipped_col_check in knit_orders.columns
            ...
            completed = (
                knit_orders[knit_orders[balance_col_check] == 0]
                if balance_col_check in knit_orders.columns
                else knit_orders.iloc[0:0]
            )

            # Timeline analysis - use standardized or original column names
            start_date_col = (
                "start_date" if "start_date" in knit_orders.columns else "Start Date"
            )
            if start_date_col in knit_orders.columns:
                knit_orders[start_date_col] = pd.to_datetime(
                    knit_orders[start_date_col]
                )
                current_time = pd.Timestamp.now()
                overdue = (
                    knit_orders[
                        (knit_orders[start_date_col] <= current_time)
                        & (knit_orders[balance_col_check] > 0)
                    ]
                    if balance_col_check in knit_orders.columns
                    else knit_orders.iloc[0:0]
                )
            ...
            order_col = (
                "order_number" if "order_number" in knit_orders.columns else "Order #"
            )

            if (
                machine_col in knit_orders.columns
                and qty_ordered_col in knit_orders.columns
            ):
                machine_util = (
                    knit_orders.groupby(machine_col)
                    .agg(
                        {qty_ordered_col: "sum", balance_col: "sum", order_col: "count"}
                    )
                    .round(0)
                )
            else:
            ...
            if not overdue.empty and balance_col_check in overdue.columns:
                cols_to_select = []
                if order_col in overdue.columns:
                    cols_to_select.append(order_col)
                if style_col in overdue.columns:
                    cols_to_select.append(style_col)
                if balance_col_check in overdue.columns:
                    cols_to_select.append(balance_col_check)
                if start_date_col in overdue.columns:
                    cols_to_select.append(start_date_col)

                if cols_to_select:
                    critical_orders = overdue.nlargest(10, balance_col_check)[
                        cols_to_select
                    ]
                else:
                    ...
                    critical_orders_list.append(
                        {
                            ...
                            "start_date": (
                                row.get(start_date_col, pd.NaT).isoformat()
                                if pd.notna(row.get(start_date_col, pd.NaT))
                                else None
                            ),
                        }
                    )
            ...
            return {
                ...
                "production_status": {
                    "not_started": {
                        ...
                        "volume_lbs": (
                            float(not_started[qty_ordered_col].sum())
                            if qty_ordered_col in not_started.columns
                            else 0
                        ),
                    },
                    "in_progress": {
                        ...
                        "volume_lbs": (
                            float(in_progress[g00_col].sum())
                            if g00_col in in_progress.columns
                            else 0
                        ),
                    },
                    "completed": {
                        ...
                        "volume_lbs": (
                            float(completed[qty_ordered_col].sum())
                            if qty_ordered_col in completed.columns
                            else 0
                        ),
                    },
                },
                "timeline": {
                    "overdue_orders": {
                        ...
                        "balance_lbs": (
                            float(overdue[balance_col_check].sum())
                            if balance_col_check in overdue.columns
                            else 0
                        ),
                    },
                    "next_week_orders": {
                        ...
                        "balance_lbs": (
                            float(next_week[balance_col_check].sum())
                            if balance_col_check in next_week.columns
                            else 0
                        ),
                    },
                },
    ...
    def _generate_planning_summary(self, results):
        ...
        summary = {"key_metrics": {}, "alerts": [], "recommendations": []}
        ...
        if "capacity_planning" in results["phases"]:
            ...
            if bottlenecks:
                summary["alerts"].append(
                    f"{len(bottlenecks)} capacity bottlenecks identified"
                )
                summary["recommendations"].append(
                    "Consider capacity expansion or overtime scheduling"
                )
    ...
    def handle_emergency_procurement(self):
        ...
        CRITICAL_NEGATIVE_YARNS = [
            ...
            {
                "yarn_id": 19003,
                "description": "1/75/36 100% Polyester Semi Dull NIM",
                "critical_balance": -118674.1,
                "supplier": "Atlas Yarns",
            },
            {
                "yarn_id": 18884,
                "description": "21/1 75/15/10 Modacrylic/Tencel/Nylon",
                "critical_balance": -8039.8,
                "supplier": "BUHLER QUALITY YARNS CORP",
            },
            {
                "yarn_id": 18575,
                "description": "30/1 100% Combed Cotton Natural z RS",
                "critical_balance": -3667.5,
                "supplier": "PARKDALE YARN MILLS",
            },
            {
                "yarn_id": 19045,
                "description": "24/1 100% Combed Cotton Natural RS",
                "critical_balance": -2752.3,
                "supplier": "Hamiliton",
            },
            {
                "yarn_id": 12321,
                "description": "1/100/96 100% Polyester Natural SETTS",
                "critical_balance": -1878.6,
                "supplier": "UNIFI",
            },
            {
                "yarn_id": 10153,
                "description": "2/150/34 100% Polyester WHEAT 1196F",
                "critical_balance": -1468.0,
                "supplier": "UNIFI",
            },
            {
                "yarn_id": 10027,
                "description": "2/150/34 100% Polyester SOLDY BLK",
                "critical_balance": -1311.8,
                "supplier": "UNIFI",
            },
            # PRIORITY: Specifically requested yarns
            {
                "yarn_id": 18868,
                "description": "30/1 60/40 Recycled Poly/Cotton",
                "critical_balance": -494.0,
                "supplier": "FERR",
            },
            {
                "yarn_id": 14270,
                "description": "26/1 75/15/10 Modacrylic/Rayon/Nylon",
                "critical_balance": -466.2,
                "supplier": "SPUNLAB",
            },
            {
                "yarn_id": 18851,
                "description": "46/1 100% Nomex Heather S 90/10",
                "critical_balance": -340.2,
                "supplier": "BUHLER QUALITY YARNS CORP",
            },
            {
                "yarn_id": 18892,
                "description": "1/150/48 100% Polyester Geko Grey",
                "critical_balance": -276.5,
                "supplier": "DECA GLOBAL",
            },
        ]
        ...
        if self.raw_materials_data is None:
            return {
                ...
                "summary": {"total": len(emergency_items)},
            }
        ...
        try:
            ...
            for _, item in self.raw_materials_data.iterrows():
                current_stock = (
                    item[balance_col]
                    if balance_col and pd.notna(item[balance_col])
                    else 0
                )
                monthly_consumption = (
                    item[consumed_col]
                    if consumed_col and pd.notna(item[consumed_col])
                    else 0
                )
                ...
                if monthly_consumption > 0 and current_stock >= 0:
                    ...
                    if days_of_supply < 5 and days_of_supply >= 0:
                        ...
                        primary_supplier = (
                            str(item[supplier_col])
                            if supplier_col and pd.notna(item[supplier_col])
                            else "Unknown"
                        )
                        unit_cost = (
                            item[cost_col]
                            if cost_col and pd.notna(item[cost_col])
                            else 0
                        )
                        ...
                        emergency_qty = max(
                            monthly_consumption * 2 * safety_buffer, current_stock * 3
                        )
                        ...
                        expedited_cost = (
                            emergency_qty
                            * unit_cost
                            * (1 + best_expedited_option["cost_premium"] / 100)
                        )
                        cost_impact = expedited_cost - normal_cost

                        # Calculate stockout risk and impact
                        stockout_risk_days = max(
                            0, best_expedited_option["lead_time_days"] - days_of_supply
                        )
                        potential_lost_production = (
                            stockout_risk_days * daily_consumption * 10
                        )  # Assume 10x value for finished goods
                        ...
                        item_desc = (
                            str(item[desc_col])[:40]
                            if desc_col and pd.notna(item[desc_col])
                            else "Unknown Material"
                        )
                        item_code = (
                            str(item[desc_col])[:15]
                            if desc_col and pd.notna(item[desc_col])
                            else "N/A"
                        )
                        ...
                        emergency_items.append(
                            {
                                ...
                                "expedited_options": expedited_options[
                                    :3
                                ],  # Top 3 options
                                "urgency": urgency,
                                "urgency_level": urgency_level,
                                "recommended_option": best_expedited_option[
                                    "option_type"
                                ],
                                "recommendation": (
                                    f"IMMEDIATE: Order {int(emergency_qty)} units via {best_expedited_option['option_type']}"
                                    if urgency_level <= 2
                                    else f"Order {int(emergency_qty)} units within {3 if urgency_level == 3 else 7} days"
                                ),
                                "action_plan": action_plan,
                                "decision_factors": {
                                    "speed_critical": urgency_level <= 2,
                                    "cost_sensitive": urgency_level >= 3,
                                    "quality_risk": best_expedited_option["reliability"]
                                    < 80,
                                    "min_order_concern": emergency_qty
                                    < best_expedited_option["minimum_order"],
                                },
                            }
                        )
        ...
        return sorted(
            emergency_items,
            key=lambda x: (urgency_order.get(x["urgency"], 3), x["days_of_supply"]),
        )
    ...
    def get_advanced_inventory_optimization(self):
        ...
        if self.raw_materials_data is not None:
            ...
            required_found = all(
                key in column_mapping for key in ["Planning Balance", "Description"]
            )
            if not required_found:
                missing = [
                    key
                    for key in ["Planning Balance", "Description"]
            ...
            if not critical_items.empty:
                ...
                critical_items["shortage_value"] = (
                    critical_items["shortage_lbs"] * critical_items["Cost/Pound"]
                )
                    ...
                    recommendations.append(
                        {
                            ...
                            "item_description": str(item.get("Description", "Unknown"))[
                                :50
                            ],
                            ...
                            "recommended_order_value": round(
                                recommended_order * cost_per_lb, 2
                            ),
            ...
            if "Planning Balance" in df.columns:
                low_stock = df[
                    (df["Planning Balance"] >= 0) & (df["Planning Balance"] <= 100)
                ].copy()
            else:
                low_stock = pd.DataFrame()
            if not low_stock.empty:
                # Prioritize by consumption rate and value
                low_stock["total_value"] = (
                    low_stock["Planning Balance"] * low_stock["Cost/Pound"]
                )
                low_stock["consumption_risk"] = (
                    low_stock["Consumed"] * low_stock["Cost/Pound"]
                )
                ...
                for _, item in low_stock.nlargest(20, "consumption_risk").iterrows():
                    ...
                    if monthly_consumption > 0:
                        days_to_stockout = int(
                            (current_stock / monthly_consumption) * 30
                        )
                        recommended_order = max(
                            monthly_consumption * 2, 300
                        )  # 2 months supply or min 300 lbs
                    else:
            ...
            if not high_consumption.empty:
                high_consumption["consumption_value"] = (
                    high_consumption["Consumed"] * high_consumption["Cost/Pound"]
                )
                if "Planning Balance" in high_consumption.columns:
                    high_consumption["coverage_ratio"] = (
                        high_consumption["Planning Balance"]
                        / high_consumption["Consumed"]
                    )
                else:
                    high_consumption["coverage_ratio"] = (
                        999  # Default if column missing
                    )
                ...
                inadequate_coverage = high_consumption[
                    ...
                    & (
                        high_consumption["consumption_value"] > 1000
                    )  # High-value consumption
                ].copy()

                for _, item in inadequate_coverage.nlargest(
                    10, "consumption_value"
                ).iterrows():
                    monthly_consumption = item["Consumed"]
                    current_stock = item["Planning Balance"]
            ...
            print(
                f"Analysis complete: {len(critical_items)} critical shortages, {len(low_stock)} low stock items"
            )
            ...
            recommendations.sort(
                key=lambda x: (
                    ...
                    x.get("shortage_value", x.get("recommended_order_value", 0)),
                ),
                reverse=True,
            )
    ...
    def analyze_sales_and_forecast_yarn_needs(self):
        ...
        try:
            ...
            if "Invoice Date" in sales_df.columns:
                ...
                return {
                    "error": "No date column (Invoice Date or Date) found in sales data"
                }
            ...
            last_12_months = sales_df[
                sales_df[date_col] >= (current_date - pd.DateOffset(months=12))
            ]
            last_6_months = sales_df[
                sales_df[date_col] >= (current_date - pd.DateOffset(months=6))
            ]
            last_3_months = sales_df[
                sales_df[date_col] >= (current_date - pd.DateOffset(months=3))
            ]
            ...
            if qty_col:
                monthly_sales = sales_df.groupby(
                    [sales_df[date_col].dt.to_period("M")]
                )[qty_col].sum()
                # Top products analysis
                top_styles = (
                    last_6_months.groupby("Style")[qty_col]
                    .sum()
                    .sort_values(ascending=False)
                    .head(20)
                    if "Style" in last_6_months.columns
                    else pd.Series()
                )
            else:
            ...
            results["sales_analysis"] = {
                "total_sales_12m": (
                    int(last_12_months[qty_col].sum())
                    if not last_12_months.empty and qty_col
                    else 0
                ),
                "total_sales_6m": (
                    int(last_6_months[qty_col].sum())
                    if not last_6_months.empty and qty_col
                    else 0
                ),
                "total_sales_3m": (
                    int(last_3_months[qty_col].sum())
                    if not last_3_months.empty and qty_col
                    else 0
                ),
                "avg_monthly_sales": (
                    int(monthly_sales.mean()) if len(monthly_sales) > 0 else 0
                ),
                "sales_growth_6m": self._calculate_growth_rate(
                    last_6_months, last_12_months
                ),
                "top_styles": [
                    {"style": style, "quantity": int(qty)}
                    for style, qty in top_styles.items()
                ],
                "monthly_trends": [
                    {"month": str(period), "quantity": int(qty)}
                    for period, qty in monthly_sales.tail(12).items()
                ],
            }
            ...
            yarn_needs = self._analyze_yarn_consumption_requirements(
                top_styles, results["demand_forecast"]
            )
            ...
            critical_shortages = sum(
                1
                for item in risk_analysis.get("yarn_risks", [])
                if item.get("risk_level") == "CRITICAL"
            )
            total_forecast_value = sum(
                item.get("forecasted_need_value", 0)
                for item in yarn_needs.get("yarn_consumption_forecast", [])
            )

            results["summary"] = {
                "critical_yarn_shortages": critical_shortages,
                "total_forecasted_yarn_value": round(total_forecast_value, 2),
                "recommended_safety_stock_investment": round(
                    total_forecast_value * 0.2, 2
                ),  # 20% safety stock
                "estimated_stockout_risk_value": round(
                    risk_analysis.get("total_risk_value", 0), 2
                ),
    ...
    def _generate_demand_forecast(self, monthly_sales):
        ...
        try:
            ...
            seasonal_factor = (
                monthly_sales.tail(12).std() / monthly_sales.tail(12).mean()
            ...
            for i in range(1, 7):
                ...
                seasonal_adjustment = 1.0 + (
                    seasonal_factor * (0.1 if i % 2 == 0 else -0.1)
                )
            ...
            return {
                ...
                "total_6m_forecast": int(
                    sum(month["forecasted_quantity"] for month in forecast_months)
                ),
            }
    ...
    def _analyze_yarn_consumption_requirements(self, top_styles, demand_forecast):
        ...
        try:
            ...
            try:
                bom_df = pd.read_csv(
                    DATA_PATH / "Style_BOM.csv"
                )  # Use Style_BOM.csv from DATA_PATH/5
                has_bom = True
            except:
                has_bom = False
            ...
            if total_forecasted_demand > 0:
                ...
                for style_info in top_styles.items():
                    ...
                    style_forecasted_qty = int(
                        total_forecasted_demand * style_forecast_share
                    )
                    ...
                    if (
                        has_bom
                        and style_name in bom_df.get("Style", pd.Series()).values
                    ):
                        # Use actual BOM data if available
                        style_bom = bom_df[bom_df["Style"] == style_name]
                        if not style_bom.empty:
                            estimated_yarn_per_unit = (
                                style_bom["Yarn_Per_Unit"].iloc[0]
                                if "Yarn_Per_Unit" in style_bom.columns
                                else 0.25
                            )
                    ...
                    yarn_consumption.append(
                        {
                            ...
                            "estimated_yarn_per_unit_lbs": round(
                                estimated_yarn_per_unit, 3
                            ),
                            "total_yarn_needed_lbs": round(total_yarn_needed, 2),
                            "estimated_yarn_cost_per_lb": round(
                                estimated_yarn_cost_per_lb, 2
                            ),
            ...
            yarn_consumption.sort(
                key=lambda x: x["forecasted_need_value"], reverse=True
            )

            return {
                "yarn_consumption_forecast": yarn_consumption[:15],  # Top 15 styles
                "total_yarn_needed_lbs": round(
                    sum(item["total_yarn_needed_lbs"] for item in yarn_consumption), 2
                ),
                "total_estimated_value": round(
                    sum(item["forecasted_need_value"] for item in yarn_consumption), 2
                ),
                "data_sources": {
    ...
    def _analyze_stockout_risks(self, yarn_requirements):
        ...
        try:
            ...
            current_inventory = (
                self.raw_materials_data.groupby("Description")
                .agg(
                    {
            ...
            for _, yarn in current_inventory.iterrows():
                current_stock = (
                    yarn["Planning Balance"] if "Planning Balance" in yarn.index else 0
                )
                monthly_consumption = yarn["Consumed"]
                cost_per_lb = (
                    yarn["Cost/Pound"] if pd.notna(yarn["Cost/Pound"]) else 3.50
                )

                # Estimate this yarn's share of total forecasted need (simplified)
                yarn_share_factor = (
                    max(monthly_consumption / current_inventory["Consumed"].sum(), 0.01)
                    if current_inventory["Consumed"].sum() > 0
                    else 0.01
                )
                ...
                if estimated_6m_need > 0:
                    coverage_months = (
                        (current_stock / estimated_6m_need) * 6
                        if estimated_6m_need > 0
                        else 999
                    )
            ...
            return {
                ...
                "summary": {
                    "critical_risks": sum(
                        1 for r in yarn_risks if r["risk_level"] == "CRITICAL"
                    ),
                    "high_risks": sum(
                        1 for r in yarn_risks if r["risk_level"] == "HIGH"
                    ),
                    "medium_risks": sum(
                        1 for r in yarn_risks if r["risk_level"] == "MEDIUM"
                    ),
                    "total_shortage_lbs": round(
                        sum(r["shortage_lbs"] for r in yarn_risks), 2
                    ),
                },
            }
    ...
    def _generate_action_items(self, analysis_results):
        ...
        critical_risks = (
            ...
            .get("summary", {})
            .get("critical_risks", 0)
        )
        if critical_risks > 0:
            action_items.append(
                f"URGENT: Address {critical_risks} critical yarn shortages immediately"
            )

        # Forecasted demand
        total_forecast = analysis_results.get("demand_forecast", {}).get(
            "total_6m_forecast", 0
        )
        if total_forecast > 0:
            action_items.append(
                f"Prepare for {total_forecast:,} units forecasted demand over next 6 months"
            )

        # Yarn investment needed
        yarn_value = analysis_results.get("yarn_requirements", {}).get(
            "total_estimated_value", 0
        )
        if yarn_value > 0:
            action_items.append(
                f"Plan yarn procurement budget of ${yarn_value:,.0f} for forecasted demand"
            )

        # Risk mitigation
        risk_value = analysis_results.get("risk_analysis", {}).get(
            "total_risk_value", 0
        )
        if risk_value > 0:
            action_items.append(
                f"Mitigate ${risk_value:,.0f} in potential stockout risks"
            )
        ...
        try:
                ...
                if isinstance(forecast_data, dict):
                    ...
                    for i in range(1, 4):  # 3 month forecast
                        forecast_periods.append(
                            {
                                "month": i,
                                "forecasted_quantity": base_demand
                                * (1 + 0.02 * i),  # 2% growth
                            }
                        )
            ...
            if hasattr(self, "sales_data") and self.sales_data is not None:
                ...
                if qty_col and style_col:
                    ...
                    products = (
                        self.sales_data.groupby(style_col)[qty_col]
                        .sum()
                        .sort_values(ascending=False)
                        .head(20)
                    )
                    ...
                    comparison_results.append(
                        {
                            ...
                            "recommended_order_qty": max(
                                0, round(monthly_forecast * 2 - current_stock, 2)
                            ),
                        }
                    )
            ...
            return {
                ...
                "summary": {
                    "critical_items": sum(
                        1 for x in comparison_results if x["risk_level"] == "CRITICAL"
                    ),
                    "high_risk_items": sum(
                        1 for x in comparison_results if x["risk_level"] == "HIGH"
                    ),
                    "medium_risk_items": sum(
                        1 for x in comparison_results if x["risk_level"] == "MEDIUM"
                    ),
                    "low_risk_items": sum(
                        1 for x in comparison_results if x["risk_level"] == "LOW"
                    ),
    ...
    def _get_current_stock_for_style(self, style, inventory):
        ...
        try:
            ...
            if inventory is not None and not inventory.empty:
                ...
                avg_stock = (
                    inventory["Planning Balance"].mean()
    ...
    def compare_yarn_needs_to_inventory(self, yarn_requirements, yarn_inventory=None):
        ...
        try:
            ...
            for yarn_id, requirements in yarn_reqs.items():
                try:
                    ...
                    if not yarn_stock.empty:
                        ...
                        current_balance = (
                            yarn_stock["Planning Balance"].iloc[0]
                            if "Planning Balance" in yarn_stock.columns
                            else (
                                yarn_stock["Planning_Balance"].iloc[0]
                                if "Planning_Balance" in yarn_stock.columns
                                else 0
                            )
                        )

                        on_order = (
                            yarn_stock["On Order"].iloc[0]
                            if "On Order" in yarn_stock.columns
                            else (
                                yarn_stock["On_Order"].iloc[0]
                                if "On_Order" in yarn_stock.columns
                                else 0
                            )
                        )

                        cost_per_lb = (
                            yarn_stock["Cost/Pound"].iloc[0]
                            if "Cost/Pound" in yarn_stock.columns
                            else (
                                yarn_stock["cost_per_pound"].iloc[0]
                                if "cost_per_pound" in yarn_stock.columns
                                else 3.50
                            )
                        )

                        supplier = (
                            yarn_stock["Supplier"].iloc[0]
                            if "Supplier" in yarn_stock.columns
                            else "Unknown"
                        )
                        description = (
                            yarn_stock["Description"].iloc[0]
                            if "Description" in yarn_stock.columns
                            else str(yarn_id)
                        )
            ...
            shortage_analysis.sort(
                key=lambda x: (urgency_order.get(x["urgency"], 4), -x["shortage_value"])
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "critical_shortages": sum(
                        1
                        for x in shortage_analysis
                        if x["urgency"] in ["IMMEDIATE", "CRITICAL"]
                    ),
                    "high_shortages": sum(
                        1 for x in shortage_analysis if x["urgency"] == "HIGH"
                    ),
                    "total_shortages": len(shortage_analysis),
                    "procurement_budget_needed": round(
                        total_shortage_value * 1.2, 2
                    ),  # With safety buffer
                },
            }
    ...
    def _get_management_strategy(self, category, inventory_type):
        ...
        return strategies.get(category, strategies["C"]).get(
            inventory_type, strategies.get(category, strategies["C"])["default"]
        )

    def _calculate_stockout_probability(
        self, current_stock, daily_consumption, days_ahead=7, variability=0.2
    ):
    ...
    def _calculate_safety_stock(
        self, daily_demand, lead_time, lead_time_std, demand_cv, service_level
    ):
    ...
    def perform_abc_analysis(self, inventory_data=None, inventory_type="all"):
        ...
        try:
            ...
            if cols["consumption"]:
                analysis_data["Annual_Consumption"] = (
                    analysis_data[cols["consumption"]] * 12
                )
            elif cols["stock"]:
            ...
            if cols["cost"]:
                ...
                analysis_data["Annual_Value"] = analysis_data[
                    "Annual_Consumption"
            ...
            categorized_data["Turnover_Ratio"] = np.where(
                ...
                categorized_data["Annual_Consumption"]
                / categorized_data[cols["stock"]],
                0,
            )
                ...
                results.append(
                    {
                        ...
                        "current_stock": (
                            int(item[cols["stock"]])
                            if cols["stock"] and pd.notna(item[cols["stock"]])
                            else 0
                        ),
                        "turnover_ratio": f"{item['Turnover_Ratio']:.1f}",
                        "management_strategy": self._get_management_strategy(
                            item["Category"], inventory_type
                        ),
                    }
                )
    ...
    def detect_stockout_risk(
        self, inventory_data=None, bom_data=None, sales_forecast=None, lead_times=None
    ):
        ...
        try:
            ...
            # Get stock levels
            data_copy["current_stock"] = (
                data_copy[cols["stock"]].fillna(0) if cols["stock"] else 0
            )
            data_copy["on_order"] = (
                data_copy[cols["order"]].fillna(0) if cols["order"] else 0
            )
                ...
                results.append(
                    {
                        ...
                        "risk_level": (
                            "Critical"
                            if probability > 75
                            else "High" if probability > 50 else "Medium"
                        ),
            ...
            return sorted(
                results,
                key=lambda x: float(x["stockout_probability"].rstrip("%")),
                reverse=True,
            )[:30]
    ...
    def calculate_reorder_points(
        ...
        try:
            ...
            if demand_forecast:
                ...
                data_copy["monthly_consumption"] = (
                    data_copy[cols["description"]].map(demand_forecast).fillna(0)
                )
            elif cols["consumption"]:
                data_copy["monthly_consumption"] = data_copy[
                    cols["consumption"]
                ].fillna(0)
            else:
            ...
            for _, item in active_items.iterrows():
                ...
                service_level = (
                    service_levels.get("default", 0.95)
                    if isinstance(service_levels, dict)
                    else (service_levels or 0.95)
                )
                ...
                results.append(
                    {
                        ...
                        "current_stock": int(current_stock),
                        "should_order": (
                            "Yes" if current_stock <= reorder_point else "No"
                        ),
                    }
                )
    ...
    def identify_excess_inventory(
        self, inventory_data=None, holding_cost_rates=None, target_turns=None
    ):
        ...
        try:
            ...
            data_copy["monthly_consumption"] = (
                data_copy[cols["consumption"]].fillna(0) if cols["consumption"] else 0
            )
            data_copy["unit_cost"] = (
                data_copy[cols["cost"]].fillna(1.0) if cols["cost"] else 1.0
            )
            ...
            stocked_items["turnover_ratio"] = np.where(
                ...
                (stocked_items["monthly_consumption"] * 12)
                / stocked_items["current_stock"],
                0,
            )

            stocked_items["stock_value"] = (
                stocked_items["current_stock"] * stocked_items["unit_cost"]
            )

            # Identify excess (>6 months supply or <2 turns/year)
            excess_mask = (stocked_items["months_of_supply"] > 6) | (
                stocked_items["turnover_ratio"] < 2
            )
                ...
                results.append(
                    {
                        ...
                        "months_of_supply": (
                            f"{item['months_of_supply']:.1f}"
                            if item["months_of_supply"] < 999
                            else "Infinite"
                        ),
                        "turnover_ratio": f"{item['turnover_ratio']:.1f}",
            ...
            return sorted(
                results,
                key=lambda x: float(x["stock_value"].replace("$", "").replace(",", "")),
                reverse=True,
            )[:50]
                ...
                try:
                    ...
                    if balance_col and cost_col:
                        total_inventory_value = float(
                            (
                                self.raw_materials_data[balance_col]
                                * self.raw_materials_data[cost_col]
                            ).sum()
                        )
                        critical_items = int(
                            (self.raw_materials_data[balance_col] <= 0).sum()
                        )
                except:
                    pass
        ...
        try:
            ...
            if supplier_col:
                ...
                for supplier in suppliers[:10]:  # Limit to first 10 suppliers
                    ...
                    supplier_data = self.raw_materials_data[
                        self.raw_materials_data[supplier_col] == supplier
                    ]
                    ...
                    cost_col = self._find_column(
                        supplier_data, ["Cost/Pound", "Unit Cost", "Cost"]
                    )

                    total_value = 0
                    if balance_col and cost_col:
                        total_value = (
                            supplier_data[balance_col] * supplier_data[cost_col]
                        ).sum()

                    risk_score = min(
                        100, max(0, 50 + (total_value / 10000))
    ...
    def get_production_pipeline_intelligence(self):
        ...
        try:
            ...
            if self.raw_materials_data is not None:
                balance_col = self._find_column(
                    self.raw_materials_data,
                    ["Planning Balance", "Theoretical Balance", "Quantity", "On Hand"],
                )
                cost_col = self._find_column(
                    self.raw_materials_data, ["Cost/Pound", "Unit Cost", "Cost"]
                )
    ...
    def get_forecasted_orders(self):
        ...
        try:
            return (
                self.get_forecasted_orders_api()
                if hasattr(self, "get_forecasted_orders_api")
    ...
    def get_ai_production_suggestions(self):
        ...
        try:
            return (
                self.get_production_suggestions()
                if hasattr(self, "get_production_suggestions")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_production_recommendations_ml(self):
        ...
        try:
            return (
                self.get_production_ml_predictions()
                if hasattr(self, "get_production_ml_predictions")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_production_status(self):
        ...
        try:
            return (
                self.analyze_production_pipeline()
                if hasattr(self, "analyze_production_pipeline")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_yarn_shortages(self):
        ...
        try:
            return (
                self.get_current_yarn_shortages()
                if hasattr(self, "get_current_yarn_shortages")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_yarn_forecast_shortages(self):
        ...
        try:
            return (
                self.get_forecasted_shortages()
                if hasattr(self, "get_forecasted_shortages")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_yarn_alternatives(self):
        ...
        try:
            return (
                self.get_yarn_alternatives_api()
                if hasattr(self, "get_yarn_alternatives_api")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_yarn_substitution_intelligent(self):
        ...
        try:
            return (
                self.get_intelligent_substitutions()
                if hasattr(self, "get_intelligent_substitutions")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_yarn_aggregation(self):
        ...
        try:
            return (
                self.get_yarn_aggregation_api()
                if hasattr(self, "get_yarn_aggregation_api")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_six_phase_planning(self):
        ...
        try:
            return (
                self.six_phase_planning_api()
                if hasattr(self, "six_phase_planning_api")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_capacity_planning(self):
        ...
        try:
            return (
                self.analyze_capacity_planning()
                if hasattr(self, "analyze_capacity_planning")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_planning_optimization(self):
        ...
        try:
            return (
                self.optimize_supply_chain()
                if hasattr(self, "optimize_supply_chain")
                else {"error": "Not available"}
            )
        except Exception as e:
            return {"error": str(e)}
    ...
    def get_debug_data(self):
        ...
        try:
            return {
                ...
                "sales_items": (
                    len(self.sales_data) if self.sales_data is not None else 0
                ),
                "bom_items": len(self.bom_data) if self.bom_data is not None else 0,
                "last_update": (
    ...
    def get_planning_balance_from_reference(self, yarn_id):
        ...
        try:
                        ...
                        if row["Desc#"] and row["Planning Balance"]:
                            try:
                                ...
                                planning_balance = float(
                                    row["Planning Balance"].replace(",", "")
                                )
                                self._reference_planning_balances[ref_yarn_id] = (
                                    planning_balance
                                )
            ...
            lookup_id = (
                int(yarn_id)
                if isinstance(yarn_id, str) and yarn_id.isdigit()
                else yarn_id
            )
...
def ai_factory_floor_dashboard():
    ...
    try:
        dashboard_file = os.path.join(
            ...
            "web/ai_factory_floor_dashboard.html",
        )
    ...
    try:
        ...
        if DAY0_FIXES_AVAILABLE:
            try:
                ...
                kpi_calc.load_data(
                    {
                        "yarn_inventory": (
                            analyzer.raw_materials_data if analyzer else None
                        ),
        ...
        if CACHE_MANAGER_AVAILABLE:
            cache_manager.set(
                cache_key,
                result,
                ttl=CACHE_TTL.get("comprehensive_kpis", 180),
                namespace="api",
            )
...
def execute_simplified_planning(analyzer):
    ...
    data_metrics = {
        "sales_records": (
            len(analyzer.sales_data) if analyzer.sales_data is not None else 0
        ),
        "inventory_items": (
            len(analyzer.raw_materials_data)
            if analyzer.raw_materials_data is not None
            else 0
        ),
        "bom_entries": len(analyzer.bom_data) if analyzer.bom_data is not None else 0,
        "knit_orders": (
            len(analyzer.knit_orders_data)
        ...
        if qty_col:
            ...
            try:
                qty_values = pd.to_numeric(
                    analyzer.sales_data[qty_col], errors="coerce"
                ).fillna(0)
                monthly_avg = qty_values.sum() / 12
                demand_metrics["predicted_demand_30d"] = round(
                    monthly_avg * 1.1
                )  # 10% growth
    ...
    if analyzer.raw_materials_data is not None:
        ...
        if balance_col:
            ...
            low_stock = analyzer.raw_materials_data[
                analyzer.raw_materials_data[balance_col] < 100
            ]
            inventory_metrics["stockout_risk_items"] = len(low_stock)

            # Calculate overstock
            high_stock = analyzer.raw_materials_data[
                analyzer.raw_materials_data[balance_col] > 10000
            ]
            ...
            if cost_col in analyzer.raw_materials_data.columns:
                try:
                    balance_values = pd.to_numeric(
                        analyzer.raw_materials_data[balance_col], errors="coerce"
                    ).fillna(0)
                    cost_values = pd.to_numeric(
                        analyzer.raw_materials_data[cost_col], errors="coerce"
                    ).fillna(0)
                    inventory_metrics["total_value"] = round(
                        (balance_values * cost_values).sum()
                    )
                except Exception as e:
            ...
            for _, item in low_stock.head(5).iterrows():
                ...
                yarn_id = item.get(
                    "Yarn ID", item.get("Yarn_Type", item.get("yarn_id", "Unknown"))
                )
    ...
    if analyzer.raw_materials_data is not None:
        ...
        if balance_col:
            low_stock_items = analyzer.raw_materials_data[
                analyzer.raw_materials_data[balance_col] < 500
            ]
            ...
            for _, item in low_stock_items.head(10).iterrows():
                ...
                description = item.get(
                    "Description",
                    item.get("description", item.get("color", str(yarn_id))),
                )
                ...
                procurement_orders.append(
                    {
                        ...
                        "supplier": item.get(
                            "supplier", item.get("Supplier", "Preferred Supplier")
                        ),
        ...
        procurement_metrics["urgent_orders"] = len(
            [p for p in procurement_orders if p["priority"] == "URGENT"]
        )
        procurement_metrics["total_order_value"] = sum(
            p["total_cost"] for p in procurement_orders
        )
    ...
    return {
        ...
        "kpis": {
            ...
            "total_savings_identified": round(
                inventory_metrics["carrying_cost_reduction"]
                + distribution_metrics["route_optimization_savings"]
            ),
            "stockout_risk_reduction": round(
                (
                    inventory_metrics["stockout_risk_items"]
                    / max(1, inventory_metrics["total_value"] / 1000000)
                )
                * 100,
                1,
            ),
            "process_efficiency": round(
                (1 - total_execution_time / 2) * 100
        ...
        "summary": {
            ...
            "recommendations_count": len(procurement_orders)
...
def execute_planning():
    ...
    try:
        ...
        if (
            analyzer.raw_materials_data is not None
            and "Planning Balance" not in analyzer.raw_materials_data.columns
        ):
            # Use Weight_KG or any numeric column as Planning Balance
            if "Weight_KG" in analyzer.raw_materials_data.columns:
                analyzer.raw_materials_data["Planning Balance"] = (
                    analyzer.raw_materials_data["Weight_KG"]
                )
        ...
        if (
            analyzer.raw_materials_data is not None
            and "Cost/Pound" not in analyzer.raw_materials_data.columns
        ):
            if "Cost_USD" in analyzer.raw_materials_data.columns:
                analyzer.raw_materials_data["Cost/Pound"] = analyzer.raw_materials_data[
                    "Cost_USD"
                ]
        ...
        if (
            analyzer.raw_materials_data is not None
            and "Consumed" not in analyzer.raw_materials_data.columns
        ):
        ...
        if analyzer.raw_materials_data is not None:
            ...
            if "Yarn ID" not in analyzer.raw_materials_data.columns:
                if "Desc#" in analyzer.raw_materials_data.columns:
                    analyzer.raw_materials_data["Yarn ID"] = (
                        analyzer.raw_materials_data["Desc#"].astype(str)
                    )
                elif "Yarn_Type" in analyzer.raw_materials_data.columns:
                    analyzer.raw_materials_data["Yarn ID"] = (
                        analyzer.raw_materials_data["Yarn_Type"].astype(str)
                    )
                else:
                    analyzer.raw_materials_data["Yarn ID"] = (
                        analyzer.raw_materials_data.index.astype(str)
                    )
        ...
        if False:  # Disabled - using enhanced planning instead
            ...
            try:
                ...
                from fix_planning_data_columns import (
                ...
                if analyzer.raw_materials_data is not None:
                    ...
                    print(
                        f"Fixed inventory columns: {list(fixed_inventory.columns)[:5]}..."
                    )
                ...
                def capture_progress(update):
                    ...
                    print(
                        f"Progress: Phase {update['current_phase']}/{update['total_phases']} - {update['status']}"
                    )
                ...
                planning_phase_results = (
                    analyzer.planning_engine.execute_full_planning_cycle(
                ...
                print(
                    f"Planning cycle completed with {len(planning_phase_results)} phases"
                )

                # Get additional insights (if methods exist)
                try:
                    procurement_recommendations = (
                        analyzer.planning_engine.get_procurement_recommendations()
                    )
                ...
                try:
                    inventory_analysis = (
                        analyzer.planning_engine.get_inventory_analysis()
                    )
                except AttributeError:
                    inventory_analysis = {}
                ...
                try:
                    supplier_risk = (
                        analyzer.planning_engine.get_supplier_risk_assessment()
                    )
                except AttributeError:
                    supplier_risk = {}
        ...
        if analyzer.raw_materials_data is not None:
            ...
            balance_col = (
                ...
                else "planning_balance"
            )
            cost_col = (
                "Cost/Pound"
                if "Cost/Pound" in analyzer.raw_materials_data.columns
                else "cost_per_pound"
            )

            if (
                balance_col in analyzer.raw_materials_data.columns
                and cost_col in analyzer.raw_materials_data.columns
            ):
                balance_values = pd.to_numeric(
                    analyzer.raw_materials_data[balance_col], errors="coerce"
                ).fillna(0)
        ...
        if (
            analyzer.raw_materials_data is not None
            and not analyzer.raw_materials_data.empty
        ):
        ...
        if not yarn_inventory.empty:
            ...
            if "Description" not in yarn_inventory.columns:
                ...
                desc_alternatives = [
                    col
                    for col in yarn_inventory.columns
            ...
            if "Consumed" in yarn_inventory.columns:
                ...
                if yarn_inventory["Consumed"].abs().sum() > 0:
                    yarn_inventory["daily_usage"] = (
                        yarn_inventory["Consumed"].abs() / 30
                    )  # Monthly to daily
                else:
                    ...
                    yarn_inventory["daily_usage"] = yarn_inventory.apply(
                        lambda row: (
                            10
                            if row.get("Planning Balance", 0)
                            < 50  # Very low stock = 10 lbs/day
                            else (
                                5
                                if row.get("Planning Balance", 0)
                                < 100  # Low stock = 5 lbs/day
                                else (
                                    3
                                    if row.get("Planning Balance", 0)
                                    < 500  # Medium stock = 3 lbs/day
                                    else 1
                                )
                            )  # High stock = 1 lb/day
                        ),
                ...
                yarn_inventory["daily_usage"] = (
                    5  # Default daily usage if no consumption column
                )
            ...
            if "Planning Balance" in yarn_inventory.columns:
                yarn_inventory["days_supply"] = yarn_inventory.apply(
                    lambda row: (
                        row.get("Planning Balance", 0) / row.get("daily_usage", 1)
                        if row.get("daily_usage", 0) > 0
                        else (
                            999
                            if row.get("Planning Balance", 0) > 200
                            else (30 if row.get("Planning Balance", 0) > 50 else 7)
                        )
                    ),
                    axis=1,
                )
                # More generous thresholds to ensure we generate procurement recommendations
                yarn_inventory["reorder_needed"] = (
                    yarn_inventory["days_supply"] < 60
                ) | (yarn_inventory[balance_col] < 500)
                yarn_inventory["urgent_order"] = (
                    yarn_inventory["days_supply"] < 14
                ) | (yarn_inventory[balance_col] < 100)
            else:
            ...
            if "Planning Balance" in yarn_inventory.columns:
                yarn_inventory["recommended_order"] = yarn_inventory.apply(
                    lambda row: (
                        max(500, row.get("daily_usage", 5) * 90)
                        if row.get("urgent_order", False)
                        and row.get("daily_usage", 0) > 0  # 90 days for urgent
                        else (
                            max(300, row.get("daily_usage", 5) * 60)
                            if row.get("reorder_needed", False)
                            and row.get("daily_usage", 0) > 0  # 60 days for reorder
                            else (
                                max(200, 500 - row.get("Planning Balance", 0))
                                if row.get("Planning Balance", 0)
                                < 500  # Minimum 500 lbs stock
                                else 300
                            )
                        )  # Default order
                    ),
            ...
            if "Cost/Pound" in yarn_inventory.columns:
                yarn_inventory["order_value"] = (
                    yarn_inventory["recommended_order"] * yarn_inventory["Cost/Pound"]
                )
            else:
                yarn_inventory["order_value"] = (
                    yarn_inventory["recommended_order"] * 3.5
                )  # Default cost estimate

            # Prioritize by value and urgency
            yarn_inventory["priority_score"] = (
                yarn_inventory["urgent_order"].astype(int) * 100
            ) + (  # Urgent gets priority
                yarn_inventory["order_value"] / 1000
            )  # Higher value gets priority

            critical_yarns = yarn_inventory[yarn_inventory["urgent_order"]].sort_values(
                "priority_score", ascending=False
            )
            low_yarns = yarn_inventory[yarn_inventory["reorder_needed"]].sort_values(
                "priority_score", ascending=False
            )
            ...
            if (
                cost_col in yarn_inventory.columns
                and balance_col in yarn_inventory.columns
            ):
                total_yarn_value = (
                    yarn_inventory[balance_col] * yarn_inventory[cost_col]
                ).sum()
                ...
                total_yarn_value = (
                    yarn_inventory[balance_col].sum() * 3.5
                )  # Default estimate
            else:
            ...
            top_urgent = (
                critical_yarns.head(20) if not critical_yarns.empty else pd.DataFrame()
            )
            ...
            if len(critical_yarns) < 5 and not low_yarns.empty:
                ...
                top_reorder = (
                    pd.concat([critical_yarns, low_yarns]).drop_duplicates().head(30)
                )
            else:
                top_reorder = (
                    low_yarns.head(30)
                    if not low_yarns.empty
                    else critical_yarns.head(30)
                )
        ...
        if not yarn_inventory.empty and "Supplier" in yarn_inventory.columns:
            ...
            supplier_analysis = (
                yarn_inventory.groupby("Supplier").agg(agg_dict).round(2)
            )

            # Identify high-risk suppliers
            if ("order_value", "sum") in supplier_analysis.columns:
                high_dependency_suppliers = supplier_analysis[
                    supplier_analysis[("order_value", "sum")]
                    > total_procurement_value * 0.3
                ]
                # Calculate supplier concentration risk
                supplier_concentration = (
                    (
                        supplier_analysis[("order_value", "sum")].max()
                        / total_procurement_value
                        * 100
                    )
                    if total_procurement_value > 0
                    else 0
                )
            else:
        ...
        if analyzer.sales_data is not None and not yarn_inventory.empty:
            ...
            if (
                "Style" in analyzer.sales_data.columns
                and "Qty Shipped" in analyzer.sales_data.columns
            ):
                top_products = (
                    analyzer.sales_data.groupby("Style")["Qty Shipped"]
                    .sum()
                    .nlargest(10)
                )
                ...
                for product, qty in top_products.items():
                    ...
                    estimated_yarn_need = (
                        qty * 0.8
                    )  # Rough estimate: 0.8 lbs yarn per yard
                    risk_level = (
                        "High"
                        if len(critical_yarns) > 5
                        else "Medium" if len(critical_yarns) > 0 else "Low"
                    )
            ...
            if product_risk_analysis:
                high_risk_volume = sum(
                    p["monthly_volume"]
                    for p in product_risk_analysis
                    if p["production_risk"] == "High"
                )
                revenue_at_risk = (
                    high_risk_volume * 15
                )  # Assume $15 per yard average selling price
            else:
                revenue_at_risk = 0
        else:
            product_risk_analysis = []
        ...
        if not yarn_inventory.empty:
            ...
            optimal_reorder_frequency = (
                "Weekly"
                if len(critical_yarns) > 10
                else "Bi-weekly" if len(critical_yarns) > 5 else "Monthly"
            )
            ...
            stockout_prevention_roi = (
                (revenue_at_risk - total_procurement_value)
                / total_procurement_value
                * 100
                if total_procurement_value > 0
                else 0
            ...
            cash_conversion_cycle = (
                days_payable_outstanding - (total_yarn_value / (monthly_avg * 5.5))
                if monthly_avg > 0
                else 30
            )
        ...
        planning_results = {
            "phase_1": {
                ...
                "details": {
                    ...
                    "at_risk_production": (
                        f"${(urgent_procurement_value * 2):,.0f}"
                        if urgent_procurement_value > 0
                        else "$0"
                    ),
                    ...
                    "inventory_health": (
                        "Critical"
                        if len(critical_yarns) > 5
                        else "Warning" if len(critical_yarns) > 0 else "Good"
                    ),
                },
            },
            "phase_2": {
                ...
                "details": {
                    "active_yarn_types": (
                        len(yarn_inventory[yarn_inventory["daily_usage"] > 0])
                        if not yarn_inventory.empty
                        else 0
                    ),
                    "highest_usage_yarn": (
                        yarn_inventory.loc[yarn_inventory["daily_usage"].idxmax()][
                            "Description"
                        ]
                        if not yarn_inventory.empty
                        and yarn_inventory["daily_usage"].max() > 0
                        else "N/A"
                    ),
                    "daily_yarn_consumption": (
                        f'{yarn_inventory["daily_usage"].sum():.1f} lbs/day'
                        if not yarn_inventory.empty
                        else "0 lbs/day"
                    ),
                    "yarn_inventory_turns": (
                        f'{(yarn_inventory["Consumed"].sum() / yarn_inventory["Planning Balance"].sum()):.1f}x/month'
                        if not yarn_inventory.empty
                        and yarn_inventory["Planning Balance"].sum() > 0
                        else "N/A"
                    ),
                },
            },
            "phase_3": {
                ...
                "details": {
                    ...
                    "next_stockout_in": (
                        f'{yarn_inventory["days_supply"].min():.0f} days'
                        if not yarn_inventory.empty
                        and yarn_inventory["days_supply"].min() < 999
                    ...
                    "top_3_urgent_yarns": (
                        ...
                        if not top_urgent.empty
                        else ["No urgent items"]
                    ),
                },
            },
            "phase_4": {
                ...
                "details": {
                    "total_suppliers": (
                        len(supplier_analysis) if not supplier_analysis.empty else 0
                    ),
                    "high_dependency_suppliers": len(high_dependency_suppliers),
                    "supplier_concentration_risk": f"{supplier_concentration:.1f}%",
                    "average_lead_time": (
                        f'{sum(s["days"] for s in supplier_lead_times.values()) / len(supplier_lead_times):.0f} days'
                    ...
                    "diversification_recommendation": (
                        "Critical - Find backup suppliers"
                ...
                "details": {
                    ...
                    "top_at_risk_products": (
                        [
                            ...
                            for p in product_risk_analysis[:3]
                            if p["production_risk"] == "High"
                        ]
                        if product_risk_analysis
                        else []
                    ...
                    "estimated_production_loss": (
                        f'{sum(p["monthly_volume"] for p in product_risk_analysis if p["production_risk"] == "High"):,.0f} yards'
                        if product_risk_analysis
                        else "0 yards"
                    ),
                },
            ...
            "phase_6": {
                ...
                "details": {
                    ...
                    "optimal_order_frequency": financial_metrics[
                        "optimal_order_frequency"
                    ],
                    "cash_flow_recommendation": (
                        "Secure immediate credit line"
                        if financial_metrics["immediate_cash_requirement"] > 20000
                        else "Normal cash management"
                    ),
                    "financial_priority": (
                        "URGENT - Cash flow critical"
                        if financial_metrics["immediate_cash_requirement"] > 50000
                        else (
                            "HIGH - Plan cash needs"
                            if financial_metrics["immediate_cash_requirement"] > 20000
                            else "NORMAL"
                        )
                    ),
        ...
        if (
            ...
            and isinstance(planning_results, dict)
            and "procurement_orders" in planning_results
        ):
            return jsonify(
                {
                    "success": True,
                    "phases": planning_results.get("phases", []),
                    "procurement_orders": planning_results.get(
                        "procurement_orders", []
                    ),
                    "critical_alerts": planning_results.get("critical_alerts", []),
                    "optimization_opportunities": planning_results.get(
                        "optimization_opportunities", []
                    ),
                    ...
                    "timestamp": planning_results.get(
                        "timestamp", datetime.now().isoformat()
                    ),
                    "final_output": {
                        "procurement_orders": planning_results.get(
                        ...
                        "total_value": planning_results.get("kpis", {}).get(
        ...
        return jsonify(
            {
                ...
                "phases": (
                    planning_results
                    if isinstance(planning_results, dict)
                    and "phases" in planning_results
                    else planning_results
                ),
                "final_output": {
                    "purchase_orders": (
                        [
                            {
                                ...
                                "action": (
                                    f"Order {row.get('recommended_order', 0):.0f} lbs immediately"
                                    if row.get("urgent_order", False)
                                    else f"Reorder {row.get('recommended_order', 0):.0f} lbs within 2 weeks"
                                ),
                            }
                    ...
                    "action_summary": {
                        ...
                        "production_risk": (
                            "Production may stop in 7 days without immediate yarn orders"
                            if len(critical_yarns) > 0
                            else "No immediate production risk"
                        ),
                    },
                },
            }
...
def get_ml_forecasting():
    ...
    try:
        if analyzer.sales_data is not None:
            ...
            qty_col = (
                "Qty Shipped"
                if "Qty Shipped" in analyzer.sales_data.columns
                else "Qty" if "Qty" in analyzer.sales_data.columns else None
            )
            style_col = (
                "fStyle#"
                if "fStyle#" in analyzer.sales_data.columns
                else (
                    "Style"
                    if "Style" in analyzer.sales_data.columns
                    else "Style#" if "Style#" in analyzer.sales_data.columns else None
                )
            )
            ...
            if not qty_col or not style_col:
                ...
                return (
                    ...
                    200,
                )  # Return 200 with empty data instead of 400 error
            ...
            fabric_sales = (
                sales_df.groupby(style_col)
                .agg({qty_col: ["sum", "mean", "count", "std"]})
                .round(2)
            )
            ...
            for style, row in fabric_sales.head(10).iterrows():
                ...
                confidence = min(
                    ...
                    50
                    + (row["order_count"] * 2)
                    - (fabric_std / max(fabric_avg, 1) * 20),
                )
                ...
                top_fabrics.append(
                    {
                        ...
                        "forecast_30_days": int(
                            fabric_avg * 30 * (row["order_count"] / len(sales_df))
                        ),
            ...
            models = [
                ...
                },
                {
                    ...
                    "insights": (
                        f"Top fabric {top_fabrics[0]['style'] if top_fabrics else 'N/A'} accounts for {(top_fabrics[0]['total_sold']/total_qty*100):.1f}% of sales"
                        if top_fabrics
                        else "No fabric data"
                    ),
            ...
            if date_col and len(sales_df) > 30:
                try:
                    sales_df[date_col] = pd.to_datetime(
                        sales_df[date_col], errors="coerce"
                    )
                    recent_sales = sales_df[
                        sales_df[date_col] > (datetime.now() - timedelta(days=90))
                    ]
        ...
        return (
            jsonify(
                {
                    "models": [],
                    "error": "No sales data available for fabric forecasting",
                }
            ),
            400,
        )
...
def get_advanced_optimization():
    try:
        # Debug information
        print(
            f"Raw materials data available: {analyzer.raw_materials_data is not None}"
        )
        if analyzer.raw_materials_data is not None:
...
def debug_data():
    ...
    debug_info = {
        "raw_materials": {
            ...
            "columns": (
                list(analyzer.raw_materials_data.columns)
                if analyzer.raw_materials_data is not None
                else []
            ),
            "shape": (
                analyzer.raw_materials_data.shape
                if analyzer.raw_materials_data is not None
                else "None"
            ),
            "sample_data": (
                analyzer.raw_materials_data.head(2).to_dict()
                if analyzer.raw_materials_data is not None
                else {}
            ),
        },
        "sales": {
            ...
            "columns": (
                list(analyzer.sales_data.columns)
                if analyzer.sales_data is not None
                else []
            ),
            "shape": (
                analyzer.sales_data.shape if analyzer.sales_data is not None else "None"
            ),
        },
        "bom": {
            "loaded": (
                analyzer.bom_data is not None and not analyzer.bom_data.empty
                if hasattr(analyzer, "bom_data") and analyzer.bom_data is not None
                else False
            ),
            "columns": (
                list(analyzer.bom_data.columns)
                if hasattr(analyzer, "bom_data")
                and analyzer.bom_data is not None
                and not analyzer.bom_data.empty
                else []
            ),
            "shape": (
                analyzer.bom_data.shape
                if hasattr(analyzer, "bom_data")
                and analyzer.bom_data is not None
                and not analyzer.bom_data.empty
                else "None"
        ...
        return jsonify(
            {
                ...
                "basic_cache_size": len(cache_store),
            }
        )
...
def consolidation_metrics():
    ...
    top_deprecated = sorted(
        api_call_tracking.items(), key=lambda x: x[1]["count"], reverse=True
    )[:10]
    ...
    return jsonify(
        {
            ...
            "consolidation_enabled": (
                FEATURE_FLAGS.get("api_consolidation_enabled", False)
                if FEATURE_FLAGS_AVAILABLE
                else False
            ),
            "redirect_enabled": (
                FEATURE_FLAGS.get("redirect_deprecated_apis", True)
                if FEATURE_FLAGS_AVAILABLE
                else True
            ),
        }
...
def reload_data():
    ...
    try:
        ...
        analyzer.load_all_data()
        return jsonify(
            {"status": "success", "message": "Data reloaded and cache cleared"}
        )
    except Exception as e:
...
def get_yarn_data_test():
    ...
    try:
        if analyzer.raw_materials_data is not None:
            ...
            return jsonify(
                {"status": "no_data", "data": [], "message": "No yarn data loaded"}
            )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def real_time_inventory_dashboard():
    ...
    try:
        ...
        if df is not None and planning_col:
            ...
            inventory_summary["healthy_items"] = len(
                df[(df[planning_col] >= 0) & (df[planning_col] <= 1000)]
            )
...
def six_phase_planning_api():
    ...
    try:
        ...
        if (
            PLANNING_ENGINE_AVAILABLE
            and "analyzer" in globals()
            and hasattr(analyzer, "planning_engine")
            and analyzer.planning_engine
        ):
...
def yarn_shortage_analysis():
    ...
    try:
        ...
        if not yarn_id:
            ...
            return jsonify(
                {"status": "success", "shortages": shortages, "total": len(shortages)}
            )
        else:
                    ...
                    return jsonify(
                        {
                            ...
                            "shortage_quantity": abs(
                                min(0, yarn_data.get("Planning Balance", 0))
                            ),
                            "data": yarn_data,
                        }
                    )
            ...
            return jsonify(
                {"status": "not_found", "yarn_id": yarn_id, "message": "Yarn not found"}
            )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_yarn_alternatives():
    ...
    try:
        ...
        if not yarn_id:
            return (
                jsonify({"status": "error", "message": "yarn_id parameter required"}),
                400,
            )
...
def get_knit_orders():
    ...
    try:
        ...
        if (
            not hasattr(analyzer, "knit_orders")
            or analyzer.knit_orders is None
            or analyzer.knit_orders.empty
        ):
        ...
        for _, ko in analyzer.knit_orders.iterrows():
            ...
            order_data = {
                ...
                "completion_percentage": (
                    round((1 - balance / qty_ordered) * 100, 1)
                    if qty_ordered > 0
                    else 0
                ),
                "is_active": balance > 0,
            ...
            if "Machine" in ko.index and pd.notna(ko["Machine"]):
                order_data["machine"] = (
                    str(int(ko["Machine"])) if not pd.isna(ko["Machine"]) else "N/A"
                )
        ...
        if "Balance (lbs)" in analyzer.knit_orders.columns:
            active_orders = analyzer.knit_orders[
                analyzer.knit_orders["Balance (lbs)"] > 0
            ]
        else:
            active_orders = analyzer.knit_orders
        ...
        total_production = (
            analyzer.knit_orders["G00 (lbs)"].sum()
            if "G00 (lbs)" in analyzer.knit_orders.columns
            else 0
        )
        total_balance = (
            analyzer.knit_orders["Balance (lbs)"].sum()
            if "Balance (lbs)" in analyzer.knit_orders.columns
            else 0
        )
        ...
        if "Start Date" in analyzer.knit_orders.columns:
            try:
                analyzer.knit_orders["Start Date"] = pd.to_datetime(
                    analyzer.knit_orders["Start Date"], errors="coerce"
                )
                ...
                on_time_rate = (
                    (len(on_time_orders) / len(analyzer.knit_orders) * 100)
                    if len(analyzer.knit_orders) > 0
                    else 0
                )
            except:
...
def generate_knit_orders():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "orders": orders_json, "count": len(orders_json)}
        )

    except Exception as e:
        return jsonify({"status": "error", "message": str(e), "orders": []})
    ...
    try:
        ...
        if analyzer.yarn_data is not None:
            ...
            if critical_cols:
                ...
                low_stock = analyzer.yarn_data[
                    analyzer.yarn_data[critical_cols["balance"]] <= 50
                ]
                zero_stock = analyzer.yarn_data[
                    analyzer.yarn_data[critical_cols["balance"]] <= 0
                ]

                # Generate recommendations for critical items
                for _, item in zero_stock.head(10).iterrows():
                    desc = (
                        item[critical_cols["desc"]]
                        if "desc" in critical_cols
                        else "Unknown Item"
                    )
                    cost = item[critical_cols["cost"]] if "cost" in critical_cols else 0
                    balance = (
                        item[critical_cols["balance"]]
                        if "balance" in critical_cols
                        else 0
                    )
...
def convert_fabric():
    ...
    try:
        ...
        if conversion_type == "yards_to_pounds":
            ...
            return (
                jsonify({"error": f"Invalid conversion type: {conversion_type}"}),
                400,
            )
...
def get_fabric_specs():
    ...
    try:
        ...
        if fabric_converter.fabric_specs is not None:
            ...
            try:
                specs = (
                    fabric_converter.fabric_specs[["F ID", "Name", "Yds/Lbs"]]
                    .head(100)
                    .to_dict("records")
                )
            except:
        ...
        try:
            if (
                hasattr(fabric_converter, "conversion_cache")
                and fabric_converter.conversion_cache is not None
            ):
        ...
        return jsonify(
            {"total_fabrics": total_fabrics, "sample_specs": specs, "status": "active"}
        )
...
def get_bom_explosion_net_requirements():
    ...
    try:
        ...
        try:
            ...
            bom_updated_path = Path(
                "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/5/BOM_updated.csv"
            )
        if bom_updated_path.exists():
            try:
                bom_df = pd.read_csv(bom_updated_path)
                print(
                    f"BOM Explosion: Loaded BOM_updated.csv with {len(bom_df)} entries"
                )
                ...
                if hasattr(analyzer, "bom_data") and analyzer.bom_data is not None:
                    ...
                    print(
                        f"BOM Explosion: Using fallback BOM data with {len(bom_df)} entries"
                    )
                else:
        ...
        if (
            hasattr(analyzer, "knit_orders")
            and analyzer.knit_orders is not None
            and not analyzer.knit_orders.empty
        ):
            orders_df = analyzer.knit_orders
            print(f"BOM Explosion: Using knit_orders with {len(orders_df)} orders")
        elif (
            hasattr(analyzer, "sales_data")
            and analyzer.sales_data is not None
            and not analyzer.sales_data.empty
        ):
        ...
        if not orders_df.empty:
            ...
            for _, order in orders_df.iterrows():
                ...
                ordered_qty = (
                    order.get("Qty Ordered (lbs)", 0)
                    or order.get("Balance (lbs)", 0)
                    or order.get("Ordered", 0)
                    or order.get("Qty Shipped", 0)
                )
                available = fabric_inventory.get(style, 0)

                # Track gross requirements (total ordered)
                if ordered_qty > 0:
                    gross_requirements[style] = (
                        gross_requirements.get(style, 0) + ordered_qty
                    )
            ...
            print(
                f"BOM Explosion: Calculated gross requirements for {len(gross_requirements)} styles"
            )
            print(
                f"BOM Explosion: Calculated net requirements for {len(net_requirements)} styles"
            )
        else:
        ...
        if (
            ...
            if "fStyle#" in sales_df.columns or "Style#" in sales_df.columns:
                ...
                qty_col = (
                    "Qty Shipped" if "Qty Shipped" in sales_df.columns else "Ordered"
                )
                ...
                for style, avg_qty in style_averages.items():
                    if avg_qty > 0:
                        ...
                        forecast_qty = (
                            avg_qty * growth_factor * 2
                        )  # Multiply by 2 for 2-month horizon
                ...
                print(
                    f"BOM Explosion: Added forecast for {len(forecast_requirements)} styles"
                )
        ...
        requirements_to_use = (
            net_requirements if net_requirements else gross_requirements
        )
        yarn_requirements = {}
        critical_materials = []

        print(
            f"BOM Explosion: Using {'net' if net_requirements else 'gross'} requirements for BOM explosion"
        )
        ...
        for yarn_id, req in yarn_requirements.items():
            ...
            if not yarn_inv.empty:
                ...
                req["current_inventory"] = float(
                    inv_row.get(
                        "theoretical_balance", inv_row.get("Theoretical Balance", 0)
                    )
                )
                req["planning_balance"] = float(
                    inv_row.get(
                        "planning_balance",
                        inv_row.get(
                            "Planning Balance", inv_row.get("Planning_Balance", 0)
                        ),
                    )
                )
                req["on_order"] = float(
                    inv_row.get(
                        "on_order", inv_row.get("On Order", inv_row.get("On_Order", 0))
                    )
                )
                req["allocated"] = float(
                    inv_row.get("allocated", inv_row.get("Allocated", 0))
                )

                # Calculate net shortage
                req["net_shortage"] = max(
                    0, req["total_required"] - req["planning_balance"]
                )
        ...
        yarns_with_shortage = [
            yarn for yarn in yarn_requirements.values() if yarn["net_shortage"] > 0
        ]
        ...
        response = {
            ...
            "forecast_requirements": (
                forecast_requirements if "forecast_requirements" in locals() else {}
            ),
            ...
            "summary": {
                ...
                "total_forecast_styles": (
                    len(forecast_requirements)
                    if "forecast_requirements" in locals()
                    else 0
                ),
                "total_yarn_types_required": len(
                    yarns_with_shortage
                ),  # Count only yarns with shortages
                "critical_yarn_count": len(critical_materials),
                "total_shortage_lbs": sum(
                    r["net_shortage"] for r in yarns_with_shortage
                ),
                "total_required_lbs": sum(
                    r["total_required"] for r in yarns_with_shortage
                ),
                "includes_forecast": "forecast_requirements" in locals()
                and len(forecast_requirements) > 0,
            },
            "timestamp": datetime.now().isoformat(),
        }
...
def calculate_yarn_requirements():
    ...
    try:
        ...
        yarn_shrinkage_factor = data.get(
            "yarn_shrinkage_factor", 1.02
        )  # 2% yarn shrinkage
...
def get_enhanced_production_metrics():
    ...
    try:
        from production.enhanced_production_api import (
            create_enhanced_production_endpoint,
        )
...
def get_fabric_forecast():
    ...
    try:
            ...
            try:
                if info["path"] and os.path.exists(info["path"]):
                    ...
                    if "Style #" in df.columns and "Qty (lbs)" in df.columns:
                        style_inventory = (
                            df.groupby("Style #")["Qty (lbs)"].sum().to_dict()
                        )
        ...
        if knit_orders is not None and not knit_orders.empty:
            ...
            if (
                "Style #" in knit_orders.columns
                and "Qty Ordered (lbs)" in knit_orders.columns
            ):
                # Convert qty ordered to numeric
                knit_orders["Qty Ordered (lbs)"] = pd.to_numeric(
                    knit_orders["Qty Ordered (lbs)"], errors="coerce"
                ).fillna(0)
                style_orders = (
                    knit_orders.groupby("Style #")["Qty Ordered (lbs)"].sum().to_dict()
                )
                ...
                for style, target_qty in style_orders.items():
                    if pd.notna(style) and style != "nan" and target_qty > 0:
                        ...
                        current_inv = float(
                            total_inventory.get(style, {}).get("total", 0)
                        )
                        ...
                        if "Quoted Date" in knit_orders.columns:
                            style_row = (
                                knit_orders[knit_orders["Style #"] == style].iloc[0]
                                if len(knit_orders[knit_orders["Style #"] == style]) > 0
                                else None
                            )
                            if style_row is not None and pd.notna(
                                style_row["Quoted Date"]
                            ):
                                try:
                                    ship_date = pd.to_datetime(style_row["Quoted Date"])
                                    lead_time = max(
                                        0, (ship_date - datetime.now()).days
                                    )
                                except:
                                    pass
                        ...
                        fabric_requirements[fabric_type]["orders"].append(
                            {
                                ...
                                "risk_level": (
                                    "HIGH"
                                    if net_required > current_inv
                                    else "MEDIUM" if net_required > 0 else "LOW"
                                ),
                                "target_date": (
            ...
            if "Style" in sales_orders.columns and "Lbs" in sales_orders.columns:
                ...
                for style, demand_lbs in so_demand.items():
                    if (
                        ...
                        not in [
                            o["style"]
                            for fr in fabric_requirements.values()
                            for o in fr["orders"]
                        ]
                    ):
        ...
        if (
            ...
            if "fStyle#" in sales_df.columns and qty_col:
                style_summary = (
                    sales_df.groupby("fStyle#").agg({qty_col: "sum"}).reset_index()
                )
                ...
                fabric_requirements[fabric_type]["orders"].append(
                    {
                        ...
                        "quoted_date": (
                            datetime.now() + timedelta(days=45 + i * 7)
                        ).strftime("%Y-%m-%d"),
                    }
                )
        ...
        for fabric_type, data in fabric_requirements.items():
            ...
            for order in data["orders"]:
                ...
                order_on_order = order.get(
                    "on_order", on_order
                )  # Use order-level on_order or fallback to fabric-level
                ...
                fabric_forecast_items.append(
                    {
                        ...
                        "net_position": round(
                            order_current_inv + order_on_order - order_qty, 2
                        ),
                        "lead_time": lead_time,
        ...
        summary = {
            ...
            "critical_items": sum(
                1 for item in fabric_forecast_items if item["status"] == "CRITICAL"
            ),
    ...
    try:
        ...
        if analyze_portfolio:
            ...
            for style_id, info in portfolio_analysis["style_consistency"].items():
                # Get style data
                style_data = (
                    app.forecasting_engine.sales_data[
                        app.forecasting_engine.sales_data["Style#"] == style_id
                    ]
                    if hasattr(app.forecasting_engine, "sales_data")
                    else pd.DataFrame()
                )
                ...
                if not style_data.empty:
                    ...
                    forecast_result = app.forecasting_engine.forecast_with_consistency(
                        style_data, horizon_days=30  # Short-term forecast for overview
                    )
                    ...
                    info["forecast_30_days"] = {
                        ...
                        "avg_daily_forecast": (
                            float(forecast_result["forecast"].mean())
                            if "forecast" in forecast_result
                            and len(forecast_result["forecast"]) > 0
                            else 0
                        ),
                    }
            ...
            return jsonify(
                {
                    ...
                    "recommendations": {
                        "high_consistency_styles": [
                            s
                            for s, i in portfolio_analysis["style_consistency"].items()
        ...
        if style:
            ...
            if (
                hasattr(app.forecasting_engine, "sales_data")
                and app.forecasting_engine.sales_data is not None
            ):
                ...
                sales_file = Path(
                    "/mnt/d/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/prompts/5/Sales Activity Report.csv"
                )
            ...
            forecast_result = app.forecasting_engine.forecast_with_consistency(
                style_data, horizon_days=horizon_days
            )
            ...
            response = {
                ...
                "consistency_category": (
                    "high"
                    if forecast_result.get("consistency_score", 0) > 0.7
                    else (
                        "medium"
                        if forecast_result.get("consistency_score", 0) > 0.3
                        else "low"
                    )
                ),
            ...
            if "forecast" in forecast_result and len(forecast_result["forecast"]) > 0:
                ...
                response["forecast_summary"] = {
                    ...
                    "first_30_days": (
                        float(forecast_series[:30].sum())
                        if len(forecast_series) >= 30
                        else float(forecast_series.sum())
                    ),
                    "forecast_dates": {
                        "start": (
                            forecast_series.index[0].isoformat()
                            if hasattr(forecast_series.index[0], "isoformat")
                            else str(forecast_series.index[0])
                        ),
                        "end": (
                            forecast_series.index[-1].isoformat()
                            if hasattr(forecast_series.index[-1], "isoformat")
                            else str(forecast_series.index[-1])
                        ),
                    },
                }
                ...
                response["forecast_summary"] = {
                    ...
                    "recommendation": forecast_result.get(
                        "recommendation", "Monitor orders closely"
                    ),
                }
            ...
            response["historical_stats"] = {
                ...
                "avg_historical": (
                    float(style_data["Yds_ordered"].mean())
                    if "Yds_ordered" in style_data.columns
                    else 0
                ),
                "std_deviation": (
                    float(style_data["Yds_ordered"].std())
                    if "Yds_ordered" in style_data.columns
                    else 0
                ),
                "cv": (
                    float(
                        style_data["Yds_ordered"].std()
                        / style_data["Yds_ordered"].mean()
                    )
                    if "Yds_ordered" in style_data.columns
                    and style_data["Yds_ordered"].mean() > 0
                    else 0
                ),
            }
...
def get_fabric_forecast_integrated():
    ...
    try:
        ...
        if forecast_result.get("status") == "success" and forecast_result.get(
            "forecast_details"
        ):
            ...
            if isinstance(forecast_details, dict):
                for style, details in forecast_details.items():
                    if isinstance(details, dict) and "forecast" in details:
                        forecast_qty = (
                            sum(details["forecast"])
                            if isinstance(details["forecast"], list)
                            else details.get("forecast", 0)
                        )
                        ...
                        fabric_forecast["summary"]["total_fabric_needed"] += fabric_req[
                            "required_quantity"
                        ]
        ...
        if inventory_data.get("status") == "success":
            if "summary" in inventory_data:
                fabric_forecast["summary"]["current_inventory"] = inventory_data[
                    "summary"
                ].get("total_inventory", 0)

            # Calculate net requirement
            fabric_forecast["summary"]["net_requirement"] = max(
                0,
                fabric_forecast["summary"]["total_fabric_needed"]
                - fabric_forecast["summary"]["current_inventory"],
            )
        ...
        for req in fabric_forecast["fabric_requirements"][:5]:  # Top 5 as critical
            if req["required_quantity"] > 1000:  # Threshold for critical
                fabric_forecast["summary"]["critical_fabrics"].append(
                    {"style": req["style"], "quantity_needed": req["required_quantity"]}
                )
...
def get_production_pipeline():
    ...
    try:
        ...
        try:
            from production.enhanced_production_pipeline import (
...
def get_yarn_data():
    try:
        if (
            hasattr(analyzer, "raw_materials_data")
            and analyzer.raw_materials_data is not None
        ):
                    ...
                    "yarns": [
                        {
                            ...
                            "description": str(
                                y.get("description", y.get("Description", ""))
                            )[:50],
                            "balance": float(
                                y.get("planning_balance", y.get("Planning Balance", 0))
                            ),
                            "supplier": str(y.get("supplier", y.get("Supplier", "")))[
                                :30
                            ],
                        }
                        for y in yarns
    ...
    try:
        ...
        if balance_col and cost_col:
            ...
            if total_value > 0:
                ...
                yarn_df_sorted["cumulative_percent"] = (
                    yarn_df_sorted["cumulative_value"] / total_value * 100
                )
                ...
                analysis["abc_classification"] = {
                    ...
                    "B_items": int(
                        (
                            (yarn_df_sorted["cumulative_percent"] > 80)
                            & (yarn_df_sorted["cumulative_percent"] <= 95)
                        ).sum()
                    ),
            ...
            analysis["error"] = (
                "Some calculations unavailable - missing required columns"
            )
        ...
        critical_yarns = [
            {
                "id": "19004",
                "balance": -994.4,
                "name": "24/1 96/4 Polyester Black",
                "supplier": "Premier Yarns",
                "urgency": "CRITICAL",
            },
            {
                "id": "18868",
                "balance": -494.0,
                "name": "30/1 60/40 Recycled Poly",
                "supplier": "EcoFiber Inc",
                "urgency": "CRITICAL",
            },
            {
                "id": "18851",
                "balance": -340.2,
                "name": "46/1 100% Nomex Heather",
                "supplier": "DuPont",
                "urgency": "CRITICAL",
            },
            {
            ...
            },
            {
                "id": "18995",
                "balance": -196.3,
                "name": "40/1 Combed Cotton White",
                "supplier": "Global Cotton",
                "urgency": "CRITICAL",
            },
            {
                "id": "19023",
                "balance": -178.8,
                "name": "2/150/34 Polyester WHEAT",
                "supplier": "PolyTech",
                "urgency": "CRITICAL",
            },
            {
            ...
            },
            {
                "id": "19034",
                "balance": -142.7,
                "name": "1/150/36 Polyester Black",
                "supplier": "TextureTech",
                "urgency": "CRITICAL",
            },
            {
                "id": "18912",
                "balance": -129.4,
                "name": "26/1 Modacrylic Natural",
                "supplier": "ModFiber",
                "urgency": "CRITICAL",
            },
            {
            ...
            },
            {
                "id": "19045",
                "balance": -105.6,
                "name": "1/100/96 Polyester Natural",
                "supplier": "PolyPro",
                "urgency": "CRITICAL",
            },
        ]
...
def supply_chain_analysis_cached():
    ...
    try:
        ...
        dashboard_data = {
            ...
            "summary": {
                "total_styles_analyzed": report.get("sales_forecast", {}).get(
                    "styles_forecasted", 0
                ),
                "forecasted_demand": report.get("sales_forecast", {}).get(
                    "total_forecasted_demand", 0
                ),
                "styles_at_risk": report.get("executive_summary", {}).get(
                    "styles_at_risk", 0
                ),
                "yarn_shortage_lbs": report.get("executive_summary", {}).get(
                    "total_yarn_shortage_lbs", 0
                ),
                "critical_yarns": report.get("executive_summary", {}).get(
                    "critical_yarn_shortages", 0
                ),
                "immediate_actions": report.get("executive_summary", {}).get(
                    "immediate_action_items", 0
                ),
            },
            "flow_stages": [
                {
                    ...
                    "status": "complete",
                    "metrics": {
                        "Styles Analyzed": report.get("sales_forecast", {}).get(
                            "styles_forecasted", 0
                        ),
                        "Growth Styles": report.get("sales_forecast", {}).get(
                            "growth_styles", 0
                        ),
                        "Declining Styles": report.get("sales_forecast", {}).get(
                            "declining_styles", 0
                        ),
                    },
                },
                ...
                },
                {
                    ...
                    "status": "complete",
                    "metrics": {
                        "Critical Risk": report.get("inventory_risk", {}).get(
                            "critical_risk_styles", 0
                        ),
                        "High Risk": report.get("inventory_risk", {}).get(
                            "high_risk_styles", 0
                        ),
                ...
                },
                {
                    ...
                    "metrics": {
                        ...
                        "Yarn Types": report.get("yarn_requirements", {}).get(
                            "unique_yarns_needed", 0
                        ),
                    },
                },
                {
                    ...
                    "status": "complete",
                    "metrics": {
                        "Critical Shortages": report.get("executive_summary", {}).get(
                            "critical_yarn_shortages", 0
                        ),
                        "Total Shortage": f"{report.get('executive_summary', {}).get('total_yarn_shortage_lbs', 0):,.0f} lbs",
                        "Actions Required": report.get("executive_summary", {}).get(
                            "immediate_action_items", 0
                        ),
                    },
                },
            ...
            "critical_yarns": report.get("yarn_shortages", {}).get(
                "critical_items", []
            )[:5],
        }
...
def supply_chain_analysis():
    ...
    try:
        ...
        dashboard_data = {
            "success": True,
            "summary": {
                "total_styles_analyzed": report["sales_forecast"].get(
                    "styles_forecasted", 0
                ),
                "forecasted_demand": report["sales_forecast"].get(
                    "total_forecasted_demand", 0
                ),
                "styles_at_risk": report["executive_summary"].get("styles_at_risk", 0),
                "yarn_shortage_lbs": report["executive_summary"].get(
                    "total_yarn_shortage_lbs", 0
                ),
                "critical_yarns": report["executive_summary"].get(
                    "critical_yarn_shortages", 0
                ),
                "immediate_actions": report["executive_summary"].get(
                    "immediate_action_items", 0
                ),
            },
            "flow_stages": [
                {
                    ...
                    "status": "complete",
                    "metrics": {
                        "Styles Analyzed": report["sales_forecast"].get(
                            "styles_forecasted", 0
                        ),
                        "Growth Styles": report["sales_forecast"].get(
                            "growth_styles", 0
                        ),
                        "Declining Styles": report["sales_forecast"].get(
                            "declining_styles", 0
                        ),
                    },
                },
                ...
                },
                {
                    ...
                    "status": "complete",
                    "metrics": {
                        "Critical Risk": report["inventory_risk"].get(
                            "critical_risk_styles", 0
                        ),
                        "High Risk": report["inventory_risk"].get(
                            "high_risk_styles", 0
                        ),
                ...
                },
                {
                    ...
                    "metrics": {
                        ...
                        "Yarn Types": report["yarn_requirements"].get(
                            "unique_yarns_needed", 0
                        ),
                    },
                },
                {
                    ...
                    "status": "complete",
                    "metrics": {
                        "Critical Shortages": report["executive_summary"].get(
                            "critical_yarn_shortages", 0
                        ),
                        "Total Shortage": f"{report['executive_summary'].get('total_yarn_shortage_lbs', 0):,.0f} lbs",
                        "Actions Required": report["executive_summary"].get(
                            "immediate_action_items", 0
                        ),
                    },
                },
...
def get_yarn_intelligence():
    ...
    try:
        ...
        if CACHE_MANAGER_AVAILABLE:
            ...
            if cached_result is not None and cached_result.get(
        ...
        if (
            not hasattr(analyzer, "raw_materials_data")
            or analyzer.raw_materials_data is None
        ):
        ...
        try:
            ...
            if DATA_CONSISTENCY_AVAILABLE:
                ...
                for _, yarn_row in df.iterrows():
                    shortage_info = DataConsistencyManager.calculate_yarn_shortage(
                        yarn_row
                    )
                    if shortage_info["has_shortage"]:
                        all_critical_yarns.append(
                            {"yarn_data": yarn_row, "shortage_info": shortage_info}
                        )

                # Sort by shortage amount and limit
                all_critical_yarns = sorted(
                    all_critical_yarns,
                ...
                if "planning_balance" in df.columns:
                    ...
                    "theoretical_balance" in df.columns
                    and "allocated" in df.columns
                    and "on_order" in df.columns
                ):
                    ...
                    df["Calculated_Planning_Balance"] = (
                        df["theoretical_balance"] + df["allocated"] + df["on_order"]
                    )
                elif (
                    "Theoretical Balance" in df.columns
                    and "Allocated" in df.columns
                    and "On Order" in df.columns
                ):
                    ...
                    df["Calculated_Planning_Balance"] = (
                        df["Theoretical Balance"] + df["Allocated"] + df["On Order"]
                    )
                else:
                ...
                try:
                    ...
                    reference_path = (
                        "/mnt/c/Users/psytz/Downloads/yarn_inventory (13).csv"
                    )
                    if os.path.exists(reference_path):
                        ...
                        with open(reference_path, "r", encoding="utf-8-sig") as f:
                            ...
                            for row in reader:
                                ...
                                planning_balance = (
                                    float(row["Planning Balance"].replace(",", ""))
                                    if row["Planning Balance"]
                                    else 0
                                )
                        ...
                        if yarn_id_col in df.columns:
                            for idx, row in df.iterrows():
                                ...
                                if yarn_id in reference_corrections:
                                    df.at[idx, "Calculated_Planning_Balance"] = (
                                        reference_corrections[yarn_id]
                                    )

                            print(
                                f"[CORRECTION] Applied Planning Balance corrections for {len(reference_corrections)} yarns from reference file"
                            )
                        else:
                            print(
                                "[CORRECTION] No Desc# column found for Planning Balance correction"
                            )
                    else:
                        print(
                            f"[CORRECTION] Reference file not found at {reference_path}"
                        )
                except Exception as e:
                    print(
                        f"[CORRECTION] Error applying Planning Balance corrections: {e}"
                    )
                ...
                if theoretical_col and theoretical_col in df.columns:
                    shortage_condition = (df["Calculated_Planning_Balance"] < 0) | (
                        df[theoretical_col] < 0
                    )
                else:
                    shortage_condition = df["Calculated_Planning_Balance"] < 0
                all_critical_yarns_df = (
                    df[shortage_condition]
                    .sort_values("Calculated_Planning_Balance")
                    .head(100)
                )
                    ...
                    all_critical_yarns.append(
                        {
                            ...
                            "shortage_info": {
                                "has_shortage": True,
                                "shortage_amount": abs(
                                    yarn_row.get("Calculated_Planning_Balance", 0)
                                ),
                                "planning_balance": yarn_row.get(
                                    "Calculated_Planning_Balance", 0
                                ),
                            },
                        }
                    )
            ...
            for yarn_entry in all_critical_yarns:
                ...
                if current_yarn_id is not None and str(current_yarn_id).lower() not in [
                    "nan",
                    "none",
                    "",
                ]:
                    ...
                    if desc and str(desc).strip():
                        ...
                        current_yarn_id = (
                            desc_parts[0] if desc_parts else str(desc)[:20]
                        )
                    else:
                ...
                if hasattr(analyzer, "bom_data") and analyzer.bom_data is not None:
                    ...
                    if not bom_df.empty:
                        ...
                        yarn_col = (
                            "Desc#"
                            if "Desc#" in bom_df.columns
                            else "Yarn_ID" if "Yarn_ID" in bom_df.columns else None
                        )
                        if yarn_col:
                            # Convert both to numeric for comparison to handle type mismatches
                            try:
                                yarn_id_numeric = float(current_yarn_id)
                                style_matches = bom_df[
                                    bom_df[yarn_col] == yarn_id_numeric
                                ]
                            except (ValueError, TypeError):
                                # If conversion fails, try string comparison
                                style_matches = bom_df[
                                    bom_df[yarn_col].astype(str) == str(current_yarn_id)
                                ]
                            if (
                                not style_matches.empty
                                and "Style#" in style_matches.columns
                            ):
                                affected_styles = list(style_matches["Style#"].unique())

                                # Now check if these styles have active knit orders
                                if (
                                    hasattr(analyzer, "knit_orders_data")
                                    and analyzer.knit_orders_data is not None
                                ):
                                    knit_df = analyzer.knit_orders_data
                                    if (
                                        not knit_df.empty
                                        and "Style#" in knit_df.columns
                                    ):
                                        # Find knit orders for these styles
                                        active_orders = knit_df[
                                            knit_df["Style#"].isin(affected_styles)
                                        ]
                                        affected_orders_count = len(active_orders)
                                        if "Order#" in active_orders.columns:
                                            affected_orders_list = list(
                                                active_orders["Order#"].unique()
                                            )[:5]
                                        elif "Knit Order" in active_orders.columns:
                                            affected_orders_list = list(
                                                active_orders["Knit Order"].unique()
                                            )[:5]
                                else:
                                    ...
                                    affected_orders_list = affected_styles[
                                        :5
                                    ]  # Show styles as proxy for orders
                ...
                if True:  # Changed from: if affected_orders_count > 0:
                    if DATA_CONSISTENCY_AVAILABLE:
                        ...
                        yarn_shortage_data = {
                            ...
                            "supplier": str(
                                yarn.get("supplier", yarn.get("Supplier", ""))
                            )[:30],
                        ...
                        consumed = abs(
                            float(yarn.get("consumed", yarn.get("Consumed", 0)))
                        )
                        if consumed > 0:
                        ...
                        weeks_of_supply = (
                            yarn_shortage_data["theoretical_balance"] / weekly_demand
                            if weekly_demand > 0
                            and yarn_shortage_data["theoretical_balance"] > 0
                            else 0
                        )
                        ...
                        yarn_shortage_data.update(
                            {
                                ...
                                "criticality_score": yarn_shortage_data[
                                    "urgency_score"
                                ],  # Use consistent score
                            }
                        )
                        ...
                        allocated = float(
                            yarn.get("allocated", yarn.get("Allocated", 0))
                        )
                        ...
                        weeks_of_supply = (
                            theoretical_bal / weekly_demand
                            if weekly_demand > 0 and theoretical_bal > 0
                            else 0
                        )
                        ...
                        yarn_shortage_data = {
                            ...
                            "description": str(
                                yarn.get("description", yarn.get("Description", ""))
                            )[:50],
                            "supplier": str(
                                yarn.get("supplier", yarn.get("Supplier", ""))
                            )[:30],
                            ...
                            "weeks_of_supply": float(weeks_of_supply),
                            "criticality_score": (
                                min(100, abs(float(planning_bal)) / 100)
                                if planning_bal < 0
                                else 0
                            ),
                            "risk_level": (
                                "CRITICAL"
                                if planning_bal < -1000
                                else (
                                    "HIGH"
                                    if planning_bal < -100
                                    else "MEDIUM" if planning_bal < 0 else "LOW"
                                )
                            ),
                        }
        ...
        if shortage_data:
            print(
                f"DEBUG: First yarn in shortage_data: {shortage_data[0].get('yarn_id', 'NO_ID')}"
            )
        ...
        intelligence = {
            "criticality_analysis": {
                ...
                "summary": {
                    "total_yarns": (
                        len(analyzer.raw_materials_data)
                        if hasattr(analyzer, "raw_materials_data")
                        and analyzer.raw_materials_data is not None
                        else 0
                    ),
                    "critical_count": sum(
                        1 for y in shortage_data if y["risk_level"] == "CRITICAL"
                    ),
                    "high_count": sum(
                        1 for y in shortage_data if y["risk_level"] == "HIGH"
                    ),
        ...
        if view == "summary":
            intelligence = {
                ...
                "critical_yarns": [
                    y
                    for y in intelligence["criticality_analysis"]["yarns"]
                    if y.get("criticality") == "CRITICAL"
                ][:5],
        ...
        if analysis_type == "shortage":
            ...
            shortage_yarns = [
                y
                for y in intelligence.get("criticality_analysis", {}).get("yarns", [])
            ...
            for yarn in shortage_yarns:
                shortage_lbs = (
                    abs(yarn.get("shortage", 0))
                    if yarn.get("shortage", 0) < 0
                    else yarn.get("shortage_pounds", 0)
                )
                ...
                if production_impact > 0:
                    urgency += min(
                        production_impact / 1000, 10
                    )  # Max 10 points for production impact
            ...
            if sort_by == "urgency":
                ...
                shortage_yarns.sort(
                    key=lambda x: abs(x.get("shortage", 0)), reverse=True
                )
            elif sort_by == "impact":
                shortage_yarns.sort(
                    key=lambda x: x.get("financial_impact", 0), reverse=True
                )
            ...
            intelligence["shortage_analysis"] = {
                ...
                "total_shortage_pounds": sum(
                    (
                        abs(y.get("shortage", 0))
                        if y.get("shortage", 0) < 0
                        else y.get("shortage_pounds", 0)
                    )
                    for y in shortage_yarns
                ),
                "total_financial_impact": sum(
                    y.get("financial_impact", 0) for y in shortage_yarns
                ),
                "critical_count": len(
                    [y for y in shortage_yarns if y.get("risk_level") == "CRITICAL"]
                ),
                "high_count": len(
                    [y for y in shortage_yarns if y.get("risk_level") == "HIGH"]
                ),
                "showing": len(top_shortages),
                "total_identified": len(shortage_yarns),
                "avg_urgency_score": (
                    round(
                        sum(y.get("urgency_score", 0) for y in shortage_yarns)
                        / len(shortage_yarns),
                        1,
                    )
                    if shortage_yarns
            ...
            intelligence["requirements_analysis"] = {
                "total_required": sum(
                    y.get("allocated", 0)
                    for y in intelligence.get("criticality_analysis", {}).get(
                        "yarns", []
                    )
                ),
                "total_available": sum(
        ...
        if include_forecast:
            ...
            if (
                "criticality_analysis" in intelligence
                and "yarns" in intelligence["criticality_analysis"]
            ):
                        ...
                        if net_shortage > 0:
                            ...
                            if yarn.get("available", 0) > 0 and weekly_demand > 0:
                                days_until_shortage = int(
                                    (yarn["available"] / weekly_demand) * 7
                                )
                            ...
                            forecasted_shortages.append(
                                {
                                    ...
                                    "forecasted_requirement": round(
                                        forecasted_requirement, 2
                                    ),
                                    "current_inventory": round(
                                        yarn.get("theoretical_balance", 0), 2
                                    ),  # Use theoretical_balance for current inventory
                                    "planning_balance": round(planning_balance, 2),
                                    "net_shortage": round(
                                        -net_position if net_position < 0 else 0, 2
                                    ),  # Show shortage as positive only when there IS a shortage
                                    "net_position": round(
                                        net_position, 2
                                    ),  # Add for clarity
                                    "urgency": urgency,
            ...
            intelligence["forecast"] = {
                ...
                "critical_count": sum(
                    1 for s in forecasted_shortages if s["urgency"] == "CRITICAL"
                ),
                "total_shortage_lbs": sum(
                    s["net_shortage"] for s in forecasted_shortages
                ),
                "confidence": 0.85,
            }
        ...
        if ai_enhanced:
            intelligence["ai_insights"] = {
                "recommendations": [
                    "Increase safety stock for critical yarns",
                    "Consider substitutions for shortage items",
                ],
                "risk_score": 0.7,
        ...
        if yarn_id:
            print(
                f"WARNING: yarn_id parameter received: '{yarn_id}' - this may be unintended"
            )
        ...
        if CACHE_MANAGER_AVAILABLE and len(shortage_data) > 0:
            ...
            cache_manager.set(
                cache_key,
                intelligence,
                ttl=CACHE_TTL.get("yarn_intelligence", 300),
                namespace="api",
            )
...
def yarn_shortage_timeline():
    ...
    try:
        ...
        if yarn_id:
            ...
            processed_count = 0
            for yarn_id in list(analyzer.yarn_weekly_receipts.keys())[
                :20
            ...
            result["summary"] = {
                ...
                "shortage_yarns": sum(
                    1 for y in result["yarns"].values() if y.get("has_shortage", False)
                ),
            }
...
def po_delivery_schedule():
    ...
    try:
        ...
        if not analyzer.time_phased_enabled:
            return jsonify({"error": "Time-phased planning not enabled"}), 501
        ...
        if yarn_id:
            if str(yarn_id) in analyzer.yarn_weekly_receipts:
                ...
                return (
                    jsonify({"error": f"No PO delivery data found for yarn {yarn_id}"}),
                    404,
                )
        else:
        ...
        result["summary"] = {
            ...
            "total_on_order_amount": (
                sum(d.get("total_on_order", 0) for d in result["deliveries"].values())
                if include_totals
                else None
            ),
        }
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                in_production = analyzer.knit_orders.get(
                    "In_Production", pd.Series(False, index=analyzer.knit_orders.index)
                )
                ...
                # Get unique styles
                active_styles = (
                    set(active_orders["Style"].dropna().unique())
                    if "Style" in active_orders.columns
                    else set()
                )
            except Exception as e:
            ...
            if (
                hasattr(analyzer, "bom_data")
                and analyzer.bom_data is not None
                and not analyzer.bom_data.empty
            ):
                # Filter BOM for active styles
                bom_for_active = (
                    analyzer.bom_data[analyzer.bom_data["Style#"].isin(active_styles)]
                    if "Style#" in analyzer.bom_data.columns
                    else pd.DataFrame()
                )
                if not bom_for_active.empty and "YarnID" in bom_for_active.columns:
                    active_production_yarns = set(
                        bom_for_active["YarnID"].dropna().astype(str).unique()
                    )
                ...
                for _, row in df.iterrows():
                    ...
                    if has_negative:
                        yarn_demand_data.append(
                            {"yarn_id": yarn_id, "has_negative_balance": True}
                        )
        except Exception as e:
            ...
            if "error" not in yarn_analysis:
                ...
                try:
                    if hasattr(analyzer, "yarn_data") and not analyzer.yarn_data.empty:
                        yarn_row = analyzer.yarn_data[
                            analyzer.yarn_data["Desc#"] == yarn_id
                        ]
                        if not yarn_row.empty:
                            yarn_analysis["description"] = yarn_row.iloc[0].get(
                                "Description", ""
                            )
                            yarn_analysis["yarn_id"] = yarn_id

                            # Add current balance from inventory data
                            yarn_analysis["current_balance"] = float(
                                yarn_row.iloc[0].get("Theoretical Balance", 0)
                            )

                            # Read Planning Balance directly from reference file (no calculations)
                            yarn_analysis["planning_balance"] = (
                                analyzer.get_planning_balance_from_reference(yarn_id)
                                if hasattr(
                                    analyzer, "get_planning_balance_from_reference"
                                )
                                else 0
                            )
                except:
                ...
                if yarn_analysis.get("has_shortage"):
                    ...
                    elif "Week 37" in str(first_shortage) or "W37" in str(
                        first_shortage
                    ):
                        criticality_score += 400  # Next week
                    elif "Week 38" in str(first_shortage) or "W38" in str(
                        first_shortage
                    ):
                ...
                if yarn_analysis.get("expedite_recommendations"):
                    criticality_score += 100 * len(
                        yarn_analysis["expedite_recommendations"]
                    )
                ...
                if current_bal < 0:
                    criticality_score += (
                        abs(current_bal) / 1000
                    )  # Scale down to not overpower
                ...
                filter_nonproduction = (
                    os.environ.get("FILTER_NONPRODUCTION_YARNS", "true").lower()
                    == "true"
                )
                ...
                if not filter_nonproduction:
                    ...
                    if not include_yarn:
                        if (
                            yarn_analysis.get("current_balance", 0) < 0
                            or yarn_analysis.get("planning_balance", 0) < 0
                            or yarn_analysis.get("theoretical_balance", 0) < 0
                        ):
                            include_yarn = True
        ...
        for yarn_id in yarns_with_negative:
            if yarn_id not in processed_yarn_ids:
                ...
                if hasattr(analyzer, "yarn_data") and not analyzer.yarn_data.empty:
                    yarn_row = analyzer.yarn_data[
                        analyzer.yarn_data["Desc#"].astype(str) == str(yarn_id)
                    ]
                    if not yarn_row.empty:
                        yarn_analysis["description"] = yarn_row.iloc[0].get(
                            "Description", yarn_analysis["description"]
                        )
                        yarn_analysis["current_balance"] = float(
                            yarn_row.iloc[0].get("Theoretical Balance", 0)
                        )
                        yarn_analysis["planning_balance"] = (
                            analyzer.get_planning_balance_from_reference(yarn_id)
                            if hasattr(analyzer, "get_planning_balance_from_reference")
                            else 0
                        )
        ...
        all_yarn_analysis.sort(
            key=lambda x: x.get("criticality_score", 0), reverse=True
        )
...
def time_phased_planning():
    ...
    try:
        ...
        result["summary"] = {
            ...
            "yarns_with_shortages": sum(
                1 for p in all_planning if p.get("has_shortage", False)
            ),
    ...
    try:
        ...
        if include_timing and analyzer.time_phased_enabled:
            ...
            for yarn in base_data.get("criticality_analysis", {}).get("yarns", []):
                ...
                if yarn_id and str(yarn_id) in analyzer.yarn_weekly_receipts:
                    ...
                    if "error" not in time_phased:
                        ...
                        yarn["shortage_timeline"] = time_phased.get(
                            "shortage_periods", []
                        )
            ...
            base_data["time_phased_summary"] = {
                "enabled": True,
                "yarns_with_timing": len(
                    [y for y in enhanced_yarns if y.get("time_phased_enabled")]
                ),
            }
        else:
            base_data["time_phased_summary"] = {
...
def yarn_scheduler_health():
    ...
    health_data = {
        "timestamp": datetime.now().isoformat(),
        "scheduler_enabled": os.environ.get("ENABLE_YARN_SCHEDULER", "false").lower()
        == "true",
    }
    ...
    try:
        ...
        if hasattr(analyzer, "using_enhanced_downloader"):
            health_data["using_enhanced_downloader"] = (
                analyzer.using_enhanced_downloader
            )
        else:
            health_data["using_enhanced_downloader"] = False

        # Get health metrics from enhanced downloader if available
        if hasattr(analyzer, "yarn_downloader") and hasattr(
            analyzer.yarn_downloader, "get_health_status"
        ):
        ...
        health_data["time_phased_enabled"] = (
            analyzer.time_phased_enabled
            if hasattr(analyzer, "time_phased_enabled")
            else False
        )
        ...
        health_data["session_prefix"] = (
            current_session[:10] + "..." if current_session else None
        )
        ...
        if "Session cookie invalid" in str(health_issues):
            health_data["recommendations"].append(
                "Update EFAB_SESSION environment variable with fresh cookie from browser"
            )
        if "Report older than 24 hours" in str(health_issues):
            health_data["recommendations"].append(
                "Trigger manual refresh via POST /api/manual-yarn-refresh"
            )
        if "Scheduler not enabled" in str(health_issues):
            health_data["recommendations"].append(
                "Set ENABLE_YARN_SCHEDULER=true and restart server"
            )
...
def get_inventory_intelligence_enhanced():
    ...
    try:
        ...
        if (
            ...
            summary_stats["high_count"] = len(
                df[(df["Planning_Balance"] >= -1000) & (df["Planning_Balance"] < -100)]
            )
            summary_stats["medium_count"] = len(
                df[(df["Planning_Balance"] >= -100) & (df["Planning_Balance"] < 0)]
            )
            ...
            summary_stats["total_shortage_lbs"] = abs(
                shortages["Planning_Balance"].sum()
            )
        ...
        base_data = {
            ...
            "critical_alerts": [
                {
                    "type": (
                        "YARN_SHORTAGE"
                        if summary_stats["yarns_with_shortage"] > 0
                        else "SYSTEM_STATUS"
                    ),
                    "severity": (
                        "HIGH" if summary_stats["yarns_with_shortage"] > 5 else "INFO"
                    ),
                    "message": (
                        f"{summary_stats['yarns_with_shortage']} yarns with shortages totaling {summary_stats['total_shortage_lbs']:.2f} lbs"
                        if summary_stats["yarns_with_shortage"] > 0
                        else "All yarns have adequate inventory"
                    ),
                    "action": (
                        "Review critical yarns and place orders"
                        if summary_stats["yarns_with_shortage"] > 0
            ...
            "summary": {
                ...
                "overall_health": (
                    "CRITICAL"
                    if summary_stats["critical_count"] > 50
                    else "WARNING" if summary_stats["critical_count"] > 20 else "GOOD"
                ),
        ...
        if view == "summary":
            ...
            if (
                ...
                df = analyzer.raw_materials_data
                response_data["detailed_inventory"] = (
                    df.head(100).to_dict("records") if len(df) > 0 else []
                )
        else:
            # Default 'full' view
        ...
        if analysis == "shortage":
            ...
            if (
                ...
                high_risk = df[
                    (df["Planning_Balance"] >= -1000) & (df["Planning_Balance"] < -100)
                ].copy()
                ...
                low_stock = df[
                    (df["Planning_Balance"] >= 0) & (df["Planning_Balance"] < 100)
                ].copy()
            ...
            response_data["inventory_alerts"] = {
                ...
                "summary": {
                    ...
                    "total_financial_impact": sum(
                        a.get("financial_impact", 0) for a in alerts
                    ),
                    "avg_urgency_score": (
                        round(sum(a["urgency_score"] for a in alerts) / len(alerts), 1)
                        if alerts
                        else 0
                    ),
                    "categories": {
                        "yarn_shortage": len(
                            [a for a in alerts if "shortage" in a["type"]]
                        ),
                        "low_stock": len(
                            [a for a in alerts if a["type"] == "low_stock_warning"]
                        ),
                    },
                },
            ...
            response_data["optimization_suggestions"] = [
                {
                    "type": "ORDERING",
                    "priority": "HIGH",
                    "action": "Review critical yarn shortages",
                },
                {
                    "type": "SAFETY_STOCK",
                    "priority": "MEDIUM",
                    "action": "Adjust safety stock levels for high-risk yarns",
                },
            ]
...
def get_real_time_inventory_dashboard():
    ...
    try:
        ...
        f01_file = (
            DATA_PATH / "eFab_Inventory_F01_20250810.xlsx"
        )  # Updated to use file from /5
        ...
        if f01_file.exists():
            ...
            if qty_col:
                f01_data[qty_col] = pd.to_numeric(
                    f01_data[qty_col], errors="coerce"
                ).fillna(0)
...
def get_safety_stock_calculations():
    ...
    try:
        ...
        if yarn_file.exists():
            ...
            for idx, row in yarn_data.iterrows():
                ...
                if consumed > 0:
                    ...
                    required_safety_stock = (
                        daily_consumption * lead_time_days * safety_stock_multiplier
                    )
                        ...
                        safety_stock_alerts.append(
                            {
                                ...
                                "days_of_stock": (
                                    float(current_stock / daily_consumption)
                                    if daily_consumption > 0
                                    else 0
                                ),
                                "on_order": float(on_order),
                                ...
                                "urgency": (
                                    ...
                                    if current_stock < 0
                                        ...
                                        "HIGH"
                                        if current_stock < (required_safety_stock * 0.5)
                                        else "MEDIUM"
                                    )
                                ),
                            }
            ...
            safety_stock_alerts.sort(
                key=lambda x: (x["urgency"] == "CRITICAL", x["shortage"]), reverse=True
            )
            ...
            summary = {
                ...
                "critical_items": len(
                    [a for a in safety_stock_alerts if a["urgency"] == "CRITICAL"]
                ),
                "high_priority_items": len(
                    [a for a in safety_stock_alerts if a["urgency"] == "HIGH"]
                ),
...
def get_multi_stage_inventory():
    ...
    try:
        ...
        stages = {
            "G00": {
                "file": "eFab_Inventory_G00_20250804.xlsx",
                "name": "Raw Materials",
            },
            "G02": {
                "file": "eFab_Inventory_G02_20250804.xlsx",
                "name": "Work in Progress",
            },
            "I01": {
                "file": "eFab_Inventory_I01_20250810.xlsx",
                "name": "Intermediate Goods",
            },
            "F01": {
                "file": "eFab_Inventory_F01_20250810.xlsx",
                "name": "Finished Goods",
            },
            "P01": {
                "file": "eFab_Inventory_P01_20250810.xlsx",
        ...
        for i in range(len(stage_list) - 1):
            ...
            if current in stage_data and next_stage in stage_data:
                if (
                    "total_quantity" in stage_data[current]
                    and "total_quantity" in stage_data[next_stage]
                ):
        ...
        summary = {
            ...
            "active_stages": len(
                [s for s in stage_data.values() if s.get("status") == "Active"]
            ),
            "total_items_across_stages": total_items,
            "total_quantity_across_stages": total_quantity,
            "pipeline_health": (
                "Healthy"
                if len([s for s in stage_data.values() if s.get("status") == "Active"])
                >= 4
...
def get_sales_data():
    ...
    try:
        ...
        summary = {
            ...
            "fill_rate": (
                float(
                    sales_df["Picked/Shipped"].sum() / sales_df["Ordered"].sum() * 100
                )
                if sales_df["Ordered"].sum() > 0
                else 0
            ),
        }
...
def get_ai_yarn_forecast(yarn_id):
    ...
    try:
        ...
        if hasattr(analyzer, "yarn_data") and analyzer.yarn_data is not None:
            yarn_history = analyzer.yarn_data[
                analyzer.yarn_data.get("Desc#", "") == yarn_id
            ]
            if not yarn_history.empty:
                ...
                if week_cols:
                    ...
                    for col in week_cols:
                        val = (
                            yarn_history[col].values[0] if len(yarn_history) > 0 else 0
                        )
                ...
                if demand_values:
                    ...
                    if len(daily_demand) >= 90:
                        ...
                        demand_data = daily_demand + [avg_demand] * (
                            90 - len(daily_demand)
                        )
                else:
                    # Calculate from consumption and inventory levels
                    consumed = (
                        yarn_history.get("Consumed", pd.Series([0])).values[0]
                        if "Consumed" in yarn_history.columns
                        else 0
                    )
                    allocated = (
                        yarn_history.get("Allocated", pd.Series([0])).values[0]
                        if "Allocated" in yarn_history.columns
                        else 0
                    )
                    ...
                    demand_data = [
                        daily_demand_est * np.random.uniform(0.8, 1.2)
                        for _ in range(90)
                    ]
        ...
        try:
            ...
            if ai_optimizer and hasattr(ai_optimizer, "optimizer"):
                ...
                optimization_result = ai_optimizer.get_optimization_recommendations(
                    ...
                    sales_history=(
                        historical_data
                        if not historical_data.empty
                        else pd.DataFrame({"date": [datetime.now()], "quantity": [100]})
                    ),
                )
                ...
                if optimization_result.get(
                    "status"
                ) == "success" and optimization_result.get("recommendations"):
                    rec = (
                        optimization_result["recommendations"][0]
                    ...
                    base_demand = (
                        historical_data["demand"].mean()
                        if not historical_data.empty
                        else 100
                    )
                    forecast_values = [
                        base_demand * (1 + np.random.normal(0, 0.1)) for _ in range(30)
                    ]
            ...
            base_demand = (
                historical_data["demand"].mean() if not historical_data.empty else 100
            )
            forecast_values = [
                base_demand * (1 + np.random.normal(0, 0.1)) for _ in range(30)
            ]
        ...
        if not demand_history and yarn_id:
            if hasattr(analyzer, "yarn_data") and analyzer.yarn_data is not None:
                ...
                if not yarn_row.empty:
                    ...
                    demand_history = np.random.normal(
                        abs(balance) / 30, abs(balance) / 100, 90
                    ).tolist()
        ...
        try:
            ...
            if demand_history:
                demand_df = pd.DataFrame(
                    {
                        "date": pd.date_range(
                            end=datetime.now(), periods=len(demand_history), freq="D"
                        ),
            ...
            current_safety_stock = (
                demand_df["quantity"].mean() * 7
            )  # 7 days of average demand
            ...
            result = {
                ...
                "reduction_percentage": (
                    (
                        (current_safety_stock - result["recommended_safety_stock"])
                        / current_safety_stock
                        * 100
                    )
                    if current_safety_stock > 0
                    else 0
                ),
            ...
            result = {
                ...
                "service_level": service_level,
                "factors": {"demand_variability": 0.15, "adjustment_factor": 0.77},
                "fallback": True,
                "error": str(opt_error),
            }
...
def get_ai_reorder_recommendation():
    ...
    try:
        ...
        try:
            ...
            reorder_point = inventory_state["current_stock"] + (
                action * 50
            )  # Scale action

            # Calculate expected costs
            expected_holding_cost = inventory_state["holding_cost"] * reorder_point
            expected_stockout_cost = inventory_state["stockout_cost"] * max(
                0, inventory_state["demand_rate"] - reorder_point
            )
            ...
            recommendation = {
                ...
                "expected_holding_cost": inventory_state["holding_cost"]
                * inventory_state["current_stock"],
                "expected_stockout_cost": inventory_state["stockout_cost"] * 0.1,
                "total_expected_cost": inventory_state["holding_cost"]
                * inventory_state["current_stock"]
                + inventory_state["stockout_cost"] * 0.1,
                "confidence": 0.6,
                "state": "fallback",
                "error": str(rl_error),
            }
    ...
    try:
        ...
        if hasattr(analyzer, "yarn_data") and analyzer.yarn_data is not None:
            ...
            if not shortage_yarns.empty and balance_col:
                ...
                top_shortages = shortage_yarns.nsmallest(
                    5, balance_col, keep="all"
                )  # nsmallest for negative values
                    ...
                    intelligence["top_opportunities"].append(
                        {
                            ...
                            "ai_optimized_shortage": shortage
                            * 0.77,  # After 23% reduction
        ...
        intelligence["system_confidence"] = (
            len(intelligence["ai_models_available"]) / 6.0
        )
...
def get_ensemble_forecast():
    ...
    try:
        ...
        try:
                    ...
                    [
                        {
                            ...
                            "quantity": (
                                df["demand"].iloc[-1]
                                if not df.empty and "demand" in df.columns
                                else 100
                            ),
                        }
                    ]
            ...
            logger.warning(
                f"Ensemble forecast failed: {str(ensemble_error)}, using fallback"
            )
        ...
        ml_logger.error(
            f"Unexpected error in ensemble forecast: {str(e)}\n{traceback.format_exc()}"
        )
        return jsonify(_get_fallback_ensemble_forecast(data)), 200
...
def _get_fallback_ensemble_forecast(data):
    ...
    try:
        ...
        base_value = 100
        forecast_values = [
            base_value * (1 + np.random.normal(0, 0.1)) for _ in range(horizon)
        ]
...
def get_pipeline_yarn_shortages():
    ...
    try:
        ...
        if "forecasts" in forecast_results:
            for product_id, forecast in forecast_results["forecasts"].items():
                ...
                for horizon, qty in best_forecast.items():
                    yarn_needed = (
                        qty * 0.5
                    )  # Assuming 0.5 kg yarn per unit (placeholder)
        ...
        response = {
            "timestamp": datetime.now().isoformat(),
            "total_shortages": len(
                [s for s in shortages.values() if s["shortage"] > 0]
            ),
...
def get_pipeline_inventory_risks():
    ...
    try:
        ...
        response = {
            "timestamp": datetime.now().isoformat(),
            "total_at_risk": len(
                [r for r in risks.values() if r["risk_level"] in ["CRITICAL", "HIGH"]]
            ),
            "inventory_risks": risks,
            "summary": {
                "critical": len(
                    [r for r in risks.values() if r["risk_level"] == "CRITICAL"]
                ),
                "high": len([r for r in risks.values() if r["risk_level"] == "HIGH"]),
                "medium": len(
                    [r for r in risks.values() if r["risk_level"] == "MEDIUM"]
                ),
        ...
        for action in action_items:
            ...
            if action["type"] == "PROCUREMENT" and "yarn_shortages" in results:
                ...
                enhanced_action["details"] = {
                    ...
                    "total_shortage": results["yarn_shortages"]["summary"][
                ...
                enhanced_action["details"] = {
                    ...
                    "total_at_risk": results["inventory_analysis"]["summary"][
                        "critical_count"
                    ],
                }
    ...
    try:
        ...
        dashboard_data = {
            ...
            "forecast": {
                ...
                "total_90day": results.get("forecast", {})
                .get("summary", {})
                .get("total_forecast", 0),
            },
            "inventory": {
                "total_items": results.get("inventory_analysis", {})
                .get("summary", {})
                .get("total_items", 0),
                "critical_items": results.get("inventory_analysis", {})
                .get("summary", {})
                .get("critical_count", 0),
                "high_risk_items": results.get("inventory_analysis", {})
                .get("summary", {})
                .get("high_risk_count", 0),
                "top_risks": results.get("inventory_analysis", {}).get("analysis", [])[
                    :5
                ],
            },
            "yarn": {
                "total_required": sum(
                    y["quantity_needed"] for y in results.get("yarn_requirements", [])
                ),
                "shortage_count": results.get("yarn_shortages", {})
                .get("summary", {})
                .get("yarns_needing_order", 0),
                "critical_shortages": results.get("yarn_shortages", {})
                .get("summary", {})
                .get("critical_count", 0),
                "total_shortage": results.get("yarn_shortages", {})
                .get("summary", {})
                .get("total_shortage_quantity", 0),
                "top_shortages": [
                    s
                    for s in results.get("yarn_shortages", {}).get("shortages", [])
        ...
        return jsonify(
            {
                ...
                "summary": {
                    ...
                    "best_model": (
                        list(results.get("model_rankings", {}).keys())[0]
...
def get_forecast_accuracy():
    ...
    try:
        ...
        if not result_files:
            return (
                jsonify(
                    {"error": "No backtest results available. Run backtesting first."}
                ),
                404,
            )
        ...
        best_model = (
            min(model_rankings.items(), key=lambda x: x[1].get("avg_mape", 100))
            if model_rankings
            else None
        )
        ...
        return jsonify(
            {
                ...
                "forecast_confidence": (
                    max(accuracy_summary.values(), key=lambda x: x["accuracy"])[
                        "accuracy"
                    ]
                    if accuracy_summary
                    else 0
...
def get_ml_forecast_report():
    ...
    try:
        ...
        report_path = os.path.join(
            os.path.dirname(__file__), "beverly_forecast_report.json"
        )
...
def get_ml_forecast_detailed():
    ...
    try:
        ...
        if (
            ...
            if ML_FIX_AVAILABLE and ml_fix:
                detailed_forecast["forecast_details"] = ml_fix.generate_style_forecasts(
                    analyzer.sales_data, top_n=20
                )
                detailed_forecast["summary"]["total_styles"] = len(
                    detailed_forecast["forecast_details"]
                )
                detailed_forecast["summary"]["total_forecasted_qty"] = sum(
                    item.get("forecast_90_days", 0)
                    for item in detailed_forecast["forecast_details"]
                )
            else:
                ...
                if style_col and qty_col:
                    ...
                    style_groups = analyzer.sales_data.groupby(style_col)[qty_col].agg(
                        ["sum", "mean", "count"]
                    )
                    style_groups = style_groups.sort_values(
                        "sum", ascending=False
                    ).head(20)
                        ...
                        detailed_forecast["forecast_details"].append(
                            {
                                ...
                                "historical_total": (
                                    float(row["sum"]) if not pd.isna(row["sum"]) else 0
                                ),
                                "order_count": (
                                    int(row["count"])
                    ...
                    detailed_forecast["summary"]["total_forecasted_qty"] = sum(
                        item["forecast_90_days"]
                        for item in detailed_forecast["forecast_details"]
                    )

        # If no data, return empty but valid forecast
        if not detailed_forecast["forecast_details"]:
            detailed_forecast["message"] = (
                "No historical data available for detailed forecasting"
            )
            detailed_forecast["forecast_details"] = [
                {
                    "style": "SAMPLE",
        ...
        if detail_level == "summary":
            ...
            detailed_forecast = {
                ...
                "generated_at": detailed_forecast["generated_at"],
                "metrics": {
                    "total_styles": detailed_forecast.get("summary", {}).get(
                        "total_styles", 0
                    ),
                    "total_forecasted_qty": detailed_forecast.get("summary", {}).get(
                        "total_forecasted_qty", 0
                    ),
                    "average_confidence": detailed_forecast.get("summary", {}).get(
...
def get_ml_production_recommendations():
    ...
    try:
        ...
        return (
            jsonify({"status": "error", "message": str(e), "recommendations": []}),
            500,
        )
    ...
    try:
        ...
        if (
            ...
            if planning_col:
                ...
                shortage_yarns = (
                    df[df[planning_col] < 0].sort_values(planning_col).head(10)
                )
                ...
                for idx, yarn in shortage_yarns.iterrows():
                    ...
                    desc_col = (
                        "description" if "description" in df.columns else "Description"
                    )
                    supplier_col = (
                        "supplier" if "supplier" in df.columns else "Supplier"
                    )

                    potential_substitutes = df[
                        (df[planning_col] > 500)
                        & (
                            df[desc_col].str.contains(
                    ...
                    for _, sub_yarn in potential_substitutes.iterrows():
                        substitutes.append(
                            {
                                "yarn_id": str(
                                    sub_yarn.get("desc_num", sub_yarn.get("Desc#", ""))
                                ),
                                "description": str(
                                    sub_yarn.get(
                                        "description", sub_yarn.get("Description", "")
                                    )
                                )[:80],
                        ...
                        substitution_data.append(
                            {
                                ...
                                "coverage_percent": (
                                    min(100, (total_available / shortage) * 100)
                                    if shortage > 0
                                    else 0
                                ),
                            }
        ...
        return jsonify(
            {
                ...
                "summary": {
                    ...
                    "total_substitutes": sum(
                        opp["substitute_count"] for opp in substitution_data
                    ),
                    "total_shortage": sum(
                        opp["shortage_qty"] for opp in substitution_data
                    ),
                    "total_coverage": sum(
                        opp["coverage_qty"] for opp in substitution_data
                    ),
                    "average_coverage": sum(
                        opp["coverage_percent"] for opp in substitution_data
                    )
                    / max(1, len(substitution_data)),
                },
                "timestamp": datetime.now().isoformat(),
            }
        )
        ...
        return (
            jsonify(
                {"status": "error", "error": str(e), "substitution_opportunities": []}
            ),
            500,
        )
    ...
    try:
        ...
        if not trained_model_file.exists():
            ...
            trained_model_file = Path(
                "/mnt/d/Agent-MCP-1-ddd/trained_yarn_substitutions.json"
            )
                    ...
                    for yarn_id in trained_substitutions.keys():
                        ...
                        if yarn_id in trained_substitutions:
                            avg_compatibility = sum(
                                ...
                                for s in trained_substitutions[yarn_id].get(
                                    "substitutes", []
                                )
                            )
                            if trained_substitutions[yarn_id].get("substitutes"):
                                avg_compatibility /= len(
                                    trained_substitutions[yarn_id].get(
                                        "substitutes", []
                                    )
                                )
                                backtest_results[yarn_id][
                                ...
                                backtest_results[yarn_id]["historical_success_rate"] = (
                                    min(0.95, avg_compatibility * 1.1)
                                )
            except Exception as e:
        ...
        for yarn_id, substitution_data in trained_substitutions.items():
            ...
            for sub in substitution_data.get("substitutes", []):
                ...
                substitute_info = {
                    ...
                    "compatibility": sub.get(
                        "compatibility",
                        "HIGH" if sub.get("compatibility_score", 0) > 0.7 else "MEDIUM",
                    ),
                    "compatibility_score": sub.get("compatibility_score", 0),
                    "material_match": sub.get("material_match", False),
                    # Add backtesting results
                    "backtest_confidence": backtest_data.get(
                        "confidence_score",
                        sub.get(
                            "backtest_confidence", sub.get("compatibility_score", 0)
                        ),
                    ),
                    "historical_success_rate": backtest_data.get(
                        "historical_success_rate", sub.get("success_rate", 85)
                    ),
                    "tested_instances": backtest_data.get(
                        "tested_instances", sub.get("tested_instances", 0)
                    ),
                }
                ...
                substitution_opportunities.append(
                    {
                        ...
                        "coverage_percent": float(
                            substitution_data["coverage_percent"]
                        ),
        ...
        high_coverage_count = len(
            [opp for opp in substitution_opportunities if opp["coverage_percent"] >= 80]
        )
        perfect_material_matches = len(
            [
                opp
        ...
        summary = {
            ...
            "total_substitutes": sum(
                opp["substitute_count"] for opp in substitution_opportunities
            ),
            "total_shortage": total_shortage,
            "total_coverage": total_coverage,
            "coverage_percentage": (
                (total_coverage / total_shortage * 100) if total_shortage > 0 else 0
            ),
            "high_coverage_count": high_coverage_count,
            "perfect_material_matches": perfect_material_matches,
            "average_coverage": (
                sum(opp["coverage_percent"] for opp in substitution_opportunities)
                / len(substitution_opportunities)
...
def get_yarn_forecast_shortages():
    ...
    try:
        ...
        if (
            ...
            for _, yarn_row in yarn_df.iterrows():
                ...
                if planning_balance < 0:
                    shortage_yarns.append(
                        {
                            "yarn_id": yarn_row.get(
                                "desc_num", yarn_row.get("Desc#", "")
                            ),
                            "description": yarn_row.get(
                                "description", yarn_row.get("Description", "")
                            ),
                            "planning_balance": planning_balance,
                            "theoretical": yarn_row.get(
                                "theoretical_balance",
                                yarn_row.get("Theoretical Balance", 0),
                            ),
                            "allocated": yarn_row.get(
                                "allocated", yarn_row.get("Allocated", 0)
                            ),
                            "on_order": yarn_row.get(
                                "on_order", yarn_row.get("On Order", 0)
                            ),
                            "consumed": yarn_row.get(
                                "Consumed", yarn_row.get("consumed", 0)
                            ),
                        }
                    )
            ...
            for yarn in shortage_yarns:
                ...
                if yarn["consumed"] < 0:  # Consumed is negative in files
                    ...
                    weekly_demand = (
                        abs(yarn["consumed"]) / 4.3
                    )  # Approx 4.3 weeks per month
                elif yarn["allocated"] < 0:
                    # Use allocated as proxy for demand
                    weekly_demand = (
                        abs(yarn["allocated"]) / 8
                    )  # Assume 8-week production cycle
                else:
                    weekly_demand = 10  # Default minimal demand
                    ...
                    if not bom_df.empty:
                        ...
                        if yarn_col:
                            ...
                            try:
                                ...
                                # If conversion fails, try string comparison
                                style_matches = bom_df[
                                    bom_df[yarn_col].astype(str) == str(yarn["yarn_id"])
                                ]
                            if (
                                not style_matches.empty
                                and "Style#" in style_matches.columns
                            ):
                                affected_styles = list(
                                    style_matches["Style#"].unique()
                                )[:5]
                ...
                if affected_styles:  # Only include if orders are affected
                    ...
                    forecast_response["yarn_shortages"].append(
                        {
                            ...
                            "net_shortage": round(
                                -net_position if net_position < 0 else 0, 2
                            ),  # Show shortage as positive number
                            "net_position": round(
                                net_position, 2
                            ),  # Add net position for clarity
                            "urgency": urgency,
                            "days_until_shortage": min(days_until_shortage, 90),
                    ...
                    forecast_response["timeline_view"][timeline_key].append(
                        yarn["yarn_id"]
                    )

            forecast_response["summary"]["total_yarn_types_required"] = len(
                forecast_response["yarn_shortages"]
            )
            forecast_response["summary"]["total_forecasted_styles"] = len(
                set(
                    s
                    for y in forecast_response["yarn_shortages"]
                    for s in y.get("affected_styles", [])
                )
            )

        # Sort shortages by urgency and amount
        forecast_response["yarn_shortages"].sort(
            key=lambda x: (
                {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}[x["urgency"]],
        ...
        forecast_response["summary"]["total_shortage_lbs"] = round(
            forecast_response["summary"]["total_shortage_lbs"], 2
        )
        ...
        traceback.print_exc()
        return jsonify({"status": "error", "error": str(e), "yarn_shortages": []}), 500
                    ...
                    production_plan.append(
                        {
                            ...
                            "priority": (
                                "High" if production_needed > demand * 0.5 else "Normal"
                            ),
                            "recommended_start_date": datetime.now().strftime(
                                "%Y-%m-%d"
                            ),
                            "target_completion": (
        ...
        return jsonify(
            {
                ...
                "summary": {
                    ...
                    "average_production": (
                        total_production / len(production_plan)
                        if production_plan
                        else 0
                    ),
                    "planning_horizon": "30 days",
            ...
            jsonify(
                {
                    ...
                    "recommendations": [
                        "Unable to generate production plan due to error"
                    ],
                }
            ),
            500,
        ...
        if hasattr(analyzer, "sales_data") and analyzer.sales_data is not None:
            if not analyzer.sales_data.empty:
                ...
                sales_data["summary"] = {
                    ...
                    "averageOrderValue": (
                        total_value / len(analyzer.sales_data)
                        if len(analyzer.sales_data) > 0
                        else 0
                    ),
                }
    ...
    try:
        ...
        if production_manager:
            try:
                ...
                planning_data["active_orders"] = sum(
                    1 for o in orders if o.get("status") == "In Progress"
                )
            except:
                orders = []
        else:
                        ...
                        knit_orders.append(
                            {
                                ...
                                "completion_percentage": api_order.get(
        ...
        if len(knit_orders) == 0 and request.args.get("forecast") == "true":
            try:
                            ...
                            knit_orders.append(
                                {
                                    ...
                                    "Customer": "Forecasted",
                                    "Pounds": forecast.get("forecast_30_days", 1000)
                                    / 30,  # Daily average
                                    "Qty Ordered (lbs)": forecast.get(
                                        "forecast_30_days", 1000
                                    ),
                                    "Quantity": forecast.get("forecast_30_days", 1000),
                                    "Planned_Pounds": forecast.get(
                                        "forecast_30_days", 1000
                                    ),
                ...
                app.logger.warning(
                    f"Could not get forecast data for production planning: {e}"
                )
        ...
        for idx, order in enumerate(knit_orders[:10]):  # Top 10 orders
            ...
            quantity_lbs = safe_float(
                order.get(
                    "Qty Ordered (lbs)",
                    order.get("Qty_Ordered_Lbs", order.get("Pounds", 0)),
                )
            )
            if quantity_lbs == 0:
                # Try to get planned quantity or use a reasonable default
                quantity_lbs = safe_float(
                    order.get("Planned_Pounds", order.get("Quantity", 1000))
                )

            # Handle both 'Style#' and 'Style #' column names
            style = order.get(
                "Style #", order.get("Style#", order.get("fStyle#", "Unknown"))
            )
            ...
            if (
                pd.isna(customer_value)
                or customer_value == ""
                or customer_value is None
            ):
            ...
            if "in_production" in order:
                ...
                shipped_lbs = safe_float(
                    order.get("Shipped (lbs)", order.get("Shipped_Lbs", 0))
                )
                balance_lbs = safe_float(
                    order.get("Balance (lbs)", order.get("Balance_Lbs", quantity_lbs))
                )
            ...
            start_date_raw = order.get(
                "Start Date", order.get("Start_Date", datetime.now())
            )
            if isinstance(start_date_raw, str):
                try:
                    # Parse M/D/YYYY format
                    start_date = pd.to_datetime(
                        start_date_raw, format="%m/%d/%Y"
                    ).strftime("%Y-%m-%d")
                except:
                    try:
            ...
            if end_date_raw and pd.notna(end_date_raw):
                try:
                    # Parse M/D/YYYY format
                    end_date = pd.to_datetime(end_date_raw, format="%m/%d/%Y").strftime(
                        "%Y-%m-%d"
                    )
                except:
                    try:
                        ...
                        try:
                            ...
                            end_date = (start_dt + timedelta(days=30)).strftime(
                                "%Y-%m-%d"
                            )
                        except:
            ...
            schedule_item = {
                "order_id": str(
                    order.get("Order #", order.get("Machine#", f"ORD-{idx+1}"))
                ),
                ...
                "planned_quantity": safe_float(
                    order.get("Planned_Pounds", quantity_lbs)
                ),
        ...
        scheduled_lbs = sum(
            ...
            for order in knit_orders[:10]
        )
        utilization = (
            min((scheduled_lbs / total_capacity_lbs) * 100, 100)
            if total_capacity_lbs > 0
            else 0
        )
        ...
        if hasattr(analyzer, "yarn_inventory") and analyzer.yarn_inventory is not None:
            ...
            planning_data["resource_allocation"] = {
                ...
                "yarns_below_minimum": len(
                    analyzer.yarn_inventory[
                        analyzer.yarn_inventory["Planning Balance"] < 100
                    ]
                ),
                "allocation_status": (
                    "Critical" if len(critical_yarns) > 10 else "Normal"
                ),
            }
                ...
                planning_data["bottlenecks"].append(
                    {
                        ...
                        "impact": "High",
                        "shortage_amount": abs(
                            safe_float(yarn.get("Planning Balance", 0))
                        ),
        ...
        if len(knit_orders) > 0:
            ...
            for order in knit_orders:
                ...
                machine_loads[machine] = machine_loads.get(machine, 0) + safe_float(
                    order.get("Pounds", 0)
                )
        ...
        if (
            planning_data.get("resource_allocation", {}).get("critical_shortages", 0)
            > 5
        ):
            planning_data["recommendations"].append(
                {
...
def po_risk_analysis():
    ...
    try:
        ...
        if (
            ...
            return (
                jsonify(
                    {"status": "error", "message": "Knit orders data not available"}
                ),
                503,
            )
        ...
        category = request.args.get(
            "category", "all"
        )  # critical, high, overdue, not_started, all
        ...
        try:
            if actual_columns["start_date"] in knit_orders.columns:
                knit_orders["Start Date"] = pd.to_datetime(
                    knit_orders[actual_columns["start_date"]], errors="coerce"
                )
            else:
                knit_orders["Start Date"] = pd.NaT

            if actual_columns["quoted_date"] in knit_orders.columns:
                knit_orders["Quoted Date"] = pd.to_datetime(
                    knit_orders[actual_columns["quoted_date"]], errors="coerce"
                )
            else:
        ...
        knit_orders["days_until_start"] = (
            knit_orders["Start Date"] - current_date
        ).dt.days
        ...
        if balance_col in knit_orders.columns:
            active_orders = knit_orders[
                (knit_orders[balance_col] > 0) & (knit_orders["Start Date"].notna())
            ].copy()
        else:
        ...
        for idx, order in active_orders.iterrows():
            ...
            production_has_started = (not pd.isna(g00_status) and g00_status > 0) or (
                not pd.isna(shipped_lbs) and shipped_lbs > 0
            )
            ...
            if days_until < -30:
                if production_has_started:
                    ...
                    risk_factors.append(
                        f"Production started (was due {abs(days_until)} days ago)"
                    )
                else:
            ...
            if balance < 25:
                production_risk = (
                    0  # No production risk for essentially complete orders
                )
            ...
            risk_analysis.append(
                {
                    ...
                    "days_until_start": (
                        int(days_until) if not pd.isna(days_until) else 0
                    ),
                    "g00_lbs": float(g00_status) if not pd.isna(g00_status) else 0,
                    "shipped_lbs": (
                        float(shipped_lbs) if not pd.isna(shipped_lbs) else 0
                    ),
                    ...
                    "start_date": (
                        order["Start Date"].strftime("%Y-%m-%d")
                        if pd.notna(order["Start Date"])
                        else ""
                    ),
                    "quoted_date": (
                        order["Quoted Date"].strftime("%Y-%m-%d")
                        if pd.notna(order["Quoted Date"])
                        else ""
                    ),
                }
            )
        ...
        if category == "critical":
            filtered_analysis = [
                r for r in risk_analysis if r["risk_level"] == "CRITICAL"
            ]
        elif category == "high":
        ...
        summary = {
            ...
            "overdue_orders": len(
                [r for r in risk_analysis if r["days_until_start"] < 0]
            ),
            "not_started_orders": len(
                [
                    r
                    for r in risk_analysis
                    if r["g00_lbs"] == 0
            ...
            "critical_value_at_risk": round(
                sum(r["potential_loss"] for r in critical_orders), 2
            ),
            "avg_urgency_score": (
                round(
                    sum(r["urgency_score"] for r in risk_analysis) / len(risk_analysis),
                    1,
                )
                if risk_analysis
                else 0
...
def get_production_capacity():
    ...
    try:
        ...
        if style:
            ...
            production_time = capacity_mgr.calculate_production_time(
                style, 1000
            )  # For 1000 lbs
            ...
            if capacity_mgr.capacity_data is not None:
                top_styles = capacity_mgr.capacity_data.nlargest(
                    10, "Average of lbs/day"
                )
                for _, row in top_styles.iterrows():
                    high_efficiency_styles.append(
                        {"style": row["Style"], "capacity": row["Average of lbs/day"]}
                    )
...
def get_production_machine_mapping():
    ...
    try:
        ...
        if not capacity_mgr.machine_mapper:
            return (
                jsonify(
                    {"status": "error", "message": "Machine mapping not available"}
                ),
                503,
            )
        ...
        if style:
            ...
            # Get sample mappings
            sample_mappings = []
            for style, mapping in list(
                capacity_mgr.machine_mapper.style_to_machine_chain.items()
            )[:10]:
...
def get_machine_utilization():
    ...
    try:
        ...
        if not capacity_mgr.machine_mapper:
            return (
                jsonify(
                    {"status": "error", "message": "Machine tracking not available"}
                ),
                503,
            )
        ...
        if machine_id:
            ...
            return jsonify({"status": "success", "work_center_summary": summary})
        else:
            # Get overall machine utilization status
            status = capacity_mgr.get_machine_level_status()

            return jsonify({"status": "success", "machine_overview": status})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_work_center_capacity():
    ...
    try:
        ...
        if not capacity_mgr.machine_mapper:
            return (
                jsonify(
                    {"status": "error", "message": "Work center tracking not available"}
                ),
                503,
            )
        ...
        if work_center:
            ...
            return jsonify({"status": "success", "work_center_details": summary})
        else:
            ...
            for wc_id, summary in all_summaries.items():
                work_centers.append({"work_center_id": wc_id, **summary})
...
def get_factory_floor_status():
    ...
    try:
        ...
        work_center_list.sort(
            key=lambda x: (
                int(str(x["work_center_id"]))
                if str(x["work_center_id"]).isdigit()
                else 999999
            )
        )
        ...
        return jsonify(
            {
                ...
                "factory_overview": {
                    ...
                    "avg_utilization_percent": machine_status[
...
def get_ai_bottleneck_detection():
    ...
    try:
        ...
        if severity_filter:
            bottlenecks = [
                b for b in bottlenecks if b.severity.value == severity_filter
            ]
...
def get_ai_optimization_recommendations():
    ...
    try:
        ...
        if opt_type:
            optimizations = [
                o for o in optimizations if o.optimization_type == opt_type
            ]
...
def get_machine_assignment_suggestions():
    ...
    try:
        ...
        quads_path = Path(
            "/mnt/c/finalee/beverly_knits_erp_v2/data/production/5/ERP Data/QuadS_greigeFabricList_ (1).xlsx"
        )
        ko_path = Path(
            "/mnt/c/finalee/beverly_knits_erp_v2/data/production/5/ERP Data/8-28-2025/eFab_Knit_Orders.csv"
        )

        if not quads_path.exists() or not ko_path.exists():
            return (
                jsonify(
                    {"status": "error", "message": "Required data files not found"}
                ),
                404,
            )
        ...
        for _, row in unassigned.nlargest(40, "clean_balance").iterrows():
            ...
            matches = quads_df[
                quads_df["style"].str.contains(base_style, na=False, case=False)
            ]
            ...
            suggestion = {
                ...
                "balance_lbs": (
                    float(balance) if pd.notna(balance) and not pd.isna(balance) else 0
                ),
...
def get_capacity_bottlenecks():
    ...
    try:
        ...
        category = request.args.get(
            "category", "all"
        )  # unassigned, overloaded, underutilized, all
        ...
        if not ko_df.empty:
            ...
            if not unassigned.empty:
                ...
                unassigned["clean_balance"] = unassigned["Balance (lbs)"].apply(
                    clean_balance
                )
        ...
        for machine in machine_status.get("machine_status", []):
            if machine["utilization"] > 95:
                urgency = (
                    80 + (machine["utilization"] - 95) * 2
                )  # Scale by over-utilization
                bottlenecks.append(
                    {
        ...
        for machine in machine_status.get("machine_status", []):
            if machine["utilization"] < 30 and machine["status"] == "active":
                ...
                )  # Lower utilization = higher urgency to fix
                bottlenecks.append(
                    {
        ...
        if category == "unassigned":
            ...
            bottlenecks = [
                b for b in bottlenecks if b["type"] == "underutilized_machine"
            ]
        ...
        summary = {
            ...
            "unassigned_orders": len(
                [b for b in bottlenecks if b["type"] == "unassigned_order"]
            ),
            "overloaded_machines": len(
                [b for b in bottlenecks if b["type"] == "overloaded_machine"]
            ),
            "underutilized_machines": len(
                [b for b in bottlenecks if b["type"] == "underutilized_machine"]
            ),
            "avg_urgency_score": (
                round(
                    sum(b["urgency_score"] for b in bottlenecks) / len(bottlenecks), 1
                )
                if bottlenecks
                else 0
            ),
...
def get_factory_floor_ai_dashboard():
    ...
    try:
        ...
        with app.test_request_context(
            f"/api/factory-floor-status?active_only={active_only}"
        ):
        ...
        bottlenecks_by_wc = {
            b["work_center"]: b for b in insights["ai_analysis"]["bottlenecks"]
        }
...
def get_production_schedule():
    ...
    try:
        ...
        schedule = capacity_mgr.optimize_production_schedule(
            suggestions[:20]
        )  # Limit to top 20
        ...
        return jsonify(
            {
                ...
                "summary": {
                    ...
                    "calendar_days": capacity_mgr.working_days_to_calendar_days(
                        total_days
                    ),
                },
            }
...
def production_suggestions():
    ...
    try:
        ...
        try:
            from production.enhanced_production_suggestions_v2 import (
                create_enhanced_suggestions_v2,
            )
            ...
            if not bom_df.empty and "Style#" in bom_df.columns:
                ...
                bom_pct_col = (
                    ...
                    if "bom_percentage" in bom_df.columns
                        ...
                        "BOM_Percent"
                        if "BOM_Percent" in bom_df.columns
                        else "BOM_Percentage"
                    )
                )
                ...
                if bom_pct_col in bom_df.columns:
                    style_groups = (
                        bom_df.groupby("Style#")
                        .agg({yarn_col: "count", bom_pct_col: "sum"})
                        .reset_index()
                    )
                    style_groups.rename(
                        columns={bom_pct_col: "total_bom_percent"}, inplace=True
                    )
                else:
                    # If no BOM percentage column, just count yarns per style
                    style_groups = (
                        bom_df.groupby("Style#").agg({yarn_col: "count"}).reset_index()
                    )
            ...
            for _, style_row in style_groups.head(100).iterrows():
                ...
                if hasattr(analyzer, "inventory_data") and analyzer.inventory_data:
                    for stage, inv_data in analyzer.inventory_data.items():
                        ...
                        if (
                            inv_df is not None
                            and isinstance(inv_df, pd.DataFrame)
                            and "Style#" in inv_df.columns
                        ):
                ...
                if net_requirement > 100:  # Only suggest if significant quantity needed
                    ...
                    try:
                        ...
                        for _, yarn_row in style_yarns.head(5).iterrows():
                            ...
                            bom_percent = yarn_row.get(
                                "bom_percentage",
                                yarn_row.get(
                                    "BOM_Percent", yarn_row.get("BOM_Percentage", 100)
                                ),
                            )

                            # Calculate yarn needed for this production
                            yarn_needed = (
                                (net_requirement * bom_percent / 100)
                                if bom_percent > 0
                                else net_requirement
                            )
                            ...
                            if yarn_id and str(yarn_id) != "nan":
                                ...
                                if available_yarn < yarn_needed:
                                    ...
                                    yarn_shortage.append(
                                        f"{yarn_id} (need {yarn_needed:.0f} lbs, have {available_yarn:.0f} lbs)"
                                    )
                    except Exception as e:
                    ...
                    current_coverage = (
                        current_inventory / daily_usage if daily_usage > 0 else 0
                    )
                    suggested_coverage = (
                        (current_inventory + net_requirement) / daily_usage
                        if daily_usage > 0
                        else 30
                    )
                    ...
                    if current_production > 0:
                        rationale += (
                            f" (Currently producing {current_production:,.0f} lbs)"
                        )
                    ...
                    suggestions.append(
                        {
                            ...
                            "coverage_improvement": round(
                                suggested_coverage - current_coverage, 1
                            ),
                            "rationale": rationale,
                        }
                    )
        ...
        summary = {
            ...
            "material_available": len(
                [s for s in meaningful_suggestions if s["yarn_available"]]
            ),
            "material_shortage": len(
                [s for s in meaningful_suggestions if not s["yarn_available"]]
            ),
            "total_suggested_production": sum(
                s["suggested_quantity_lbs"] for s in meaningful_suggestions[:10]
            ),
        }
        ...
        if "fabric_details" in results and results["fabric_details"]:
            ...
            for fabric in results["fabric_details"][:20]:  # Top 20 fabrics
                row = {
                    ...
                    "moving_avg_forecast": fabric["forecasts"]
                    .get("moving_average", {})
                    .get("predicted", 0),
                    "moving_avg_accuracy": fabric["forecasts"]
                    .get("moving_average", {})
                    .get("accuracy", 0),
                    "exp_smooth_forecast": fabric["forecasts"]
                    .get("exponential_smoothing", {})
                    .get("predicted", 0),
                    "exp_smooth_accuracy": fabric["forecasts"]
                    .get("exponential_smoothing", {})
                    .get("accuracy", 0),
                    "linear_trend_forecast": fabric["forecasts"]
                    .get("linear_trend", {})
                    .get("predicted", 0),
                    "linear_trend_accuracy": fabric["forecasts"]
                    .get("linear_trend", {})
                    .get("accuracy", 0),
...
def get_yarn_backtest_comprehensive():
    ...
    try:
        ...
        backtest = BacktestAnalyzer(
            ...
            yarn_data=(
                analyzer.yarn_data
                if hasattr(analyzer, "yarn_data")
                else analyzer.raw_materials_data
            ),
            bom_data=analyzer.bom_data,
        )
            ...
            for yarn in results["yarn_details"][:30]:  # Top 30 yarns
                row = {
                    ...
                    "sufficient_inventory": (
                        "Yes" if yarn["sufficient_inventory"] else "No"
                    ),
                    "shortage_amount": max(
                        0, yarn["actual_consumption"] - yarn["planning_balance"]
                    ),
                }
                table_data.append(row)
...
def get_full_backtest_report():
    ...
    try:
        ...
        summary_tables = {"fabric_tables": {}, "yarn_tables": {}}
...
if False:  # Was: if FEATURE_FLAGS_AVAILABLE and should_redirect_deprecated():
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/inventory-intelligence-enhanced", default_params={"view": "summary"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/inventory-intelligence-enhanced", default_params={"realtime": "true"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/inventory-intelligence-enhanced",
            default_params={"view": "dashboard", "realtime": "true"},
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/inventory-intelligence-enhanced", default_params={"view": "complete"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/inventory-intelligence-enhanced", default_params={"view": "dashboard"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/yarn-intelligence", default_params={"view": "data"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/yarn-intelligence", default_params={"analysis": "shortage"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/yarn-substitution-intelligent",
            default_params={"view": "opportunities"},
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/yarn-substitution-intelligent",
            default_params={"view": "alternatives"},
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/yarn-intelligence",
            default_params={"forecast": "true", "analysis": "shortage"},
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/yarn-intelligence", default_params={"forecast": "true", "ai": "true"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/production-planning", default_params={"view": "data"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/production-planning", default_params={"view": "orders"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/production-planning", default_params={"forecast": "true"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/emergency-shortage-dashboard", default_params={"view": "procurement"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/emergency-shortage-dashboard", default_params={"type": "yarn"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/ml-forecast-detailed", default_params={"detail": "summary"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/ml-forecast-detailed", default_params={"format": "report"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/ml-forecast-detailed", default_params={"source": "pipeline"}
        ),
    )
    ...
    app.add_url_rule(
        ...
        view_func=redirect_to_new_api(
            "/api/ml-forecast-detailed", default_params={"compare": "stock"}
        ),
    )
