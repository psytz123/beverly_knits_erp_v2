...
class ManufacturingSupplyChainAI:
    ...
    def __init__(self, data_path, column_mapping=None):
        ...
        self.ml_models = {}
        self.ml_models_cache = {}  # Cache for ML model performance metrics
    ...
    try:
        ...
        try:
            ...
            for idx, yarn in all_critical_yarns.iterrows():
                ...
                yarn_id = yarn.get('Desc#') or yarn.get('desc_num') or yarn.get('YarnID') or yarn.get('Yarn_ID') or yarn.get('ID') or yarn.get('yarn_id')
                
                # Convert to string and handle NaN/None values
                if yarn_id is not None and str(yarn_id).lower() not in ['nan', 'none', '']:
                    # Convert to int first to remove decimals, then to string
                    try:
                        yarn_id = str(int(float(yarn_id)))
                    ...
                    if desc and str(desc).strip():
                        ...
                        yarn_id = desc_parts[0] if desc_parts else str(desc)[:20]
                    else:
                        # Use index as last resort
                        yarn_id = f"YARN_{idx}"
                
                yarn_id = str(yarn_id)[:20]  # Limit length
        ...
        if include_forecast:
            intelligence['forecast'] = {
                'horizon_days': 90,
                'predicted_shortages': [],  # Would integrate with forecasting engine
                'confidence': 0.85
            }
            if yarn_id:
                # Specific yarn forecast
                intelligence['forecast']['yarn_id'] = yarn_id
                intelligence['forecast']['predicted_demand'] = 1000  # Placeholder
