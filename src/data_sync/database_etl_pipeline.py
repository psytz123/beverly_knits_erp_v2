...
class BeverlyKnitsETL:
    ...
    def __init__(self, config_path: str = "database_config.json"):
        ...
        self.cursor = None
        self.data_path = Path(
            self.config.get("data_path", "/mnt/c/Users/psytz/sc data/ERP Data")
        )
            ...
            try:
                ...
                if missing_count > 0:
                    self.log_data_quality_issue(
                        "yarns",
                        "MISSING_ID",
                        f"{missing_count} yarns with missing Desc#",
                        "WARNING",
                    )
                        ...
                        if yarn_result:
                            ...
                            """,
                                (
                                    ...
                                    (
                                        float(row.get("Weeks of Supply", 0))
                                        if pd.notna(row.get("Weeks of Supply"))
                                        else None
                                    ),
                    ...
                    if so_number:
                        ...
                        """,
                            (
                                ...
                                (
                                    pd.to_datetime(row.get("Ship Date"))
                    ...
                    if ko_number:
                        ...
                        """,
                            (
                                ...
                                style_number,
                                (
                                    pd.to_datetime(row.get("Start Date"))
                                    if pd.notna(row.get("Start Date"))
                                    else None
                                ),
                                (
                                    pd.to_datetime(row.get("Quoted Date"))
        ...
        if file_path.exists():
            try:
                ...
                week_columns = [
                    ...
                    if "Week" in str(col) or col.startswith("W")
                ]
                        ...
                        if yarn_result:
                            ...
                            for week_num, week_col in enumerate(week_columns, 1):
                                ...
                                if demand_qty and demand_qty > 0:
                                    week_date = self.snapshot_date + timedelta(
                                        weeks=week_num - 1
                                    )
