...
class FabricMappingConfig:
    ...
    def get_standard_mapping(
        cls, filename: str, df_columns: List[str]
    ) -> Dict[str, str]:
        ...
        if file_config:
            ...
            # Special handling for Sales Activity Report
                ...
                if style_col in df_columns:
                    mapping[style_col] = cls.STANDARD_COLUMNS[
                        "style_version"
                    ]  # Maps to cFVersion
                ...
                for col in df_columns:
                    ...
                    if col == style_col:
                        ...
                        if filename.startswith(
                            tuple(cls.CONSOLIDATION_RULES["use_fstyle"])
                        ):
    ...
    for col in df.columns:
        ...
        if any(pattern in col_lower for pattern in style_patterns):
            if col not in mapping:
                ...
                corrections.append(
                    f"Note: Found potential style column '{col}' not in mapping"
                )
...
if __name__ == "__main__":
    ...
    for filename in test_files:
        ...
        if config:
            ...
            if filename.startswith(
                tuple(FabricMappingConfig.CONSOLIDATION_RULES["use_fstyle"])
            ):
                print(f"  Maps to: fStyle#")
            elif filename.startswith(
                tuple(FabricMappingConfig.CONSOLIDATION_RULES["use_style"])
            ):
                print(f"  Maps to: Style#")
            elif filename.startswith(
                tuple(FabricMappingConfig.CONSOLIDATION_RULES["use_fbase"])
            ):
                print(f"  Maps to: fBase")
            elif filename.startswith(
                tuple(FabricMappingConfig.CONSOLIDATION_RULES["use_cfversion"])
            ):
