            ...
            for col in df_columns:
                for attr_name in attr_names:
                    if col.lower() == attr_name.lower():
                        ...
                        standard_name = (
                            attr_type.replace("_", " ").title().replace(" ", "_")
                        )
    ...
    if mapping:
        ...
        for old_col, new_col in mapping.items():
            if old_col in df.columns:
                ...
                corrections.append(
                    f"Mapped yarn column '{old_col}' to standard '{new_col}'"
                )
                ...
                if new_col == YarnMappingConfig.STANDARD_YARN_COLUMN:
                    ...
                    # Validate yarn IDs
                    invalid_count = (
                        ~df[new_col].apply(YarnMappingConfig.validate_yarn_id)
                    ).sum()
                    if invalid_count > 0:
                        corrections.append(
                            f"Warning: Found {invalid_count} invalid yarn IDs"
                        )

    # Check for duplicate yarn columns after mapping
    yarn_cols = [
        col for col in df.columns if "desc#" in col.lower() or "yarn" in col.lower()
    ]
    if len(yarn_cols) > 1:
