...
class DataParserCleaner:
    ...
    def identify_file_type(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if any(
                "planning" in col.lower() and "balance" in col.lower()
                for col in columns
            ):
                return "yarn_inventory"
            elif any("style" in col.lower() for col in columns) and any(
                "desc" in col.lower() for col in columns
            ):
                return "style_bom"
        ...
        for col in df.columns:
            ...
            if col in self.data_types["numeric"]:
                try:
                    ...
                    if nulls > 0:
                        ...
                        corrections.append(
                            f"Filled {nulls} null values in '{col}' with 0"
                        )
                except Exception as e:
                    corrections.append(
                        f"Warning: Could not convert '{col}' to numeric: {e}"
                    )
            ...
            # Date columns
                ...
                try:
                    ...
                    corrections.append(
                        f"Warning: Could not convert '{col}' to date: {e}"
                    )
        ...
        try:
            ...
            if len(df) < before_drop:
                result["corrections"].append(
                    f"Removed {before_drop - len(df)} empty rows"
                )
            ...
            logger.info(
                f"Cleaned {file_path.name}: {len(col_corrections)} column fixes, {len(type_corrections)} type fixes"
            )
...
"""
        ...
        for file_result in report["files_processed"]:
            status_emoji = {"success": "✅", "warnings": "⚠️", "error": "❌"}.get(
                file_result["status"], "?"
            )
...
def clean_sharepoint_data(data_dir: Path) -> bool:
    ...
    try:
        ...
        if report["summary"]["errors"] == 0:
            ...
            logger.error(
                f"Data cleaning encountered {report['summary']['errors']} errors"
            )
            return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir",
        default="C:/finalee/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/sharepoint_sync",
