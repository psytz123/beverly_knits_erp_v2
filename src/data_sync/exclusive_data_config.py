...
class ExclusiveDataConfig:
    ...
    DATABASE_CONFIG_PATH = (
        Path(__file__).parent.parent / "database" / "database_config.json"
    )
    ...
    def _cleanup_old_data_sources(cls):
        ...
        for old_path in old_paths:
            if old_path.exists() and old_path != cls.EXCLUSIVE_DATA_PATH:
                ...
                backup_path = (
                    old_path.parent
                    / f"{old_path.name}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
                logger.warning(
                    f"Moving old data source to backup: {old_path} -> {backup_path}"
                )
                try:
...
def configure_exclusive_data_source(erp_instance):
    ...
    erp_instance.data_path = (
        exclusive_path.parent
    )  # Set to parent to maintain /5 structure
