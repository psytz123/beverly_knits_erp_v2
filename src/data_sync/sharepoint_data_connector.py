...
try:
    ...
    print(
        "SharePoint libraries not available. Install with: pip install Office365-REST-Python-Client"
    )
...
class SharePointDataConnector:
    ...
    def _load_metadata(self) -> Dict:
        ...
        if self.metadata_file.exists():
            with open(self.metadata_file, "r") as f:
                return json.load(f)
        return {"last_sync": None, "files": {}, "sync_interval_minutes": 60}
    ...
    def authenticate_interactive(
        ...
        if not SHAREPOINT_LIBS_AVAILABLE:
            raise ImportError(
                "SharePoint libraries not installed. Run: pip install Office365-REST-Python-Client"
            )
    ...
    def list_files(self, file_extensions: List[str] = [".xlsx", ".csv"]) -> List[Dict]:
        ...
        try:
            ...
            for file in files:
                if any(
                    file.properties["Name"].endswith(ext) for ext in file_extensions
                ):
                    file_list.append(
                        {
    ...
    def sync_folder(self, force: bool = False) -> Dict[str, int]:
        ...
        if not force and self.metadata["last_sync"]:
            ...
            if datetime.now() - last_sync < timedelta(
                minutes=self.metadata["sync_interval_minutes"]
            ):
                logger.info(
                    f"Skipping sync - last sync was {(datetime.now() - last_sync).seconds // 60} minutes ago"
                )
                return stats
        ...
        for file_info in files:
            ...
            if file_name in self.metadata["files"]:
                if (
                    self.metadata["files"][file_name]["modified"]
                    == file_info["modified"]
                ):
    ...
    @staticmethod
    def create_from_url(
        sharepoint_url: str, local_data_path: str
...
def integrate_sharepoint_with_erp(
    erp_instance, sharepoint_url: str, username: str = None, password: str = None
):
    """
    ...
    connector = SharePointDataConnector.create_from_url(
        sharepoint_url, str(erp_instance.data_path / "5")  # Use existing data directory
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync SharePoint data for Beverly Knits ERP"
    )
    parser.add_argument(
        "--url",
        default="https://beverlyknits-my.sharepoint.com/:f:/p/psytz/EjOE8ifm4IpGpot4KsLgFy4B9H_dO-vtPFuA4ergdB__Og?e=9GncTp",
        help="SharePoint folder URL",
    )
    parser.add_argument(
        "--local-path",
        default="C:/finalee/Agent-MCP-1-ddd/Agent-MCP-1-dd/ERP Data/5",
        help="Local data directory",
    )
    parser.add_argument("--username", help="SharePoint username")
    parser.add_argument("--password", help="SharePoint password")
    parser.add_argument(
        "--force", action="store_true", help="Force sync even if recently synced"
    )
    ...
    if connector.authenticate_interactive(args.username, args.password):
        ...
        print(
            f"Sync completed: Downloaded {stats['downloaded']}, Skipped {stats['skipped']}, Failed {stats['failed']}"
        )
    else:
