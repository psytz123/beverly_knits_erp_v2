...
def ml_forecast_report():
    ...
    try:
        ...
        if (
            hasattr(handler.forecasting, "ml_available")
            and handler.forecasting.ml_available
        ):
        ...
        if hasattr(handler.forecasting, "calculate_forecast_accuracy"):
            accuracy = handler.forecasting.calculate_forecast_accuracy(
                actual=sales_data, predicted=forecast if "forecast" in locals() else {}
            )
    ...
    try:
        ...
        if (
            ...
            if hasattr(handler.forecasting, "forecast_with_random_forest"):
                rf_forecast = handler.forecasting.forecast_with_random_forest(
                    sales_data
                )
            ...
            if hasattr(handler.forecasting, "forecast_with_prophet"):
                ...
                detailed_forecast["models"]["prophet"] = {
                    ...
                    "items_forecasted": (
                        len(prophet_forecast) if prophet_forecast else 0
                    ),
                }
...
def ml_forecasting():
    ...
    try:
        ...
        forecast_info = {
            ...
            "models": [],
            "last_training": (
                handler.last_training.isoformat() if handler.last_training else None
            ),
            "forecast_available": False,
        }

        if handler.forecasting:
            forecast_info["ml_enabled"] = getattr(
                handler.forecasting, "ml_available", False
            )
...
def ml_validation_summary():
    ...
    try:
        ...
        if handler.forecasting:
            ...
            best_model = max(
                validation_summary["accuracy_scores"].items(), key=lambda x: x[1]["r2"]
            )
    ...
    try:
        ...
        for model in models:
            try:
                ...
                retrain_result["errors"].append({"model": model, "error": str(e)})
