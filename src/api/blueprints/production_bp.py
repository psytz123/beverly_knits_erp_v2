...
def production_planning():
    ...
    try:
        ...
        planning_data = {
            "total_orders": (
                len(production_data) if hasattr(production_data, "__len__") else 0
            ),
            "view": view,
        ...
        if handler.capacity and hasattr(
            handler.capacity, "calculate_capacity_utilization"
        ):
            capacity_data = handler.capacity.calculate_capacity_utilization(
                production_data
            )
...
def production_pipeline():
    ...
    try:
        ...
        if handler.capacity and hasattr(
            handler.capacity, "identify_capacity_bottlenecks"
        ):
            bottlenecks = handler.capacity.identify_capacity_bottlenecks(
                production_data
            )
...
def production_suggestions():
    ...
    try:
        ...
        production_data = handler.get_production_data()
        yarn_data = (
            handler.data_loader.load_yarn_inventory() if handler.data_loader else None
        )
        ...
        if handler.pipeline and hasattr(handler.pipeline, "run_complete_analysis"):
            analysis = handler.pipeline.run_complete_analysis(
                inventory_data=yarn_data, yarn_data=yarn_data
            )
