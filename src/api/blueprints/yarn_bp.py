...
def init_blueprint(
    yarn_intelligence, interchangeability, data_loader, service_manager=None
):
        ...
        if analysis_type in ["shortage", "all"]:
            ...
            if hasattr(yarn_data, "iterrows"):
                if CONSISTENCY_AVAILABLE:
                    ...
                    yarn_data_standardized = DataConsistencyManager.standardize_columns(
                        yarn_data.copy()
                    )
                    ...
                    for _, yarn in yarn_data.iterrows():
                        planning_balance = yarn.get(
                            "Planning Balance", yarn.get("planning_balance", 0)
                        )
                        if planning_balance < 0:
                            shortage_yarns.append(
                                {
                                    "yarn_id": yarn.get(
                                        "Desc#", yarn.get("yarn_id", "")
                                    ),
                                    ...
                                    "risk_level": (
                                        "HIGH" if planning_balance < -500 else "MEDIUM"
                                    ),
                                }
                            )
            ...
            intelligence["analysis"]["total_shortage_value"] = sum(
                y["shortage_amount"] for y in shortage_yarns
            )

        # Analyze inventory health
        if hasattr(yarn_data, "describe"):
            planning_balance = yarn_data.get(
                "Planning Balance", yarn_data.get("planning_balance", pd.Series())
            )
            if not planning_balance.empty:
                intelligence["analysis"]["inventory_health"] = {
                    ...
                    "warning_items": len(
                        planning_balance[
                            (planning_balance >= 0) & (planning_balance <= 100)
                        ]
                    ),
            ...
            shortage_analysis["action_items"].append(
                {
                    ...
                    "yarns": [
                        y["yarn_id"]
                        for y in shortage_analysis["critical_shortages"][:5]
                    ],
                }
            )
...
def yarn_substitution_intelligent():
    ...
    try:
        ...
        substitution_data["savings_potential"] = (
            len(substitution_data["opportunities"]) * 1000
        )
...
def yarn_aggregation():
    ...
    try:
        ...
        aggregation = {"total_yarns": len(yarn_data), "groups": {}, "summary": {}}

        # Group by supplier if column exists
        if "supplier" in yarn_data.columns:
            supplier_groups = (
                yarn_data.groupby("supplier")
                .agg({"Planning Balance": "sum", "Desc#": "count"})
                .to_dict("index")
            )
        ...
        if "Planning Balance" in yarn_data.columns:
            aggregation["summary"] = {
                ...
                "items_overstocked": len(
                    yarn_data[yarn_data["Planning Balance"] > 5000]
                ),
            }
