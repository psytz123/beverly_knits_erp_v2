...
def init_blueprint(analyzer_instance, pipeline_instance, data_loader_instance):
    ...
    global handler
    handler = InventoryAPIHandler(
        analyzer_instance, pipeline_instance, data_loader_instance
    )
...
def inventory_intelligence_enhanced():
    ...
    try:
        ...
        if analysis_type == "shortage":
            critical_items = [
                item
                for item in analysis_results.get("critical_items", [])
        ...
        return jsonify(
            {
                ...
                "summary": {
                    "critical": len(
                        [s for s in shortages if s["shortage_amount"] > 1000]
                    ),
                    "high": len(
                        [s for s in shortages if 500 < s["shortage_amount"] <= 1000]
                    ),
                    "medium": len(
                        [s for s in shortages if 100 < s["shortage_amount"] <= 500]
                    ),
...
def stock_risks():
    ...
    try:
        ...
        # Analyze risk levels
        analysis = handler.analyzer.analyze_inventory_levels(
...
def real_time_inventory():
    ...
    try:
        ...
        status = {
            "last_updated": "Now",
            "total_items": (
                len(inventory_data) if hasattr(inventory_data, "__len__") else 0
            ),
            "critical_items": 0,
            "healthy_items": 0,
            "items": [],
                    ...
                    status["items"].append(
                        {
                            ...
                            "status": (
                                "Critical"
                                if balance < 0
                                else "Healthy" if balance > 100 else "Warning"
                            ),
                        }
                    )
