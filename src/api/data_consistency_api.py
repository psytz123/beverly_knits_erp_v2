...
def data_consistency_check():
    ...
    try:
        ...
        if not analyzer:
            return jsonify({"error": "Analyzer not available", "available": False}), 503
        ...
        production_df = (
            analyzer.knit_orders_data
            if hasattr(analyzer, "knit_orders_data")
            else pd.DataFrame()
        )
        ...
        validation_results = {
            ...
            "bom": (
                DataValidationRules.validate_bom(bom_df)
                if not bom_df.empty
                else {"is_valid": True, "warnings": ["No BOM data available"]}
            ),
            "production": (
                DataValidationRules.validate_production_orders(production_df)
                if not production_df.empty
                else {"is_valid": True, "warnings": ["No production data available"]}
            ),
        }

        # Cross-validate data between sources
        cross_validation = (
            DataValidationRules.cross_validate_data(inventory_df, bom_df, production_df)
            if not bom_df.empty and not production_df.empty
            else {"is_valid": True, "cross_checks": []}
        )

        # Combine all results
        consistency_report = {
            "summary": {
                "overall_consistent": report["validation"]["is_consistent"]
                and all(v["is_valid"] for v in validation_results.values()),
                ...
                "data_sources_available": sum(
                    1 for df in [inventory_df, bom_df, production_df] if not df.empty
                ),
            },
...
def shortage_consistency():
    ...
    try:
        if not CONSISTENCY_AVAILABLE:
            return jsonify({"error": "Data consistency tools not available"}), 503
        ...
        inventory_standardized = DataConsistencyManager.standardize_columns(
            inventory_df
        )
                ...
                legacy_shortages.append(
                    {
                        ...
                        "risk_level": (
                            ...
                            else "HIGH" if planning_balance < -500 else "MEDIUM"
                        ),
                        "method": "legacy",
                    }
                )
        ...
        for consistent_yarn in consistent_shortages:
            legacy_yarn = next(
                (
                    y
                    for y in legacy_shortages
                    if y["yarn_id"] == consistent_yarn["yarn_id"]
                ),
                None,
            )
            if legacy_yarn:
                diff = abs(
                    consistent_yarn["shortage_amount"] - legacy_yarn["shortage_amount"]
                )
        ...
        return jsonify(
            {
                ...
                "consistency_check": {
                    ...
                    "match_percentage": len(consistent_yarn_ids & legacy_yarn_ids)
                    / max(len(consistent_yarn_ids | legacy_yarn_ids), 1)
                    * 100,
                },
                "recommendations": (
                    [
        ...
        return jsonify({"error": f"Shortage consistency check failed: {str(e)}"}), 500
...
def data_quality_metrics():
    ...
    try:
        ...
        if (
            ...
            duplicate_count = (
                inventory_df.duplicated(subset=["Desc#"], keep=False).sum()
                if "Desc#" in inventory_df.columns
                else 0
            )
            negative_balance_count = (
                (inventory_df["Planning Balance"] < 0).sum()
                if "Planning Balance" in inventory_df.columns
                else 0
            )
            ...
            metrics["data_sources"]["inventory"] = {
                ...
                "quality_score": (
                    sum(column_completeness.values()) / len(column_completeness)
        ...
        if hasattr(analyzer, "bom_data") and analyzer.bom_data is not None:
            ...
            unique_styles = (
                bom_df["Style#"].nunique() if "Style#" in bom_df.columns else 0
            )
            total_mappings = len(bom_df)
            avg_yarns_per_style = (
                total_mappings / unique_styles if unique_styles > 0 else 0
            )
        ...
        if (
            ...
            if "Machine" in production_df.columns:
                assigned_orders = (
                    production_df["Machine"].notna() & (production_df["Machine"] != "")
                ).sum()
            ...
            metrics["data_sources"]["production"] = {
                ...
                "assignment_rate": (
                    (assigned_orders / total_orders * 100) if total_orders > 0 else 0
                ),
                "quality_score": (
                    (assigned_orders / total_orders * 100) if total_orders > 0 else 0
                ),
        ...
        quality_scores = [
            ...
            for source in metrics["data_sources"].values()
        ...
        return jsonify({"error": f"Data quality metrics failed: {str(e)}"}), 500
