    ...
    def production_flow_status():
        ...
        try:
            if not production_tracker:
                return (
                    jsonify({"error": "Production Flow Tracker not initialized"}),
                    503,
                )
    ...
    def generate_production_plan():
        ...
        try:
            ...
            if not replenishment_needs:
                replenishment_needs = (
                    production_tracker.calculate_f01_replenishment_needs()
                )

            # Generate production plan
            plan = production_tracker.generate_production_plan(
                replenishment_needs=replenishment_needs, bom_data=bom_data
            )

            return jsonify({"production_plan": plan, "total_orders": len(plan)})
        except Exception as e:
            return jsonify({"error": str(e)}), 500
    ...
    def get_capacity_status():
        ...
        try:
            ...
            for stage, capacity in production_tracker.capacity_constraints.items():
                current_wip = sum(
                    production_tracker.stage_inventory.get(stage, {}).values()
                )
                utilization = (
                    (current_wip / capacity * 100) if capacity < float("inf") else 0
                )
            ...
            for style_id, bom in test_bom.items():
                try:
                    ...
                    if style_id == "STYLE001":
                        ...
                        production_tracker.move_batch_to_stage(
                            batch.batch_id, ProductionStage.G00
                        )
                        production_tracker.move_batch_to_stage(
                            batch.batch_id, ProductionStage.G02
                        )
                except Exception as e:
