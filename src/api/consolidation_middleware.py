...
def log_deprecated_usage(old_endpoint: str, new_endpoint: str):
    ...
    logger.warning(
        f"Deprecated endpoint used: {old_endpoint} -> {new_endpoint} "
...
def get_deprecation_report() -> Dict[str, Any]:
    ...
    report = {
        "total_deprecated_calls": sum(
            info["count"] for info in deprecated_usage.values()
        ),
        ...
        report["endpoints"].append(
            {
                ...
                "first_seen": (
                    info["first_seen"].isoformat() if info["first_seen"] else None
                ),
                "last_seen": (
                    info["last_seen"].isoformat() if info["last_seen"] else None
                ),
            }
        )
...
class APIConsolidationMiddleware:
        ...
        if current_path in ENDPOINT_MAPPINGS:
            ...
            def add_deprecation_headers(response):
                ...
                response.headers["Warning"] = (
                    f'299 - "This endpoint is deprecated. Use {new_endpoint}"'
                )
                return response
    ...
    def feature_flags_endpoint(self):
        ...
        if request.method == "GET":
            ...
            return jsonify({"updated": updated, "current_flags": get_feature_flags()})
