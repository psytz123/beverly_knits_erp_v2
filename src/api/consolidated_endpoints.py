    ...
    def unified_endpoint():
        ...
        try:
            ...
            include_intelligence = (
                request.args.get("intelligence", "true").lower() == "true"
            )
            include_yarn = request.args.get("yarn", "true").lower() == "true"
            include_production = (
                request.args.get("production", "true").lower() == "true"
            )
            ...
            if style_filter and "production" in response["data"]:
                filtered = [
                    item
                    for item in response["data"]["production"].get("items", [])
            ...
            response["summary"] = {
                "total_yarn_items": len(
                    response["data"].get("yarn", {}).get("inventory", [])
                ),
                "total_production_items": len(
                    response["data"].get("production", {}).get("items", [])
                ),
            ...
            return (
                jsonify({"error": "Failed to get inventory data", "message": str(e)}),
                500,
            )
...
class ConsolidatedForecastAPI:
    ...
    def unified_endpoint():
        ...
        try:
            ...
            if style_filter:
                for key in ["sales", "demand", "orders"]:
                    if key in response["data"] and "items" in response["data"][key]:
                        filtered = [
                            item
                            for item in response["data"][key]["items"]
                            if style_filter.lower()
                            in str(item.get("style", "")).lower()
                        ]
            ...
            logger.error(traceback.format_exc())
            return (
                jsonify({"error": "Failed to get forecast data", "message": str(e)}),
                500,
            )
...
class ConsolidatedProductionAPI:
    ...
    def unified_endpoint():
        ...
        try:
            ...
            include_suggestions = (
                request.args.get("suggestions", "true").lower() == "true"
            )
            ...
            if order_filter:
                for key in response["data"]:
                    if (
                        isinstance(response["data"][key], dict)
                        and "orders" in response["data"][key]
                    ):
                        filtered = [
                            order
                            for order in response["data"][key]["orders"]
                            if order_filter.lower()
                            in str(order.get("order_id", "")).lower()
                        ]
            ...
            logger.error(traceback.format_exc())
            return (
                jsonify({"error": "Failed to get production data", "message": str(e)}),
                500,
            )
    ...
    def unified_endpoint():
        ...
        try:
            ...
            include_substitutions = (
                request.args.get("substitutions", "true").lower() == "true"
            )
                ...
                for key in response["data"]:
                    if isinstance(response["data"][key], dict):
                        ...
                        if "yarns" in response["data"][key]:
                            filtered = [
                                yarn
                                for yarn in response["data"][key]["yarns"]
                                if yarn_filter.lower()
                                in str(yarn.get("id", "")).lower()
                            ]
            ...
            logger.error(traceback.format_exc())
            return jsonify({"error": "Failed to get yarn data", "message": str(e)}), 500
    ...
    def unified_endpoint():
        ...
        try:
            ...
            include_optimization = (
                request.args.get("optimization", "true").lower() == "true"
            )
            ...
            if phase_filter and "six_phase" in response["data"]:
                if "phases" in response["data"]["six_phase"]:
                    filtered_phases = {
                        k: v
                        for k, v in response["data"]["six_phase"]["phases"].items()
            ...
            logger.error(traceback.format_exc())
            return (
                jsonify({"error": "Failed to get planning data", "message": str(e)}),
                500,
            )
...
class ConsolidatedSystemAPI:
    ...
    def unified_endpoint():
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": "Failed to execute system operation", "message": str(e)}
                ),
                500,
            )
