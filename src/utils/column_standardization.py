...
class ColumnStandardizer:
    ...
    COLUMN_MAPPINGS = {
        ...
        "fStyle#": [
            "fStyle#",
            "fStyle",
        ],  # Don't include 'Style #' here - it maps to Style# in most cases
        ...
        if file_type == "yarn_inventory":
            ...
            for col in required_cols:
                if col not in df.columns:
                    print(
                        f"Warning: Required column '{col}' not found in yarn_inventory"
                    )

        elif file_type == "yarn_id":
            # Yarn_ID.csv and Yarn_ID_Master.csv have Planning_Ballance typo
            if (
                "Planning_Ballance" in df.columns
                and "Planning Balance" not in df.columns
            ):
            ...
            file_type == "inventory_f01"
            or file_type == "inventory_g00"
            or file_type == "inventory_i01"
        ):
            ...
            if "fStyle#" not in df.columns and "Style#" not in df.columns:
                print(
                    "WARNING: eFab_SO_List is missing style columns (fStyle# and Style#)"
                )
