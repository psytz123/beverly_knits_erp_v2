...
class CacheManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Cache manager initialized with TTL={default_ttl}s, max_items={max_memory_items}"
        )
...
class ERPCacheManager(CacheManager):
    ...
    def cache_bom(self, style: str, bom_data: List) -> bool:
        ...
        return self.set(
            style, bom_data, ttl=self.NAMESPACES["bom_data"], namespace="bom_data"
        )
...
def test_cache_manager():
    ...
    complex_data = {"inventory": [1, 2, 3], "orders": {"order1": 100, "order2": 200}}
