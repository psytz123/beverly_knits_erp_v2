...
def validate_numeric_field(
    ...
    if min_value is not None and value < min_value:
        raise ValidationError(
            f"Field '{field}' must be at least {min_value}", field=field
        )

    if max_value is not None and value > max_value:
        raise ValidationError(
            f"Field '{field}' must not exceed {max_value}", field=field
        )
...
def validate_date_field(
    data: Dict, field: str, date_format: str = "%Y-%m-%d"
) -> datetime:
    """
    ...
    try:
        ...
        raise ValidationError(
            f"Field '{field}' must be a valid date in format {date_format}", field=field
        )
...
class ErrorLogger:
    ...
    def log_validation_error(self, field: str, value: Any, reason: str):
        ...
        self.logger.warning(
            f"Validation failed for field '{field}': {reason} (value: {value})"
        )
