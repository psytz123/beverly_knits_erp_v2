...
def create_inventory_netting_endpoint(analyzer):
    ...
    try:
        ...
        if (
            ...
            if "Qty" in analyzer.sales_data.columns:
                ...
                result["netting_summary"]["total_demand"] = (
                    float(total_demand) if not pd.isna(total_demand) else 0
                )
        ...
        if (
            ...
            if "Stock (LBS)" in analyzer.raw_materials_data.columns:
                ...
                result["netting_summary"]["total_inventory"] = (
                    float(total_inventory) if not pd.isna(total_inventory) else 0
                )
            ...
            if stock_col and id_col:
                ...
                yarn_data = analyzer.raw_materials_data.sort_values(by=stock_col).head(
                    50
                )
                ...
                if hasattr(analyzer, "bom_data") and analyzer.bom_data is not None:
                    if (
                        "Component" in analyzer.bom_data.columns
                        and "Quantity" in analyzer.bom_data.columns
                    ):
                        yarn_demand = (
                            analyzer.bom_data.groupby("Component")["Quantity"]
                            .sum()
                            .to_dict()
                        )
                ...
                for _, row in yarn_data.iterrows():
                    ...
                    available = (
                        float(row.get(stock_col, 0))
                        if not pd.isna(row.get(stock_col, 0))
                        else 0
                    )
                    ...
                    yarn_item = {
                        "yarn_id": yarn_id,
                        "description": (
                            str(row.get(desc_col, "N/A")) if desc_col else "N/A"
                        ),
                ...
                total_allocated = sum(
                    item["allocated"] for item in result["yarn_netting"]
                )
        ...
        if (
            result["netting_summary"]["total_inventory"]
            > result["netting_summary"]["total_demand"] * 2
        ):
            result["recommendations"].append(
                {
                    "type": "excess",
