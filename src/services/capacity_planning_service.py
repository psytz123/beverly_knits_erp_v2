...
class CapacityPlanningService:
    ...
    def __init__(self, config: Optional[CapacityConfig] = None):
        ...
        logger.info(
            f"CapacityPlanningService initialized with bottleneck threshold: {self.config.bottleneck_threshold}"
        )

    def calculate_finite_capacity_requirements(
        self, production_plan: Dict[str, float], time_horizon_days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        for product, quantity in production_plan.items():
            ...
            machine_hours = (quantity * machine_rate) + (
                setup_time * quantity / batch_size
            )
            labor_hours = (quantity * labor_rate) + (
                setup_time * 0.5 * quantity / batch_size
            )
        ...
        logger.debug(
            f"Calculated capacity requirements for {len(production_plan)} products"
        )
        return capacity_requirements

    def identify_capacity_bottlenecks(
        self, capacity_utilization: Dict[str, float]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if bottlenecks:
            logger.warning(
                f"Identified {len(bottlenecks)} bottlenecks, "
    ...
    def optimize_capacity_allocation(
        ...
        if capacity_constraints:
            for product, max_capacity in capacity_constraints.items():
                if product in available_capacity:
                    available_capacity[product] = min(
                        available_capacity[product], max_capacity
                    )
        ...
        for product, demand in demand_forecast.items():
            ...
            allocation_plan[product] = {
                ...
                "deficit": deficit,
                "utilization": (
                    allocated / product_capacity if product_capacity > 0 else 0
                ),
        ...
        allocation_plan["_summary"] = {
            ...
            "overall_fulfillment_rate": (
                total_allocated / sum(demand_forecast.values())
                if demand_forecast
                else 0
            ),
        }
        ...
        logger.info(
            f"Optimized allocation for {len(demand_forecast)} products, "
    ...
    def _calculate_bottleneck_impact(
        self, resource: str, utilization: float
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "priority": (
                "high" if utilization > self.config.critical_threshold else "medium"
            ),
            "overtime_hours_needed": excess_utilization
            * 8,  # Approximate overtime needed
        }
    ...
    def _get_product_capacity(
        self, product: str, available_capacity: Dict[str, float]
    ) -> float:
        """
...
def test_capacity_planning_service():
    ...
    config = CapacityConfig(bottleneck_threshold=0.80, critical_threshold=0.90)
    ...
    if bottlenecks:
        ...
        for bottleneck in bottlenecks:
            print(
                f"    {bottleneck['resource']}: {bottleneck['utilization']:.1%} "
