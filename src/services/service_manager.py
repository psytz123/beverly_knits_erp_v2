...
class ServiceManager:
    ...
    def analyze_inventory(self, current_inventory, forecast):
        ...
        if inventory_service:
            return inventory_service.analyze_inventory_levels(
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        for name, service in self.services.items():
            service_status = {"available": True, "class": service.__class__.__name__}
    ...
    def perform_integrated_analysis(self, inventory_data, sales_history):
        ...
        if forecast_service and sales_history:
            ...
            forecast_dict = {}
            for style_id, forecast_data in forecast_summary.get(
                "forecasts", {}
            ).items():
...
def test_service_manager():
    ...
    config = {
        "inventory": {"safety_stock_multiplier": 2.0, "lead_time_days": 45},
    ...
    if results["forecasts"]:
        print(
            f"  Average Confidence: {results['forecasts'].get('average_confidence', 0):.3f}"
        )
        print(
            f"  Method Distribution: {results['forecasts'].get('method_distribution', {})}"
        )
    ...
    if results["inventory_analysis"]:
        ...
        critical = sum(
            1
            for item in results["inventory_analysis"]
    ...
    if results["recommendations"]:
        ...
        for rec in results["recommendations"][:3]:
            print(
                f"  - {rec['product_id']}: {rec['action']} ({rec['quantity']:.0f} units)"
            )
