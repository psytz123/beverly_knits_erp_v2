...
class YarnRequirementCalculatorService:
    ...
    def __init__(self, config: Optional[YarnRequirementConfig] = None):
        ...
        logger.info(
            f"YarnRequirementCalculatorService initialized with data path: {self.data_path}"
        )
        ...
        if bom_file.exists():
            try:
                ...
                if self.column_standardizer:
                    self.bom_data = self.column_standardizer.standardize_columns(
                        self.bom_data
                    )
                    logger.debug("Columns standardized")

                logger.info(
                    f"Loaded {len(self.bom_data)} BOM entries from {bom_file.name}"
                )
                return True
    ...
    def process_yarn_requirements(self) -> Dict[str, Any]:
        ...
        if self.bom_data is not None:
            ...
            logger.debug(
                f"Total yarn requirements calculated: {len(self.yarn_requirements)}"
            )
    ...
    def get_critical_yarns(
        ...
        return sorted(
            critical,
            key=lambda x: (x["criticality_score"], x["total_required"]),
            reverse=True,
        )

    def calculate_procurement_needs(
            ...
            for yarn_id, req in self.yarn_requirements.items():
                ...
                if shortage > 0:
                    priority = self._calculate_priority(
                        current_stock, required, shortage
                    )
    ...
    def get_yarn_usage_summary(self) -> Dict[str, Any]:
        ...
        total_requirement = sum(
            req["total_required"] for req in self.yarn_requirements.values()
        )
        ...
        summary = {
            ...
            "average_requirement_per_yarn": (
                total_requirement / total_yarns if total_yarns > 0 else 0
            ),
        ...
        if requirement_values:
            ...
            summary["median_requirement"] = sorted(requirement_values)[
                len(requirement_values) // 2
            ]
    ...
    def _calculate_priority(
        self, current_stock: float, required: float, shortage: float
    ) -> str:
...
def test_yarn_requirement_service():
    ...
    config = YarnRequirementConfig(critical_threshold=500, high_priority_factor=0.6)
    ...
    for yarn in critical_yarns[:3]:
        print(
            f"    {yarn['yarn_id']}: {yarn['total_required']} units "
    ...
    sample_inventory = pd.DataFrame(
        {"Yarn ID": ["YARN001", "YARN002", "YARN003"], "Balance": [100, 50, 200]}
    )
    ...
    for item in procurement_needs[:3]:
        print(
            f"    {item['yarn_id']}: Shortage {item['shortage']} "
