...
from services.inventory_pipeline_service import (
    InventoryManagementPipelineService,
    PipelineConfig,
)
from services.yarn_requirement_service import (
    YarnRequirementCalculatorService,
    YarnRequirementConfig,
)
...
class OptimizedServiceManager:
    ...
    def _initialize_services(self):
        ...
        try:
            inventory_config = InventoryConfig(
                safety_stock_multiplier=self.config.get("inventory", {}).get(
                    "safety_stock_multiplier", 1.5
                ),
                lead_time_days=self.config.get("inventory", {}).get(
                    "lead_time_days", 30
                ),
            )
        ...
        try:
            forecast_config = ForecastConfig(
                forecast_horizon=self.config.get("forecast", {}).get(
                    "horizon_days", 90
                ),
                target_accuracy=self.config.get("forecast", {}).get(
        ...
        try:
            capacity_config = CapacityConfig(
                bottleneck_threshold=self.config.get("capacity", {}).get(
                    "bottleneck_threshold", 0.85
                ),
                critical_threshold=self.config.get("capacity", {}).get(
        ...
        try:
            pipeline_config = PipelineConfig(
                ...
                growth_forecast=self.config.get("pipeline", {}).get(
                    "growth_forecast", 1.1
                ),
                critical_threshold=self.config.get("pipeline", {}).get(
                    "critical_threshold", 5
                ),
            )
            pipeline_service = InventoryManagementPipelineService(pipeline_config)
            pipeline_service.set_dependencies(
                inventory_analyzer=self.services.get("inventory"), supply_chain_ai=None
            )
            self.services["pipeline"] = pipeline_service
            logger.info("  ✓ InventoryManagementPipelineService initialized")
        except Exception as e:
            logger.error(
                f"  ✗ Failed to initialize InventoryManagementPipelineService: {e}"
            )

        # Initialize Yarn Requirement Calculator Service
        try:
            yarn_config = YarnRequirementConfig(
                critical_threshold=self.config.get("yarn", {}).get(
                    "critical_threshold", 1000
                ),
                high_priority_factor=self.config.get("yarn", {}).get(
                    "high_priority_factor", 0.5
                ),
            )
            ...
            logger.error(
                f"  ✗ Failed to initialize YarnRequirementCalculatorService: {e}"
            )
    ...
    def perform_integrated_analysis(
        self, inventory_data: pd.DataFrame = None, sales_history: pd.DataFrame = None
    ) -> Dict[str, Any]:
        """
        ...
        if pipeline_service:
            try:
                pipeline_results = pipeline_service.run_complete_analysis(
                    sales_data=sales_history, inventory_data=inventory_data
                )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        for name, service in self.services.items():
            try:
                ...
                health["services"][name] = {"available": False, "error": str(e)}
    ...
    def _calculate_health_score(self, health: Dict[str, Any]) -> int:
        ...
        unavailable = sum(
            1 for s in health["services"].values() if not s.get("available")
        )
    ...
    def optimize_all_services(self):
        ...
        logger.info(
            f"  Memory after: {self.memory_optimizer.get_memory_usage():.2f} MB"
        )
    ...
    def _log_memory_status(self):
        ...
        logger.info(
            f"Memory Status: {stats['rss_mb']:.2f} MB ({stats['percent']:.1f}%), "
...
def test_optimized_service_manager():
    ...
    for name, status in health["services"].items():
        ...
        print(
            f"  {symbol} {name}: {status['class'] if status['available'] else status.get('error', 'Unknown')}"
        )
    ...
    if "pipeline_analysis" in results:
        ...
        if "summary" in pipeline:
            print(
                f"  Pipeline Analysis: {pipeline['summary'].get('components_analyzed', 0)} components"
            )

    if "yarn_analysis" in results:
        yarn = results["yarn_analysis"]
        print(
            f"  Yarn Analysis: {yarn['total_yarns']} yarns, {yarn['critical_yarns']} critical"
        )
