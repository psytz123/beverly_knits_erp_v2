...
class InventoryManagementPipelineService:
    ...
    def run_complete_analysis(
        ...
        yarn_data: Optional[pd.DataFrame] = None,
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.supply_chain_ai and hasattr(
                ...
                logger.info(
                    f"Generated simple forecast for {len(sales_forecast)} items"
                )
            ...
            if inventory_data is not None:
                ...
                if self.inventory_analyzer:
                    inventory_analysis = (
                        self.inventory_analyzer.analyze_inventory_levels(
    ...
    def generate_production_plan(
        self, inventory_analysis: List[Dict], forecast: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        production_plan["_summary"] = {
            ...
            "high_priority_items": sum(
                1
                for k, v in production_plan.items()
                if k != "_summary" and v["priority"] == "HIGH"
            ),
        }

        logger.info(
            f"Generated production plan for {production_plan['_summary']['total_items']} items"
        )
                ...
                inventory_list.append(
                    {
                        ...
                        "quantity": float(
                            row.get("Planning Balance", row.get("Stock", 0))
                        ),
                    }
                )
    ...
    def _generate_simple_forecast(
        self, sales_data: Optional[pd.DataFrame]
    ) -> Dict[str, float]:
        """
    ...
    def _calculate_material_requirements(
        self, production_plan: Dict, yarn_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_material_shortages(
        self, requirements: Dict, yarn_data: pd.DataFrame
    ) -> List[Dict]:
        """
                ...
                shortages.append(
                    {
                        ...
                        "coverage_days": (
                            int(current_stock / (req["quantity_needed"] / 30))
                ...
                recommendations.append(
                    {
                        ...
                        "items": [
                            item["product_id"] for item in critical_items[:5]
                        ],  # Top 5
                    }
                )
            elif critical_items:
        ...
        if "shortage_analysis" in analysis_results:
            ...
            if shortages:
                high_priority_shortages = [
                    s for s in shortages if s["priority"] == "HIGH"
                ]
                    ...
                    recommendations.append(
                        {
                            ...
                            "total_shortage_value": sum(
                                s["shortage"] for s in high_priority_shortages
                            ),
                        }
                    )
                else:
    ...
    def _simple_inventory_analysis(
        self, current_inventory: List[Dict], forecast: Dict[str, float]
    ) -> List[Dict]:
        """
    ...
    def _generate_summary(self, results: Dict) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "components_analyzed": len(
                [k for k in results.keys() if k not in ["error", "summary"]]
            ),
        ...
        if "inventory_analysis" in results:
            ...
            summary["inventory"] = {
                ...
                "critical_items": sum(
                    1 for i in inventory if i.get("shortage_risk") == "CRITICAL"
                ),
...
def test_inventory_pipeline_service():
    ...
    config = PipelineConfig(
        waste_factor=1.25, growth_forecast=1.15, critical_threshold=3
    )
    ...
    if "sales_forecast" in results:
        print(
            f"  ✓ Sales forecast generated for {len(results['sales_forecast'])} items"
        )
    ...
    if "production_plan" in results:
        ...
        if "_summary" in plan:
            print(
                f"  ✓ Production plan created for {plan['_summary']['total_items']} items"
            )
    ...
    if "summary" in results:
        ...
        if "inventory" in summary:
            print(
                f"  Inventory: {summary['inventory']['total_items']} items, "
                f"{summary['inventory']['critical_items']} critical"
            )
        if "shortages" in summary:
            print(
                f"  Shortages: {summary['shortages']['total_shortages']} total, "
