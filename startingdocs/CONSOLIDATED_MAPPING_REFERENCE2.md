...
> **⚠️ IMPORTANT - Actual API Field Names:**
>
> The actual eFab API returns field name `desc_number` (lowercase, no hash symbol).
> This document shows `Desc#` for historical reference, but **all code must use `desc_number`**.
>
> **Correct field name**: `desc_number`
> **Deprecated notations**: `Desc#`, `yarn_id`, `Yarn_ID`
>
> See `WRAPPER_ENDPOINT_DATA_REFERENCE.md` for complete eFab→Wrapper field mappings.

| API Endpoint                   | Source Column   | Target Column   | Data Type   | Transformation |
| ------------------------------ | --------------- | --------------- | ----------- | -------------- |
| `/api/yarn/active`           | `desc_number` | `desc_number` | VARCHAR(50) | Direct mapping |
| `/api/report/yarn_expected`  | `Desc`        | `desc_number` | VARCHAR(50) | Standardize    |
| `/api/report/yarn_demand`    | `Yarn`        | `desc_number` | VARCHAR(50) | Standardize    |
| `/api/report/yarn_demand_ko` | `Yarn`        | `desc_number` | VARCHAR(50) | Standardize    |
| BOM Files                      | `Yarn_ID`     | `desc_number` | VARCHAR(50) | Standardize    |
...
| ------------------------------------------ | -------------------- | ------------- |
| `Planning Balance`, `Planning_Balance` | `Planning_Balance` | DECIMAL(15,3) |
| `On Order`, `On-Order`, `OnOrder`    | `On_Order`         | DECIMAL(15,3) |
| `Allocated`, `Alloc`, `Reserved`     | `Allocated`        | DECIMAL(15,3) |
| `BOM%`, `BOM_Pct`, `BOM Percent`     | `BOM_Percent`      | DECIMAL(5,2)  |
| `Yds_ordered`, `Yards Ordered`         | `Yds_ordered`      | DECIMAL(15,3) |
| `Unit Price`, `UnitPrice`              | `Unit_Price`       | DECIMAL(15,4) |
...
| Quantities      | DECIMAL(15,3)            | Remove commas → round(3)    |
...
# Pattern Validation
VALIDATION_PATTERNS = {
    'order_number': r'^[A-Z0-9\-]+$',      # SO-2025-001
    'Style#': r'^[A-Z0-9\-]+$',            # ABC123-001
    'Desc#': r'^[A-Z0-9]+$',               # Y12345
